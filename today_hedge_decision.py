...
from hedging.tail_risk_hedge import MarketRegime
...
# 风格 Beta 代理 (基于风格标签的 rough estimation)
style_beta_proxy = {
    '宽基': 0.95,
    '高端制造': 1.15,
    '科技': 1.20,
...
print('今日开仓对冲决策 - 优化版')
print('=' * 60)
print(f'日期: 2026-07-06')
print(f'阶段: 第一阶段-底仓建立')
...
print('-' * 60)
print('按风格分布:')
for style in ['宽基', '高端制造', '科技', '制造', '新能源', '医药', '化工', '银行', '防御', '顺周期', '避险', '红利', '成长']:
...
defense_weak = (style_amounts.get('防御', 0) + style_amounts.get('避险', 0)) / day_capital < 0.15 if day_capital > 0 else True
