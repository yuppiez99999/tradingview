...
def main():
    ...
    print(f"\n{'='*70}")
    print("  步骤2：不同 n_trials 的 DSR 对比")
    print(f"{'='*70}")
    ...
    print(f"  {'-'*65}")
    print("\n  说明:")
    print("    - n_trials=23: 每个标的1次独立试验（最保守的合理估计）")
    print("    - n_trials=100: 含特征组合搜索（中等保守）")
    print("    - n_trials=200: 含超参数搜索（最保守）")
    ...
    for n_trials in range(1, 300):
        ...
        print("    DSR 在 n_trials=1~299 范围内均未通过 0.95 阈值")
