...
class FactorCommittee:
    ...
    def review(self, factor_name: str, factor_report: dict[str, Any]) -> CommitteeVerdict:
        ...
        for agent in self.agents:
            try:
                vote = agent.vote(factor_report)  # type: ignore[union-attr]
                votes.append(vote)
                logger.info(
                    "[Committee] %s | %s score=%.1f veto=%s | %s",
                    factor_name, agent.name, vote.score, vote.veto, vote.rationale,  # type: ignore[union-attr]
                    )
            except (ValueError, TypeError, KeyError, AttributeError, RuntimeError, OSError, TimeoutError, ConnectionError) as e:
                # 数据处理/计算/IO 异常: 格式/类型/字段/属性/运行时/网络/超时
                logger.error("[Committee] %s | %s 评分失败：%s", factor_name, agent.name, e)  # type: ignore[misc]
                error_vote = AgentVote(
                    agent_name=agent.name, score=0.0, veto=True,  # type: ignore[misc]
