...
def main() -> int:
    ...
    logger.info("=" * 70)
    logger.info("S3 第十五批次：MARGIN_EXP + VT_MICRO_VOL_SKEW_INV 组合验证（P2.2 v6.3）")
    logger.info("=" * 70)
    logger.info("v6.3 验证目标:")
    logger.info(f"  - Factor A: {FACTOR_A}（微观结构类，第八批次 approved）")
    logger.info(f"  - Factor B: {FACTOR_B}（质量变化类，第十四批次 approved）")
    logger.info("  - 组合方法：cross-sectional rank 标准化 + 等权相加")
    logger.info("  - 验证组合 IC_IR 和 Shadow 表现是否优于单因子")
    ...
    logger.info(f"  factor_history 包含 {len(factor_history)} 个因子")
    logger.info(f"  valid_dates: {len(valid_dates)} 天")

    # ============ Step 4: 提取单因子历史 + 构建组合信号 ============
    logger.info("\n[4/6] 提取单因子历史并构建组合信号")
    if FACTOR_A not in factor_history:
        logger.info(f"[ERROR] 因子 {FACTOR_A} 不在 factor_history 中")
        return 1
    if FACTOR_B not in factor_history:
        logger.info(f"[ERROR] 因子 {FACTOR_B} 不在 factor_history 中")
        return 1
    ...
    logger.info(f"  Combined (equal weight): {len(combined_hist)} 天")
    ...
    if not overall_success:
        if not ic_ir_improved:
            logger.info("    - IC_IR 未提升，可能两因子相关性较高")
        if not max_dd_improved:
            logger.info("    - max_dd 未降低，可能两因子回撤期重叠")
