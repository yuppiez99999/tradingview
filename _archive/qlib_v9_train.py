...
def main():
    ...
    aux_symbols = [f"{exch}{code}" for code, exch, name in CSI50_AUX if f"{exch}{code}" not in portfolio_symbols]
    ...
    X_train = train_data["feature"].values[:, top_feature_indices]  # noqa: N806
    y_train = train_data["label"].values.flatten()

    X_valid = valid_data["feature"].values[:, top_feature_indices]  # noqa: N806
    y_valid = valid_data["label"].values.flatten()

    X_test = test_data["feature"].values[:, top_feature_indices]  # noqa: N806
    y_test = test_data["label"].values.flatten()

    mask_train = np.isfinite(y_train)
    X_train = X_train[mask_train]  # noqa: N806
    y_train = y_train[mask_train]

    mask_valid = np.isfinite(y_valid)
    X_valid = X_valid[mask_valid]  # noqa: N806
    ...
    rank_ic_test = (
        np.corrcoef(pd.Series(y_test_clean).rank(), pd.Series(y_pred_test_clean).rank())[0, 1]
