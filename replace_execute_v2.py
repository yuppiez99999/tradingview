...
# 找到 def execute_decision( 到 def _simulate_fill( 之间的内容
...
if match:
    ...
    with open(file_path, 'w', encoding='utf-8') as f:
        f.write(new_content)
    print('替换成功')
    print('旧函数体长度:', len(old_body))
    print('新函数体长度:', len(new_body))
else:
    print('未找到匹配')
    # 调试信息
    idx = content.find('def execute_decision(')
    print('execute_decision 位置:', idx)
    idx2 = content.find('def _simulate_fill(')
    print('_simulate_fill 位置:', idx2)
