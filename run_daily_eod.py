...
def run_kill_switch() -> dict:
    try:
        ...
        logger.info(f"[Guard1] KillSwitch: margin_status={margin_result.get('level', 'N/A')}")
...
def run_drawdown_controller() -> dict:
    try:
        ...
        if not shadow_state_path.exists():
            logger.warning("[Guard2] DrawdownController: shadow_state.json 不存在, 跳过")
        ...
        if peak <= 0 or current <= 0:
            logger.warning(f"[Guard2] DrawdownController: 数据异常 peak={peak} current={current}")
            return {"success": False, "reason": "invalid nav data"}

        result = dc.check_drawdown(peak, current)
        level = result.get("level", 0)
        logger.info(f"[Guard2] DrawdownController: level={level} ({result.get('level_name', '')})")
        if level >= 2:
            logger.warning(f"[Guard2] 回撤触发 Level {level}, 执行响应: {result.get('actions', [])}")
...
def run_rebalance(risk_cfg: dict) -> dict:
    try:
        ...
        max_w = risk_cfg.get("thresholds", {}).get("max_single_weight", 0.15)
        mw_orders = generate_max_weight_reduction_orders(positions, prices, styles, max_weight=max_w)
        reb_orders = generate_rebalance_orders(style_allocation, TARGET_ALLOCATION, positions, prices)
        ...
        out_path = _BASE / "reports" / f"rebalance_execution_orders_{datetime.now():%Y%m%d}.json"
        out_path.parent.mkdir(parents=True, exist_ok=True)
        with open(out_path, "w", encoding="utf-8") as f:
            json.dump(report, f, ensure_ascii=False, indent=2)
        return {"success": True, "orders_count": len(all_orders), "mw_orders": len(mw_orders), "report_path": str(out_path)}
    except Exception as e:
