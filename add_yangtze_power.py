# -*- coding: utf-8 -*-
"""
把长江电力加入 500万建仓计划，并重新生成 7月6日计划文件
"""
import os
import sys
import json
...
_GENERATOR = os.path.join(_BASE_DIR, "_archive_dead_code", "generate_500w_build_plan.py")
...
def _scale_existing_weights(data: dict, scale: float) -> None:
    ...
    for code, cfg in list(position_plan.items()):
        ...
        cfg["est_price"] = target_portfolio.get(code, {}).get("est_price", cfg.get("est_price"))
        ...
        for phase in cfg.get("phases", []):
            cumulative_ratio = round(cumulative_ratio + float(phase.get("capital_ratio", 0)), 3)
            phase["target_amount"] = round(cfg["target_amount"] * float(phase.get("capital_ratio", 0)), 0)
            phase["shares"] = int(phase["target_amount"] / cfg["est_price"] / lots) * lots
        ...
        "phases": [
            {
                ...
                "actual_amount": round(int(round(target_amount * 0.35, 0) / est_price / lots) * lots * est_price, 0),
            ...
            },
            {
                ...
                "actual_amount": round(int(round(target_amount * 0.30, 0) / est_price / lots) * lots * est_price, 0),
            ...
            },
            {
                ...
                "actual_amount": round(int(round(target_amount * 0.20, 0) / est_price / lots) * lots * est_price, 0),
            ...
            },
            {
                ...
                "actual_amount": round(int(round(target_amount * 0.15, 0) / est_price / lots) * lots * est_price, 0),
...
def _update_style_summary(data: dict) -> None:
    ...
    for code, info in data.get("position_plan", {}).items():
        ...
        if style not in style_summary:
            style_summary[style] = {"amount": 0, "weight": 0, "codes": [], "risk_distribution": {}}
        ...
        style_summary[style]["risk_distribution"][risk] = style_summary[style]["risk_distribution"].get(risk, 0) + info.get("actual_amount", 0)
    data["style_summary"] = style_summary


def _update_phase_summary(data: dict) -> None:
    phase_summary = []
    for i, phase in enumerate([{"phase": 1, "name": "第一阶段-底仓建立", "start": "2026-07-06", "duration_days": 10, "capital_ratio": 0.35, "strategy": "优先建立高端制造核心仓位（科创50、半导体、高端装备），同步配置黄金ETF和中国神华作为稳定器"},
                               {"phase": 2, "name": "第二阶段-配置完善", "start": "2026-07-20", "duration_days": 15, "capital_ratio": 0.30, "strategy": "利用月中波动窗口分批加仓，关注大宗商品价格趋势，择机增加资源板块"},
                               {"phase": 3, "name": "第三阶段-防御补充", "start": "2026-08-10", "duration_days": 15, "capital_ratio": 0.20, "strategy": "结合中报披露窗口，优选医药板块回调时点建仓，配置债券类资产"},
                               {"phase": 4, "name": "第四阶段-最终调整", "start": "2026-09-01", "duration_days": 20, "capital_ratio": 0.15, "strategy": "审视前三阶段执行偏差，补齐偏离标的，配置短融ETF管理剩余现金"}]):
