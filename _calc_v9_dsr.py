...
print(f'  公式: DSR = (SR_observed - SR_max_expected) / SR_std')
print(f'  SR_max_expected = sqrt(2*ln(n_trials)) * sqrt(1/(T-1)) * sqrt(1 - skewness*SR*sqrt(1/T) + (kurtosis-1)/(4*(T-1))*SR^2)')
...
# === 综合评估 ===
print(f'\n{"=" * 70}')
print(f'综合评估 (新标准: Sharpe CV<1.0 + DSR>=5 + 年化>=15% + 回撤<=10%)')
print(f'{"=" * 70}')
...
dsr_pass = dsr_10 > 0
checks.append((f'DSR > 0 (n_trials=10)', dsr_pass, f'DSR={dsr_10:.4f}, p={dsr_results.get(10, {}).get("p_value", 0):.4f}'))
