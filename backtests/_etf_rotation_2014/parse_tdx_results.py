...
def parse_files(res_dir):
    ...
    if not files:
        print("!! 未找到 tdx_kline 结果文件:", res_dir); return None
    for fp in files:
        with open(fp, encoding="utf-8", errors="replace") as f:
            text = f.read()
        m = re.search(r"详细K线数据:\s*\n?(\{)", text)
        if not m:
            print("!! 无JSON起始:", os.path.basename(fp)); continue
        try:
            obj, _ = json.JSONDecoder().raw_decode(text[m.start(1):])
        except Exception as e:
            print("!! JSON解析失败:", os.path.basename(fp), e); continue
        code = str(obj.get("Code"))
        startxh = int(obj.get("Startxh", 0) or 0)
        rows = obj.get("Rows") or []
        if not rows:
            print(f"!! {code} startxh={startxh} 空段(历史不足或超界)"); continue
        for r in rows:
            records.append(dict(code=code, startxh=startxh, date=str(r["Data"]),
                                open=float(r["Open"]), high=float(r["High"]),
                                low=float(r["Low"]), close=float(r["Close"]),
                                volume=float(r.get("Volume", 0)), amount=float(r.get("Amount", 0))))
        print(f"  {code} startxh={startxh:>4} rows={len(rows):>4} "
              f"range={rows[0]['Data']}..{rows[-1]['Data']}")
    ...
    for mf in sorted(glob.glob(os.path.join(res_dir, "manual_*.csv"))):
        ...
        with open(mf, encoding="utf-8") as f:
            for line in f:
                ...
                if not line:
                    continue
                d, o, h, l, c = line.split(",")
                records.append(dict(code=code, startxh=startxh, date=d.strip(),
                                    open=float(o), high=float(h),
                                    low=float(l), close=float(c),
                                    volume=0.0, amount=0.0))
        print(f"  [manual] {code} startxh={startxh:>4} rows=补入")
    return pd.DataFrame(records)

def main():
    res_dir = sys.argv[1] if len(sys.argv) > 1 else r"C:\Users\Administrator\.workbuddy\projects\e-各种PY程序-28-终极量化交易系统8.4\e0b32441-aef1-499f-9b65-f13b6aaa4ef5\tool-results"
    out_dir = sys.argv[2] if len(sys.argv) > 2 else r"E:\各种PY程序\28-终极量化交易系统8.4\backtests\_etf_rotation_2014\data"
    ...
    for code, g in df.groupby("code"):
        print(f"{code}: {len(g)}根  {g['date'].min().date()} .. {g['date'].max().date()}")
    print("\n总记录:", len(df), "| 代码:", sorted(df.code.unique()))
