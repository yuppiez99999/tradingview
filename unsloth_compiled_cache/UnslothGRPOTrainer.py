...
def grpo_compute_loss(
    ...
    with torch.no_grad():
        if use_vllm and sampling_per_token_logps is not None:
            ...
            if vllm_importance_sampling_mode in ["sequence_mask", "sequence_truncate"]:
                importance_sampling_ratio = importance_sampling_ratio.sum(
                    dim=-1, keepdim=True
                )
    ...
    if loss_type == "cispo":
        ...
        temperatures = torch.where(
            advantages > 0, sapo_temperature_pos, sapo_temperature_neg
        )
...
def grpo_accumulated_loss(
    ...
    **kwargs,
    ...
    prev_max_left_pad = kwargs.get(
        "max_left_pad", 0
    )  # max_left_pad for LLM training, enabled by default.

    # Pop from kwargs to avoid downstream issues.
    _ = kwargs.pop("sampling_per_token_logps", None)
    kwargs["vllm_importance_sampling_cap"] = getattr(
        trainer.args, "vllm_importance_sampling_cap", None
    )
    # Older TRL lacks this arg; fall back to token_truncate (legacy clamp(max=cap) behavior).
    kwargs["vllm_importance_sampling_mode"] = (
        getattr(trainer.args, "vllm_importance_sampling_mode", None) or "token_truncate"
    )
    kwargs["vllm_importance_sampling_clip_min"] = getattr(
        trainer.args, "vllm_importance_sampling_clip_min", None
    )
    kwargs["vllm_importance_sampling_clip_max"] = getattr(
        trainer.args, "vllm_importance_sampling_clip_max", None
    )
    ...
    kwargs["get_gamma_weights"] = (
        trainer.get_gamma_weights if hasattr(trainer, "get_gamma_weights") else None
    )
    kwargs["vespo_k_pos"] = (
        trainer.args.vespo_k_pos if hasattr(trainer.args, "vespo_k_pos") else 2.0
    )
    kwargs["vespo_k_neg"] = (
        trainer.args.vespo_k_neg if hasattr(trainer.args, "vespo_k_neg") else 3.0
    )
    kwargs["vespo_lambda_pos"] = (
        trainer.args.vespo_lambda_pos
        if hasattr(trainer.args, "vespo_lambda_pos")
        else 3.0
    )
    kwargs["vespo_lambda_neg"] = (
        trainer.args.vespo_lambda_neg
        if hasattr(trainer.args, "vespo_lambda_neg")
        else 2.0
    )
    ...
    if (
        kwargs["vllm_importance_sampling_clip_max"] is None
        and kwargs["vllm_importance_sampling_cap"] is not None
    ):
        kwargs["vllm_importance_sampling_clip_min"] = 0
        kwargs["vllm_importance_sampling_clip_max"] = kwargs[
    ...
    _pg_layout = None
    _pg_trusted = (
        False  # signature already verified -> skip the full-row forward this step
    )
    if _pg_engage:
        try:
            ...
            _pg_unsafe = getattr(
                unwrapped_model, "_unsloth_prefix_grouper_grad_unsafe", None
            )
            ...
            if _pg_layout is not None and _pg_layout.signature in _pg_unsafe:
                ...
                _pg_verified = getattr(
                    unwrapped_model, "_unsloth_prefix_grouper_grad_verified", None
                )
            ...
            if UNSLOTH_ENABLE_LOGGING:
                print(
                    f"[Unsloth] GRPO PrefixGrouper (grad) disabled (fell back to packed): {_pg_err!r}",
                    flush=True,
                )
    ...
    _pack_result = None
    _pack_use = False
    _pack_enabled = os.environ.get("UNSLOTH_GRPO_SEQ_PACKING", "1").lower() not in (
        "0",
        "false",
        "no",
        "off",
    ...
    if (
        _pack_enabled
        and not _pg_skip_pack
        and pixel_values is None
        and token_type_ids is None
        and mm_token_type_ids is None
        and _pack_ok is not False
    ):
        try:
            ...
            _pack_lengths_cpu = (
                _pack_lengths.tolist()
            )  # single GPU->CPU sync, reused below
            ...
            _pack_sw_ok = not (
                isinstance(_pack_sw, int) and _pack_sw > 0 and _pack_maxseg > _pack_sw
            )
            _pack_active = int((completion_mask.sum(dim=1) > 0).sum())
            _pack_unsafe = getattr(
                unwrapped_model, "_unsloth_seq_packing_grad_unsafe_T", None
            )
            # skip the whole packed forward for a known-unsafe length region (a prior moderate mismatch)
            if (
                _pack_T >= 2
                and len(_pack_nz_cpu) > 0
                and _pack_sw_ok
                and (_pack_ok is True or _pack_active >= 2)
                and not (_pack_unsafe is not None and _pack_T >= _pack_unsafe)
            ):
                _pack_psl = torch.tensor(
                    _pack_nz_cpu, dtype=torch.int32, device=input_ids.device
                )
                ...
                _pack_cstart = (
                    _pack_L - logits_to_keep
                ) - left_pad_tokens_per_prompt  # [rows]
                _pack_ctgt = (
                    _pack_nz_idx[1:, 1] >= _pack_cstart[_pack_nz_idx[1:, 0]]
                ) & _pack_within
                with autocaster:
                    ...
                    if _unsloth_grpo_returns_hidden_states(
                        unwrapped_model, _pack_h, lm_head
                    ):
                ...
                _pack_tgt = (_pack_nz_idx[1:, 0] * _pack_L + _pack_nz_idx[1:, 1])[
                    _pack_ctgt
                ]
                _pack_result = (
                    torch.zeros(
                    ...
                    .index_put((_pack_tgt,), _pack_sel.to(torch.float32))
                    .view(total_rows, _pack_L)[:, -_pack_W:]
                )
                ...
                _pack_vT = int(
                    getattr(unwrapped_model, "_unsloth_seq_packing_grad_verified_T", 0)
                )
                _pack_vS = int(
                    getattr(
                        unwrapped_model, "_unsloth_seq_packing_grad_verified_seg", 0
                    )
                )
                _pack_force_verify = (
                ...
                if (
                    (not _pack_force_verify)
                    and _pack_ok is True
                    and _pack_T <= _pack_vT
                    and _pack_maxseg <= _pack_vS
                ):
                    ...
                    with torch.no_grad(), autocaster:
                        for _pack_i in range(total_rows):
                            ...
                            _pack_rpos = torch.arange(
                                _pack_ni, device=input_ids.device
                            ).unsqueeze(0)
                            _pack_rh = unwrapped_model(
                                input_ids=_pack_real,
                                position_ids=_pack_rpos,
                                use_cache=False,
                            ).logits
                            ...
                            if _unsloth_grpo_returns_hidden_states(
                                unwrapped_model, _pack_rh, lm_head
                            ):
                                _pack_rsel = (
                            ...
                            _pack_rcols = _pack_rmask.nonzero(as_tuple=False).squeeze(
                                1
                            )[1:] - (_pack_L - _pack_W)
                            _pack_rkeep = _pack_rcols >= 0
                            _pack_ref[_pack_i, _pack_rcols[_pack_rkeep]] = _pack_rsel[
                                _pack_rkeep
                            ].to(torch.float32)
                    device_synchronize()
                    ...
                    _pack_diff = float(
                        ((_pack_result.detach() - _pack_ref).abs() * _pack_cm).max()
                    )
                    if UNSLOTH_ENABLE_LOGGING:
                        print(
                            f"[Unsloth] GRPO seq-packing (grad) verify: T={_pack_T} maxseg={_pack_maxseg} packed-vs-perrow max|d|={_pack_diff:.4f}",
                            flush=True,
                        )
                    ...
                    if _pack_diff < 7e-1:
                        ...
                        if _pack_active >= 2:
                            unwrapped_model._unsloth_seq_packing_grad_verified_T = max(
                                _pack_vT, _pack_T
                            )
                            unwrapped_model._unsloth_seq_packing_grad_verified_seg = (
                                max(_pack_vS, _pack_maxseg)
                            )
                        _pack_ok = True
                        _pack_use = True
                    else:
                        ...
                        if _pack_diff >= 1.5:
                            ...
                            unwrapped_model._unsloth_seq_packing_grad_unsafe_T = (
                                _pack_T
                                if _pack_unsafe is None
                                else min(_pack_unsafe, _pack_T)
                            )
                        if UNSLOTH_ENABLE_LOGGING:
                            print(
                                f"[Unsloth] GRPO seq-packing (grad) fell back at T={_pack_T} (diff={_pack_diff:.3f})",
                                flush=True,
                            )
            ...
            if UNSLOTH_ENABLE_LOGGING:
                print(
                    f"[Unsloth] GRPO sequence-packing disabled (fell back to padded): {_pack_err!r}",
                    flush=True,
                )
    ...
    def _pg_grad_forward():
        ...
        with autocaster:
            ...
            if not _unsloth_grpo_returns_hidden_states(unwrapped_model, _h, lm_head):
                ...
                def _pg_fn(
                    _pg_h, _pg_lm, _pg_ids, _pg_n, _pg_lsm, _pg_lsd, _pg_lsc, _pg_t
                ):
    ...
    if _pg_layout is not None:
        ...
        try:
            if not _pg_trusted:
                ...
                if _pack_use and _pack_result is not None:
                    ...
                    if _pg_diff < _pg_tol_ok():
                        _pg_v = getattr(
                            unwrapped_model,
                            "_unsloth_prefix_grouper_grad_verified",
                            None,
                        )
            ...
            if UNSLOTH_ENABLE_LOGGING:
                print(
                    f"[Unsloth] GRPO PrefixGrouper (grad) forward failed -> packed/padded fallback: {_pg_err2!r}",
                    flush=True,
                )
    ...
    class Unsloth_Offloaded_Log_Softmax(torch.autograd.Function):
        ...
        def forward(
            ...
            if device_module is not None:
                ...
                try:
                    pinned_buffer = torch.empty_like(
                        detached_hidden_states, device="cpu", pin_memory=True
                    )
                    if pinned_buffer is not None:
                        current_stream = device_module.current_stream(
                            detached_hidden_states.device
                        )
                        copy_stream = device_module.Stream(
                            device=detached_hidden_states.device
                        )
                        copy_stream.wait_stream(current_stream)
                        with device_module.stream(copy_stream):
                            pinned_buffer.copy_(
                                detached_hidden_states, non_blocking=True
                            )
            ...
            if saved_hidden_states is None:
                ...
                saved_hidden_states = detached_hidden_states.to(
    ...
    def compute_logprobs_chunk(
        new_hidden_states_chunk, completion_ids, input_ids_chunk
    ):
        ...
        if _unsloth_grpo_returns_hidden_states(
            unwrapped_model, new_hidden_states_chunk, lm_head
        ):
    ...
    for (
        ...
        with autocaster:
            if pixel_values is None:
                ...
                logprobs_chunk = compute_logprobs_chunk(
                    new_hidden_states_chunk, completion_ids, input_ids_chunk
                )
            else:
                new_hidden_states_chunk = unwrapped_model(
...
UNSLOTH_ENABLE_LOGGING = _unsloth_os.environ.get("UNSLOTH_ENABLE_LOGGING", "0") in (
    "1",
    "True",
    "true",
)

UNSLOTH_GRPO_SEQ_PACKING_ON = _unsloth_os.environ.get(
...
try:
    ...
    UNSLOTH_ZOO_HAS_MASKED_COL_GUARD = (
        "torch.where(_keep, new"
        in _unsloth_inspect.getsource(_unsloth_zoo_RL["grpo_compute_loss"])
    )
except Exception:
    UNSLOTH_ZOO_HAS_MASKED_COL_GUARD = False

_pg_build_layout = _pg_enabled_fn = _pg_verify_on = _pg_tol_ok = _PG_TOL_KILL = None
UNSLOTH_GRPO_PREFIX_GROUPER_ON = _unsloth_os.environ.get(
...
class UnslothGRPOConfig(GRPOConfig):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__(
            **_unsloth_filter_config_init_kwargs(GRPOConfig, _unsloth_config_arguments)
        )
...
class _UnslothGRPOTrainer(BaseTrainer):
    ...
    def __init__(
        ...
        if hasattr(model, "vllm_engine") and hasattr(args, "use_vllm"):
            ...
            _unsloth_esm = getattr(
                getattr(
                    getattr(
                        getattr(model.vllm_engine, "llm_engine", None),
                        "vllm_config",
                        None,
                    ),
                    "model_config",
            ...
            if (
                _unsloth_esm
                if _unsloth_esm is not None
                else os.environ.get("UNSLOTH_VLLM_STANDBY", "0") != "0"
            ):
        ...
        if self.use_vllm:
            ...
            if self.vllm_mode == "server":
                ...
                if (
                    ...
                    max_model_len = None
                if getattr(
                    getattr(model, "vllm_engine", None), "shared_weights", False
                ):
                ...
                if _pk_use and _pk_result is not None:
                    try:
                        ...
                        if _pg_diff < _pg_tol_ok():
                            ...
                            if _pg_diff >= _PG_TOL_KILL:
                                _pg_u.add(_pg_layout.signature)
                                unwrapped_model._unsloth_prefix_grouper_nograd_unsafe = (
                                    _pg_u
                                )
                            _pg_use = False
    ...
    def _generate_single_turn(self, prompts: list[str], images: Optional[list]):
        ...
        if self.use_vllm:
            ...
            if (
                "mm_token_type_ids" in generate_inputs
                or "image_grid_thw" in generate_inputs
            ):
                mm_token_type_ids = _unsloth_fix_mm_token_type_ids(
    ...
    def _generate_and_score_completions(
        ...
        try:
            ...
            if images is None:
                ...
                max_left_pad = torch.max(left_pad_tokens_per_prompt).item()
        _use_gc = (
            self.model._unsloth_gradient_checkpointing
            if hasattr(self.model, "_unsloth_gradient_checkpointing")
            else getattr(self.args, "gradient_checkpointing", True)
        )
