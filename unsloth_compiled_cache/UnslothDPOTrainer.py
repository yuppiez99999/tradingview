...
class UnslothDPOConfig(DPOConfig):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__(
            **_unsloth_filter_config_init_kwargs(DPOConfig, _unsloth_config_arguments)
        )
...
class _UnslothDPOTrainer(BaseTrainer):
        ...
        with PartialState().main_process_first():
            ...
            dataset = dataset.map(
                (
                    ...
                    else dpo_trainer_vision_process_row
                ),
    ...
    def concatenated_inputs(
        ...
        if "pixel_position_ids" in batch:
            output["pixel_position_ids"] = torch.cat(
                (batch["pixel_position_ids"], batch["pixel_position_ids"]), dim=0
            )
        if "image_position_ids" in batch:
            output["image_position_ids"] = torch.cat(
                (batch["image_position_ids"], batch["image_position_ids"]), dim=0
            )
        if "mm_token_type_ids" in batch:
            output["mm_token_type_ids"] = torch.cat(
                (batch["mm_token_type_ids"], batch["mm_token_type_ids"]), dim=0
            )
    ...
    def _compute_loss_liger(
        ...
        if "pixel_position_ids" in concatenated_batch:
            model_kwargs["pixel_position_ids"] = concatenated_batch[
                "pixel_position_ids"
            ]
        if "image_position_ids" in concatenated_batch:
            model_kwargs["image_position_ids"] = concatenated_batch[
                "image_position_ids"
            ]
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if hasattr(train_dataset, "column_names"):
            ...
            if is_dpo_dataset and isinstance(
                data_collator, TransformersDataCollatorForLanguageModeling
            ):
                data_collator = None
        ...
        if not hasattr(DataCollatorForPreference, "_unsloth_vision_keys_patch"):
            ...
            def _unsloth_dpo_torch_call(self, examples):
                ...
                for _k in (
                    ...
                    _values = [
                        _unsloth_torch.as_tensor(example[_k]) for example in examples
                    ]
                    try:
                        if _unsloth_trl_pad is not None:
                            output[_k] = _unsloth_trl_pad(
                                _values,
                            ...
                            from torch.nn.utils.rnn import (
                                pad_sequence as _unsloth_pad_sequence,
                            )

                            output[_k] = _unsloth_pad_sequence(
                                _values, batch_first=True, padding_value=_padding_value
                            )
                    except Exception:
                        from torch.nn.utils.rnn import (
                            pad_sequence as _unsloth_pad_sequence,
