        ...
        **kwargs,
        ...
        if _unsloth_get_dataset_num_proc is not None:
            dataset_num_proc = _unsloth_get_dataset_num_proc(
                dataset_num_proc, serial_as_none=False
            )
        ...
        super().__init__(
            **_unsloth_filter_config_init_kwargs(SFTConfig, _unsloth_config_arguments)
        )
...
class _UnslothSFTTrainer(BaseTrainer):
    ...
    def __init__(
        ...
        if not self._is_vlm and self._is_vision_dataset:
            ...
            if hasattr(_m, "model"):
                _m = _m.model
            if hasattr(
                getattr(_m, "config", None), "vision_config"
            ) or _m.__class__.__name__.endswith("ForConditionalGeneration"):
                self._is_vlm = True
    ...
    def _prepare_dataset(
        ...
        try:
            _unsloth_pack_has_strategy = (
                "strategy" in _inspect.signature(pack_dataset).parameters
            )
        except Exception:
        ...
        _model = getattr(self, "_unsloth_model_ref", None) or getattr(
            self, "model", None
        )
        ...
        if do_tokenize:
            ...
            if do_prompt_completion:
                ...
                def _tokenize_pc(example):
                    ...
                    if (
                        do_truncation
                        and not _unsloth_wrapped_packing
                ...
                try:
                    from unsloth_zoo.dataset_num_proc import (
                        map_failure_diagnostics as _unsloth_map_diagnostics,
                    )
                except ImportError:
                    from unsloth.dataset_num_proc import (
                        map_failure_diagnostics as _unsloth_map_diagnostics,
                ...
                with (
                    _w.catch_warnings(),
                    _unsloth_map_diagnostics(map_kwargs.get("num_proc")),
                ):
                    _w.filterwarnings(
                        "ignore", message=".*couldn't be hashed properly.*"
                    )
                    dataset = dataset.map(
                        _tokenize_pc,
                        batched=False,
            ...
            if (
                is_vlm
                and not hasattr(processing_class, "pad")
                and not do_prompt_completion
            ):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if "max_length" not in locals() and not hasattr(args, "max_length"):
            ...
            if (
                hasattr(args, "max_seq_length")
                and args.max_seq_length is not None
                and args.max_seq_length > 0
            ):
                ...
                if model_max_length is None:
                    model_max_length = getattr(model, "max_length", None)
                if model_max_length is not None:
                    ...
                    print(
                        "Unsloth: We did not find `max_seq_length` or `max_length` in the model or args. We will set it to 1024."
                    )
        ...
        IGNORED_TOKENIZER_NAMES = os.environ.get(
