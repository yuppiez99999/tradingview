...
def test_black_litterman_optimizer():
    ...
    print(f"  无观点: w_BL={result_a.optimal_weights}, Sharpe={result_a.sharpe_ratio:.3f}")
    ...
    # 测试 1b: 有观点 (绝对观点)
    views = [
        BLView(type="absolute", assets=["600519"], weights=[1.0],
               expected_return=0.15, confidence=0.7),
        BLView(type="relative", assets=["000858", "601318"], weights=[1.0, -1.0],
    ...
    print(f"  有观点: w_BL={result_b.optimal_weights}, Sharpe={result_b.sharpe_ratio:.3f}")
    ...
    # 后验收益应与隐含收益不同 (因为加了观点)
    assert not np.allclose(result_b.posterior_returns, result_b.implied_equilibrium_returns)
...
def test_tca_engine():
    ...
    fills = [
        FillRecord(symbol="600519", side="BUY", shares=3000, price=1500.0, timestamp="2026-07-14T09:35"),
        FillRecord(symbol="600519", side="BUY", shares=4000, price=1502.0, timestamp="2026-07-14T10:00"),
        FillRecord(symbol="600519", side="BUY", shares=3000, price=1505.0, timestamp="2026-07-14T10:30"),
    ]
    ...
    fills_by_symbol = {
        "600519": fills,
        "000858": [
            FillRecord(symbol="000858", side="SELL", shares=5000, price=200.0, timestamp="2026-07-14T09:40"),
        ],
    }
    benchmarks = {
        "600519": benchmark,
        "000858": BenchmarkPrices(
            decision_price=205.0, arrival_price=202.0, vwap=201.0, close_price=198.0,
        ),
    }
...
def test_barra_decomposer():
    ...
    factor_exposures = {
        "600519": {"Size": 1.5, "Beta": 0.8, "Momentum": 0.3, "ResidualVolatility": -0.2,
                   "NonLinearSize": 0.1, "BookToPrice": -0.5, "Liquidity": 0.4,
                   "EarningsYield": 0.6, "Growth": 0.2, "Leverage": -0.3},
        "000858": {"Size": 1.2, "Beta": 1.1, "Momentum": -0.1, "ResidualVolatility": 0.3,
                   "NonLinearSize": 0.2, "BookToPrice": -0.3, "Liquidity": 0.5,
                   "EarningsYield": 0.4, "Growth": 0.3, "Leverage": -0.1},
        "601318": {"Size": 1.8, "Beta": 0.9, "Momentum": 0.5, "ResidualVolatility": -0.1,
                   "NonLinearSize": 0.0, "BookToPrice": 0.2, "Liquidity": 0.6,
                   "EarningsYield": 0.3, "Growth": 0.1, "Leverage": -0.5},
    }
