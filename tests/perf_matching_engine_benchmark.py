...
def run_scenario(name: str, n_events: int, n_orders_per_event: int, mode: str = "BAR"):
    ...
    engine = MatchingEngine(
        mode=mode, allow_partial_fill=True, max_participation_rate=0.10
    )
    ...
    for ev_idx in range(n_events):
        ...
        if mode == "BAR":
            events_data.append(
                (
                    code,
                    make_bar(code, ev_idx),
                    make_orders(code, n_orders_per_event, ev_idx),
                )
            )
        else:
            events_data.append(
...
def main():
    ...
    elapsed_a, calls_a, avg_us_a = run_scenario(
        "A", n_events=2500, n_orders_per_event=10, mode="BAR"
    )
    ...
    elapsed_b_full, calls_b, avg_us_b = run_scenario(
        "B", n_events=600_000, n_orders_per_event=10, mode="BAR"
    )
    ...
    print(
        f"  Rust 预估 (10x): {elapsed_b_full/10:.3f}s, 节省 {elapsed_b_full*0.9:.3f}s ({elapsed_b_full*0.9/60:.2f}min)"
    )
    print()

    # ---- 场景 C: TICK 级 ----
    print("【场景 C】TICK 级: 240 事件 × 10 orders/event (TICK 模式, 五档撮合)")
    elapsed_c, calls_c, avg_us_c = run_scenario(
        "C", n_events=240, n_orders_per_event=10, mode="TICK"
    )
    ...
    print("=" * 80)
    profile_output = run_cprofile_scenario(
        n_events=2500, n_orders_per_event=10, mode="BAR"
    )
    ...
    print(
        f"    Rust 预估节省: {elapsed_b_full*0.9:.3f}s ({elapsed_b_full*0.9/60:.2f}min)"
    )
