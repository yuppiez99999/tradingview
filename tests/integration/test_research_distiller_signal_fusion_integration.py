...
class TestResearchDistillerSignalFusionIntegration:
    ...
    def test_load_snapshot_then_inject_then_fuse(self, distiller, engine):
        ...
        # 融合
        result = engine.fuse(
            alpha_signals={"600276.SH": {"strength": 0.5, "confidence": 0.8}}
        )
        r = result[0]
        assert r.meta["research_distilled_applied"] is True
        assert r.sources["research_distilled_strength"] == 0.7

    def test_to_signal_map_then_inject(
        self, distiller, engine, sample_distilled_signals
    ):
...
class TestResearchDistillerPersistenceRoundTrip:
    ...
    def test_save_load_roundtrip_preserves_signals(
        self, distiller, sample_distilled_signals
    ):
        """save → load 保留信号值"""
...
class TestMultiSourceCoexistence:
    ...
    def test_research_and_pipeline_coexist(
        self, distiller, engine, sample_distilled_signals
    ):
        """研究蒸馏 + Pipeline 因子共存"""
        # 研究蒸馏信号
