...
class TestConsumeFillsFromStore(unittest.TestCase):
    ...
    def test_consume_fills_basic(self):
        ...
        account = ShadowAccount(
            account_id="test_basic", strategy_id="build", initial_capital=1_000_000
        )
        ...
        with patch(
            ...
            result = account.consume_fills_from_store(
                ["2026-08-20", "2026-08-21"], strategies=("build",)
            )
    ...
    def test_consume_fills_empty_dates(self):
        ...
        account = ShadowAccount(
            account_id="test_empty", strategy_id="build", initial_capital=500_000
        )
    ...
    def test_consume_fills_no_matching_date(self):
        ...
        account = ShadowAccount(
            account_id="test_nomatch", strategy_id="build", initial_capital=500_000
        )
        with patch(
            "utils.execution.fills_store.FillsStore.load_day",
            return_value=[],
    ...
    def test_consume_fills_strategy_filter(self):
        ...
        account = ShadowAccount(
            account_id="test_filter", strategy_id="build", initial_capital=1_000_000
        )
        ...
        with patch(
            "utils.execution.fills_store.FillsStore.load_day", side_effect=mock_load_day
        ):
            result = account.consume_fills_from_store(
                ["2026-08-20"], strategies=("build",)
            )
    ...
    def test_consume_fills_sell_reduces_holding(self):
        ...
        account = ShadowAccount(
            account_id="test_sell", strategy_id="build", initial_capital=1_000_000
        )
    ...
    def test_consume_fills_terminated_account(self):
        ...
        account = ShadowAccount(
            account_id="test_term", strategy_id="build", initial_capital=500_000
        )
...
class TestFillsStoreStrategyFilter(unittest.TestCase):
    ...
    def test_load_day_strategies_filter(self):
        ...
        with patch.object(
            FillsStore, "load_day", return_value=mock_records
        ) as mock_method:
            FillsStore()
    ...
    def test_load_day_strategies_none_returns_all(self):
        ...
        filtered = (
            mock_records
            if True
            else [r for r in mock_records if r.get("strategy") in strategies_set]
        )
        self.assertEqual(len(filtered), 2)
...
class TestBridgeStrategyFilter(unittest.TestCase):
    ...
    def test_augment_market_prices_strategies_param_exists(self):
        ...
        with patch(
            "utils.execution.fills_store.FillsStore.latest_avg_price_by_symbol",
            return_value={},
        ):
    ...
    def test_realized_pnl_strategies_param_exists(self):
        ...
        with patch(
            "utils.execution.fills_store.FillsStore.realized_pnl",
