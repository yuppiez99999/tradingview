...
class TestTradeRecord:
    def test_construct(self):
        tr = TradeRecord(
            date=pd.Timestamp("2020-01-01"), code="000001", side="BUY",
            qty=100, price=10.0, notional=1000.0,
            commission=0.25, stamp_duty=0.0, transfer_fee=0.02,
            market_impact=1.0, total_cost=1.27,
        )
...
class TestCostAwareBacktestInit:
    ...
    def test_custom_init(self):
        bt = CostAwareBacktest(initial_capital=1_000_000, commission_rate=0.0005,
                               min_cost_bps=5.0)
...
class TestComputeTradeCost:
    ...
    def test_market_impact_with_cost_model(self):
        bt = CostAwareBacktest()
        result = bt.compute_trade_cost(notional=10_000.0, side="BUY", qty=100,
                                        daily_volume=1_000_000,
                                        volatility=0.02, price=100.0)
    ...
    def test_no_market_impact_when_qty_zero(self):
        bt = CostAwareBacktest()
        result = bt.compute_trade_cost(notional=100_000.0, side="BUY", qty=0,
                                        daily_volume=1_000_000, price=100.0)
        assert result["market_impact"] == 0.0

    def test_no_market_impact_when_volume_zero(self):
        bt = CostAwareBacktest()
        result = bt.compute_trade_cost(notional=100_000.0, side="BUY", qty=100,
                                        daily_volume=0, price=100.0)
        assert result["market_impact"] == 0.0
    ...
    def test_keys_present(self):
        bt = CostAwareBacktest()
        result = bt.compute_trade_cost(notional=1000.0, side="BUY")
        for key in ("commission", "stamp_duty", "transfer_fee",
...
class TestRunStrategy:
    def _make_data(self, n_days=60, n_codes=2):
        ...
        weights = pd.DataFrame(
            [[0.5, 0.3]] * n_days, index=dates, columns=codes,
        )
    ...
    def test_zero_target_weights(self):
        ...
        codes = ["000001"]
        prices = pd.DataFrame(
            np.random.uniform(9, 11, (30, 1)), index=dates, columns=codes,
        )
...
class TestCompareWithNoCost:
    def test_compare_keys(self):
        ...
        codes = ["000001", "000002"]
        prices = pd.DataFrame(
            np.random.uniform(9, 11, (60, 2)), index=dates, columns=codes,
        )
        weights = pd.DataFrame([[0.5, 0.3]] * 60, index=dates, columns=codes)
        bt = CostAwareBacktest(initial_capital=1_000_000)
        result = bt.run_strategy(prices, weights)
        cmp = bt.compare_with_no_cost(result)
        for key in ("total_return_with_cost", "total_return_without_cost",
                    "cost_drag", "cost_breakdown", "sharpe_with_cost",
                    "turnover", "n_trades", "avg_cost_bps"):
            assert key in cmp
    ...
    def test_cost_drag_positive(self):
        ...
        codes = ["000001"]
        prices = pd.DataFrame(
            np.random.uniform(9, 11, (60, 1)), index=dates, columns=codes,
        )
