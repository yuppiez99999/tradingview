...
def _router(available: float = 1e9, ntp=None, slip: float = 0.0, **kw) -> SmartOrderRouter:
    return SmartOrderRouter(broker=_AccountBroker(available, slip=slip),
...
def test_execute_buy_iceberg_fills():
    ...
    r = _router()
    fills = r.execute("510300.SH", target_qty=5000, side="BUY",
                      decision_price=4.000, algo="ICEBERG")
    assert len(fills) > 0
...
def test_execute_zero_qty_returns_empty():
    ...
    r = _router()
    assert r.execute("510300.SH", target_qty=0, side="BUY",
                     decision_price=4.0) == []
...
def test_execute_option_sell_margin_block():
    ...
    fills = r.execute("510050C2609M03500.SH", target_qty=10, side="SELL",
                      decision_price=0.05)
    assert len(fills) == 1
...
def test_execute_paused_symbol_skipped():
    ...
    fills = r.execute("510300.SH", target_qty=1000, side="BUY",
                      decision_price=4.0)
    assert len(fills) == 1
...
def test_order_fill_dataclass():
    """OrderFill 数据类字段完整性"""
    of = OrderFill(
        order_id="O1", symbol="X", side="BUY", fill_price=4.0, fill_qty=100,
        decision_price=4.0, slippage=0.0,
        server_ts=datetime.utcnow(), local_ts=datetime.utcnow(),
        ntp_offset=0.0, status="FILLED",
    )
