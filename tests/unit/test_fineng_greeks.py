...
def test_aggregate_stock_futures_option_mixed():
    ...
    holdings = [
        ...
        {"code": "IF.CFE", "qty": 2, "price": 4000.0, "multiplier": 1000.0,
         "type": "FUTURES", "beta": 0.5},
        # 期权: 多头 Put, S=2.75 K=2.50 T=0.25 sigma=0.22 → delta 负
        {"code": "510050P2500M6.SH", "qty": 5, "price": 0.15, "type": "OPTION",
         "S": 2.75, "K": 2.50, "T": 0.25, "sigma": 0.22, "is_call": False,
         "multiplier": 10000},
    ]
    agg = PortfolioGreeksAggregator()
    pg = agg.aggregate(holdings)
    ...
    expected = sum(
        p.delta * (1.0 if p.quantity > 0 else -1.0) * p.multiplier
    ...
    assert pg.theta != 0.0
    # 组合市值 = 股票 + 期货 + 期权
    assert pg.total_market_value == pytest.approx(10000 * 4.0 + 2 * 4000.0 * 1000.0 + 5 * 0.15 * 10000.0)
...
def test_portfolio_greeks_properties():
    ...
    pg = PortfolioGreeks(
        delta=5000.0, gamma=200.0, vega=30000.0, theta=-1500.0,
    ...
    d = pg.to_dict()
    assert set(d.keys()) >= {"delta", "gamma", "vega", "theta", "delta_pct", "daily_theta", "n_positions"}
...
def test_rebalance_signal_all_ok():
    ...
    agg = PortfolioGreeksAggregator()
    pg = PortfolioGreeks(
        delta=0.02 * 1_000_000.0, gamma=0.0, vega=0.0, theta=0.0,
        total_market_value=1_000_000.0,
    )
    sig = agg.rebalance_signal(pg, delta_tolerance=0.05, gamma_tolerance=0.02, vega_tolerance=0.02)
...
def test_rebalance_signal_delta_breach():
    ...
    pg = PortfolioGreeks(delta=0.10 * 1_000_000.0, gamma=0.0, vega=0.0, theta=0.0,
                         total_market_value=1_000_000.0)
    sig = agg.rebalance_signal(pg, delta_tolerance=0.05, gamma_tolerance=0.02, vega_tolerance=0.02)
...
def test_rebalance_signal_vega_breach():
    ...
    agg = PortfolioGreeksAggregator()
    pg = PortfolioGreeks(delta=0.0, gamma=0.0, vega=0.03 * 1_000_000.0, theta=0.0,
                         total_market_value=1_000_000.0)
    sig = agg.rebalance_signal(pg, delta_tolerance=0.05, gamma_tolerance=0.02, vega_tolerance=0.02)
    assert sig["need_rebalance"] is True
