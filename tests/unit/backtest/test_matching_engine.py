...
def test_bar_limit_buy_full_match(recorder: _CallbackRecorder, sample_bar: BarData) -> None:
    """BAR 限价买: order.price >= bar.low → 全成于 max(price, open)。"""
    # Arrange: sample_bar.open=1780, low=1770, high=1860, close=1820, volume=50000
    engine = MatchingEngine(mode="BAR", allow_partial_fill=True, max_participation_rate=1.0)
    order = OrderData(
        order_id="buy-001", code="600519.SH", exchange="SSE",
        direction="BUY", offset="OPEN", order_type="LIMIT",
        price=1800.0, volume=100.0,
    )
...
def test_bar_limit_buy_below_low_no_match(recorder: _CallbackRecorder, sample_bar: BarData) -> None:
    """BAR 限价买: order.price < bar.low → 不成交(挂起)。"""
    engine = MatchingEngine(mode="BAR")
    order = OrderData(
        order_id="buy-002", code="600519.SH", exchange="SSE",
        direction="BUY", order_type="LIMIT",
        price=1700.0, volume=100.0,  # 低于 low=1770
    )

    events = engine.match([order], sample_bar,
                          on_fill=recorder.on_fill, on_reject=recorder.on_reject)
...
def test_bar_market_buy_at_open(recorder: _CallbackRecorder, sample_bar: BarData) -> None:
    """BAR 市价买: 按 bar.open 成交。"""
    engine = MatchingEngine(mode="BAR", max_participation_rate=1.0)
    order = OrderData(
        order_id="buy-003", code="600519.SH", exchange="SSE",
        direction="BUY", order_type="MARKET",
        price=0.0, volume=100.0,
    )
...
def test_bar_partial_fill_with_participation_rate(recorder: _CallbackRecorder) -> None:
    ...
    bar = BarData(
        code="510300.SH", exchange="SSE", period="1d",
        open=4.0, high=4.1, low=3.9, close=4.05,
        volume=1000.0, amount=4000.0, date=20231114, time=0,
    )
    engine = MatchingEngine(mode="BAR", allow_partial_fill=True, max_participation_rate=0.10)
    order = OrderData(
        order_id="buy-004", code="510300.SH", exchange="SSE",
        direction="BUY", order_type="MARKET",
        price=0.0, volume=500.0,  # 需要 500,但 max_volume=100
    )

    events = engine.match([order], bar,
                          on_fill=recorder.on_fill,
                          on_partial_fill=recorder.on_partial_fill)
...
def test_bar_no_partial_fill_when_disabled(recorder: _CallbackRecorder) -> None:
    ...
    engine = MatchingEngine(mode="BAR", allow_partial_fill=False, max_participation_rate=0.10)
    order = OrderData(
        order_id="buy-005", code="510300.SH", exchange="SSE",
        direction="BUY", order_type="MARKET",
        price=0.0, volume=500.0,
    )
...
def test_bar_limit_sell_full_match(recorder: _CallbackRecorder, sample_bar: BarData) -> None:
    """BAR 限价卖: order.price <= bar.high → 全成于 min(price, open)。"""
    engine = MatchingEngine(mode="BAR", max_participation_rate=1.0)
    order = OrderData(
        order_id="sell-001", code="600519.SH", exchange="SSE",
        direction="SELL", offset="CLOSE", order_type="LIMIT",
        price=1850.0, volume=100.0,
    )
...
def test_tick_limit_buy_consumes_ask_levels(recorder: _CallbackRecorder, sample_tick: TickData) -> None:
    ...
    order = OrderData(
        order_id="tick-buy-001", code="600519.SH", exchange="SSE",
        direction="BUY", order_type="LIMIT",
        price=1801.0, volume=100.0,  # 高于 ask[0]=1800.5
    )
...
def test_tick_limit_buy_below_ask_no_match(recorder: _CallbackRecorder, sample_tick: TickData) -> None:
    """TICK 限价买: order.price < ask[0] → 不成交(挂起)。"""
    engine = MatchingEngine(mode="TICK")
    order = OrderData(
        order_id="tick-buy-002", code="600519.SH", exchange="SSE",
        direction="BUY", order_type="LIMIT",
        price=1799.0, volume=100.0,  # 低于 ask[0]=1800.5
    )
...
def test_tick_partial_fill_consumes_five_levels(recorder: _CallbackRecorder) -> None:
    ...
    tick = TickData(
        ...
        price=1800.0, open=1780.0, high=1860.0, low=1770.0, pre_close=1780.0,
        volume=10000.0, amount=18000000.0,
    ...
    order = OrderData(
        order_id="tick-buy-003", code="600519.SH", exchange="SSE",
        direction="BUY", order_type="LIMIT",
        price=1900.0, volume=1000.0,  # 需要 1000,但五档仅 480
    )

    events = engine.match([order], tick,
                          on_fill=recorder.on_fill,
                          on_partial_fill=recorder.on_partial_fill)

    # 部分成交 480 股,加权均价 = (180*1800.5 + 120*1800.6 + 90*1800.7 + 60*1800.8 + 30*1800.9) / 480
    expected_value = (180 * 1800.5 + 120 * 1800.6 + 90 * 1800.7 + 60 * 1800.8 + 30 * 1800.9)
...
def test_tick_market_buy_at_ask(recorder: _CallbackRecorder, sample_tick: TickData) -> None:
    """TICK 市价买: 按 ask[0] 成交。"""
    engine = MatchingEngine(mode="TICK")
    order = OrderData(
        order_id="tick-buy-004", code="600519.SH", exchange="SSE",
        direction="BUY", order_type="MARKET",
        price=0.0, volume=100.0,
    )
...
def test_reject_on_limit_up(recorder: _CallbackRecorder, sample_bar: BarData) -> None:
    ...
    engine = MatchingEngine(mode="BAR", enforce_price_limit=True, max_participation_rate=1.0)
    order = OrderData(
        order_id="buy-limit-up", code="600519.SH", exchange="SSE",
        direction="BUY", order_type="LIMIT",
        price=1820.0, volume=100.0,  # 价格 = close = 1820
    )
...
def test_reject_on_limit_down(recorder: _CallbackRecorder, sample_bar: BarData) -> None:
    ...
    engine = MatchingEngine(mode="BAR", enforce_price_limit=True, max_participation_rate=1.0)
    order = OrderData(
        order_id="sell-limit-down", code="600519.SH", exchange="SSE",
        direction="SELL", order_type="LIMIT",
        price=1820.0, volume=100.0,  # 价格 = close = 1820
    )
...
def test_fok_rejects_partial_fill(recorder: _CallbackRecorder) -> None:
    ...
    engine = MatchingEngine(
        mode="BAR", allow_partial_fill=True,  # 即使允许部分成交
        max_participation_rate=0.10,  # max_volume=100
    )
    order = OrderData(
        order_id="fok-001", code="510300.SH", exchange="SSE",
        direction="BUY", order_type="FOK",  # Fill Or Kill
        price=0.0, volume=500.0,
    )
...
def test_hybrid_mode_auto_selects(
    ...
    tick_order = OrderData(
        order_id="hybrid-tick", code="600519.SH", exchange="SSE",
        direction="BUY", order_type="LIMIT",
        price=1801.0, volume=100.0,
    )
    ...
    # BarData → BAR 模式
    bar_order = OrderData(
        order_id="hybrid-bar", code="600519.SH", exchange="SSE",
        direction="BUY", order_type="LIMIT",
        price=1800.0, volume=100.0,
    )
...
def test_immutability_order_not_modified(
    ...
    engine = MatchingEngine(mode="BAR", max_participation_rate=1.0)
    order = OrderData(
        order_id="immut-001", code="600519.SH", exchange="SSE",
        direction="BUY", order_type="LIMIT",
        price=1800.0, volume=100.0,
        traded_volume=0.0,
        status="NOT_REPORTED",
...
def test_tick_limit_sell_consumes_bid_levels(
    ...
    engine = MatchingEngine(mode="TICK")
    order = OrderData(
        order_id="tick-sell-001", code="600519.SH", exchange="SSE",
        direction="SELL", offset="CLOSE", order_type="LIMIT",
        price=1799.0, volume=100.0,  # 低于 bid[0]=1799.5
    )
...
def test_tick_limit_sell_above_bid_no_match(
    ...
    engine = MatchingEngine(mode="TICK")
    order = OrderData(
        order_id="tick-sell-002", code="600519.SH", exchange="SSE",
        direction="SELL", order_type="LIMIT",
        price=1800.0, volume=100.0,  # 高于 bid[0]=1799.5
    )
...
def test_tick_market_sell_at_bid(
    ...
    engine = MatchingEngine(mode="TICK")
    order = OrderData(
        order_id="tick-sell-003", code="600519.SH", exchange="SSE",
...
def test_tick_sell_partial_fill_consumes_bid_levels(
    ...
    engine = MatchingEngine(mode="TICK", allow_partial_fill=True)
    order = OrderData(
        order_id="tick-sell-004", code="600519.SH", exchange="SSE",
        direction="SELL", order_type="LIMIT",
        price=1799.0, volume=1000.0,  # 需要 1000,但五档仅 580
    )

    events = engine.match([order], sample_tick,
                          on_partial_fill=recorder.on_partial_fill)

    expected_value = (200 * 1799.5 + 150 * 1799.4 + 100 * 1799.3
                      + 80 * 1799.2 + 50 * 1799.1)
...
def test_tick_sell_no_liquidity_rejects(
    ...
    tick = TickData(
        ...
        ask_prices=[1800.5], ask_volumes=[100],
        timestamp=1700000000.0, datetime_str="2023-11-14 10:00:00",
        date=20231114, time=100000,
    )
    engine = MatchingEngine(mode="TICK")
    order = OrderData(
        order_id="tick-sell-005", code="600519.SH", exchange="SSE",
        direction="SELL", order_type="MARKET",
        price=0.0, volume=100.0,
    )
...
def test_tick_buy_no_ask_liquidity_rejects(
    ...
    tick = TickData(
        ...
        price=1800.0, open=1780.0, high=1860.0, low=1770.0, pre_close=1780.0,
        volume=10000.0, amount=18000000.0,
        bid_prices=[1799.5], bid_volumes=[100],
    ...
    engine = MatchingEngine(mode="TICK")
    order = OrderData(
        order_id="tick-buy-005", code="600519.SH", exchange="SSE",
        direction="BUY", order_type="MARKET",
        price=0.0, volume=100.0,
    )
...
def test_tick_market_buy_zero_ask_volume_rejects(
    ...
    tick = TickData(
        ...
        volume=10000.0, amount=18000000.0,
        bid_prices=[1799.5], bid_volumes=[100],
        ask_prices=[1800.5], ask_volumes=[0],  # 价位有,量为 0
        timestamp=1700000000.0, datetime_str="2023-11-14 10:00:00",
        date=20231114, time=100000,
    )
    engine = MatchingEngine(mode="TICK")
    order = OrderData(
        order_id="tick-buy-006", code="600519.SH", exchange="SSE",
        direction="BUY", order_type="MARKET",
        price=0.0, volume=100.0,
    )
...
def test_bar_limit_sell_above_high_no_match(
    ...
    engine = MatchingEngine(mode="BAR", max_participation_rate=1.0)
    order = OrderData(
        order_id="sell-002", code="600519.SH", exchange="SSE",
        direction="SELL", order_type="LIMIT",
        price=1900.0, volume=100.0,  # 高于 high=1860
    )
...
def test_bar_market_sell_at_open(
    ...
    engine = MatchingEngine(mode="BAR", max_participation_rate=1.0)
    order = OrderData(
        order_id="sell-003", code="600519.SH", exchange="SSE",
        direction="SELL", order_type="MARKET",
        price=0.0, volume=100.0,
    )
...
def test_bar_sell_no_volume_rejects(recorder: _CallbackRecorder) -> None:
    ...
    bar = BarData(
        code="510300.SH", exchange="SSE", period="1d",
        open=4.0, high=4.1, low=3.9, close=4.05,
        volume=0.0, amount=0.0, date=20231114, time=0,
    )
    engine = MatchingEngine(mode="BAR", enforce_price_limit=False,
                            max_participation_rate=0.10)
    order = OrderData(
        order_id="sell-004", code="510300.SH", exchange="SSE",
        direction="SELL", order_type="LIMIT",
        price=4.0, volume=100.0,
    )
...
def test_bar_buy_no_volume_rejects(recorder: _CallbackRecorder) -> None:
    ...
    engine = MatchingEngine(mode="BAR", enforce_price_limit=False,
                            max_participation_rate=0.10)
    order = OrderData(
        order_id="buy-006", code="510300.SH", exchange="SSE",
        direction="BUY", order_type="LIMIT",
        price=4.0, volume=100.0,
    )
...
def test_tick_mode_with_bar_event_rejects(
    ...
    engine = MatchingEngine(mode="TICK")
    order = OrderData(
        order_id="mode-001", code="600519.SH", exchange="SSE",
        direction="BUY", order_type="LIMIT",
        price=1800.0, volume=100.0,
    )
...
def test_bar_mode_with_tick_event_rejects(
    ...
    engine = MatchingEngine(mode="BAR")
    order = OrderData(
        order_id="mode-002", code="600519.SH", exchange="SSE",
        direction="BUY", order_type="LIMIT",
        price=1800.0, volume=100.0,
    )
...
def test_hybrid_tick_without_ask_rejects(recorder: _CallbackRecorder) -> None:
    ...
    order = OrderData(
        order_id="mode-003", code="600519.SH", exchange="SSE",
        direction="BUY", order_type="LIMIT",
        price=1800.0, volume=100.0,
    )
...
def test_tick_unknown_direction_rejects(
    ...
    engine = MatchingEngine(mode="TICK")
    order = OrderData(
        order_id="dir-001", code="600519.SH", exchange="SSE",
        direction="HOLD", order_type="LIMIT",  # 非法方向
        price=1800.0, volume=100.0,
    )
...
def test_bar_unknown_direction_rejects(
    ...
    engine = MatchingEngine(mode="BAR", max_participation_rate=1.0)
    order = OrderData(
        order_id="dir-002", code="600519.SH", exchange="SSE",
        direction="HOLD", order_type="LIMIT",
        price=1800.0, volume=100.0,
    )
...
def test_full_fill_without_fill_callback(
    ...
    engine = MatchingEngine(mode="BAR", max_participation_rate=1.0)
    order = OrderData(
        order_id="cb-001", code="600519.SH", exchange="SSE",
        direction="BUY", order_type="LIMIT",
        price=1800.0, volume=100.0,
    )
...
def test_partial_fill_without_partial_callback(
    ...
    engine = MatchingEngine(mode="BAR", allow_partial_fill=True,
                            max_participation_rate=0.10)
    order = OrderData(
        order_id="cb-002", code="600519.SH", exchange="SSE",
        direction="BUY", order_type="MARKET",
        price=0.0, volume=10000.0,  # 需要 10000,但 max_volume=5000
    )
...
def test_reject_without_reject_callback(
    ...
    engine = MatchingEngine(mode="BAR", allow_partial_fill=False,
                            max_participation_rate=0.10)
    order = OrderData(
        order_id="cb-003", code="600519.SH", exchange="SSE",
        direction="BUY", order_type="MARKET",
        price=0.0, volume=10000.0,
    )
...
def test_enforce_price_limit_false_skips_check(
    ...
    order = OrderData(
        order_id="nolimit-001", code="600519.SH", exchange="SSE",
        direction="BUY", order_type="LIMIT",
        price=1820.0, volume=100.0,
    )
    limit_up = {"600519.SH": 1820.0}  # 恰好涨停
...
def test_constraint_reject_without_reject_callback(
    ...
    engine = MatchingEngine(mode="BAR", enforce_price_limit=True,
                            max_participation_rate=1.0)
    order = OrderData(
        order_id="nolimit-002", code="600519.SH", exchange="SSE",
        direction="BUY", order_type="LIMIT",
        price=1820.0, volume=100.0,
    )
...
def test_fak_partial_fill_then_cancel(
    ...
    engine = MatchingEngine(mode="BAR", allow_partial_fill=True,
                            max_participation_rate=0.10)
    order = OrderData(
        order_id="fak-001", code="600519.SH", exchange="SSE",
        direction="BUY", order_type="FAK",
        price=1800.0, volume=10000.0,  # 需要 10000,但 max_volume=5000
    )
