...
def test_option_spec_call_put_flags():
    """is_call / is_put 标志正确"""
    call = OptionSpec(
        underlying="510050.SH", option_type=OptionType.CALL, strike=3.5,
        expiry=date(2026, 6, 25), multiplier=10000,
        exercise_style=ExerciseStyle.EUROPEAN,
    )
    put = OptionSpec(
        underlying="510050.SH", option_type=OptionType.PUT, strike=3.5,
        expiry=date(2026, 6, 25), multiplier=10000,
        exercise_style=ExerciseStyle.EUROPEAN,
    )
...
def test_option_spec_days_to_expiry():
    ...
    opt = OptionSpec(
        underlying="510050.SH", option_type=OptionType.CALL, strike=3.5,
        expiry=exp, multiplier=10000, exercise_style=ExerciseStyle.EUROPEAN,
    )
