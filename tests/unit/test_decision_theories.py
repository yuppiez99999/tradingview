...
class TestSorosReflexivityEngine:
    ...
    def test_high_reflexivity_sell_signal(self):
        ...
        result = engine.compute_reflexivity_score(
            ...
            volume_data={
                "TEST": {"current_vol": 10000, "avg_vol_20d": 3000}
            },  # vol_ratio=3.33 → vol_score=1.0
            valuation_data={
                "TEST": {"pe_ttm": 60, "pe_historical_median": 30}
            },  # pe_dev=1.0 → val_score=1.0
            sentiment_data={
                "TEST": {"sentiment_score": 0.9, "narrative_strength": 0.9}
            },  # sent_score=0.9
        )
...
class TestTheoryFusionEngine:
    ...
    def test_hold_signal_threshold(self):
        ...
        decisions = [
            TheoryDecision(
                theory="t1", signal="HOLD", score=0.5, conviction="MEDIUM", summary="A"
            ),
            TheoryDecision(
                theory="t2", signal="HOLD", score=0.5, conviction="MEDIUM", summary="B"
            ),
        ]
        result = engine.fuse_decisions(decisions)
...
class TestReexportCompat:
    ...
    @pytest.mark.skip(
        reason="v8.3_institutional/src/factors/ 目录已移除, v8.4 统一到 utils/alpha/decision_theories.py (2026-08-16)"
    )
