...
class TestLiteLLMRouterChat:
    def test_chat_success(
        self, router: LiteLLMRouter, mock_inner_router: MagicMock
    ) -> None:
    ...
    def test_chat_scene_temperature(
        self, router: LiteLLMRouter, mock_inner_router: MagicMock
    ) -> None:
        """场景路由自动选择温度."""
    ...
    def test_chat_explicit_temperature(
        self, router: LiteLLMRouter, mock_inner_router: MagicMock
    ) -> None:
        """显式温度覆盖场景温度."""
    ...
    def test_glm5_client_test_connection(self) -> None:
        ...
        mock_router.chat.return_value = ChatResponse(
            ...
            provider=ProviderInfo(
                name="deepseek", model="deepseek-chat", latency_ms=50.0
            ),
        )
        client = GLM5Client()
        client._router = mock_router
