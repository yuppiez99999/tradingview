...
def provider(tmp_path: Path) -> MarketDataProvider:
    ...
    with patch.object(
        MarketDataProvider,
        "__init__",
        lambda self, cache_size=1000, backtest_mode=False: None,
    ):
...
class TestInitAndConfig:
    ...
    def test_init_default(self, tmp_path: Path) -> None:
        with (
            patch.object(MarketDataProvider, "_init_wind_mcp"),
    ...
    def test_cache_suffix_backtest_with_date(
    ...
    def test_cache_suffix_backtest_without_date(
        self, provider: MarketDataProvider
    ) -> None:
        provider.backtest_mode = True
        provider._backtest_date = None
    ...
    def test_init_wind_mcp_missing_file(
        self,
        provider: MarketDataProvider,
        tmp_path: Path,
    ...
    def test_init_wind_mcp_spec_failure(
        self,
        provider: MarketDataProvider,
        monkeypatch: pytest.MonkeyPatch,
        caplog: pytest.LogCaptureFixture,
        ...
        assert (
            "无法创建 importlib spec"
            in provider.source_health["wind_mcp"]["last_error"]
        )

    def test_init_wind_mcp_missing_functions(
        self,
        provider: MarketDataProvider,
        tmp_path: Path,
        monkeypatch: pytest.MonkeyPatch,
        ...
        monkeypatch.setattr(
            importlib.util, "spec_from_file_location", lambda *args, **kwargs: fake_spec
        )
        monkeypatch.setattr(
            importlib.util, "module_from_spec", lambda spec: fake_module
        )
        with caplog.at_level(logging.WARNING):
            provider._init_wind_mcp()
        assert provider._wind_mcp_client is None
    ...
    def test_init_wind_mcp_load_exception(
        self,
        provider: MarketDataProvider,
        monkeypatch: pytest.MonkeyPatch,
        caplog: pytest.LogCaptureFixture,
        ...
        monkeypatch.setattr(Path, "is_file", lambda self: True)
        monkeypatch.setattr(
            importlib.util, "spec_from_file_location", lambda *args, **kwargs: boom()
        )
        with caplog.at_level(logging.WARNING):
            provider._init_wind_mcp()
        assert provider._wind_mcp_client is None
    ...
    def test_init_tdx_import_error(
        self,
        provider: MarketDataProvider,
        monkeypatch: pytest.MonkeyPatch,
        caplog: pytest.LogCaptureFixture,
    ...
    def test_init_tdx_init_failure(
        self,
        provider: MarketDataProvider,
        tmp_path: Path,
        monkeypatch: pytest.MonkeyPatch,
    ...
    def test_init_tdx_runtime_error(
        self,
        provider: MarketDataProvider,
        tmp_path: Path,
        monkeypatch: pytest.MonkeyPatch,
        ...
        fake_module = type(sys)("utils.tdx_data_source")
        fake_module.get_tdx_source = lambda: (_ for _ in ()).throw(
            RuntimeError("tdx boom")
        )
        monkeypatch.setitem(sys.modules, "utils.tdx_data_source", fake_module)
        with caplog.at_level(logging.WARNING):
            provider._init_tdx()
    ...
    def test_init_akshare_import_error(
        self,
        provider: MarketDataProvider,
        monkeypatch: pytest.MonkeyPatch,
        caplog: pytest.LogCaptureFixture,
...
class TestRealtimeFlow:
    ...
    def test_fetch_real_time_data_wind_success(
        self, provider: MarketDataProvider
    ) -> None:
        provider._wind_mcp_client = {
            "quote": lambda *args, **kwargs: {
                "price": 100.0,
                "prev_close": 99.0,
                "open": 99.5,
                "high": 101.0,
    ...
    def test_fetch_real_time_data_wind_empty(
        self, provider: MarketDataProvider
    ) -> None:
    ...
    def test_fetch_real_time_data_wind_exception(
        self, provider: MarketDataProvider
    ) -> None:
        provider._wind_mcp_client = {
            "quote": lambda *args, **kwargs: (_ for _ in ()).throw(
                RuntimeError("wind error")
            )
        }
        provider._tdx_source = None
        provider._akshare_source = None
    ...
    def test_fetch_real_time_data_tdx_success(
        self, provider: MarketDataProvider
    ) -> None:
        provider._wind_mcp_client = {"quote": lambda *args, **kwargs: {}}
        tdx = MagicMock()
    ...
    def test_fetch_real_time_data_akshare_success(
        self, provider: MarketDataProvider
    ) -> None:
        provider._wind_mcp_client = {"quote": lambda *args, **kwargs: {}}
        provider._tdx_source = None
    ...
    def test_fetch_real_time_data_sina_success(
        self, provider: MarketDataProvider
    ) -> None:
        ...
        provider._akshare_source = None
        with patch.object(
            provider, "_try_sina_http_realtime", return_value=sina_data
        ) as mock_sina:
            data = provider._fetch_real_time_data("600519")
    ...
    def test_fetch_real_time_data_sina_empty_payload(
        self, provider: MarketDataProvider
    ) -> None:
        provider._wind_mcp_client = {"quote": lambda *args, **kwargs: {}}
        provider._tdx_source = None
    ...
    def test_try_wind_mcp_realtime_success(self, provider: MarketDataProvider) -> None:
        provider._wind_mcp_client = {
            "quote": lambda *args, **kwargs: {"price": 100.0, "prev_close": 99.0}
        }
    ...
    def test_try_wind_mcp_realtime_exception(
        self, provider: MarketDataProvider
    ) -> None:
        provider._wind_mcp_client = {
            "quote": lambda *args, **kwargs: (_ for _ in ()).throw(RuntimeError("boom"))
        }
        data = provider._try_wind_mcp_realtime("600519")
        assert data is None
        assert provider.source_health["wind_mcp"]["ok"] is False
    ...
    def test_try_sina_http_realtime_zero_price(
        self, provider: MarketDataProvider
    ) -> None:
        fake_resp = MagicMock()
        fake_resp.text = (
            'var hq_str_sh600519="贵州茅台,0.0,99.0,0.0,101.0,99.0,0.0,0,0,0";'
        )
        fake_resp.encoding = "gbk"
        with patch("utils.data_provider._SINA_SESSION") as mock_session:
            mock_session.get.return_value = fake_resp
    ...
    def test_try_sina_http_realtime_insufficient_fields(
        self, provider: MarketDataProvider
    ) -> None:
        fake_resp = MagicMock()
        ...
        assert data is None
        assert (
            provider.source_health["sina_http"]["last_error"] == "insufficient_fields"
        )

    def test_try_sina_http_realtime_no_match(
        self, provider: MarketDataProvider
    ) -> None:
        fake_resp = MagicMock()
        fake_resp.text = 'var hq_str_sh600519="";'
    ...
    def test_get_market_data_fetch_failure(self, provider: MarketDataProvider) -> None:
        with patch.object(
            provider, "_fetch_real_time_data", side_effect=RuntimeError("fail")
        ):
    ...
    def test_get_market_data_default_symbol(self, provider: MarketDataProvider) -> None:
        with patch.object(
            provider, "_fetch_real_time_data", return_value={"index_price": 3000.0}
        ) as mock_fetch:
...
class TestHistoricalFlow:
    ...
    def test_fetch_historical_data_wind_success(
        self, provider: MarketDataProvider
    ) -> None:
        provider._wind_mcp_client = {
            "kline": lambda *args, **kwargs: [
                {
                    "date": "2026-01-01",
                    "close": 100.0,
                    "open": 99.0,
    ...
    def test_fetch_historical_data_wind_empty(
        self, provider: MarketDataProvider
    ) -> None:
    ...
    def test_fetch_historical_data_tdx_success(
        self, provider: MarketDataProvider
    ) -> None:
        provider._wind_mcp_client = {"kline": lambda *args, **kwargs: []}
        tdx = MagicMock()
    ...
    def test_fetch_historical_data_akshare_success(
        self, provider: MarketDataProvider
    ) -> None:
        provider._wind_mcp_client = {"kline": lambda *args, **kwargs: []}
        provider._tdx_source = None
    ...
    def test_fetch_historical_data_sina_success(
        self, provider: MarketDataProvider
    ) -> None:
        provider._wind_mcp_client = {"kline": lambda *args, **kwargs: []}
        provider._tdx_source = None
    ...
    def test_fetch_historical_data_sina_null_payload(
        self, provider: MarketDataProvider
    ) -> None:
        provider._wind_mcp_client = {"kline": lambda *args, **kwargs: []}
        provider._tdx_source = None
    ...
    def test_try_wind_mcp_historical_success(
        self, provider: MarketDataProvider
    ) -> None:
        provider._wind_mcp_client = {
            "kline": lambda *args, **kwargs: [
    ...
    def test_try_wind_mcp_historical_empty_after_parse(
        self, provider: MarketDataProvider
    ) -> None:
        provider._wind_mcp_client = {
            "kline": lambda *args, **kwargs: [
    ...
    def test_try_sina_http_historical_success(
        self, provider: MarketDataProvider
    ) -> None:
        payload = json.dumps(
            [{"day": "2026-01-01", "close": "100.0", "volume": "1000"}]
        )
        fake_resp = MagicMock()
        fake_resp.text = payload
        fake_resp.raise_for_status.return_value = None
    ...
    def test_get_historical_data_persistent_cache_hit(
        self, provider: MarketDataProvider
    ) -> None:
        pytest.importorskip("pyarrow")
        pd.DataFrame({"close": [100.0]}).to_parquet(
            provider.persistent_cache_dir / "historical_600519_1y.parquet", index=True
        )
        df = provider.get_historical_data("600519", "1y")
        assert not df.empty

    def test_get_historical_data_persistent_cache_expired(
        self, provider: MarketDataProvider
    ) -> None:
        pytest.importorskip("pyarrow")
        ...
        with patch.object(
            ...
            return_value=pd.DataFrame({"close": [101.0]}),
        ) as mock_fetch:
            df = provider.get_historical_data("600519", "1y")
        assert not df.empty
        mock_fetch.assert_called_once_with("600519", "1y")

    def test_get_historical_data_fetch_failure(
        self, provider: MarketDataProvider
    ) -> None:
        with patch.object(
            provider, "_fetch_historical_data", side_effect=RuntimeError("hist fail")
        ):
...
class TestCacheLogic:
    ...
    def test_memory_cache_write_and_evict(self, provider: MarketDataProvider) -> None:
        ...
        for i in range(3):
            with patch.object(
                ...
                return_value={"index_price": 100.0 + i},
            ):
    ...
    def test_persistent_cache_expired_deleted(
        self, provider: MarketDataProvider
    ) -> None:
    ...
    def test_persistent_cache_corrupt_file(
...
class TestSentimentAndTechnical:
    ...
    def test_get_sentiment_data_none(
        self, provider: MarketDataProvider, caplog: pytest.LogCaptureFixture
    ) -> None:
        with caplog.at_level(logging.WARNING):
    ...
    def test_get_technical_indicators_short_data(
        self, provider: MarketDataProvider
    ) -> None:
    ...
    def test_get_technical_indicators_failure(
        self, provider: MarketDataProvider, caplog: pytest.LogCaptureFixture
    ) -> None:
    ...
    def test_deprecated_historical_data_raises(
        self, provider: MarketDataProvider
    ) -> None:
        with pytest.raises(RuntimeError, match="已废弃"):
            provider._get_default_historical_data()

    def test_deprecated_sentiment_data_raises(
        self, provider: MarketDataProvider
    ) -> None:
        with pytest.raises(RuntimeError, match="已废弃"):
...
class TestTechnicalIndicatorsDetail:
    ...
    def test_calculate_technical_indicators_full(
        self, provider: MarketDataProvider
    ) -> None:
    ...
    def test_calculate_technical_indicators_exception(
        self, provider: MarketDataProvider, caplog: pytest.LogCaptureFixture
    ) -> None:
...
class TestModuleHelpers:
    ...
    def test_get_market_data_convenience(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        monkeypatch.setattr(
            "utils.data_provider._data_provider", fake_provider, raising=False
        )
    ...
    def test_get_historical_data_convenience(
        self, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        fake_provider = MagicMock()
        fake_provider.get_historical_data.return_value = pd.DataFrame(
            {"close": [100.0]}
        )
        import utils.data_provider as dp_mod
    ...
    def test_get_price_prediction_short_history(
        self, provider: MarketDataProvider, caplog: pytest.LogCaptureFixture
    ) -> None:
        hist = pd.DataFrame({"close": [100.0, 101.0]})
        with (
            patch.object(provider, "get_historical_data", return_value=hist),
            caplog.at_level(logging.WARNING),
        ):
            result = provider.get_price_prediction("600519", horizon=5)
        assert result == {}

    def test_get_price_prediction_failure(
        self, provider: MarketDataProvider, caplog: pytest.LogCaptureFixture
    ) -> None:
        ...
        with patch.object(provider, "get_historical_data", return_value=hist):
            with patch.dict(
                sys.modules,
                {
                    "utils.tf_price_predictor": MagicMock(
                        PricePredictor=MagicMock(return_value=fake_predictor)
                    )
                },
            ):
                with caplog.at_level(logging.WARNING):
    ...
    def test_get_external_macro_failure(
        self, provider: MarketDataProvider, caplog: pytest.LogCaptureFixture
    ) -> None:
        with patch(
    ...
    def test_get_risk_sentiment_failure(
        self, provider: MarketDataProvider, caplog: pytest.LogCaptureFixture
    ) -> None:
        with patch(
            "utils.external_data_source.ExternalDataManager",
    ...
    def test_get_news_sentiment_success(self, provider: MarketDataProvider) -> None:
        ...
        with patch.dict(
            sys.modules,
            {
                "utils.web_scraper": MagicMock(
                    WebScraper=MagicMock(
                        return_value=MagicMock(
                            fetch_announcements=MagicMock(return_value=fake_news)
                        )
                    )
                ),
                "utils.ai_report_agent": MagicMock(
                    AIReportAgent=MagicMock(
                        return_value=MagicMock(
                            analyze_news_sentiment=MagicMock(
                                return_value=fake_sentiments
                            )
                        )
                    )
                ),
        ...
        with patch.dict(
            sys.modules,
            {
                "utils.web_scraper": MagicMock(
                    WebScraper=MagicMock(
                        return_value=MagicMock(
                            fetch_announcements=MagicMock(return_value="bad")
                        )
                    )
                ),
            },
        ...
        with patch.dict(
            sys.modules,
            {
                "utils.ai_report_agent": MagicMock(
                    AIReportAgent=MagicMock(
                        return_value=MagicMock(
                            generate_daily_report=MagicMock(return_value=fake_report)
                        )
                    )
                ),
            },
    ...
    def test_get_hfq_factor_failure(self, provider: MarketDataProvider) -> None:
        with patch.dict(
            ...
            {
                "utils.adjust_factor_provider": MagicMock(
                    get_adjust_factor_provider=MagicMock(
                        side_effect=RuntimeError("hfq fail")
                    )
                )
            },
        ):
    ...
    def test_enrich_realtime_with_hfq_success(
        self, provider: MarketDataProvider
    ) -> None:
        ...
        with patch.dict(
            sys.modules,
            {
                "utils.adjust_factor_provider": MagicMock(
                    get_adjust_factor_provider=MagicMock(return_value=fake_provider),
                    unadjusted_to_hfq=MagicMock(
                        side_effect=lambda price, factor: price * factor
                    ),
                )
            },
        ):
    ...
    def test_enrich_realtime_with_hfq_failure(
        self, provider: MarketDataProvider
    ) -> None:
        quote = {"index_price": 100.0}
        with patch.dict(
            sys.modules,
            {
                "utils.adjust_factor_provider": MagicMock(
                    get_adjust_factor_provider=MagicMock(
                        side_effect=RuntimeError("hfq enrich fail")
                    )
                )
            },
        ):
...
class TestErrorLogging:
    ...
    def test_fetch_real_time_data_logs_all_sources_failed(
        self, provider: MarketDataProvider, caplog: pytest.LogCaptureFixture
    ) -> None:
        provider._wind_mcp_client = {"quote": lambda *args, **kwargs: {}}
        provider._tdx_source = None
        ...
        fake_resp.encoding = "gbk"
        with (
            patch("utils.data_provider._SINA_SESSION") as mock_session,
            caplog.at_level(logging.WARNING),
        ):
            mock_session.get.return_value = fake_resp
    ...
    def test_fetch_historical_data_logs_all_sources_failed(
        self, provider: MarketDataProvider, caplog: pytest.LogCaptureFixture
    ) -> None:
        provider._wind_mcp_client = {"kline": lambda *args, **kwargs: []}
        provider._tdx_source = None
    ...
    def test_source_health_updated_on_failure(
        self, provider: MarketDataProvider
    ) -> None:
        provider._wind_mcp_client = {
            "quote": lambda *args, **kwargs: (_ for _ in ()).throw(
                RuntimeError("wind fail")
            )
        }
