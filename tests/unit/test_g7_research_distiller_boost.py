...
class TestDistilledSignalValidation:
    ...
    def test_to_dict_roundtrip(self):
        s = DistilledSignal(symbol="600276.SH", strength=0.6, confidence=0.8, key_factors=["a", "b"])
        d = s.to_dict()
    ...
    def test_from_dict_roundtrip(self):
        original = DistilledSignal(symbol="000001.SZ", strength=0.5, confidence=0.9, key_factors=["x"])
...
class TestRuleDistill:
    def test_bullish_keywords(self, distiller_default):
        signals = distiller_default._rule_distill("600276 业绩超预期, 净利润大增, 强烈推荐", "report", "test")
        assert len(signals) == 1
        assert signals[0].strength > 0

    def test_bearish_keywords(self, distiller_default):
        signals = distiller_default._rule_distill("000001 业绩预警, 亏损, 减持", "report", "test")
        assert len(signals) == 1
        assert signals[0].strength < 0

    def test_critical_negative_triggers_strong_signal(self, distiller_default):
        signals = distiller_default._rule_distill("600276 公司被立案调查, 财务造假", "news", "test")
        assert len(signals) == 1
    ...
    def test_no_keywords_returns_weak_signal(self, distiller_default):
        signals = distiller_default._rule_distill("600276 这是一条没有任何信号词的普通文本内容", "news", "test")
        assert len(signals) == 1
        assert signals[0].confidence == 0.3

    def test_forced_symbol_overrides_ner(self, distiller_default):
        signals = distiller_default._rule_distill("无标的文本", "news", "test", forced_symbol="600276.SH")
        assert len(signals) == 1
...
class TestDistillNewsBatch:
    def test_single_positive(self, distiller_default):
        signals = distiller_default.distill_news_batch([
            {"title": "恒瑞医药业绩超预期", "content": "净利润增长30%", "symbol": "600276.SH"},
        ])
        assert len(signals) == 1
        assert signals[0].symbol == "600276.SH"
        assert signals[0].strength > 0
    ...
    def test_single_negative(self, distiller_default):
        signals = distiller_default.distill_news_batch([
            {"title": "某公司爆雷", "content": "财务造假, 被立案调查", "symbol": "000001.SZ"},
        ])
        assert len(signals) == 1
        assert signals[0].strength < 0
...
class TestSignalAggregation:
    ...
    def test_save_load_snapshot_roundtrip(self, distiller_default, tmp_path):
        signals = [
            DistilledSignal(symbol="600276.SH", strength=0.5, confidence=0.8, valid_until="2099-01-01"),
        ]
...
class TestSnapshotEdgeCases:
    ...
    def test_load_by_dash_date(self, distiller_default):
        signals = [DistilledSignal(symbol="600276.SH", strength=0.5, valid_until="2099-01-01")]
...
class TestKeywordAndFreshness:
    def test_rule_distill_bullish_keywords(self, distiller_default):
        signals = distiller_default.distill_news_batch([
            {"title": "业绩超预期", "content": "净利润大增, 强烈推荐", "symbol": "600276.SH"},
        ])
        assert len(signals) == 1
        assert signals[0].strength > 0
    ...
    def test_rule_distill_bearish_keywords(self, distiller_default):
        signals = distiller_default.distill_news_batch([
            {"title": "业绩预警", "content": "业绩不及预期, 毛利率下滑", "symbol": "000001.SZ"},
        ])
        assert len(signals) == 1
        assert signals[0].strength < 0
