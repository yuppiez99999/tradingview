...
class TestCheckKillSwitchInactive:
    ...
    def test_no_state_file_returns_true(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setattr("phase_b_progressive_enabler._KILL_SWITCH_STATE", tmp_path / "nonexistent.json")
        ok, reason = _check_kill_switch_inactive("2026-08-20")
        assert ok is True
        assert reason == ""

    def test_triggered_on_date_returns_false(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        state_file = tmp_path / "kill_switch_state.json"
        state_file.write_text(json.dumps({
            "USE_DRIFT_DETECTOR": {"triggered_date": "2026-08-20"},
        }), encoding="utf-8")
        monkeypatch.setattr("phase_b_progressive_enabler._KILL_SWITCH_STATE", state_file)
        ok, reason = _check_kill_switch_inactive("2026-08-20")
        assert ok is False
        assert "USE_DRIFT_DETECTOR" in reason

    def test_triggered_on_different_date_returns_true(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        state_file = tmp_path / "kill_switch_state.json"
        state_file.write_text(json.dumps({
...
class TestCheckNoLookaheadBias:
    ...
    def test_no_report_returns_true(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setattr("phase_b_progressive_enabler._HONEST_VALIDATION_DIR", tmp_path)
        ok, reason = _check_no_lookahead_bias("2026-08-20")
        assert ok is True
        assert reason == ""

    def test_lookahead_detected_returns_false(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        report = tmp_path / "2026-08-20.json"
        report.write_text(json.dumps({"lookahead_bias_detected": True}), encoding="utf-8")
        monkeypatch.setattr("phase_b_progressive_enabler._HONEST_VALIDATION_DIR", tmp_path)
        ok, reason = _check_no_lookahead_bias("2026-08-20")
        assert ok is False
        assert "lookahead" in reason

    def test_no_lookahead_returns_true(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        report = tmp_path / "2026-08-20.json"
        report.write_text(json.dumps({"lookahead_bias_detected": False}), encoding="utf-8")
        monkeypatch.setattr("phase_b_progressive_enabler._HONEST_VALIDATION_DIR", tmp_path)
        ok, reason = _check_no_lookahead_bias("2026-08-20")
        assert ok is True
...
class TestEvaluateDailyShadowHealth:
    ...
    def _setup_shadow_data(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch,
        ...
        monkeypatch.setattr("phase_b_progressive_enabler._KILL_SWITCH_STATE", tmp_path / "no_ks.json")
        monkeypatch.setattr("phase_b_progressive_enabler._HONEST_VALIDATION_DIR", tmp_path / "hv")

    def test_data_missing(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setattr("phase_b_progressive_enabler.OBSERVATION_DATA", tmp_path / "nonexistent.jsonl")
        verdict = evaluate_daily_shadow_health("2026-08-20")
        assert verdict.healthy is False
        assert verdict.reason == "shadow_data_missing"

    def test_sample_insufficient(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    def test_no_daily_return_for_date(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        records = [{"date": f"2026-08-{i:02d}", "daily_return": 0.01} for i in range(1, 21)]
        self._setup_shadow_data(tmp_path, monkeypatch, records)
        verdict = evaluate_daily_shadow_health("2026-08-25")
        assert verdict.healthy is False
        assert verdict.reason == "no_daily_return"

    def test_healthy_when_all_three_green(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        records = [{"date": f"2026-08-{i:02d}", "daily_return": 0.001} for i in range(1, 21)]
    ...
    def test_kill_switch_triggered(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        records = [{"date": f"2026-08-{i:02d}", "daily_return": 0.001} for i in range(1, 21)]
        self._setup_shadow_data(tmp_path, monkeypatch, records)
        ks_file = tmp_path / "kill_switch_state.json"
        ks_file.write_text(json.dumps({"USE_DRIFT_DETECTOR": {"triggered_date": "2026-08-20"}}), encoding="utf-8")
...
class TestUpdateStableDays:
    ...
    def test_healthy_increments(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    def test_unhealthy_resets_to_zero(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setattr("phase_b_progressive_enabler._SHADOW_DAILY_HEALTH_LOG", tmp_path / "health.jsonl")
    ...
    def test_appends_to_health_log(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setattr("phase_b_progressive_enabler._SHADOW_DAILY_HEALTH_LOG", tmp_path / "health.jsonl")
        status = PhaseBStatus(consecutive_stable_days=0)
        verdict = DailyHealthVerdict(date="2026-08-20", healthy=True, reason="ok", daily_return=0.005)
        new_status = update_stable_days(status, verdict)
...
class TestGenerateShadowStableReport:
    ...
    def test_report_generated(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setattr("phase_b_progressive_enabler.PROJECT_ROOT", tmp_path)
        health_log = [
            {"date": f"2026-08-{i:02d}", "healthy": True, "reason": "ok", "daily_return": 0.001}
            for i in range(1, 21)
        ]
        status = PhaseBStatus(
    ...
    def test_target_not_met(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setattr("phase_b_progressive_enabler.PROJECT_ROOT", tmp_path)
        status = PhaseBStatus(
            consecutive_stable_days=3,
            stable_days_target=7,
