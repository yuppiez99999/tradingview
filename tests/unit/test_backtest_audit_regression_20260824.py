...
class TestMarketImpactClamp:
    ...
    def test_large_order_clamped(self):
        ...
        impact = self.cm.market_impact(qty=200_000, daily_volume=100_000,
    ...
    def test_normal_order(self):
        """BT-2: 正常参与率 (1%) 冲击合理."""
        impact = self.cm.market_impact(qty=1000, daily_volume=100_000,
                                       volatility=0.02, price=10.0)
    ...
    def test_cost_aware_simplified_clamp(self):
        ...
        costs = self.cab.compute_trade_cost(
            notional=2_000_000, side="BUY", qty=200_000,
            daily_volume=100_000, volatility=0.02, price=10.0,
        )
...
class TestMarketImpactGuard:
    ...
    def test_negative_qty(self):
        ...
        assert self.cm.market_impact(qty=-1000, daily_volume=100_000,
                                     volatility=0.02, price=10.0) == 0.0

    def test_zero_price(self):
        """BT-8: price<=0 返回 0."""
        assert self.cm.market_impact(qty=1000, daily_volume=100_000,
                                     volatility=0.02, price=0.0) == 0.0
    ...
    def test_zero_daily_volume(self):
        ...
        assert self.cm.market_impact(qty=1000, daily_volume=0,
                                     volatility=0.02, price=10.0) == 0.0
