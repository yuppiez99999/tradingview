...
class TestFetchEtfOptions:
    ...
    def test_fetch_etf_options_empty_whitelist(
        self, fetcher: HedgeToolDataFetcher
    ) -> None:
        # Arrange & Act
    ...
    def test_fetch_etf_options_all_fallback(
        self, fetcher: HedgeToolDataFetcher
    ) -> None:
        # Arrange & Act
    ...
    def test_fetch_etf_options_from_akshare(
        self, fetcher: HedgeToolDataFetcher
    ) -> None:
        # Arrange
        mock_df = MagicMock()
    ...
    def test_fetch_etf_options_strike_missing(
        self, fetcher: HedgeToolDataFetcher
    ) -> None:
...
class TestFetchReverseEtf:
    ...
    def test_fetch_reverse_etf_empty_whitelist(
        self, fetcher: HedgeToolDataFetcher
    ) -> None:
        # Arrange & Act
    ...
    def test_fetch_reverse_etf_all_fallback(
        self, fetcher: HedgeToolDataFetcher
    ) -> None:
        # Arrange & Act
    ...
    def test_fetch_reverse_etf_from_akshare(
        self, fetcher: HedgeToolDataFetcher
    ) -> None:
        # Arrange
        mock_row = MagicMock()
        mock_row.get = MagicMock(
            side_effect=lambda key, default=0: 1.5 if key == "收盘" else default
        )
        mock_df = MagicMock()
        mock_df.__len__ = MagicMock(return_value=1)
...
class TestFallbackStatus:
    ...
    def test_flags_reset_on_each_fetch(self, fetcher: HedgeToolDataFetcher) -> None:
        ...
        # Act — 第二次获取成功，标记应重置
        with patch(
            "utils.hedge_engine.get_live_futures_prices", return_value={"IF": 3900.0}
        ):
            fetcher.fetch_futures(["IF"])
        # Assert
    ...
    def test_fetch_all_success(self, fetcher: HedgeToolDataFetcher) -> None:
        ...
        with patch(
            ...
            result = fetcher.fetch_all(
                futures_codes=["IF"], etf_option_codes=[], reverse_etf_codes=[]
            )
        # Assert
