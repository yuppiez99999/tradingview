...
def test_signal_generation():
    ...
    for s in signals:
        print(
            f"  {s.symbol} ({s.name}): 方向 {s.direction}, 强度 {s.strength:.2f}, 置信 {s.confidence:.2f}"
        )
        print(
            f"    MA20 {s.ma20:.2f}, MA60 {s.ma60:.2f}, RSI {s.rsi:.1f}, MACD {s.macd_hist:+.4f}"
        )
...
def test_position_sizing():
    ...
    print(
        f"  单张保证金 ¥{expected_one_margin:,.0f}, 期望最大 {expected_max_contracts} 张"
    )
...
def test_risk_control():
    ...
    status, pause = trader.check_risk(
        {}, daily_pnl_pct=0.0, weekly_consecutive_loss_pct=0.0
    )
    ...
    # 场景 B: 日亏损 15% → 警告
    status, pause = trader.check_risk(
        {}, daily_pnl_pct=-0.15, weekly_consecutive_loss_pct=0.0
    )
    ...
    # 场景 C: 周连续亏损 25% → 暂停
    status, pause = trader.check_risk(
        {}, daily_pnl_pct=0, weekly_consecutive_loss_pct=0.25
    )
...
def test_order_generation():
    ...
    for o in orders:
        print(
            f"  {o.symbol} {o.action} {o.contracts} 张 @ {o.price:.2f}, 保证金 ¥{o.required_margin:,.0f}"
        )
        assert o.action in (
            "open_long",
            "open_short",
    ...
    orders_b = trader.generate_orders(
        signals, positions, prices, date(2026, 7, 14), "paused"
    )
...
def test_full_workflow():
    ...
    assert (
        result.margin_usage_ratio <= 0.60 + 0.01
    ), f"保证金占用率应 ≤ 60%, 实际 {result.margin_usage_ratio:.1%}"
