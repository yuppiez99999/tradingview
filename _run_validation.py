...
def main():
    print("=" * 70)
    print("修复后真实性能评估: Walk-Forward + Purged CV + Deflated Sharpe")
    print("=" * 70)

    bt = load_backtest_result()
    if not bt:
        print("[FAIL] 无法加载回测结果, 请先运行 research/backtest_runner.py")
        return
    ...
    print(f"验收: {acceptance.get('passed', False)}")

    print("\n" + "-" * 70)
    print("1. Deflated Sharpe Ratio (多重测试修正)")
    print("-" * 70)
    dsr_result = run_deflated_sharpe(monthly_returns)
    print(json.dumps(dsr_result, ensure_ascii=False, indent=2))

    print("\n" + "-" * 70)
    print("2. Purged K-Fold CV (前视偏差隔离)")
    print("-" * 70)
    pcv_result = run_purged_cv_summary(len(records))
    print(json.dumps(pcv_result, ensure_ascii=False, indent=2))

    print("\n" + "-" * 70)
    print("3. Walk-Forward 稳定性检验")
    print("-" * 70)
    wf_result = run_walk_forward_stability(monthly_returns)
    print(json.dumps(wf_result, ensure_ascii=False, indent=2))
