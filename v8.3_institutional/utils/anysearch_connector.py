...
class AnySearchConnector:
    ...
    def search(self, query: str, domain: str = None, sub_domain: str = None,
               sub_domain_params: str = None, max_results: int = 10) -> Optional[List[Dict]]:
        return self._search_impl(query, domain, sub_domain, sub_domain_params, max_results)

    def get_finance_news(self, cn_code: str = None, period: str = '1d',
    ...
    def get_stock_quote(self, cn_code: str) -> Optional[Dict[str, float]]:
        ...
        if results and len(results) > 0:
            ...
            if 'close' in data or 'open' in data:
                numeric_fields = ['amount', 'change', 'circ_mv', 'close', 'high', 'low', 'open',
                                'pb', 'pct_chg', 'pe', 'pe_ttm', 'pre_close', 'ps', 'ps_ttm',
                                'total_mv', 'turnover_rate', 'vol']
                result = {}
    ...
    def _search_impl(self, query: str, domain: str = None, sub_domain: str = None,
                     sub_domain_params: str = None, max_results: int = 10) -> Optional[List[Dict]]:
        cli_path = os.path.join(ANYSEARCH_SKILL_DIR, 'scripts', 'anysearch_cli.py')
        ...
        if parsed and len(parsed) > 0 and 'title' not in parsed[0]:
            for i, item in enumerate(parsed):
                if 'url' in item:
                    parsed[i]['title'] = item['url'].split('/')[-1].replace('.html', '').replace('.htm', '').replace('-', ' ').title()
        return parsed
