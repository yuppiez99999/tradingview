...
class FiveFactorModel:
    ...
    def evaluate_stocks(self, klines: Dict[str, pd.DataFrame], fundamental_data: Optional[Dict] = None) -> Dict:
        """
        评估所有股票的因子得分
    ...
    def _extract_features(self, df: pd.DataFrame, code: str, fundamental_data: Optional[Dict]) -> Dict:
        """提取因子特征"""
        features = {}
    ...
    def _calculate_composite_score(self, factor_scores: Dict) -> float:
        """计算综合因子得分"""
        # 权重分配
        weights = {"value": 0.20, "quality": 0.25, "momentum": 0.20, "growth": 0.20, "safety": 0.15}
    ...
    def _rank_stocks(self, evaluation_results: Dict) -> Dict:
        """对股票进行排名"""
        # 按综合得分排序
        sorted_stocks = sorted(evaluation_results.items(), key=lambda x: x[1]["composite_score"], reverse=True)

        # 分配排名
        for rank, (_code, result) in enumerate(sorted_stocks):
            result["rank"] = rank + 1
    ...
    def _log_evaluation_results(self, evaluation_results: Dict):
        ...
        sorted_results = sorted(evaluation_results.items(), key=lambda x: x[1]["composite_score"], reverse=True)
    ...
    def select_portfolio(self, evaluation_results: Dict, top_n: int = 12) -> Dict:
        ...
        # 按排名排序
        sorted_results = sorted(evaluation_results.items(), key=lambda x: x[1]["rank"])
        ...
        for code, _result in selected_stocks:
        ...
        return {
            ...
            "composite_scores": {code: result["composite_score"] for code, result in evaluation_results.items()},
        }
...
def main():
    ...
    try:
        ...
        # 创建五因子模型
        FiveFactorModel(portfolio, settings)
