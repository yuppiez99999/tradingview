...
def get_adaptive_rebalance_interval(current_volatility, base_volatility=0.25):
    ...
    return max(STRATEGY_CONFIG["min_rebalance_days"], min(STRATEGY_CONFIG["max_rebalance_days"], interval))
...
def get_volatility_adjusted_weights(target_weights, volatility_dict):
    ...
    """
    adjusted = {}
    total_inv_vol = sum(w / max(0.05, volatility_dict.get(c, 0.25)) for c, w in target_weights.items())

    for code, _weight in target_weights.items():
        vol = volatility_dict.get(code, 0.25)
        inv_vol = 1.0 / max(0.05, vol)
        adjusted[code] = min((inv_vol / total_inv_vol), STRATEGY_CONFIG["max_single_weight"])
...
def main():
    ...
    n_days = len(dates)
    years = n_days / 252.0
    print(f"\n📅 回测区间: {dates[0].date()} ~ {dates[-1].date()}, 共 {n_days} 个交易日 ({years:.1f} 年)")
    ...
    print("\n🚀 开始回测...")
    print(
        f"   参数: 止损{STRATEGY_CONFIG['stop_loss'] * 100:.0f}% | "
    ...
    for i, date in enumerate(dates):
        ...
        if portfolio_prices:
            vols = []
            for _code, price_arr in portfolio_prices.items():
                if len(price_arr) > 1:
    ...
    avg_loss = (
        abs(np.mean([t["pnl"] for t in trade_log if t["pnl"] < 0])) if any(t["pnl"] < 0 for t in trade_log) else 1
    )
    ...
    print(f"\n{'=' * 70}")
    print("  🎯 目标达成:")
    print(f"  年化≥8%:  {'✅ 达标' if annual_return >= 0.08 else '❌ 未达标'} ({annual_return * 100:.2f}%)")
    print(f"  回撤≤10%: {'✅ 达标' if max_drawdown <= 0.10 else '❌ 未达标'} ({max_drawdown * 100:.2f}%)")
    ...
    for code in sorted(positions.keys()):
        ...
        print(
            f"{names.get(code, ''):<12} {code:<10} {shares:<8} "
