...
class RiskManager:
    ...
    def kelly_weight(self, symbol: str, mu_hist: float, sigma: float, beta: float = 1.0, n: int = 252) -> float:
    ...
    def size_position(self, mu_hist: float, sigma: float, beta: float, n: int = 252) -> float:
        """计算单标的 Kelly 仓位权重"""
        return self.budgeter.kelly_weight(mu_hist, sigma, beta, n)
    ...
    def size_portfolio(
        ...
        return self.budgeter.size_positions(
            symbols, mu_hist, sigma, beta, returns, kelly_weight=kelly_weight, rp_weight=1.0 - kelly_weight
        )
    ...
    def run_risk_cycle(
        ...
        if mode not in ("CIRCUIT_BREAKER", "CIRCUIT_BREAK_ACTIVE"):
            hedge_actions = self.auto_hedge(portfolio_beta, vix_level, avg_correlation, equity)
