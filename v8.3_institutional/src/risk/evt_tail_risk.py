...
class ExtremeValueAnalyzer:
    ...
    def __init__(self,
        ...
        logger.info(f"[ExtremeValueAnalyzer] 初始化完成 | "
                    f"Confidence={confidence_level} | "
                    f"N Blocks={n_blocks}")

    def fit_gpd(self,
                returns: np.ndarray,
                threshold: Optional[float] = None) -> GPDParameters:
        """
        使用POT方法拟合GPD分布
        ...
        if len(excesses) < 30:
            logger.warning(
                f"极值数量不足({len(excesses)}<30),GPD拟合可能不可靠"
            )
        ...
        result = minimize(
            ...
            x0,
            method='Nelder-Mead',
            options={'maxiter': 10000, 'xatol': 1e-8, 'fatol': 1e-6}
        )
    ...
    def _kolmogorov_smirnov_test(self,
        ...
        # GPD累积分布函数
        if abs(xi) > 1e-6:
            cdf = lambda x: 1 - (1 + xi*x/sigma)**(-1/xi)
        else:
            cdf = lambda x: 1 - np.exp(-x/sigma)
    ...
    def calculate_risk_metrics(self,
                               ...
                               daily_volatility: float = 0.015) -> RiskMetrics:
        """
        基于GPD参数计算风险指标
    ...
    def run_stress_tests(self,
                         ...
                         portfolio_value: float = 5_000_000) -> List[StressTestResult]:
        """
        历史极端情景压力测试
    ...
    def monte_carlo_cvar(self,
                         n_simulations: int = 2000,
                         horizon_days: int = 1,
                         portfolio_value: float = 5_000_000,
                         annual_return: float = 0.08,
                         annual_vol: float = 0.20) -> float:
        """
        蒙特卡洛模拟计算CVaR
    ...
    def generate_report(self,
                        ...
                        stress_tests: List[StressTestResult],
                        mc_cvar: float,
                        portfolio_value: float = 5_000_000) -> EVTRiskReport:
        """
        生成EVT风险报告
        ...
        if risk_metrics.is_fat_enough:
            warnings.append(
                f"[CRITICAL] 检测到严重肥尾效应(Tail Index={risk_metrics.tail_index:.3f}>0.5)"
            )
            ...
            warnings.append(
                f"[WARNING] 检测到肥尾效应(Tail Index={risk_metrics.tail_index:.3f}>0)"
            )
            recommendations.append(
                "考虑使用Vega对冲模块管理波动率风险"
            )
        ...
        if var_99_pct > 0.05:  # 日VaR超过5%
            warnings.append(
                f"[CRITICAL] 99% VaR过高({var_99_pct*100:.2f}%>5%)"
            )
            recommendations.append(
                "立即减少风险敞口,日频调仓检查Delta中性"
            )
            status = "CRITICAL"
        ...
        if severe_scenarios:
            ...
            warnings.append(
                f"[CRITICAL] 极端情景最大损失{worst_loss:,.0f}元 "
                f"({worst_loss/portfolio_value*100:.1f}%)"
            )
            recommendations.append(
                "必须为黑天鹅事件保留缓冲,期权对冲或现金储备至少覆盖99% VaR敞口"
            )
            status = "CRITICAL"
        ...
        if mc_cvar > portfolio_value * 0.01:  # CVaR超过净值1%
            warnings.append(
                f"[WARNING] 蒙特卡洛99% CVaR={mc_cvar:,.0f}元 "
                f"(占NAV {mc_cvar/portfolio_value*100:.2f}%)"
            )
            recommendations.append(
                "CVaR超过预期alpha收益的30%,需重新评估对冲方案"
            )

        # 5. KS检验
        if gpd_params.ks_pvalue < 0.05:
            warnings.append(
                f"[WARNING] GPD拟合不佳(KS p-value={gpd_params.ks_pvalue:.4f}<0.05)"
            )
            recommendations.append(
                "考虑使用更高阈值或更换分布拟合方法"
            )
...
if __name__ == "__main__":
    ...
    for st in stress_tests:
        ...
        print(f"   {severity} {st.scenario_name}: "
              f"损失{st.loss:,.0f}元 ({st.loss_pct*100:.1f}%)")

    # 4. 蒙特卡洛模拟
    print("\n4. 蒙特卡洛模拟(2000路径)...")
    mc_cvar = analyzer.monte_carlo_cvar(
        n_simulations=2000,
        portfolio_value=5_000_000
    )
