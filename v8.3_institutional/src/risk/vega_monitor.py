...
class VegaMonitor:
    ...
    def __init__(self,
                 ...
                 max_call_skew: float = 0.10):  # Call Skew上限10%
        self.nav = nav
        ...
        logger.info(f"[VegaMonitor] 初始化完成 | NAV={nav:,.0f} | "
    ...
    def calculate_skew_metrics(self, positions: List[OptionPosition],
                               atm_iv: float) -> SkewMetrics:
        """
        计算Skew指标
        ...
        # 异常检测
        is_anomalous = (abs(put_skew) > self.max_put_skew or
                        abs(call_skew) > self.max_call_skew)
    ...
    def generate_report(self, positions: List[OptionPosition]) -> VegaRiskReport:
        ...
        if exposure.vega_as_pct_nav > self.max_vega_pct:
            warnings.append(
                f"[CRITICAL] Vega暴露过高: {exposure.vega_as_pct_nav*100:.2f}% "
                f"(上限{self.max_vega_pct*100:.1f}%)"
            )
        ...
        if exposure.concentration_risk == "HIGH":
            warnings.append(
                ...
                if exposure.total_vega != 0 else "总Vega为零"
            )
        ...
        if skew.is_anomalous:
            warnings.append(
                f"[WARNING] Skew异常: Put Skew={skew.put_skew*100:.1f}%, "
                f"Call Skew={skew.call_skew*100:.1f}%"
            )
            if skew.put_skew > self.max_put_skew:
                recommendations.append(
                    "Put Skew过高,买Put对冲成本极高,考虑用Put Spread替代裸买Put"
                )
        ...
        if total_theta < -10000:  # 每日Theta损失超过1万
            warnings.append(
                f"[WARNING] Theta衰减严重: 每日损失{total_theta:,.0f}"
            )
            recommendations.append(
                "持有期权每日支付大量时间价值,若年化Theta超过预期alpha收益30%需重新评估对冲方案"
            )
