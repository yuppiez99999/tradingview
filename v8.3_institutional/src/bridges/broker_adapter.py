    ...
    def submit_order(self, order: BrokerOrder) -> bool:
        ...
        try:
            ...
            if not market_data.get("data"):
                logger.warning(f"无行情数据,使用限价单模拟")
            ...
            if random.random() < self.simulation_settings["fill_probability"]:
                ...
                stamp_tax = (commission * 0 if order.side == OrderSide.BUY else 
                            fill_price * order.quantity * self.simulation_settings["stamp_tax"])
                ...
                logger.info(f"[成交] {order.symbol} {order.side.value} "
    ...
    def generate_simulation_report(self) -> Dict:
        ...
        return {
            ...
            "performance_summary": {
                ...
                "win_rate": sum(1 for o in self.filled_orders if o.side == OrderSide.SELL and o.avg_fill_price > o.price) / max(len(self.filled_orders), 1),
                "total_commission": sum(o.avg_fill_price * o.filled_quantity * self.simulation_settings["commission_rate"] for o in self.filled_orders)
            }
        }
