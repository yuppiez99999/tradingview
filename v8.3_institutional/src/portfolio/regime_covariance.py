...
class RegimeConditionalCovariance:
    ...
    def fit(
        ...
        for regime_name in ["low_vol", "normal", "high_vol", "crisis"]:
            ...
            if len(regime_data) < 10:
                logger.warning(f"机制 {regime_name}: 数据点不足 (n={len(regime_data)}), 使用全局协方差")
                cov = aligned_returns.cov().values
            else:
                if use_ewma and len(regime_data) > 20:
                    cov = self._ewma_cov(regime_data, halflife=min(self.decay_halflife, len(regime_data) // 3))
                else:
    ...
    def _apply_tail_correlation(self, cov: np.ndarray, target_corr: float) -> np.ndarray:
    ...
    def predict_covariance(self, horizon: int = 20, current_vix_proxy: Optional[float] = None) -> np.ndarray:
            ...
            corr_estimate=float(
                np.mean(
                    np.abs(
                        pred_cov
                        / np.outer(
                            np.maximum(np.sqrt(np.diag(pred_cov)), 1e-8), np.maximum(np.sqrt(np.diag(pred_cov)), 1e-8)
                        )
                    )
                )
            ),
        ...
        logger.info(
            f"前瞻协方差: 机制={max_regime} (P={current_probs[max_regime]:.1%}), "
    ...
    def report(self) -> Dict:
        ...
        for name, cov in self.regime_covs_.items():
            ...
            corr = float(
                np.mean(
                    np.abs(
                        cov / np.outer(np.maximum(np.sqrt(np.diag(cov)), 1e-8), np.maximum(np.sqrt(np.diag(cov)), 1e-8))
                    )
                )
            )
