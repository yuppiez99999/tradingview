...
class BlackLittermanEngine:
    ...
    def blend_views(self, prior_returns: np.ndarray, cov_matrix: np.ndarray, views: list[dict]) -> np.ndarray:
        ...
        posterior_returns = posterior_cov @ (tau_sigma_inv @ prior_returns + P.T @ omega_inv @ Q)
        ...
        logger.info(
            ...
            k,
            float(prior_returns.mean()) * 100,
            float(posterior_returns.mean()) * 100,
        )

        return posterior_returns

    def optimize_weights(
        self, returns: np.ndarray, cov_matrix: np.ndarray, max_weight: float = 0.15, min_weight: float = 0.0
    ) -> np.ndarray:
        """均值-方差优化 → 最优权重"""
