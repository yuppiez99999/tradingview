...
class AICoordinator:
    ...
    def __init__(self, daily_token_budget: int = 500000, db_path: Optional[str] = None):
        if db_path is None:
    ...
    def record_usage(self, model: str, task_type: str, input_tokens: int, output_tokens: int):
        """记录Token用量"""
        config = self.MODEL_CONFIG.get(model, {})
        cost = (
            input_tokens * config.get("cost_per_1k_input", 0) + output_tokens * config.get("cost_per_1k_output", 0)
        ) / 1000
        ...
        try:
            ...
            conn.execute(
                ...
                VALUES (?, ?, ?, ?, ?, ?)
            """,
                (datetime.now().strftime("%Y-%m-%d"), model, task_type, input_tokens, output_tokens, cost),
            )
            conn.commit()
            conn.close()
    ...
    def resolve_conflicts(self, decisions_by_source: Dict[str, Dict[str, str]]) -> Dict[str, Dict[str, Any]]:
    ...
    def get_daily_usage(self, date: Optional[str] = None) -> Dict[str, Any]:
        ...
        return {
            ...
            "budget_used_pct": round((total_input + total_output) / self.daily_token_budget * 100, 1)
    ...
    def get_decision_history(
        self, ticker: Optional[str] = None, source: Optional[str] = None, days: int = 7
    ) -> List[Dict[str, Any]]:
        """查询AI决策历史"""
    ...
    def record_accuracy(
        self, decision_id: int, predicted_action: str, actual_outcome: str = "", pnl_if_followed: float = 0.0
    ):
        """记录AI决策的5日验证结果。"""
        try:
            conn = sqlite3.connect(self.db_path)
            conn.execute(
                ...
                VALUES (?, ?, ?, ?, ?)
            """,
                (decision_id, predicted_action, actual_outcome, pnl_if_followed, datetime.now().isoformat()),
            )
            conn.commit()
            conn.close()
        except Exception as e:
