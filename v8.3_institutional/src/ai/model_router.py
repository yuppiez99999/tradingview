...
class ModelRouter:
    ...
    def __init__(self, config_path: Optional[str] = None):
        ...
        self.stats: Dict[str, Dict] = {
            provider_key: {"calls": 0, "successes": 0, "total_latency_ms": 0} for provider_key in providers
        }
    ...
    def route(
        ...
        if parallel_enabled:
            # 盘中决策: 并行对冲模式
            result = self._execute_parallel_hedge(scene_config, prompt, system_prompt, rag_context, timeout, start_time)
        ...
        logger.info(
            f"[路由] 完成, 延迟={result.latency_ms:.0f}ms, 模型={result.model_path}, 置信度={result.confidence:.2f}"
        )
    ...
    def _execute_cross_validation(
        ...
        with ThreadPoolExecutor(max_workers=2) as executor:
            ...
            # 主模型 DeepSeek (R1 深度推理 / V3 快速对话) → 逻辑推理
            ...
            # 交叉验证模型 DeepSeek (V3 对话 / R1 推理, 同一 API 通道) → 财报细读和消息面
            if not self._is_circuit_open(secondary_cfg["provider"]):
        ...
        try:
            ...
            if provider == "volcengine":
                ...
                result = self._call_openai_compatible(
                    session, provider, api_base, api_key, model, messages, temperature, max_tokens, timeout
                )
            ...
            if result:
                ...
                return ModelCallResult(
                    provider=provider, model=model, content="", latency_ms=latency_ms, success=False, error="API 返回空"
                )
            ...
            return ModelCallResult(
                provider=provider, model=model, content="", latency_ms=latency_ms, success=False, error=str(e)
            )
    ...
    def _call_volcengine(
        ...
        input_messages = []
        for msg in messages:
            input_messages.append({"role": msg["role"], "content": [{"type": "input_text", "text": msg["content"]}]})
        ...
        for provider, s in self.stats.items():
            ...
            if calls > 0:
                result[provider] = {
                    ...
                    if self.circuit_breakers.get(provider, CircuitBreaker(provider=provider)).is_open
                    else "CLOSED",
                }
        return result
