    ...
    def calculate_sentiment_score(self, market_data: Dict) -> float:
        ...
        try:
            ...
            composite_sentiment = sum(
                sentiment_components[key] * weight for key, weight in self.sentiment_weights.items()
            )
...
class TechnicalIndicatorMonitor:
    ...
    def calculate_technical_score(self, market_data: Dict) -> float:
        ...
        try:
            ...
            weights = {"trend_score": 0.4, "momentum_score": 0.3, "volatility_score": 0.2, "volume_score": 0.1}

            composite_score = sum(technical_scores[key] * weight for key, weight in weights.items())
...
class SmartHedgeTrigger:
    ...
    def should_trigger_hedge(self, market_data: Dict) -> Dict:
        ...
        try:
            ...
            if self._is_in_cooldown():
                return {"should_trigger": False, "reason": "在冷却期中", "confidence": 0.0, "trigger_type": "cooldown"}
            ...
            return {"should_trigger": False, "reason": f"系统错误: {e}", "confidence": 0.0, "trigger_type": "error"}
    ...
    def _evaluate_ml_trigger(
        self, direction_pred: float, volatility_pred: float, direction_conf: float, volatility_conf: float
    ) -> Dict:
        """评估ML预测触发"""
        # 检查方向预测
    ...
    def _make_trigger_decision(
        self, sentiment_trigger: Dict, technical_trigger: Dict, ml_trigger: Dict, market_data: Dict
    ) -> Dict:
        """做出最终触发决策"""
        try:
            # 统计触发信号数量
            ...
            return {"should_trigger": False, "reason": f"决策错误: {e}", "confidence": 0.0, "trigger_type": "error"}
    ...
    def get_trigger_summary(self) -> Dict:
        ...
        try:
            ...
            # 平均置信度
            avg_confidence = np.mean([t["trigger_decision"]["confidence"] for t in recent_triggers])
    ...
    def _generate_simulation_conclusions(self, results: List[Dict]) -> List[str]:
        ...
        for result in results:
            ...
            if trigger_decision["should_trigger"]:
                ...
                conclusions.append(f"{scenario}: 未触发对冲 (原因: {trigger_decision['reason']})")
