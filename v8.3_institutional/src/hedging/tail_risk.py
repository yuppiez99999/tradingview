...
class TailRiskHedge:
    ...
    def analyze_market_regime(self, market_data: Dict) -> str:
        ...
        try:
            ...
            # 计算尾部风险指标
            tail_risk_score = self._calculate_tail_risk_score(returns, volatility, liquidity, var_95, es_95)
    ...
    def _calculate_tail_risk_score(
        self, returns: List[float], volatility: float, liquidity: float, var_95: float, es_95: float
    ) -> float:
        """
        计算尾部风险评分（0-1之间）
        ...
        # 综合评分（加权平均）
        weights = {"volatility": 0.25, "liquidity": 0.20, "var": 0.20, "es": 0.25, "extreme": 0.10}
    ...
    def _calculate_extreme_value_risk(self, returns: List[float]) -> float:
        ...
        try:
            ...
            # 计算阈值
            threshold = np.percentile(block_maxima, self.extreme_value_params["threshold_percentile"])
    ...
    def calculate_protection_needed(self, portfolio_value: float, market_data: Dict) -> Dict:
        """
        计算需要的尾部保护
        ...
        # 计算期权对冲需求
        option_hedge = max(self.protection_config["min_option_hedge"], var_protection * 0.6)
        ...
        if market_data.get("liquidity", 1.0) < 0.5:
            liquidity_hedge = min(protection_ratio * 0.2, (1.0 - market_data["liquidity"]) * 0.1)
    ...
    def execute_protection_strategy(self, protection_needed: Dict, market_data: Dict) -> List[Dict]:
        """
        执行尾部保护策略
    ...
    def _execute_option_protection(
        self, option_ratio: float, protection_capital: float, market_data: Dict
    ) -> List[Dict]:
        """
        执行期权保护策略
    ...
    def _estimate_option_premium(self, strategy: str, moneyness: float, market_data: Dict) -> float:
        """
        估算期权权利金
        ...
        try:
            ...
            if strategy == "call":
                premium = underlying_price * 0.5 * (1 + 0.5 * (d1 - 1)) - strike_price * np.exp(
                    -risk_free_rate * time_to_expiry
                ) * 0.5 * (1 + 0.5 * (d2 - 1))
            else:  # put
                premium = strike_price * np.exp(-risk_free_rate * time_to_expiry) * 0.5 * (
                    1 + 0.5 * (1 - d2)
    ...
    def _execute_volatility_hedge(
        self, volatility_ratio: float, protection_capital: float, market_data: Dict
    ) -> List[Dict]:
        """
        执行波动率对冲策略
    ...
    def _execute_liquidity_protection(
        self, liquidity_ratio: float, protection_capital: float, market_data: Dict
    ) -> List[Dict]:
        """
        执行流动性保护策略
    ...
    def _calculate_hedge_effectiveness(self, market_shock: float, volatility: float, market_data: Dict) -> float:
        """
        计算对冲有效性
    ...
    def monitor_early_warning(self, market_data: Dict) -> Dict:
        ...
        for alert in alerts:
            self.alert_history.append(
                {**alert, "timestamp": datetime.now().isoformat(), "protection_level": self.tail_protection_level}
            )
...
# v5.9 compat alias for hedge_coordinator
TailRiskHedger = TailRiskHedge
