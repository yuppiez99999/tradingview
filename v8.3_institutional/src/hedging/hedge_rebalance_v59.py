...
def _get_ifind_prices_batch(codes: List[str]) -> Dict[str, float]:
    ...
    try:
        ...
        client = IFindClient()  # Token 从环境变量自动读取
...
try:
    ...
    _HEDGE_OK = False
    # 兜底类型定义: 避免 NameError 阻止模块加载 (方法签名引用 PortfolioRisk)
    # 真正调用 assess_risk 等方法时仍会因 _HEDGE_OK=False 而提前返回
    PortfolioRisk = object  # type: ignore[assignment,misc]
    HedgeEngine = None  # type: ignore[assignment]
    HedgeSignalStrength = None  # type: ignore[assignment]
    HedgeType = None  # type: ignore[assignment]
    HedgeRecommendation = None  # type: ignore[assignment]
    INDEX_FUTURES_SPECS = {}  # type: ignore[assignment]
    ETF_OPTIONS_SPECS = {}  # type: ignore[assignment]
    get_live_futures_prices = None  # type: ignore[assignment]
    DEFAULT_FUTURES_PRICES = {}  # type: ignore[assignment]
...
class HedgeRebalanceIntegrator:
    ...
    def load_prices(self) -> Dict[str, float]:
        ...
        if IFIND_AVAILABLE:
            try:
                ...
                logger.warning(f"[P0-iFinD] 行情获取失败: {e}")

        missing = [c for c in codes if c not in self._prices]
        if missing:
            logger.info(f"[P0-iFinD] 已覆盖 {len(codes) - len(missing)}/{len(codes)}, 降级至 P1-Wind")

        # P1: Wind MCP 实时行情快照
        if missing:
            try:
                from ..bridges.wind_mcp import get_realtime_prices_batch
                ...
                logger.warning(f"[P1-Wind] 行情获取失败: {e}")

        missing = [c for c in codes if c not in self._prices]
        if missing:
            logger.info(f"[P1-Wind] 仍缺失 {len(missing)} 个标的, 降级至 P2-AKShare")
            ...
            try:
                ...
                for code in missing:
                    try:
                        ...
                        logger.warning(f"[P2-AKShare] {code} 获取失败: {e}")
            except Exception as e:
                logger.warning(f"[P2-AKShare] 批量获取失败: {e}")

        missing = [c for c in codes if c not in self._prices]
        if missing:
            logger.info(f"[P2-AKShare] 仍缺失 {len(missing)} 个标的, 降级至 P3-efinance")
            ...
            try:
                ...
                for code in missing:
                    try:
                        ...
                        logger.warning(f"[P3-efinance] {code} 获取失败: {e}")
            except Exception as e:
                logger.warning(f"[P3-efinance] 批量获取失败: {e}")

        missing = [c for c in codes if c not in self._prices]
        if missing:
            logger.warning(f"[P3-efinance] 仍缺失 {len(missing)} 个标的, 降级至 P4-兜底价格")

        # P4: 兜底价格
        for code in codes:
            if code not in self._prices:
                fallback_px = self._estimate_default_price(code)
                self._prices[code] = fallback_px
                fallback_used.append(code)

        if fallback_used:
            logger.critical(
                f"[P4-兜底] 以下 {len(fallback_used)} 个标的使用了硬编码兜底价格, "
                f"所有数据源均已失效: {fallback_used[:10]}" + ("..." if len(fallback_used) > 10 else "")
            )

        self._prices_loaded = True
        self._fallback_codes = fallback_used
        # CR1 修复: 标记兜底价格, 下游风控层可通过 is_price_safe() 拒绝交易
        self._fallback_stale_codes: List[str] = []
        for code in fallback_used:
            # _estimate_default_price 内部已记录过期状态, 这里仅收集过期代码
            # 通过检查价格年龄判断是否过期 (复用同一逻辑)
            fallback_db = {
                "300308.SZ": 20260715,
                "688041.SH": 20260715,
                "002371.SZ": 20260715,
            ...
            ts = fallback_db.get(code)
            if ts is None:
                self._fallback_stale_codes.append(code)
                continue
            try:
                age = (datetime.now() - datetime.strptime(str(ts), "%Y%m%d")).days
                if age > 30:
                    self._fallback_stale_codes.append(code)
            except Exception:
                self._fallback_stale_codes.append(code)

        if self._fallback_stale_codes:
            logger.critical(
                f"[CR1-拦截] {len(self._fallback_stale_codes)} 个标的兜底价格已过期, "
                f"风控层应拒绝交易: {self._fallback_stale_codes[:5]}"
            )
        return self._prices

    def is_price_safe(self, code: str) -> bool:
        """CR1 修复: 检查标的价格是否安全可用于交易

        Returns:
            True = 价格来自实时数据源, 可交易
            False = 价格来自过期兜底, 风控层应拒绝交易
        """
        if not hasattr(self, "_fallback_stale_codes"):
            return True
        return code not in self._fallback_stale_codes

    def _estimate_default_price(self, code: str) -> float:
        """CR1 修复: 兜底价格加时间戳 + 时效性检查 + 告警通知

        所有数据源失效时返回硬编码兜底价格, 但:
        1. 每个价格附带时间戳 (YYYYMMDD 整数)
        2. 超过 30 天视为过期, 触发 CRITICAL 告警
        3. 仍返回价格 (避免下游除零), 但调用方应通过 _fallback_codes 拒绝交易
        """
        # 兜底价格表 (price, timestamp YYYYMMDD)
        # 时间戳为最近一次审计校准日期, 后续应通过定期校准脚本更新
        _FALLBACK_PRICE_DB = {
            "300308.SZ": (105.0, 20260715),
            "688041.SH": (62.0, 20260715),
            "002371.SZ": (320.0, 20260715),
            "688981.SH": (55.0, 20260715),
            "300750.SZ": (230.0, 20260715),
            "000425.SZ": (8.5, 20260715),
            "601088.SH": (38.0, 20260715),
            "600219.SH": (5.0, 20260715),
            "600019.SH": (7.5, 20260715),
            "518880.SH": (5.2, 20260715),
            "000792.SZ": (28.0, 20260715),
            "600276.SH": (48.0, 20260715),
            "603259.SH": (65.0, 20260715),
            "002422.SZ": (32.0, 20260715),
        }
        _FALLBACK_MAX_AGE_DAYS = 30  # 兜底价格最大有效期 (天)

        price_info = _FALLBACK_PRICE_DB.get(code)
        if not price_info:
            # 未知标的, 返回中性占位价 + 告警
            logger.critical(f"[CR1-兜底] 未知标的 {code} 无兜底价格记录, 所有数据源已失效, 返回占位价 50.0 拒绝交易")
            self._notify_data_source_failure(code, 50.0, None)
            return 50.0

        px, ts_yyyymmdd = price_info
        # 计算价格年龄 (天)
        try:
            ts_date = datetime.strptime(str(ts_yyyymmdd), "%Y%m%d")
            age_days = (datetime.now() - ts_date).days
        except Exception:
            age_days = _FALLBACK_MAX_AGE_DAYS + 1  # 时间戳解析失败视为已过期

        if age_days > _FALLBACK_MAX_AGE_DAYS:
            logger.critical(
                f"[CR1-兜底] {code} 兜底价格已过期 {age_days} 天 "
                f"(价格={px}, 校准日={ts_yyyymmdd}, 阈值={_FALLBACK_MAX_AGE_DAYS}天), "
                f"所有数据源失效, 拒绝基于过期价格交易"
            )
            self._notify_data_source_failure(code, px, ts_yyyymmdd)
        else:
            logger.warning(
                f"[CR1-兜底] {code} 使用兜底价格 {px} (校准日={ts_yyyymmdd}, 年龄={age_days}天), 所有数据源已失效"
            )
        return px

    def _notify_data_source_failure(self, code: str, price: float, ts: Optional[int]) -> None:
        """CR1 修复: 数据源全失效时触发告警通知 (短信/钉钉)

        fail-safe 设计: 通知失败不影响主流程, 仅记录日志
        """
        try:
            # 优先尝试 utils.notify 模块 (短信告警)
            from utils.notify import send_sms_alert

            msg = f"[量化系统-数据源告警] {code} 所有行情源失效, 兜底价={price}, 校准日={ts or '未知'}"
            send_sms_alert(msg)
        except ImportError:
            # notify 模块不可用, 仅记录日志
            logger.warning(f"[CR1-通知] utils.notify 不可用, 数据源失效告警未发送: {code} price={price} ts={ts}")
        except Exception as e:
            logger.warning(f"[CR1-通知] 告警发送失败: {e}")
    ...
    def _compute_tail_hedge_ratio(
        self, portfolio_volatility: Optional[float] = None, portfolio_drawdown_60d: Optional[float] = None
    ) -> float:
        """v5.9: 计算尾部对冲比率
    ...
    def _get_sector_adjusted_weights(self) -> Dict[str, float]:
        ...
        try:
            # [V75] from .kondratiev import  # 已迁移到 src/macro/kondratiev.py KondratievCycleAnalyzer
            from src.macro.kondratiev import KondratievCycleAnalyzer
    ...
    def joint_optimize(
        ...
        discrepancy = (
            abs(after_hedge_exposure - after_rebalance_exposure) / risk.stock_exposure if risk.stock_exposure > 0 else 0
        )

        if discrepancy > 0.15:
            adj_ratio = (
                1 - (after_rebalance_exposure / risk.stock_exposure) if risk.stock_exposure > 0 else hedge.hedge_ratio
            )
    ...
    def _estimate_performance(
        ...
        rebalance_boost = (
            0.015
            if rebalance.needed and rebalance.rebalance_type == "strategic"
            else (0.008 if rebalance.needed else 0)
        )
    ...
    def generate_execution_plan(
        ...
        stress_tests: Optional[Dict[str, Dict[str, Any]]] = None,
    ) -> JointPlan:
        """Phase 5: 生成联合执行计划"""
        now = datetime.now()
    ...
    def run_full_workflow(
        ...
        if self.hedge_engine:
            try:
                ...
                pos_for_stress = {}
                for code, pos in (
                    self.positions.get("positions", {}) if isinstance(self.positions, dict) else {}
                ).items():
    ...
    def format_report(self, plan: JointPlan) -> str:
        ...
        lines.append(
            f"  股票敞口: {plan.stock_exposure:,.0f} ({plan.stock_exposure / plan.portfolio_value * 100:.0f}%)"
        )
        ...
        if plan.hedge:
            ...
            lines.append(
                f"  期货品种: {', '.join(h.futures_instruments) if h.futures_instruments else '无'} (多指数Beta加权)"
            )
            if h.futures_contracts:
                ...
                lines.append(
                    f"  总保证金: {h.total_margin:,.0f} (占组合 {h.total_margin / plan.portfolio_value * 100:.1f}%)"
                )
        ...
        if plan.rebalance:
            ...
            lines.append("")
            lines.append(
                f"  {'代码':<12s} {'名称':<10s} {'板块':<8s} {'操作':<6s} "
                f"{'目标权重':>8s} {'当前权重':>8s} {'偏差':>8s} {'调整金额':>10s}"
            )
            lines.append("  " + "-" * 75)
            for pw in r.positions_to_adjust:
                lines.append(
                    f"  {pw.code:<12s} {pw.name:<10s} {pw.category:<8s} {pw.action:<6s} "
    ...
    def save_report(self, plan: JointPlan, output_dir: Optional[str] = None) -> str:
