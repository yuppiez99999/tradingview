...
class HedgeCommanderConfig:
    ...
    urgency_thresholds: Dict[str, float] = field(default_factory=lambda: {
        'routine': 0.10, 'elevated': 0.25, 'urgent': 0.50
    })
...
class HedgeExecutionCommander:
    ...
    def assess(self, actual_portfolio_beta: float,
               current_hedge_beta_offset: float,
               current_futures_contracts: int,
               portfolio_notional: float) -> dict:
        """核心: 评估对冲对齐状态 → 返回行动指令"""
        
        # 1. 计算偏差
        hedge_ratio = -current_hedge_beta_offset / max(actual_portfolio_beta, 0.001)
        ...
        if days_since_hedge >= self.cfg.max_unhedged_days:
            ...
            logger.warning(
                ...
                self._consecutive_misalign_days, effective_beta, self.cfg.target_beta
            )
        else:
        ...
        target_contracts = round(
            portfolio_market_value * required_hedge_beta_units / notional_per_contract
        )
        ...
        if urgency >= HedgeUrgency.ELEVATED:
            logger.warning(
                ...
                urgency.name, effective_beta, current_futures_contracts,
                target_contracts, contracts_to_adjust
            )
