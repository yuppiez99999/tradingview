...
v8.5 新增集成:
    - EVT肥尾建模: EVTTailRisk 提供 99% CVaR 估计,输入至 compute_hedge()
    - Vega 监控: 与 src.risk.vega_monitor.VegaMonitor 联动,超限自动触发 crisis 状态
    - 流动性监控: LiquidityMonitor 检测盘口深度,低于阈值暂停期权建仓
    - 因子衰减: FactorDecayMonitor 跟踪对冲信号 IC 衰减,自动调整 OTM 阶梯深度
...
v8.5 风控增强:
    - 与 KillSwitch 生命周期集成,任何阶段触发风险阈值自动终止对冲建仓
    - 与 ShadowAccount 偏离度联动,偏离>5% 时自动加深 OTM 阶梯
    - 与 PurgedKFoldCV 交叉验证结合,防止对冲参数过拟合
"""
