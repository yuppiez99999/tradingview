...
class CorrelationHedger:
    ...
    def compute_hedge(self, returns: pd.DataFrame, portfolio_value: float) -> dict[str, object]:
        ...
        #      无论是否跳跃都需配置避险资产)
        triggered = (avg_corr > self.corr_trigger and jump > self.corr_jump) or avg_corr > 0.95

        if not triggered:
            return {
                "action": "NO_HEDGE",
                "reason": (f"ρ̄={avg_corr:.3f}, 基线={baseline:.3f}, 跳跃={jump:.3f} 未达触发条件"),
                "avg_corr": avg_corr,
        ...
        # 避险权重
        w_gold = self.max_gold * min(1.0, (avg_corr - self.corr_trigger) / 0.15)
        ...
        logger.warning(
            "相关性对冲触发: ρ̄=%.3f (基线 %.3f, 跳跃 +%.3f), 黄金 ETF %.2f%%, 逆回购 %.2f%%",
            avg_corr,
            baseline,
            jump,
            w_gold * 100,
