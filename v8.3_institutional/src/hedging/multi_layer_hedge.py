...
class MultiLayerHedgeManager:
    ...
    def execute_multi_layer_hedge(self, market_data: Dict) -> Dict:
        ...
        try:
            ...
            # 5. 执行第三层：尾部风险对冲
            tail_risk_needed = self.tail_risk_hedge.calculate_protection_needed(self.total_capital, market_data)
            tail_trades = self.tail_risk_hedge.execute_protection_strategy(tail_risk_needed, market_data)
            ...
            # 7. 计算执行效果
            execution_effectiveness = self._calculate_execution_effectiveness(all_trades, market_data)
    ...
    def _calculate_overall_risk(self, market_data: Dict) -> float:
        ...
        try:
            ...
            # 综合风险评分
            overall_risk = vol_score * 0.3 + var_score * 0.3 + es_score * 0.3 + liquidity_score * 0.1
    ...
    def _generate_alerts(self, market_data: Dict) -> List[Dict]:
        ...
        if self.current_state["market_regime"] == "crisis":
            alerts.append({"type": "CRISIS_MODE", "severity": "critical", "message": "系统进入危机模式", "value": 1.0})
...
if __name__ == "__main__":
    ...
    # 创建多层次对冲管理器
    hedge_manager = MultiLayerHedgeManager(total_capital=5000000, hedge_ratio=0.20)
