...
class ImplementationShortfall:
    ...
    def decompose(self,
                  ...
                  execution_time_s: float = 0,
                  num_fills: int = 1,
                  daily_volume: int = 0) -> ISDecomposition:
        """
        Perold 1988 框架: 五段分解
        ...
        # 5. 固定成本
        notional = fill_qty * avg_fill_price
        ...
        logger.info(f"[IS] {symbol} {side}: total={total_is:.1f}bps, "
    ...
    def generate_suggestions(self, decomp: ISDecomposition) -> List[ISOptimizationSuggestion]:
        ...
        for cat, current, target, rec, priority in components:
            ...
            if saving > 0.5:
                suggestions.append(ISOptimizationSuggestion(
                    category=cat,
    ...
    def summary(self, window_days: int = 30) -> Dict:
        ...
        return {
            ...
            'annual_savings_if_target': round(
                (_avg(recent, 'total_is_bps') - 8.0) / 10000 * self.nav * 2.0, 2
            ),
        }
    ...
    def report(self) -> Dict:
        ...
        return {
            **s,
            ...
            'total_potential_saving_bps': round(sum(
                sg.potential_saving_bps for sg in all_suggestions
            ), 2),
