...
class PnLAttributionEngine:
    ...
    def attribute_daily(self, positions: List[PositionSnapshot],
                        total_pnl: float, total_nav: float) -> dict:
        """分解单日收益"""
        ...
        # Beta 贡献
        weighted_beta = sum(p.weight * p.factor_exposure.beta
                            for p in positions if p.weight > 0.001)
        ...
        sorted_by_pnl = sorted(
            [(p.symbol, p.weight * p.daily_return) for p in positions],
    ...
    def record_trade(self, symbol: str, side: str, quantity: int,
        ...
        if abs(slippage) > 0.005:  # > 50bp
            logger.warning(
                "TCA: %s %s ×%d 滑点 %.1fbp 成本%.2f",
                symbol, side, quantity, slippage * 10000, cost
            )
        return tca
