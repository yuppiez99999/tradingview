...
class HedgeStrategyExecutor:
    ...
    def calculate_optimal_hedge_ratio(self, 
        ...
        final_ratio = min(1.0, max(0.0, 
            base_ratio + beta_adjustment + vol_adjustment + 
            regime_adjustments.get(market_regime, 0)
        ))
        
        logger.info(f"最优对冲比例: {final_ratio:.2%}")
        return final_ratio
    
    def allocate_hedge_budget(self, 
                               total_budget: float,
                               hedge_ratio: float,
        ...
        return {
            'futures': futures_budget,
            'options': options_budget
        }
    
    def select_option_strategy(self, 
                                underlying: str,
                                budget: float,
                                delta_target: float,
                                max_cost_ratio: float = 0.15) -> Dict:
        """
        选择期权策略
    ...
    def calculate_future_contracts(self,
                                    target_delta: float,
                                    future_price: float,
                                    multiplier: int = 200,
    ...
    def generate_hedge_plan(self, 
                            positions: List[Position],
                            market_data: Dict) -> HedgePlan:
        """
        生成完整对冲计划
        """
        # 1. 计算组合总Delta
