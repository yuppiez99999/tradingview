...
class ParameterSensitivity:
    ...
    def run_local_sensitivity(self, perturbation_levels: Optional[List[float]] = None) -> SensitivityReport:
        """
            ...
            for level in perturbation_levels:
                ...
                for perturb_val in perturb_values:
                    ...
                    if self.evaluator_func:
                        ...
                        # 模拟性能变化(高斯噪声+线性衰减)
                        new_perf = self._simulate_performance(param_name, perturb_val, level)

                    # 计算性能变化率
                    base_perf_value = next(iter(self.base_performance.values()))
                    if isinstance(new_perf, dict):
                        new_perf_value = next(iter(new_perf.values()))
                    else:
                        new_perf_value = new_perf

                    change_rate = (new_perf_value - base_perf_value) / base_perf_value if base_perf_value > 0 else 0
        ...
        for param_range in self.param_ranges:
            param_name = param_range.name
            all_avg_magnitudes = [
                performance_at_each_level[level][param_name]["avg_magnitude"] for level in perturbation_levels
            ]

            max_deviation = np.max(all_avg_magnitudes)
            cv = np.std(all_avg_magnitudes) / np.mean(all_avg_magnitudes) if np.mean(all_avg_magnitudes) > 0 else 0
    ...
    def _simulate_performance(self, param_name: str, param_value: float, perturbation_level: float) -> Dict[str, float]:
        """
        ...
        deviation = abs(param_value - optimal_value) / optimal_value if optimal_value > 0 else 0
        ...
        return {next(iter(self.base_performance.keys())): max(0, final_perf)}
    ...
    def print_summary(self):
        ...
        print(f"\n{'=' * 80}")
        print(f"{'参数名称':<20} {'基准值':>10} {'弹性':>10} {'CV':>10} {'稳定性评分':>12} {'等级':<12}")
        print("-" * 80)

        for result in self.results.parameter_results:
            print(
                f"{result.parameter_name:<20} {result.base_value:>10.4f} "
    ...
    def save_report(self, output_path: str, include_plots: bool = True):
        """
...
def quick_sensitivity_check(
    base_params: Dict[str, float], base_performance: Dict[str, float], param_names: List[str]
) -> SensitivityReport:
    """
    快速敏感性检查(便捷函数)
