...
def bootstrap_confidence_interval(
    ...
    for i in range(n_bootstrap):
        ...
        # 修复 BUG-R3: 时序数据必须用 TimeSeriesSplit, 不能随机打乱
        # 原代码: train_test_split(X_boot, y_boot, random_state=i) 随机分割导致数据泄漏
        try:
            split_idx = int(len(X_boot) * (1 - test_size))
...
def permutation_test(
    ...
    # 修复 BUG-R3: 时序数据必须用时序分割, 不能随机打乱
    # 原代码: train_test_split(X, y, random_state=42) 随机分割导致数据泄漏
...
def calculate_rank_ic(
    ...
    for i in range(n_periods):
        ...
        # 修复 BUG-R2: 训练集只使用测试段之前的数据, 删除 [end, n_samples) 未来数据段
        # 原代码: train_indices = list(range(start)) + list(range(end, n_samples))  ← 包含未来数据
...
def prepare_features_fast(df: pd.DataFrame, min_periods: int = 60) -> Tuple[Optional[np.ndarray], Optional[np.ndarray]]:
    ...
    try:
        ...
        # 修复 BUG-R1: 标准化参数只在整个数据集被 Walk-Forward 划分为训练/测试后,
        # 在训练集上 fit, 再 transform 测试集。此处仅做整体 nan_to_num,
        # 具体的 fit/transform 交由调用方在时序分割后完成。
        # 不再使用全样本均值/标准差做标准化（前视偏差）
