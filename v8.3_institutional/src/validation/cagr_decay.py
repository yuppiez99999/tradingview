...
class CAGRDetection:
    ...
    def __init__(
        ...
        if len(daily_returns) < min_trading_days:
            raise ValueError(
                f"数据不足:需要至少{min_trading_days}个交易日,实际只有{len(daily_returns)}个"
            )
    ...
    def detect(self) -> CAGRDetectionResult:
        ...
        first_half_max_dd = self._calculate_max_drawdown(first_half)
        second_half_max_dd = self._calculate_max_drawdown(second_half)
        ...
        # 生成警告信息
        warning_msg = self._generate_warning(has_decay, decay_rate, severity,
                                            first_half_cagr, second_half_cagr)
