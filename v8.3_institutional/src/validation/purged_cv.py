    ...
    def split(
        ...
        for i in range(self.n_splits):
            ...
            if timestamps is not None:
                train_idx = self._time_purge(train_idx, test_idx, timestamps)
...
def check_lookahead_bias(
    ...
    lookback_windows: Optional[dict] = None,
) -> dict:
    ...
    for feat_name, max_lookback in lookback_windows.items():
        ...
        if na_count == 0:
            ...
            report["suspected_leaks"].append(f"{feat_name}: 前{max_lookback}行无NaN (可能含前视偏差)")
...
if __name__ == "__main__":
    ...
    for i, (train_idx, test_idx) in enumerate(pwf.split(X, y)):
        print(f"Fold {i}: train={len(train_idx)}, test={len(test_idx)}, overlap={len(set(train_idx) & set(test_idx))}")
