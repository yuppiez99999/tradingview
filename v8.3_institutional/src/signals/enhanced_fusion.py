...
class EnhancedSignalFusionEngine(SignalFusionEngine):
    ...
    def __init__(self, db_path: Optional[str] = None, config: WeightAdjustmentConfig = None):
    ...
    def _compute_multi_dimensional_scores(self, sources: List[str]) -> Dict[str, float]:
        ...
        for source in sources:
            ...
            # 综合分数
            total_score = accuracy_score * 0.4 + volatility_score * 0.3 + response_score * 0.2 + diversity_score * 0.1
    ...
    def _apply_weight_constraints(
        self, performance_scores: Dict[str, float], correlation_penalty: Dict[str, float], sources: List[str]
    ) -> Dict[str, float]:
        """应用权重约束和优化"""
        # 应用相关性惩罚
        adjusted_scores = {}
    ...
    def _update_weight_history(self, new_weights: Dict[str, float]):
        ...
        for source_name, new_weight in new_weights.items():
            ...
            if abs(new_weight - old_weight) > 0.05:  # 5%阈值
                ...
                # 记录权重变化
                self._record_weight_change(source_name, new_weight, "significant_change", performance_score)
    ...
    def update_performance_metrics(
        self, source_name: str, actual_outcome: str, predicted_action: str, response_time: Optional[float] = None
    ):
        """更新信号源性能指标"""
        ...
        if response_time is not None:
            if metrics.response_time_avg == 0:
                ...
                metrics.response_time_std = np.sqrt(
                    metrics.response_time_std**2 * 0.9 + (response_time - old_avg) ** 2 * 0.1
                )
