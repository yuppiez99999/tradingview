...
class SignalCrowdingDetector:
    ...
    def update(self, etf_flows: Dict[str, float],
               sector_correlations: Optional[Dict[str, float]] = None) -> dict:
        """更新拥挤度 → 返回信号调整系数"""
        adjustments = {}
        
        for sector, flow in etf_flows.items():
            self._flow_history.setdefault(sector, 
                deque(maxlen=self.cfg.lookback_days))
            self._flow_history[sector].append(
                (datetime.now().isoformat()[:10], flow))
            ...
            # 衰减累计拥挤分
            self._crowding_scores.setdefault(sector, 
                deque(maxlen=self.cfg.lookback_days))
            ...
            # 累计: 衰减 + 新增
            prev = (self._crowding_scores[sector][-1] 
                    if self._crowding_scores[sector] else 0.0)
            ...
            if status != 'NORMAL':
                ...
                logger.warning(
                    ...
                    sector, status, score, flow / 1e8, factor * 100
                )
