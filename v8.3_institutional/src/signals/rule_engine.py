...
class ConservativeStrategy(BaseStrategy):
    ...
    def __init__(self):
        super().__init__("保守型策略", "基于价值投资理念，追求稳健收益，适合风险厌恶型投资者")
        self.generate_rules()
...
class AggressiveStrategy(BaseStrategy):
    ...
    def __init__(self):
        super().__init__("激进型策略", "基于趋势跟踪理念，追求最大化收益，适合风险偏好型投资者")
        self.generate_rules()
...
class BalancedStrategy(BaseStrategy):
    ...
    def __init__(self):
        super().__init__("平衡型策略", "结合价值投资和成长投资，平衡风险与收益，适合大多数投资者")
        self.generate_rules()
...
class RuleEngine:
        ...
        for rule in enabled_rules:
            try:
                if self._evaluate_rule_condition(rule, context):
                    ...
                    rule_results.append(
                        {"rule": rule.name, "action": rule.action, "score": score, "weight": rule.weight}
                    )
    ...
    # 条件表达式安全校验: 仅允许数字、运算符、括号、空白
    _SAFE_CONDITION_PATTERN = re.compile(r"^[\d\s\.\+\-\*\/\%\(\)\<\=\>\!\,andor]+$")

    # AST 节点白名单: 仅允许比较、布尔运算、一元运算、二元运算、常量
    ...
    @classmethod
    def _validate_ast_safe(cls, node: ast.AST) -> bool:
        """递归验证 AST 节点是否在白名单内"""
        if type(node) not in cls._SAFE_AST_NODES:
            return False
        if isinstance(node, ast.Name):
            if node.id not in ("True", "False", "None"):
                return False
        for child in ast.iter_child_nodes(node):
            if not cls._validate_ast_safe(child):
                return False
        return True

    def _safe_eval_condition(self, condition: str) -> bool:
        """安全求值条件表达式 (AST白名单 + 受限命名空间)"""
        try:
            tree = ast.parse(condition, mode="eval")
        except SyntaxError:
            return False

        if not self._validate_ast_safe(tree):
            logger.warning("条件表达式包含不安全的 AST 节点, 拒绝执行: %s", condition[:80])
            return False

        try:
            code = compile(tree, "<trading_rule>", "eval")
            return bool(eval(code, {"__builtins__": {}}))  # nosec B307  AST白名单+空builtins已隔离
        except Exception:
            return False

    def _evaluate_rule_condition(self, rule: TradingRule, context: DecisionContext) -> bool:
        """评估规则条件 (安全版本: AST白名单验证)"""
        try:
            ...
            if not self._SAFE_CONDITION_PATTERN.match(condition):
                logger.warning("条件表达式包含非安全字符, 拒绝执行: %s", condition[:80])
                return False

            return self._safe_eval_condition(condition)

        except (SyntaxError, TypeError, ValueError, NameError) as e:
            logger.warning("评估规则条件时出错 [%s]: %s", type(e).__name__, e)
            return False
