...
def phase_v10_risk(ctx: WorkflowContext) -> dict[str, Any]:
    ...
    try:
        if ctx.phase_manager is not None:
            sim_date = datetime.strptime(ctx.trade_date, "%Y-%m-%d").date() if ctx.trade_date else None
            ...
            if is_pm_quarter_end:
                ...
                # 2030 清仓年: 输出清仓动作
                if ctx.current_phase_info and ctx.current_phase_info.is_liquidation_year:
