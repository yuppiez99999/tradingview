...
def apply_ifind_news_adjustments(
    ...
    *,
    ...
    try:
        insights = ctx.ifind_analyzer.batch_analyze(
    ...
    def _apply(orders: list[dict[str, Any]]) -> list[dict[str, Any]]:
        ...
        for order in orders:
            ...
            factor = ifind_signal_to_factor(
                insight.direction, float(insight.confidence)
            )
            if factor <= 0.0:
                logger.info(
                    "iFinD 跳过订单 [%s] %s confidence=%.2f",
                    code,
                    insight.direction,
                    insight.confidence,
                )
            ...
            if factor >= 1.2:
                logger.info(
                    "iFinD 加仓 [%s] %s confidence=%.2f -> factor=%.2f, %d 股",
                    code,
                    insight.direction,
                    insight.confidence,
                    factor,
                ...
                logger.info(
                    "iFinD 减仓 [%s] %s confidence=%.2f -> factor=%.2f, %d 股",
                    code,
                    insight.direction,
                    insight.confidence,
                    factor,
    ...
    *,
    ...
    def _apply(orders: list[dict[str, Any]]) -> list[dict[str, Any]]:
        ...
        for order in orders:
            ...
            new_order["fifteen_five_score"] = round(
                float(getattr(score_info, "fifteen_five_score", 1.0)), 4
            )
            new_order["kondratiev_score"] = round(
                float(getattr(score_info, "kondratiev_score", 1.0)), 4
            )
            adjusted.append(new_order)

            if factor >= 1.2:
                logger.info(
                    "宏观加仓 [%s] combined=%.4f -> factor=%.2f, %d 股",
                    code,
                    combined,
                    factor,
                    new_shares,
                ...
                logger.info(
                    "宏观减仓 [%s] combined=%.4f -> factor=%.2f, %d 股",
                    code,
                    combined,
                    factor,
                    new_shares,
