...
def screenshot(name):
    filepath = f"last_{name}_{int(time.time())}.png"
    pyautogui.screenshot(filepath)
    print(f"截图: {filepath}")

print("\n[1] 点击期权下单标签")
options_tab_x = left + width * 0.292
...
time.sleep(2)
screenshot("after_keyboard")

print("\n[9] 点击委托列表查看")
order_list_x = left + width * 0.42
order_list_y = top + height * 0.685
print(f"点击: ({int(order_list_x)}, {int(order_list_y)})")
pyautogui.click(int(order_list_x), int(order_list_y))
time.sleep(1)
screenshot("after_order_list")
