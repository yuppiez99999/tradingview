...
| P3 | 通达信(pytdx) | 免费A股行情数据，实时/历史K线、盘口数据 |
| P4 | AKShare / AnySearch | 免费回退，A股/期货/指数公开数据 |
| P5 | 新浪财经 API | 免费实时行情兜底 |
| P6 | 本地缓存 | Parquet/JSON 缓存 |
| P7 | 预定义价格 | 保证系统永不崩溃的兜底价格 |

强制规则：Wind → Wind MCP → iFinD → 通达信 → AKShare → 免费源，不可跳级。需 `WIND_API_KEY`、`IFIND_TOKEN` 和 `TDX_IP/TDX_PORT` 环境变量。

### 8.1 通达信(pytdx)数据源集成指南
...
$env:TDX_IP = "119.147.212.81"  # 通达信主服务器
$env:TDX_PORT = "7709"
...
# 获取股票数据（支持A股/ETF/期货/指数）
df = tdx.get_kline_data("600519", 1, 800)  # 贵州茅台日线
df = tdx.get_kline_data("159919", 1, 100)   # 沪深300ETF日线
```

**核心优势：**
- ✅ 免费A股实时行情（Level-1）
- ✅ 支持日/周/月/分钟K线（5/15/30/60分钟）
- ✅ 盘口五档行情数据
- ✅ 历史除权因子（复权价格）
- ✅ 多服务器自动切换（内置10+服务器列表）
- ✅ 并发连接池管理
...
**适用场景：**
- ETF资金流向实时监控（v8.3核心功能）
- 日频/周频/月频K线数据获取
- 实时行情快照（盘口买卖价量）
- 历史除权因子计算复权价格

**注意事项：**
- ⚠️ 通达信数据为免费源，稳定性不如商业终端
- ⚠️ 建议作为P3优先级，在Wind/iFinD不可用时使用
- ⚠️ 分钟级数据建议用于盘中实时对冲监控
- ⚠️ 高频交易场景不建议使用（延迟较高）
