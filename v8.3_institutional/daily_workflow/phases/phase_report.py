#!/usr/bin/env python3
"""
Phase implementation: phase_report

Extracted from original DailyWorkflow class for modularization.
This module contains the standalone phase function implementing the phase_report phase.
...
def phase_report(workflow) -> Path:
    """盘后报告生成"""
    logger.info("=" * 60)
    ...
    report_dir = workflow.config.REPORT_DIR / workflow.trade_date
    try:
        report_dir.mkdir(parents=True, exist_ok=True)
    except Exception as e:
        logger.warning(f"[PhaseReport] 创建报告目录失败: {e}, 回退到本地目录")
        report_dir = BASE_DIR / "reports" / workflow.trade_date
        report_dir.mkdir(parents=True, exist_ok=True)
    report_path = report_dir / f"v75_daily_workflow_{workflow.trade_date.replace('-', '')}.md"

    # === 报告头部 ===
    lines = [
        f"# v7.5 每日交易工作流报告 — {workflow.trade_date}",
        "",
        f"**生成时间**: {datetime.now():%Y-%m-%d %H:%M:%S}",
        f"**资金规模**: {workflow.capital:,.0f}",
        f"**执行模式**: {'DRY-RUN' if workflow.dry_run else ('模拟盘' if getattr(self, '_sim_mode_requested', getattr(self, 'sim_mode', False)) else 'MOCK_EXECUTION')}",
    ...
    for phase_name, phase_data in workflow.state["phases"].items():
    ...
    if workflow.current_phase_info is not None:
        ...
        v10_result = workflow.state.get("phases", {}).get("v10_risk", {})
    ...
    # === Phase 1: 自检 ===
    if "check" in workflow.state["phases"]:
        lines.extend(["", "## 1. 系统自检", ""])
        checks = workflow.state["phases"]["check"].get("checks", {})
        lines.append("| 检查项 | 状态 |")
        lines.append("|--------|------|")
    ...
    if "calibrate" in workflow.state["phases"]:
        lines.extend(["", "## 1.5 收益预测动态校准", ""])
        cal = workflow.state["phases"]["calibrate"]
    ...
    # === Phase 4: 对冲评估 ===
    if "hedge" in workflow.state["phases"]:
        lines.extend(["", "## 4. 对冲评估", ""])
        h = workflow.state["phases"]["hedge"]
    ...
    # === Phase 5: 交易信号 (计划标的) ===
    if "signal" in workflow.state["phases"]:
        lines.extend(["", "## 5. 交易信号 (计划标的)", ""])
        s = workflow.state["phases"]["signal"]
        action = s.get("action", "")
        ...
        # AnySearch 实时新闻回顾
        if "market" in workflow.state["phases"]:
            anysearch_news = workflow.state["phases"]["market"].get("anysearch_news", [])
            if anysearch_news:
                lines.append("")
    ...
    # === 执行记录 (计划标的) ===
    if "execute" in workflow.state["phases"]:
        lines.extend(["", "## 6. 执行记录 (计划标的)", ""])
        # P3-B FIX (2026-07-26): 变量名 e 与 except 块命名冲突, 改为 exec_phase
        exec_phase = workflow.state["phases"]["execute"]
        mode = "模拟盘" if getattr(self, '_sim_mode_requested', exec_phase.get("sim_mode", False)) else ("DRY-RUN" if workflow.dry_run else "MockBroker")
    ...
    # === 对冲基金视角: P&L 八维归因分析 ===
    if workflow.pnl_attribution_engine is not None:
        try:
            positions = (workflow._get_portfolio_positions_for_stress_test()
                         if hasattr(self, "_get_portfolio_positions_for_stress_test") else [])
            # 当日成交
            fills = workflow.state.get("phases", {}).get("execute", {}).get("fills", [])
            trading_costs = sum(
                float(f.get("amount", 0)) * 0.001  # 估算 10bps 综合成本
                for f in fills
            )
            # 对冲盈亏
            hedge_pnl = float(workflow.state.get("phases", {}).get("hedge", {}).get("hedge_pnl", 0.0))
            ...
            attribution = workflow.pnl_attribution_engine.attribute(
                positions=positions,
            ...
            workflow.state["phases"]["report_pnl_attribution"] = {
    ...
    # === 机构级: Barra 风险因子暴露分解 ===
    if workflow.barra_decomposer is not None:
        try:
            positions = (workflow._get_portfolio_positions_for_stress_test()
                         if hasattr(self, "_get_portfolio_positions_for_stress_test") else [])
            logger.info("[Barra] 持仓数量: %d", len(positions))
            if positions:
                barra_result = workflow.barra_decomposer.decompose_from_positions(
                    positions=positions,
                    risk_budget=0.05,
                )
                logger.info(
                ...
                workflow.state["phases"]["report_barra"] = {
    ...
    all_pass = all(
        p.get("status") == "PASS" for p in workflow.state.get("phases", {}).values()
    ...
    try:
        ...
        _integrator = RiskGuardIntegrator(report_date=workflow.trade_date)
    ...
    lines.extend([
        ...
        f"- 数据源: 2026年交易计划.md + trade_plan_{workflow.trade_date.replace('-', '')}.json",
        "",
    ])
    ...
    if not success:
        # P3-B FIX (2026-07-26): workflow.log_dir 未定义, 改用 workflow.config.REPORT_DIR 作为 fallback
        fallback_dir = getattr(workflow.config, "REPORT_DIR", None) or Path("logs")
    ...
    try:
        ...
        with open(json_path, "w", encoding="utf-8") as f:
            json.dump(workflow.state, f, ensure_ascii=False, indent=2, default=str)
        logger.info(f"状态 JSON: {json_path}")
    except (MemoryError, OSError) as e:
        logger.warning(f"状态 JSON 保存失败（内存不足），尝试无缩进模式: {e}")
        try:
            with open(json_path, "w", encoding="utf-8") as f:
                json.dump(workflow.state, f, ensure_ascii=False, default=str)
