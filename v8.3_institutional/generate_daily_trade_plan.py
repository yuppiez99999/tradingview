...
def _save_alpha_signals_for_drift(trade_date: str) -> None:
    ...
    try:
        ...
        # 5. 裁剪到 [-1, 1]
        signals = {
            sym: float(np.clip(val, -1, 1))
            for sym, val in scores.items()
            if sym in clean_symbols
        }
...
def _load_hedge_execution_plan(
    trade_date: str, hedge_capital: float = 1_060_000
) -> dict:
...
def _load_hedge_fund_overlays(trade_date: str) -> dict:
    ...
    try:
        ...
        overlays["kill_switch"] = {
            ...
            "actions": (
                ks_status.get("actions", []) if isinstance(ks_status, dict) else []
            ),
    ...
    try:
        ...
        if not theta_cfg.get("enabled", False):
            ...
            overlays["theta"] = {
                ...
                "portfolio_yield_annualized": theta_plan.get(
                    "portfolio_yield_annualized", 0
                ),
                "positions": positions,
            }
...
PHASES = [
    {
        ...
        "end": "2026-08-15",
        "capital_ratio": 0.90,
        "phase_capital": 4_500_000,
        "duration_days": 30,
        "daily_capital": 150_000,
...
def generate_orders(
    trade_date: str, phase: dict, build_plan: dict, stock_capital: float = 3_000_000
) -> dict:
    """生成当日买卖订单 (上午 + 下午批次)
    ...
    if MACRO_SCORE_READY and target_portfolio:
        try:
            ...
            for symbol, info in target_portfolio.items():
                # 白名单直接保留
                if symbol in MACRO_WHITELIST_CODES or info.get("name", "") in {
                    "中国神华",
                    "创业板ETF易方达",
                }:
...
def generate_trade_plan(trade_date: str, capital: float = 5_000_000) -> dict:
    ...
    total_orders = len(morning_orders) + len(afternoon_orders)
    total_amount = sum(
        o.get("est_amount", 0) for o in morning_orders + afternoon_orders
    )
    ...
    hedge_fund_overlays = {
        ...
        "v77_notes": {
            ...
            "reason_if_blocked": (
                f"KillSwitch L{ks.get('level', 0)}"
                if not ks.get("build_allowed", True)
                else (
                    f"清仓阶段 Phase {liq.get('phase', 0)}"
                    if not liq.get("build_allowed", True)
                    else "Gamma尾部对冲触发" if gamma_triggered else None
                )
            ),
        },
    }
    ...
    return {
        ...
        "phase": {
            ...
            "daily_capital": phase.get(
                "daily_capital",
                round(phase["phase_capital"] / phase["duration_days"], 2),
            ),
            "day_capital": round(phase["phase_capital"] / phase["duration_days"], 2),
            "asset_count": orders.get(
        ...
        "hedge_config": {
            ...
            "layers": {
                ...
                "layer5_covered_call": (
                    "v7.7 已由 Theta引擎自动生成月度计划"
                    if overlays["theta"].get("plan_loaded")
                    else "不启动 (建仓初期)"
                ),
            },
        },
        ...
        "futures_options_hedge": {
            ...
            "execution_timing": hedge_exec_plan.get(
                "execution_timing",
                {
                    "options_window": "09:30-10:00",
                    "futures_window": "10:30-11:00",
            ...
            "total_premium": hedge_exec_plan.get("budget_check", {}).get(
                "total_premium", 0
            ),
            "total_notional": sum(
                o.get("notional", 0)
                for o in hedge_exec_plan.get("hedge_orders", [])
                if o.get("type") == "FUTURES"
            ),
            "total_safe_haven": hedge_exec_plan.get("budget_check", {}).get(
                "safe_haven", 0
            ),
...
def main():
    ...
    parser.add_argument(
        "date", nargs="?", default=None, help="交易日期 YYYY-MM-DD (默认: 下一交易日)"
    )
    parser.add_argument(
        "--capital", type=float, default=5_000_000, help="总资金 (默认: 5000000)"
    )
