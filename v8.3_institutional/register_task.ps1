# ============================================================
# v7.5 每日交易工作流 - Windows 任务计划程序注册脚本
# ============================================================
# 功能:
#   1. 注册 "v75_DailyWorkflow" 任务, 每个交易日 07:00 自动执行
#   2. 周末自动跳过 (脚本内已判断)
#   3. 任务以当前用户身份运行, 不需要登录即可触发
#
# 用法 (以管理员身份运行 PowerShell):
#   cd e:\各种PY程序\28-终极量化交易系统7.1\v7.5_institutional
#   .\register_task.ps1              # 注册任务
#   .\register_task.ps1 -Uninstall   # 卸载任务
#   .\register_task.ps1 -Test        # 立即测试执行一次
# ============================================================

param(
    [switch]$Uninstall,
    [switch]$Test,
    [string]$TaskName = "v75_DailyWorkflow",
...
Write-Host "  v7.5 Daily Workflow - Task Scheduler Registration" -ForegroundColor Cyan
Write-Host "============================================================" -ForegroundColor Cyan
...
Register-ScheduledTask `
    ...
    -Settings $settings `
    -Principal $principal `
    -Description "v7.5 机构级每日交易工作流 - 每个交易日 07:00 自动执行" `
    -Force | Out-Null
...
if ($task) {
    ...
    Write-Host "  卸载:    .\register_task.ps1 -Uninstall"
    Write-Host "  测试:    .\register_task.ps1 -Test"
} else {
