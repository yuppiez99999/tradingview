...
### karpathy 4 原则（GH+-1 通用 Skill 约束 — 08-28 落地 / 08-29 补全检验标准）

> **来源**：`multica-ai/andrej-karpathy-skills` — Karpathy 针对 LLM 编码陷阱的 4 条行为准则。
> **权威原文**：`e:\各种PY程序\andrej-karpathy-skills\CLAUDE.md`（借思想不引代码：skill 本体**不安装**，仅取原则文本；改原则以原文为准，勿以摘要为准）。
> **适用范围**：所有 Skill（FinClaw 1031 + ECC 64 + agent-skills 24）与本项目的一切代码改动，优先级高于各 Skill 自带流程。
> **权衡提示（原文自带）**：本准则偏谨慎而非速度。琐碎任务（拼写修复、显而易见的一行改动）自行判断，不必每次走完整流程 —— 目标是非琐碎工作少犯错，不是拖慢简单任务。
...
- 有更简单的路就说出来，该反对时反对。
- 卡住了就停下，指名哪里不清楚并要求澄清。
- **量化落地**：动手前先跑代码验证当前状态，勿信过时的诊断报告/注释/文档声明
  （memory ID 24715792 #3；DTE-1 教训：注释称 SimulatedBroker 实为内部算术记账）。
- **检验标准**：我的假设写进方案了么？有没有更简单但我没说的路？

**2. 简洁优先** — 用最少的代码解决问题，不推测未来

- 不加要求之外的功能；不为一次性代码建抽象。
- 不加未被要求的"灵活性/可配置性"（本系统尤忌：新 flag 必须确有开关需求）。
- 不为不可能发生的场景做错误处理。
- **200 行能写成 50 行就重写**。
- **检验标准**：资深工程师会觉得这过度设计么？新增的抽象/配置/分支，是需求还是我在预防想象中的问题？

**3. 精准修改** — 只碰必须碰的，只清理自己造成的混乱

- 不"顺手改进"相邻的代码、注释、格式；不重构没坏的东西。
- 匹配现有风格，即使你更想用另一种写法。
- 看到**无关的**死代码：提一句，**不要删**。
- 自己的改动造成的孤儿（未用导入/变量/函数）：必须清掉。
- **量化落地**：删 dead code 前先跑 `check_dangling_refs.py` + `archive_dead_code.py`
  （memory ID 55922757）；P0 文件改动会被 `ruff_incremental_gate` 拦截，最小 diff 也是最省事的 diff。
- **检验标准**：diff 里每一行都能直接追溯到用户请求么？有没有顺带改无关文件？

**4. 目标驱动执行** — 定义成功标准，循环验证直到达成

- 把指令式任务**转写成可验证目标**再动手：
  - "加校验" → "为非法输入写测试，然后让它们通过"
  - "修 bug" → "先写复现该 bug 的测试，然后让它通过"
  - "重构 X" → "重构前后测试都通过"
- 多步骤任务先给出简短计划：`1. [步骤] → 验证: [检查]`。
- **量化落地**：验证手段就是下方 §1 门禁三件套（ruff + debt_gate + pytest）；
  声明完成前必跑，零未验证交付（memory ID 87513358 DoD；08-29 CTX-A 复现教训：
  94 用例全绿 ≠ 完成，ruff/black 未跑）。
- **检验标准**：这次任务的成功标准是可判定的（命令 + 退出码）么？还是只有"让它工作"这种弱标准？

> **与本文其他章节的关系**：§3 DoD 是本系统在"门禁三件套 / 执行闭环"维度上的**具体成功标准**；
> §4 反理性化表是原则 1（编码前思考）在本系统历史踩坑上的**反例清单**。
> 用法：先按原则 1-3 决定"怎么做、做到哪停"，再按 §3 定义成功标准，最后按原则 4 循环验证直到达成。
