...
| 资金口径唯一定义（现存 5M/5M/5M/2M/3M 五套并存） | 用户（口径拍板） | 尽快 | 再平衡目标高估 ~82%、对冲手数 ~1.82×；详见同报告 §P1-2。**工程半边已落地 (2026-09-11, AUTO 批)**：11 处活跃消费点收敛至 `config/risk_thresholds.yaml` → `capital_base` 段单一事实源（默认 5M 现行为不变）——**拍板后只需改 capital_base 段一处即全链生效** |
