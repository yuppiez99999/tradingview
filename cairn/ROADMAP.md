...
8. **CI 门禁维护与遗留清理（2026-08-12 已完成）** — R1/R4 已落地（ci.yml 6 脚本真实实现 + quality-gate.yml PR 增量门禁），C6 稳定 PASS。✅ **工作区未跟踪文件 99 → 0 收敛闭环**：经甄别 99 个文件绝大多数是有效新增代码/测试（Wave6 因子、contracts、execution 闭环、fineng 测试、AI hedge fund 模块、workflow 拆分等），非垃圾；改用分批提交（5 批共 129 文件，全部门禁 0 阻止性）而非删除，零有效工作丢失。剩余待办：① `_run_v9_regression.py` 暴露的 D1 压力测试既有问题（非 R1 引入，独立跟踪）；② 后续任何人修改 ci.yml 的 `python scripts/xxx.py` 引用，须同步 `ci_integrity_check.py` 机检。经验见 `cairn/ci-repair-and-gate-lessons-20260812.md`。
