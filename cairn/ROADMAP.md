...
8. **CI 门禁维护与遗留清理（2026-08-12 新增）** — R1/R4 已落地（ci.yml 6 脚本真实实现 + quality-gate.yml PR 增量门禁），C6 稳定 PASS。待办：① `_run_v9_regression.py` 暴露的 D1 压力测试既有问题（非 R1 引入，独立跟踪）；② 工作区剩 99 个未跟踪临时文件（`.tmp_pip/`、`.venv/`、`_ms_*.txt` 等）需清理避免污染门禁；③ 后续任何人修改 ci.yml 的 `python scripts/xxx.py` 引用，须同步 `ci_integrity_check.py` 机检。经验见 `cairn/ci-repair-and-gate-lessons-20260812.md`。
