...
- **DTZ005 全仓清零**：771→0（12 批次，~250 文件，`datetime.now()`→`now_bj()`），详见同日上方 LOG 条目。
- **ruff 全仓清理 556→0**：
  - F401 未使用导入删除 + F821 `now_bj` 补 import + I 排序：556→72（ruff --fix 自动 + 手动补 101 文件 import）
  - T201 print 豁免（`backtests/` + `300万计划/` CLI 脚本）：72→27
  - E701/E741/F841/B905/N806 豁免（scripts/ + backtests/ + guards/）：27→4
  - C901 阈值 15→18 + guards/ C901 豁免（`guard_kill_switch` 26 / `_extract_holding_symbols` 19 需后续重构）：4→0
- **三端一致**：`be3b5331`（主系统 + tradingview + CNB + GitHub）
- **指针**：`ruff.toml`（豁免配置）；`utils/datetime_utils.py`（`now_bj()` 定义）
...
- **进度条口径终稿（三条并存、非替代；分母一律按「12-31 = 第 120 天」日历累加）**：

| 条 | 窗口 | 分母 | 09-12 实测 |
|---|---|---|---|
| **整体** | 2026-09-03 → 策略六线末 **2027-06-30** | **301 天** | **9/301 ≈ 3.0%** |
| 2027 Q1 收口 | → 2027-03-31 | 210 天 | 69.8%（刻度点） |
| v8.7 发布段（子条） | → 2026-12-31 | 120 天 | 9/120 = **7.5%** |

- **中途一次修订（用户拍板，非自行选择）**：整体条终点先取 `2027-03-31`（210 天 / 4.3%），用户答复「2027-06（策略六线末」⇒ 改取 `ROADMAP §策略六线（2026-09 ~ 2027-06）` 的**末日 2027-06-30**，分母 = 120 + 90（2027 Q1）+ 91（2027 Q2）= **301 天**。刻度重算（占 301）：09-19 冻结窗 5.3% / 10-13 评估计算 13.3% / 12-10 功能冻结 32.6% / 12-31 发布 39.9% / 2027-01-02~09 生产切换窗 40.5~42.9% / 03-31 Q1 收口 69.8% / 06-30 策略六线末 100%。
- **顺带修正一处 off-by-one（自我更正）**：按「12-31 = 第 120 天」累加，2027-01-02 应为第 **122** 天、01-09 为第 **129** 天；上一轮误按「+1 起点」写成 121/128 ⇒ 生产切换窗刻度由 58.1%/61.4% 修正为 **40.5%/42.9%**。
- **三端同源判据（可复用）**：全仓 grep `9/120|7.5%|9/210|9/301` ⇒ 仅 3 处持有总进度口径且已互相对齐 = `cairn/ROADMAP.md §TIME PROGRESS` + `docs/排期计划总览_20260912.md §1.1`（L32/L44）+ 同名 `.html §1.1`（L228/L247）。**旧快照 `_20260910.md/.html` 按「刷新即新快照、不追溯改写」不动**。⇒ **改「快照型数值」必须先 grep 全仓找齐所有持有该口径的文件并一并改**；新增口径用「**整体条 + 子条**」两段式而**非覆盖**（保留原刻度，不破既有对外引用）。
- **HTML 标签重叠处置**：`§1.1 ①` 行 marks 收敛为 4 个粗刻度（09-19 / 10-13 / 12-10 / 12-31）防标签重叠，细刻度交由 `②` 发布段行承担。
...
- **SC-29（P0）`build_plan_executor` 排期契约错配**：`metadata.build_phases` 在 `500万建仓计划_20260706.json` 中是**整数 4（阶段数量）**，旧码却按 `[{start,duration}]` 迭代 → `for i,p in enumerate(4)` → `TypeError: 'int' object is not iterable`。**实跑复现**：`get_active_phase` / `generate_daily_orders` / `get_build_status` **任意日期三处整条崩溃** ⇒ 盘前建仓指令生成链彻底不可用。根因 = 真实排期事实源其实是 `phase_summary[].start/duration_days`（`build_phases` 只是计数，被误当配置载体）。修复 = 新增 `_resolve_phase_schedule()` 严进严出解析 + 非法/缺失 **WARNING + 兜底排期**（绝不 TypeError）。**修复后实测**：07-10→phase1 / 07-25→phase2 / 08-20→phase3 / 09-13→phase4 / 10-15→completed / 06-01→not_started；`generate_daily_orders(2026-09-13)` → **14 上午 + 17 下午，day_capital 742,184**。
- **SC-30（P1）`daily_build_and_hedge` 无排期谎报"已完成"**：`_load_plan` 回退候选 = `500万建仓计划_20260706.json`，该文件**无 `execution_plan` 段**（真实排期同在 `phase_summary[]`）；`get_active_phase` 只认 `execution_plan.phase1..4` → 取不到即 `return None, "completed"` → `run()` 见 phase 空 → `{"status":"no_active_phase"}` → CLI `sys.exit(1)`。**实跑复现**：`run() -> {'status': 'no_active_phase'}`。**影响**：`run()` 是"建仓指令+市场状态+风险预算+对冲计划+报告"唯一入口 —— **整条链每日直接退出，退出原因却是"已完成"这种看似正常的语义，零告警**。修复 = 新增 `_phase_from_phase_summary()`（start+duration_days 推 end）+ 无事实源显式标 `no_schedule`（不再冒充 completed）+ `run()` ERROR 留痕并回传 phase_key。**修复后实测**：`get_active_phase()->'phase4'`、`run()->{'status':'success'}`。
- **SC-31（P1）幽灵 API + 异常元组缺 ImportError**：`wind_get_index_data(windcode, days=70)` 在 `tools/wind_mcp_fetcher.py` 重构后**已不存在**（真实名 `wind_get_index_kline(windcode, begin_date, end_date, period)`，返回 `list[dict]` 而非 DataFrame）；**更关键**：调用处异常元组列了 8 类却**不含 `ImportError`** ⇒ `ImportError` **穿透所有 fail-safe**。**实跑复现**：`_fetch_index_returns('000300.SH')` / `assess_market_state()` / `gamma._get_market_ma60()` / `theta._get_etf_spots()` 全部 `ImportError` 穿透。**影响**：① 市场状态评估（VIX/指数收益/regime）**整段崩溃**；② Gamma 尾部保护触发器（ma60 破位）**永久失效**；③ 备兑看涨现价链失效（`theta_engine` 同源幽灵 `wind_get_etf_quote`）。修复 = 新增 `_fetch_index_kline_wind()`（daily_build_and_hedge + gamma_engine 各一份）封装**真实 API** 并归一为含 `close` 列的 DataFrame；`theta_engine` 改走 `fetch_realtime_price`；**三处异常元组补 `ImportError`/`IndexError`**。**修复后实测**：`assess_market_state()` 正常返回（`regime=cautious, data_degraded=True` 属沙箱无 Wind 余额的**预期降级**）；`gamma._get_market_ma60()->None`、`theta._get_etf_spots()->{}`（降级不穿透）。
- **SC-32（P2）`ETFMonitor` 幽灵类 → ETF 资金流信号从未进决策**：`from utils.etf_flow_monitor import ETFMonitor` —— 该模块真实导出 `ETFRealTimeTracker` / `get_etf_flow_summary`，**无 `ETFMonitor`**；`except (ImportError, AttributeError)` 静默吞成 `etf_data = {}` ⇒ `etf_signal` 恒 `neutral`、`etf_net_flow` 恒 0 ⇒ **紧急协议里所有 ETF 资金流维度判据永不触发**（含"宏观过热+ETF大幅流出→EXTREME"、"ETF净流出>50亿→CRITICAL"），而这些判据是建仓暂停/减仓的直接触发条件。修复 = 改走 `get_etf_flow_summary()` + 导入缺失/取数失败分别 WARNING（不静默为空）。
- **SC-33（P2）`lgb_enhanced_trainer.load_model_meta` 幽灵符号**：`system_integration.py:611` 的 `from lgb_enhanced_trainer import POSITION_SYMBOLS, load_model_meta` —— 后者真实归属 `lgb_trainer.persistence`；且异常被 `except Exception` + **`logger.debug`** 吞掉 ⇒ "QLib 预热失败 → 历史模型 CV IC 兜底预热"这条**唯一兜底路径从未生效** ⇒ 漂移检测器在 QLib 不可用时长期**0 样本冷启动**，检测灵敏度形同虚设（且失效在 debug 级下不可见）。修复 = 真实归属导入 + 日志 debug→warning。（`POSITION_SYMBOLS` 的再导出真实存在，保留不动。）
- **SC-34（P2）`load_historical_returns_from_cache` 只有调用方无实现**：`cli/modes/risk_monitor.py:21` **模块顶**导入 `utils.cli_helpers.load_historical_returns_from_cache`，而该函数在 `cli_helpers` 中不存在 ⇒ `--risk-monitor` 模式**整个模块加载失败**（CLI 以 WARNING 跳过，不报错）⇒ 组合滚动 60 日相关性监控（P0-7 能力）**实际缺失**且长期不可见。修复 = 在 `utils/cli_helpers` 补齐实现（走真实 `data_provider.get_historical_data`，纯码/`.SH`/`.SZ` 多后缀尝试，无数据返回 `{}` + WARNING 留痕；调用方已按"无数据→提示跳过"处理，不构成假通过）。
- **共性根因（第六次同一把尺子）**：本批与前五批（伪审计来源串 / 物理量级无护栏 / 静态数字冒充运行时值 / 同一兜底值兼作分子分母 / NaN 比较恒 False）同族，同属"**缺数据 ≠ 通过**"铁律的失败路径变体；本批**新增两个可复用检查项**：① **「符号存在性」未纳入任何门禁** —— 幽灵类/幽灵函数/幽灵导出全部通过所有静态门禁（ruff F821 只查**未定义名**，不查 **import 目标是否存在**）；② **「异常元组缺 ImportError」** —— 依赖缺失属"取不到数据"，但常见异常元组只列数据处理类异常，于是依赖缺失**穿透 fail-safe**。另 ③ **「回退候选与解析口径不匹配」**（SC-29/30）：同一模块"能读到的文件"与"期望的结构"不一致，失败被默认值兜成**语义相反的结论**（无排期 → 已完成）。
- **验证（【R】本机实测，先红后绿）**：新增 `tests/unit/test_sc29_34_ghost_contract_unit.py` **37 例**。**先红**：stash 全部源修复后旧码跑 → **32 failed / 5 passed**（核心断言如实显示旧码对 09-13 抛 `TypeError`、对无排期返回 `completed`、对幽灵 API 抛 `ImportError`）；**后绿**：37 passed。受影响面 6 文件 **210 passed / 1 failed / 1 skipped**，唯一失败为 **pre-existing** 基线失败（沙箱缺 `configs/account_structure.yaml`，基线复跑确认与本次无关）。**遗留测试口径迁移**：`test_g7_daily_build_hedge_boost.py`（5 例 `completed`→`no_schedule` + 6 例幽灵 API mock→真实封装）+ `test_gamma_engine_unit.py`（6 例同源迁移），迁移后同量级全绿。
- **门禁实证**：`ruff check` 改动 9 文件 **All checks passed**；`ruff --select BLE001,F401,F811` 全仓 **5 条 / 与基线逐条一致（零新增，全在他文件）**；`f821_fullscan_gate` 1949 文件 **0 条 PASSED**；`industrial_grade_check` **10 PASS / 2 WARN / 0 FAIL** 与 09-11 基线逐项一致；`check_utf8_mojibake` 401 文件 **exit 0**；`py_compile` 改动 6 源文件 OK。
- **未做（如实声明）**：**【P】生产机运行时行为未观测**（本批全部沙箱实跑；`assess_market_state` 的 `data_degraded=True` 属沙箱无 Wind 余额的预期降级）；`research/` 消费方未改；`theta_engine`/`gamma_engine` 的**调用方参数口径**未重审（本次只修幽灵符号）；排期口径存在**两个事实源**（`build_plan_executor._FALLBACK_PHASE_SCHEDULE` 与计划文件 `phase_summary`），兜底表仅在计划文件无相位时使用，是否收敛属计划文件 schema 治理（非本批 bug），建议后续单独评估。
- **指针**：Issue #13；`build_plan_executor.py` `_resolve_phase_schedule`；`utils/execution/daily_build_and_hedge.py` `_phase_from_phase_summary` / `_fetch_index_kline_wind`；`utils/gamma_engine.py` `_fetch_index_kline_wind`；`utils/theta_engine.py` `_get_etf_spots`；`utils/cli_helpers.py` `load_historical_returns_from_cache`；`system_integration.py`；`tests/unit/test_sc29_34_ghost_contract_unit.py`；`docs/代码质量与系统Bug审查_20260912.md` §07 / §05 缺口4 / 附录B 台账 SC-29~34。

## 2026-09-12 · Issue #13 续批：SC-28 — mvsk_regime_detector 数据源失败路径静默降级（审查缺口3 闭环）

- **接单判定**：用户「继续开发」。09-13~18 主线节点（shadow 30 天窗首日 / ER-2.x 双签 / D11 复验）全卡生产机与人工；AUTO-1~10 全闭环；SC-19~27 已修。故接审查报告 §05 **覆盖缺口 3「correlation_regime / mvsk 数据源失败路径」**（前轮仅核验判定方向，未审失败路径）。另核实 SC-15（P1）**已由 `25c1f2d1` 修复**（实测约束后总和=1.0、现金≥floor、单源主导 0.833→0.5），本批不复做。
- **一、缺口拆解**：`correlation_regime.py` 前轮已确认为**零引用孤儿模块**（登记不动），其 `_fetch_price_series` 失败路径为 `raise ValueError` **fail-fast 非静默** → 无需本类修复。`mvsk_regime_detector.py` 前轮"实测正常"只核了**正常路径单调性**，**失败路径未审** → 本轮在此挖出 SC-28。
- **二、SC-28（P2）实跑复现（HEAD 旧码）**：`RegimeDetector.detect` 无输入校验，`_compute_moments` 对非有限值不处理 → NaN 使 `m2 <= 0` 恒 False（`m2=NaN`）、阈值比较 `vol_rank>=q`/`kurt>t` 恒 False → **全 NaN 输入静默返回 `low_vol_normal` + `use_mvsk=False` + `volatility=NaN` + 零告警**；含 ±inf、空数组同；标量输入直接 `IndexError`。**缺数据与"正常低波 regime"不可区分** —— 与 SC-15/16/19~27 同族：**主路径判据对、失败路径继续静默**。
- **二、影响面（真实消费点）**：`research/regime_fixed_backtest.py:88,100` + `research/regime_switch_backtest.py:84` **仅消费 `r_det.use_mvsk`** → 数据源故障静默回退 MV，`volatility=NaN` 无人读 → **回测在"看似正常"的 regime 判定下产出，结论可信度被静默污染**（与 §六 P3/P4 结论同源消费点）。
- **三、修复（fail-closed 显式化）**：新增 `Regime.UNKNOWN` + `RegimeResult.data_available`（默认 True 向后兼容）+ `unavailable_reason`。判据 = `n_valid==0` 或 `n_valid/T < MIN_VALID_RATIO(0.5)` 或标量/空/无资产列 → `_unavailable_result()`：`use_mvsk=False` / `trigger="data_unavailable"` / **诊断字段输出 0.0 而非 NaN**（阻断 NaN 向下游传播）/ WARNING 带 `n_valid/total`。少数非有限值 → 剔除后判定 + WARNING（不静默）；清洗后长度 < `min_periods` → 既有短历史回退。**全零收益（合法常量输入）仍判 `LOW_VOL_NORMAL`**，与"无数据"严格区分。
- **四、验证（【R】本机实测，先红后绿）**：新增 `tests/unit/test_mvsk_regime_detector_unit.py::TestDataUnavailablePath` **13 例**。**先红**：stash 源修复后旧码跑 → **13 failed**（核心断言如实显示旧码对全 NaN 返回 `LOW_VOL_NORMAL` 且 `volatility=nan`；标量输入甚至 `IndexError`）；**后绿**：13 passed，该文件 **12→25 passed**（原 12 例零回归）。相关面 7 文件（mvsk_regime + mvsk_optimizer + vol_regime_weighter + auto_trading_vol_regime + regime_aware_allocator + regime_gated_transformer + enhanced_signal_fusion）**181 passed / 0 failed**（+13）。
- **五、门禁实证**：`ruff check` 两文件 **All checks passed**；`check_utf8_mojibake` 401 文件 **exit 0**；`f821_fullscan_gate` **0 条 PASSED**；`industrial_grade_check` **10 PASS / 2 WARN / 0 FAIL** 与 09-11 基线逐项一致（C1 xtquant 物理阻塞 / C10 沙箱无 fills）；`py_compile` OK。
...
- **接单**：用户「按照排期计划继续」。09-13~18 主线节点（shadow 30 天窗首日 / ER-2.x 双签 / D11 复验）全卡生产机与人工；AUTO-1~10 全闭环。故接 0912 审查报告 §05 **覆盖缺口 2「S4 假策略修复质量」**（`96739a83` 未复审）。
- **一、S4 主体修复独立复核：✅ 通过**。实测四腿成本率互异 —— S1 0% / S2 1.50% / S3 1.25% / S4 1.55%，tail 确实在 collar 之上独立叠加，「S3=S4 假策略」已消除。
- **二、但顺同一张成本解析链扫出 SC-25~27 三处静默降级**（与 SC-19~24 同族：**主路径改对、兜底路径继续静默**）：
  1. **SC-25（P2）`_tail_pct` 来源串冒充配置值**：`scripts/run_200w_etf_backtest.py:320-323` 缺配置时返回裸 float `0.005`，而调用方来源串**恒**宣称 `tail_protection.budget_annual_pct` —— 用的是兜底值却宣称是配置值。对比 `_protective_put_pct` 会明确写「内置兜底 1.5%」，**口径可审计性不一致**（审查报告自身判据要求成本口径可审计）。
  2. **SC-26（P3）非法值静默兜底**：`budget_annual_pct` 写成 `"0.3%"` 或 `True` 时静默落兜底。**bool 是 int 子类**，`True` 会被当 1.0 成本率（同 SC-9 的 bool 混入风格权重同源）。
  3. **SC-27（P1）保护成本率无区间护栏**：**实测**单位错写（`0.3` 意即 0.3%）→ S2 成本率 **50%**、S4 **31.25%**，仅成本项一年把净值打到 **0.73**（腰斩再腰斩），且**零告警** → 回测据此产出「策略不可交易」的假结论。
- **修复**：① `_tail_pct` 改为返回 `(值, 来源串)` 三元组，与 `_protective_put_pct` 同构，兜底/非法均显式标注；② 新增 `_cost_rate_guard(rate, src, leg)` 区间护栏 —— 越界 `(0, 0.10]` 或非有限值一律**归零 + WARNING**，不静默放行（对齐铁律「缺数据 ≠ 通过」）；③ `validate_configs.py` 新增**护栏10**：`tail_protection.enabled` 为真但 `budget_annual_pct` 缺失/非法即报错，且量级须落 `(0, 0.02]`（疑单位错写即拦），并登记 2 例自测变异。
- **验证（【R】本机实测，先红后绿）**：新增 5 例回归 `tests/unit/test_backtest_circuit_breaker.py`。**先红**：stash 掉源修复后在旧码上跑 → **4 failed / 2 passed**（其中 S2 实测输出 `0.5` 未被任何护栏拦下，直接坐实 SC-27）；**后绿**：修复后全绿。受影响面 3 文件基线对照 —— 基线 **50 passed / 25 skipped** → 修复后 **55 passed / 25 skipped**（+5 新增，零回归，skip 数不变，全为沙箱缺 `config/portfolio_200w_etf_v91.yaml`）。
- **门禁实证**：`ruff check` 改动 3 文件 **All checks passed**；专项 `BLE001,F401,F811,E402,DTZ005,F821,W291` 全绿；`python scripts/validate_configs.py --selftest` **PASS**（含新增 2 例变异被正确拦下）；`check_utf8_mojibake` 400 文件 **exit 0**；`py_compile` OK。
- **未做（如实声明）**：`config/portfolio_200w_etf_v91.yaml` **不在版本库**（`config/*` gitignored，R-10 决策把事实源录 ROADMAP/LOG），故 v9.1 实配置下的端到端回测数值**未复跑**，护栏10 的实配置命中性未在生产配置上验证；**【P】生产机运行时行为未观测**。
- **方法论沉淀（第三次同一把尺子）**：「成本口径统一」类修复必须配**兜底路径残留扫描**——主路径改对后，兜底路径因其平时不触发而长期潜伏；本次新增的坏味道检查项 = **「来源串宣称的出处 ≠ 实际取值出处」**（伪审计）与 **「物理量级无护栏」**（单位错写放大 100 倍静默通过）。
- **指针**：Issue #13；`docs/代码质量与系统Bug审查_20260912.md` §04 评级汇总 SC-25~27 / §05 缺口 2；`scripts/run_200w_etf_backtest.py` `_tail_pct` / `_cost_rate_guard`；`scripts/validate_configs.py` 护栏10；`tests/unit/test_backtest_circuit_breaker.py`。
...
- **背景**：Issue #30「11个安全问题」。仓库无该扫描产物，故以项目自带安全门禁（`.github/workflows/quality-gate.yml` → `bandit -c bandit.yaml -r <files> -ll -ii`，中危即 FAIL）自主复扫首方代码，**实测精确复现 11 项**（排除 `external/`、`unsloth_compiled_cache/`、`qlib/` 第三方）：B301 反序列化 ×3、B310 urlopen scheme ×6、B314 XML ×1、B608 SQL ×1。纳入同源面后另有 B314 ×1（mobius 远程论文 XML）、B615 HF 下载 revision 未固定 ×4，一并修复。
- **修复（实质防护，非仅消警）**：① 新增 `utils/safe_xml.py`（defusedxml 优先 + stdlib `forbid_dtd=True` 兜底，DTD/实体炸弹在解析阶段拒绝）与 `utils/safe_url.py`（scheme 白名单 http/https，`file://`/`ftp://`/`data:` fail-closed 拒绝）。② B301：`ms_strategy/cloud_train/{backtest,simple_backtest}.py` 保留 SHA256 侧车校验语义 + `# nosec B301` 附前置控制说明；`research/backtest_current_portfolio.py` 缓存加载新增**路径越界校验**（resolve 后必须落在 `cache/` 内）+ 非空/50MB 上限校验。③ B608：`checkpointer.py` 去 f-string，改显式字面量 SQL 元组 + `?` 参数绑定（表名不再参与拼接）。④ B314：`scripts/run_unit_tests.py` + `mobius_addon/agents/retriever.py` 改走 `safe_xml_parse/fromstring`；arXiv 端点 `http://` → `https://`。⑤ B615：`lgb_trainer/llm_finetune/finetune_sentiment_model.py` 新增 `FinetuneConfig.base_model_revision` 并透传到 4 处 `from_pretrained/revision=`。⑥ B310：`tools/start_and_test_ollama{2,3,_gpu}.py` 6 处 `urlopen` 改走 `safe_urlopen`。
- **验证（【R】本机实测）**：`bandit -r . -x <第三方> -ll -ii` 首方 **11 → 0**（含新增 B615 面 15 → 0）；CI 口径对改动文件 `bandit -c bandit.yaml -r <changed> -ll -ii` **No issues identified**；新增回归 `tests/unit/test_security_hardening_20260912.py` **18 passed / 4 skipped**（skip 为沙箱缺 torch/langgraph 的运行时分支，附源码级断言兜底），连同 `test_adversarial_news_guard_unit` 共 **57 passed**；`test_g7_dqc_checkpoints_boost` 面 **83 passed**；`ruff check` 改动 12 文件 All checks passed；`ruff_incremental_gate` 通过；`check_utf8_mojibake` exit 0；`check_no_print_p0` OK；`industrial_grade_check` **10 PASS / 2 WARN / 0 FAIL** 与 09-11 基线逐项一致。**基线对照**：`git stash` 下 `test_item12_*` / `test_ai_hedge_fund_sprint2_real_links` 5 例失败与改动后**逐项一致**（沙箱缺 `_archive/`、`config/portfolio.yaml`），零新增失败。
...
- **新增 `cairn/merge-and-gate-playbook-20260911.md`**（跨线合并首份 playbook）：①合并前必算**脏文件 ∩ 入站改动**，交集非空 → `Copy-Item` 备份 + `git stash push -- <具体文件>`；②append-only 日志冲突**取并集**（只删标记行，大文件用脚本按行扫描）；③merge 提交被 **DTZ005** 拦（门禁把暂存 `.py` 整体交 ruff）→ 改 `now_bj()`，**不得**顺手把审计时间戳改 `utc_iso()`；④批量插 import 用 **`ast` 顶层 import 的 `end_lineno`**（按行号/正则会劈开多行导入）；⑤双端推送 `-c http.version=HTTP/1.1` + 密钥扫描 + `0/0` 收口。
- **ROADMAP §专项文档指针**增两行（ETF 模型文档指向 §v9.1；新增合并 playbook 指针）。
- **可复用方法论（已入长期记忆）**: 「关闭新机制的对照组 + 与旧口径数值重合」= 重构/数据源变更的自证法；「配置写了机制 ≠ 引擎消费了它」= 下结论前先 grep 消费方。

## 2026-09-11 · 4.3% 入 ROADMAP + Wind 五年证据基座 + L1~L4 减仓熔断入净值 + 基准对照

- **R-10 口径入 ROADMAP**: `§CURRENT STATE` 新增 `etf_option_submodel.p9_200w_v91`（净年化 4.3% = 毛 5.5% − Collar 1.2%，区间 [3.5%,5.5%]，回撤预算 15%/范围 10%~18%，四条验收口径，`scope_note` 明示**仅适用 ETF 子组合**）；`performance_targets` 增 `etf_option_subportfolio` 行，明确与 8%~18%（证券200+期货100）**不是一个口径、禁止互相引用**；决策登记表增 R-10；顺带修正 gates_trio 实测值并关闭 D1 遗留。
- **Wind 五年证据基座**: `tools/wind_mcp_fetcher.py` 新增 `wind_get_kline_range()`（**显式日期区间**，支持 stock/fund/index；日期风格 `YYYYMMDD`→`YYYY-MM-DD` 双试；复权参数名按域区分 —— 基金 `price_type=1`、指数 `aftype=0`）+ `wind_get_index_kline()` + `WIND_INDEX_ENDPOINT`（`vserver_index_data`）。新增 `scripts/fetch_wind_v91_ohlc.py`：8 标的 + 000300 基准各 1365 行（2021-01-04~2026-08-20）落盘 `data/etf_option_backtest/wind_2021_2026/`（逐标的 parquet + 长表 + **manifest.json** 记录来源/窗口/复权/行数/复现命令）；任一标的为空即非零退出。
- **L1~L4 减仓熔断入净值**（`scripts/run_200w_etf_backtest.py`）: ①按配置分层拆**权益腿(核心+卫星)/防御腿(压舱+现金)**，减仓只动权益腿、腾出权重按原比例放大防御腿（不另造收益假设）；②档位由**已实现回撤**触发、权重**次日生效**（无前视，有专门回归测试守卫）；③回补滞回 2pp 为显式参数 `--cb-recovery-buffer`（配置未定义回补规则，假设不藏代码）；④新增 `--data-source local|wind`、`--benchmark`、`--no-circuit-breaker`；⑤报告/JSON 增熔断统计（各档天数、切换次数、权益权重最低/均值、**累计单边换手**）与基准对照段。
- **三组对照（同一 1365 交易日）**: A 本地+无 CB → S1 **+5.4205%**/15.7581%、S3 **+4.1112%**/18.7265%；B Wind+无 CB → +5.4200%/15.7602%、+4.1107%/18.7286%（**A≈B 差 ≤0.0005pp** ⇒ 跨源等价 + sleeve 重构在基准档与原口径完全等价）；C Wind+CB → S1 **+4.9014%**/15.4757%、S3 **+3.4508%**/16.9448%。
- **L1~L4 净效果（诚实结论: 买回撤卖收益）**: S3 削回撤 **−1.78pp**(18.73→16.94)、代价 **−0.66pp 年化**；S1 削 −0.28pp、代价 −0.52pp；换手 134%×4bp = 0.054% 全期 ≈ **0.010%/年**（可忽略）。**L4(dd≥18%) 全期未触发** —— 档位由已实现回撤触发、减仓本身压低回撤，存在结构性自限。
- **基准**: 000300 同期 年化 **−2.50%** / 回撤 **45.60%**；S3+CB 超额 **+5.95pp**、回撤低 28.7pp。
- **验收达成度**: ①净年化 ∈[3.5%,5.5%] → S3+CB **3.45% 边缘未达**(差 0.05pp)、S3 无 CB 4.11% 达标；②回撤 ≤15% → **未达成**(最好 15.48%)；③Wind 价格基零容差 PASS 8/8；④门禁 0 FAIL。
- **门禁实测（09-11 17:23）**: `industrial_grade_check` **10P/2W/0F**（C1 xtquant 物理阻塞 + C10 fills 最新为 09-10 非当日）；`assert_data_validity` **12P/0F（D1 遗留已闭环）**；`engineering_debt_gate` **BLOCK**（D11 shadow 12/20，09-17 消解，属已知）。新增 `tests/unit/test_backtest_circuit_breaker.py` **7 passed**（含前视守卫、fail-closed、等价性），`test_portfolio_v91_config.py` 24 passed；ruff + 增量门禁全绿。
- **已知缺陷（独立跟踪）**: `test_etf_option_combo/test_performance.py::TestStatePersistenceLatency` 计时阈值(50ms)在未改动代码上连跑 3 次出现 1 次失败(64.06ms) ⇒ 既存 flaky，非本次引入。
- **交付**: 汇总报告 `每日报告归档/2026-09-11/ETF期权5年证据基座_含L1-L4与基准_20260911.md` + 自动报告/JSON `ETF期权对冲回测_Phase2_v91_wind_cb_20260911.*`；取数脚本 `scripts/fetch_wind_v91_ohlc.py`。

## 2026-09-11 · v9.1 目标口径统一为自下而上 4.3%（含基据块）+ 清除 4 处旧口径残留

- **问题本体**: 诊断书自身卷四(毛5.5%/净4.3%) vs 卷六(目标5.5%~6.5%)矛盾 → 目标高于自身测算上限。上轮已改 `target.annual_return: 0.043`，但**旧口径仍以注释/派生值/情景表形态留在配置里**，而护栏看不见注释 ⇒ 全套门禁依旧全绿（典型假 PASS），本次一并收口。
- **口径唯一事实源**: `annual_return = net_return_center = gross_return_center(0.055) − hedge_cost_target_pct(0.012) = 0.043`；`target_basis` 基据块（method=bottom_up + gross/net_formula + cross_check）；`legacy_target_infeasible`(0.08) 并存证**禁止删除**（"8% 与 15% 不可兼得"本身是诊断书核心证据）。
- **配置清除 4 处残留**: ①文件头"目标修正 5.5%~6.5%/回撤10%~15%" → 4.3%/15%(范围10%~18%)；②`collar.call_leg.strike_floor_pct` 0.14(=6%+8pp) → **0.123**(=4.3%+8pp)；③`risk_disclaimers[0]` "下调至 5.5%~6.5%" → "净 4.3%(毛 5.5%)"；④`scenario_analysis.base` [5.5,6.5] → **[4.0,5.0]**（概率加权 0.25×9.5+0.5×4.5+0.25×(−1.0) = 4.375% ≈ 净中枢 4.3%，差 0.075pp）。
- **护栏扩 5 条**（`validate_configs.py::check_portfolio_v91`）: 基据块必须存在且 method/formula 可复核 → 拦"删基据只留数字"；`annual_return == net_return_center`(±5e-4) → 拦第二口径；`strike_floor_pct == target + 0.08`(±0.005) → 拦派生值不联动；情景概率合计=1 且加权中枢=净中枢(±0.5pp) → 拦情景另立口径；`legacy_target_infeasible.verdict` 非空 → 拦删证。
- **回归测试**: `test_portfolio_v91_config.py` 新增 5 个变异算子（删基据块/目标脱钩/地板过期/情景抬高/删并存证）接入参数化负向用例 → **24 passed**（16 个负向变异全绿）。
- **「修复前会失败」证明**: 把新增护栏块**手术剥离**成基线副本，与当前版对同一批变异双跑 → 5/5 **基线未拦 / 现在命中**（防"护栏看起来接好实则空转"）。
- **验证**: `validate_configs.py` exit 0（32 config）｜`--selftest` PASS（含 7 硬伤变异）｜pytest 24 passed｜`ruff_incremental_gate` 通过。
- **附带修复（验证中暴露的真缺陷）**: `validate_configs.py` 成功路径 `print("全部通过 ✓")` 在 **GBK 控制台**抛 `UnicodeEncodeError`（`\u2713`）⇒ **校验成功反以非零退出码收场**（门禁假失败，易被误读成配置有问题）。改为 `print("全部通过 (PASS)")` + 注释禁用非 GBK 字符；实测不设 `PYTHONIOENCODING` 的原生 GBK 控制台下现在 exit 0（修复前直接抛栈）。教训：门禁脚本的**成功路径输出**也必须过 GBK 兼容性检查，否则"绿灯"永远走不到 return 0。
- **边界**: `strike_floor_pct` 全仓**无引擎消费者**（仅护栏/测试命中，`collar.py` 的 call_leg 是本地 Leg 变量）⇒ 0.14→0.123 是口径修正、**不改变任何回测数值**（回测成本取 `collar.cost_target_pct` 中值），本次无需重跑回测。4.3% 是期望值口径而非承诺；静态权重实测净 4.11%/回撤 15.76% 仍属未含 L1~L4 减仓的上界。
- **交付**: 报告 `每日报告归档/2026-09-11/v9.1目标口径统一_自下而上4.3_20260911.md`；复现 = `.venv/Scripts/python.exe scripts/validate_configs.py --selftest` + `pytest tests/unit/test_portfolio_v91_config.py -q`。

## 2026-09-11 · ETF 回测价格基 vs Wind MCP 逐日一致性校验（PASS 8/8，数据出处处处可复核）

- **动机**: `scripts/run_200w_etf_backtest.py` 的价格基是本地 parquet（`data/etf_option_backtest/`），**原始出处未在仓库留痕**——历史教训是 sina 未复权把份额折算 ETF `512100` 的约 +19.8% 放大成 +221.5%，一旦某条腿被未复权数据污染，回测结论会静默失真。Wind 为口径事实源 P0/P1，故对 v9.1 组合**每个持仓代码**做逐日交叉比对。
- **方法**: 本地序列解析顺序**与回测严格一致**（`all_etf_daily.parquet` → 主目录单标的 parquet → `p2_candidates_long/` → `p2_candidates/`）；Wind 侧取 `fund_data.get_fund_kline` 前复权日线（HTTP 直连优先 / CLI 兜底）；在共同交易日上算 `|Δclose|/close`，**零容差**（1e-6）；缺本地数据 / Wind 为空 / 无共同交易日 / 超阈值**任一命中即 FAIL**（缺数据不得判通过）。
- **结果**: **PASS 8/8，零偏差**。510300/510500/588000/511260/518880/511880 各 1365 个共同交易日，159915 1364，513100 1380（窗口 2020-12-11~2026-08-20），平均与最大相对偏差**全部 0.00000000** → v9.1「毛收益 5.42% / Collar 净 4.11% / 回撤 15.76%」建立在可被 Wind 复核的价格基上。
- **负向验证**: 注入本地不存在的 `999999` → exit 1（`999999: FAIL reason=本地无该代码价格数据`）；`--tolerance -1` 强制任意偏差超阈 → exit 1 / PASS 0/8；正常路径 exit 0。证明门禁真在拦而非空转。
- **交付**: 新增 `scripts/verify_etf_price_source.py`（ruff 全绿，`--config/--days/--tolerance/--out`）；机读证据 `reports/etf_price_source_check.json`；报告 `每日报告归档/2026-09-11/ETF价格基Wind交叉校验_20260911.md`。复现：`.venv/Scripts/python.exe scripts/verify_etf_price_source.py`（`NO_PROXY` 需含 `mcp.wind.com.cn`）。
- **边界（不掩盖）**: Wind 回看 `--days 1400` 覆盖 2020-11 之后，513100 的 2015–2020 段**未参与比对**，仍属未交叉验证区；只比对收盘价（回测未用成交量/额）；159915 本地少 1 天，差异日不在共同样本内，且该标的 `option_available: false` 仅作现货腿。

## 2026-09-11 · ETF 期权组合诊脉书七处硬伤修复（v9.1「守正」配置 + 期权数据链 + 成本口径重构）

- **背景**: 《贰佰万 ETF 期权组合诊脉书_20260911》`ETF期权组合诊脉书_20260911.html` 判定原方案「200万ETF+期权 / 持有5年 / 年化>8% / 回撤<15%」结构不合理、四目标不可同时达成；现实可达 = 年化 5.5%~6.5% / 回撤 10%~15%。用户指令「开始修复」。
- **七处硬伤修复落点**: ①核心仓清一色 A 股无跨资产 → v9.1 新增**压舱层** 国债15% + 黄金10%；②把最贵的科创50当压舱石 → 588000 降级出核心，卫星基准权重限 ≤7% 并标 `demoted_from_core`；③期权预算会计口径错误（本金划拨 vs 年度计提）→ 统一 `budget.basis = annual_return_provision`（1.0%~1.5%/年净值，硬上限 1.5%），且 budget / option_params / greeks.theta **三处同源单一口径**；④熊市配置自相矛盾（Put 35% 名义 vs 预算 10%）→ 删熊市条款，备兑名义 `max_notional_pct ≤ 1.0` 显式封顶 + `collar.cost_target_pct` 上限受预算硬上限约束；⑤卫星 alpha 未实证（S13 已证伪 -3.41pp）→ `alpha_aggregation=false` + `evidence_status=unproven` + `max_active_tilt_pct ≤ 3%`；⑥期权数据链未通（σ 硬编码 0.20）→ 见下条；⑦康波「五年之锚」不可证伪 → `regime_state_machine` 只由 RV/回撤/均线/宽度/估值分位/熔断驱动，`removed_anchors` 留痕。
- **硬伤⑥ 代码修复（期权数据链）**: `utils/option_data_fetcher.py` v2 — 接入真实期权链（AKShare `option_finance_board`，上交所 T 型行情），新增 `get_chain_snapshot()`（进程 TTL → 磁盘当日快照 → AKShare）、`_implied_vol()` 二分法反解 IV、`_enrich_with_greeks()`、`_rv_sigma()`（复用 `utils.alpha.iv_rank`）。审计字段 `iv_source ∈ {explicit, chain_iv, chain_atm_iv, rv_proxy, assumed_default}`；`_AKSHARE_SYMBOL_MAP` 仅覆盖 510050/510300/510500/588000/588080（**不含 159915**，诚实边界不伪造）；行权价量纲修正（优先行情「行权价」列，兜底 ÷1000，修复前 ÷10000 把 4.000 读成 0.4 导致整链被 OTM 过滤清空）；单点缓存仅在 `sigma is None` 时生效（不再静默覆盖调用方显式 σ）。消费侧 `utils/etf_option_combo/combo_base.py` 优先取真实链、无真实链回落 BS 合成网格。实测 510300 Put DTE≈103 → IV≈23%（非假设 20%），source=`akshare_chain`；`OPTION_CHAIN_OFFLINE=1` 可强制离线降级。
- **新增配置（本地事实源）**: `config/portfolio_200w_etf_v91.yaml` — 权益 53%（沪深300 28% / 中证500 7%）+ 卫星 18% + 压舱 25% + 现金 22%；`options_strategy` 以 collar 为主 + `option_params` + `data_chain`（含 iv_source 契约）+ `regime_state_machine` + `circuit_breaker.levels` L1-L4；`state_file/nav_history_file/audit_db` 与 v1.0 **隔离**。⚠️ `config/*` 被 .gitignore，本配置为本机事实源，须与本条目同读。
- **护栏（可机读，防复发）**: `scripts/validate_configs.py` 新增 `check_portfolio_v91()` —— 七处硬伤逐条固化 + 权重/目标/熔断阶梯自洽 + 状态隔离，错误以 `[硬伤N]` 前缀汇报；`--selftest` 新增 7 个变异负向用例（缺压舱 / 科创50 回核心 / 口径回退 / 熊市条款 / 启用 alpha 聚合 / 缺数据链 / 回退康波），任一变异未被拦即 FAIL。
- **回测入口修正 `scripts/run_200w_etf_backtest.py`**: ① 新增 `--config / --report-dir / --tag`（此前硬编码 v1.0，v9.1 无法回测）；② `_build_weight_map` 补 `ballast_holdings`（修复前 v9.1 的 25% 压舱权重被静默丢弃后重归一 → 风险虚高）；③ **成本口径重构**：旧实现把年度对冲成本当**一次性乘数**施加（年化拖累仅 ~1e-4pp，期权"显得免费"），且 `hedge_cost_annual_pct` 用「绝对金额 ÷ 归一化净值」导出 → 报出 3002199% 的荒唐值；现改为**逐日计提**（`portfolio_ret − annual_pct/252`）并把口径来源随报告落盘（collar 取 `cost_target_pct` 中值 1.25%；legacy 走 `budget_annual_pct` 并标注「本金划拨遗留口径」）；④ 数据覆盖缺口显式化（配置权重合计 vs 数据覆盖权重 + 缺数据标的告警，不再静默重归一）；⑤ 并入本地 `p2_candidates*/513100.parquet` 等候选标的（严格对齐主数据日期窗口，不改回测区间）；⑥ `datetime.now()` → `now_bj()`。
- **验证**: `validate_configs.py --selftest` PASS（含 7 变异）；`tests/unit/test_portfolio_v91_config.py` **16 passed**（含"修复前必失败"的成本拖累/压舱丢仓/legacy 口径回归）；`ruff check` 全绿；报告落 `每日报告归档/2026-09-11/ETF期权对冲回测_Phase2_20260911_v91.{md,json}`。
- **数据覆盖补齐（Wind MCP）**: 511880（银华日利货币ETF，22% 权重）本地无日线 → 用 Wind MCP `fund_data.get_fund_kline` 取 2021-01-04~2026-08-20 前复权日 K（**1365 条，与主数据集交易日数完全一致**，隐含年化 1.59% 符合货币基金量级），落 `data/etf_option_backtest/511880.parquet`；`_load_etf_prices()` 扩展为同时扫描**主目录按代码命名的单标的 parquet**（此前只扫 `p2_candidates*`）。补齐后 v9.1 回测 **数据覆盖 1.0000/1.0000、0 只缺失**，S1 年化 5.42% / 夏普 0.229 / **回撤 15.76%**（补齐前 0.78 覆盖时为 6.29%/0.243/21.27% —— 补齐前因漏掉 22% 货币仓而**高估**收益、**低估**回撤，方向相反故不可互替）。
- **Wind MCP 调用坑（必记）**: `C:\Users\Administrator\.codebuddy\skills\` 是 **junction**，指向 `E:\各种PY程序\_win_machine_assets\_agents\skills\`；`skills/wind-mcp-skill/scripts/cli.mjs` 用 `import.meta.url === pathToFileURL(process.argv[1]).href` 判定 `IS_MAIN`，Node 把 `import.meta.url` 解析成 junction **真实路径**、而 `argv[1]` 是传入路径 → 判定为 false → **CLI 静默无输出、exit 0**（含 `setup-key` 也会"假成功"）。解法：一律用**真实路径**调用 `E:\各种PY程序\_win_machine_assets\_agents\skills\wind-mcp-skill\scripts\cli.mjs`。Key 存 `~/.wind-aifinmarket/config`（scope=global，仓库外 0600；**不存** skill 内 `config.json`，因其未被 gitignore 而 28仓有自动提交）。
- **遗留/诚实边界**: ① 场内无 A 股个股期权 → 159915 标 `option_available: false` 且护栏禁止其进入期权 targets；② 回测只对期权**计费**、不模拟到期赔付 → 夏普排序必然偏向对冲最少者，报告已加「口径警示」，不得据此否定对冲价值；③ v9.1 回撤 15.76% 略高于诊脉书可达区间上限（10%~15%），年化 5.42% 略低于下限（5.5%~6.5%）—— 属"未计期权削尾增益"的保守侧，非配置缺陷。
- **指针**: `config/portfolio_200w_etf_v91.yaml`、`utils/option_data_fetcher.py`、`utils/etf_option_combo/combo_base.py`、`scripts/validate_configs.py`、`scripts/run_200w_etf_backtest.py`、`tests/unit/test_portfolio_v91_config.py`、`每日报告归档/2026-09-11/`
...
- **全仓体检顺手复核（AUTO-9 口径）**: 当前 394 个知识层文件 UTF-8 全部有效、无 mojibake 特征 — 09-11 编码事故修复后无残留；唯一 U+FFFD 堆积为 `每日报告归档/2026-08-23/...` 4 处（0f554c92 收敛提交时已损坏，原始字节不可逆，登记豁免）。
- **指针**: `scripts/check_utf8_mojibake.py`；`tests/unit/test_check_utf8_mojibake.py`；`cairn/ROADMAP.md` §云端任务池 AUTO-10；事故背景 `cairn/LOG.md` 2026-09-11 编码事故条目
## 2026-09-11 · 修复 cnb 合并引入的编码事故（ROADMAP / LLM权限边界规范）

- **现象**: `9e884552`（cnb/main 合并冲突解决，13:27）对两个冲突文件**内容选对了、编码写坏了**——mojibake 双重编码（UTF-8 字节被按 GBK 写回后存为 UTF-8），`cairn/ROADMAP.md` 与 `docs/LLM权限边界规范.md` 全文花屏且已入 main。
- **判定**: 合并解决记录（`24376828` LOG）明示意图「ROADMAP 保留 HEAD R-1 精简版 / LLM权限边界保留 HEAD 成文版」；逐字节逆变换 + ASCII 骨架比对确认损坏件语义确为主线版本、无 cnb 侧新增内容。
- **修复**: 按 `9e884552^1` 精确恢复两文件（UTF-8），ROADMAP 追加本日扫描 2 条 DECISION NEEDED；损坏原样 + 逆变换副本留档（scan 工作台 `incident-20260911/`）。
- **范围核查**: 合并涉及 25 文件 + 全仓 `cairn/`、`docs/`、根目录 `*.md` 全扫描——仅上述 2 文件受伤，其余无恙。
- **教训（后续会话注意）**: 冲突解决写中文文件必须显式 UTF-8（python `encoding='utf-8'` 写回；勿用 PowerShell 默认编码直写）；现有门禁对编码损坏无感，建议 backlog 增「cairn/docs UTF-8 有效性」轻量检查（可入 AUTO 池）。
## 2026-09-11 · 系统 Bug / 策略逻辑专项扫描（审查报告）

- **触发**: 用户指令「扫描系统bug和策略逻辑」；3 路初扫（信号链 / 组合执行 / 近改面）＋ 2 路定向复扫 ＋ 主线复现，全程只读
- **产物**: `docs/代码质量与系统Bug审查_20260911.md`（+同名 HTML 预览）
- **关键结论**: 新增 **P1×2**（① 建仓流水静默短路〔已复现：空计划→假「已完成」〕；② 资金口径五套并存→再平衡目标高估 ~82%、对冲手数 ~1.82×〔已量化〕）；**P2×5**（约束→归一化顺序〔复现〕 / IC_IR 假告警 / GTJA191 死链＋factor_scorer 悬挂 import / DTZ 残留×2 / 情绪链吞词既有项＋下游新证据）；**P3/观察×7**；关闭疑点×4
- **待拍板**: ① 建仓流水处置（恢复 or 停用任务）；② 资金口径唯一定义；③ 情绪链「规则命中只升不降」 → ①② 已挂 ROADMAP DECISION NEEDED
- **排除/缺口**: 09-09 审计 33 项不重复；信号侧 regime/复权/增强融合快扫列下一批
- **指针**: `docs/代码质量与系统Bug审查_20260911.md`（附录 B：建议台账编号 SC-1~14）

## 2026-09-11 · 09-11 EOD 复跑执行包（B4 首 EOD 跳 warmup + T3 七项 + 三 shadow cron 三合一收口）

- **背景**: 09-10 版 sprint1 材料 §附二留有 3 行**待终态项**（09-10 三 cron 触发结果、09-10 T3「时点性 FAIL」、09-10 影子线实绩），且 B4 已于当日 11:59 双签启用 → 首 EOD 应跳 warmup。用户要求一次复跑全部收口。生产机三线（S12 16:30 / Shadow30Day 16:35 / GNN S6 16:50）在 16:30~16:50，沙箱无生产数据源与运行时产物 → 本轮交付**执行包 + 判定口径 + 结果记录表**，不伪造终态。
- **交付 A `scripts/_eod_verify_911.py`（新增，离线只读）**: A 段 B4 跳 warmup 三硬证据（`eod_workflow_summary_<date>.phases.phase4_86_b4_shadow.skipped == true` 且 `reason == "B4 already enabled, warmup complete"`；日志含跳过行且**不含**预热执行行；`b4_shadow_status.json.last_run` **不以当日开头**= runner 确实没跑）；B 段 T3 七项结构化明细（按 `t3_check_<date>.md` 表格解析，键名与 `t3_post_market_check.py` 的 checks 字典严格同构）；C 段三 shadow cron（schtasks Last Result + 三 jsonl 当日产出 + wrapper 日志 + S12 `--status`）。**四项设计约束**：只读（不写 reports/）、离线（不触网）、fail-closed（缺失记 MISSING 计 INCOMPLETE）、**不冒充 T3 判定主体**（七项判据仍由官方脚本出，本脚本只做解析与完整性计数）。
- **交付 B 契约回归护栏（新增）**: `tests/unit/test_daily_workflow_unit.py::TestB4SkipWarmupContract` 4 用例（纯 AST，不导入 EOD 宿主、无副作用）——① 脚本存在 ② **必须从 `system_config.json` 读 `USE_MLOPS_PIPELINE`**（禁止硬编码跳过）③ 跳过分支必须落 `skipped/reason/flag_invariant` 三字段 ④ **跳过判定必须在 `"B4 Shadow Warmup"` runner 调用之前**（否则 runner 会因 flag 不变式每日 FAIL）。**负向实证**：把跳过块机械移到 runner 之后 → ④ 立即红（`assert 57838 < 57023`），其余 3 绿；恢复后 23 passed。
- **交付 C 文档**: ① `docs/sprint1_收尾判定材料_20260911.md`（正式版，判据一/二/三结论不变，附二增补三段收口口径 + 待填表位）；② `docs/b4_mlops_enable_checklist_20260909.md` §五 首个 EOD 验证项挂上口径与工具；③ `docs/排期计划总览_20260910.md` NEXT 表新增「09-11 盘后」待填行，明确**不用盘前口径结账**；④ 新建 `docs/eod_复跑执行包_20260911.md`（前置 → 复跑五步 → 三段验收判据 → 结果记录表 → 回滚触发 → 交付清单）。
- **口径要点（防再次误判）**: ① T3 ② 依赖 `reports/gate/gate_daily_<date>.json` → **必须先跑 `scripts/gate_check_daily.py`**，否则该项天然 FAIL（本沙箱实测：`.venv/bin/python` 缺失 → 三件套全 FAIL，属环境差异非回归）；② T3 脚本自 2026-09-07 起**不再写 stdout**，只落盘 → 结论只能读 `reports/operations/t3_check_<date>.md`；③ T3 ⑤ 判据是 **mvsk ≥1 + S6 ≥1 且非全 skeleton**，qlib 当日 0 条为 R-6 停跑**预期**；④ 验收 B 判据是"与 09-09 PASS 基线**同构**"而非绝对达标；⑤ 09-10 T3 的 FAIL 是**盘前时点性**，不得当日结账。
- **【R】本机复现事实**: `v87_release_gate.py --sprint 1` → phase_b_stable_days 因 `reports/evolution/phase_b_status.json` 不入库 = MISSING（沙箱差异，非回归）/ r10 0 处 / coverage 0.8330 / **D7 报告 2017 行** ≤3000；`v8.3_institutional/daily_workflow.py` **1543 行**（与 09-10 材料一致）；`system_config.json` 9 flag 全 True（含 B4）。**ruff 基线差异（保留原样不修）**：沙箱 `ruff 0.16.1` 全仓 **41 行 / 40 errors**、F401 等专项 **1 处**（`tests/test_trendcast_audit.py:7` 未用 `pytest`，09-10 生产机报 0）→ 判定为**工具/Python 版本差异**（本沙箱 Py3.11 + ruff 0.16.1，与 `.venv` 的 Py3.14 不同），**非本 PR 引入**（`git stash` 屏蔽本 PR 全部改动后基线完全一致），故不顺手修改他人文件以保持 PR 聚焦。
- **硬约束**: 未触 `config/positions.json`、`.env*`、实盘下单/风控参数、冻结模块；未改任何 flag；未新增 reports/ 产物（`.cbmignore` 同目录，未动）；不代替生产机产出运行时结论。
- **下一步（全为生产/人工依赖）**: ① 生产机今晚复跑并回填 §6 结果记录 → `_eod_verify_911.py` 转全绿后 §附二待终态行结清；② 09-14 B4 连续 2 EOD 复验（09-12/13 非交易日）；③ 09-13 shadow 30 天正式窗首日；④ 09-17/18 D11 复验（发布门禁唯一阻断项）。
- **指针**: Issue #13；`docs/eod_复跑执行包_20260911.md`；`scripts/_eod_verify_911.py`；`tests/unit/test_daily_workflow_unit.py::TestB4SkipWarmupContract`；`cairn/eod-rerun-911-execution-pack-20260911.md`
...
- **item 11 首份文件已完成**: `utils/risk_guard_integrator.py` **2344 → 452 行 (-1892, -81%)**, 仅剩编排骨架(`__init__` + `run_all_guards` / `_run_guard_step` / `_enforce_risk_field_consistency` / `_write_guard_log` / `main`)。四个 commit: C1 `6f588bad`(guards 包骨架 plan_context/kill_switch_level) → C2 `4d0c2052`(drawdown/vol_target/hedge) → C3 `0e00014c`(margin_kill_switch/market) → C4 `e62b2330`(sentiment/correlation)。
- **最终结构**: `utils/risk/guards/` 9 模块 (`__init__`/`plan_context`/`kill_switch_level`/`drawdown`/`vol_target`/`hedge`/`margin_kill_switch`/`market`/`sentiment`/`correlation`); 宿主类 MRO = `RiskGuardIntegrator → Drawdown → VolTarget → Hedge → MarginKillSwitch → Market → Sentiment → Correlation → PlanContextMixin → object`。未新增任何 ruff per-file-ignores(N806/C901 未触发)。
- **剩余 (续做锚点)**: item 11 另 3 个巨型文件 —— `量化策略系统_统一入口_v8.6.py`(2784 行, **0 个 .py 引用方**, 已有 `cli/modes/` 13 模式注册表可作落点, 但该文件 L397-410 已明确回退为"不再 import cli.modes"+ 21 个 `_deprecated_mode_stub`)、`v8.3_institutional/daily_workflow.py`(2180 行, 须先解 `workflow/phases/execute.py` 经 `ctx.__getattr__` 的反向代理链; DTZ005 存量 2 处)、`daily_trade_executor.py`(2275 行, 12 处引用, 执行链核心, 需保护跨进程锁/幂等键/fail-closed 写序; DTZ005 为 0); 以及 item 12(LLM 五处收敛 + `utils/_legacy`(仅 13 行 docstring 的兼容占位, 0 引用, 但 `archive_dead_code.py:18` 注明"严禁删除——破坏 V9 基线") / `utils/backup`(有 1 脚本 + 1 测试 + `EOD_Backup` 定时任务依赖) 移出)。
- **实测证据**: C4 后 guard 全量回归 **386 passed / 0 failed / 16 skipped** (pytdx 本轮未复现, 属网络抖动; 早前 8 个失败已经 worktree 基线确认为既存); mypy 基线 317=317 (四轮共补 22 处泛型 + mixin 宿主属性声明); ruff 增量门禁每轮通过; DTZ005 全程通过。
...
- **用户拍板接受替代口径**: 年化 8~18% (组合 300 万期望值) + 最大回撤 ≤10% + 月度胜率 ≥70% (8-9/12 月正); "每天稳定盈利 1000"废止
- **期货 100 万账户首次登记**: 定位 = 对冲/套利载体 (IC/IM/IF Beta 对冲 + 期现基差), 非独立收益机器; 杠杆纪律 ≤2 倍 (杠杆 >4 倍爆仓风险, 2015/2024 教训); 启用随生产切换窗
- **登记**: ROADMAP §CURRENT STATE `release.performance_targets` (accounts/annual_return/max_drawdown/monthly_win_rate/daily_target 废止声明/acceptance_basis) + 决策登记表
...
- **知识层（已完成）**: 18 本推荐书目已于 08-19 解析归档（weread 10 本含章节目录 + douban 7 本），模块映射沉淀 `cairn/recommended-reading-20260819.md` — 无待办
- **代码层（10 增强方向）**: 现状不并入（Wave 7/9 排期已满 08-13~12-31，且核心方法已覆盖，剩余属增强非缺口）；**最早评估时点 = Wave 9 收尾（12-15）或 v8.7 发布（12-31）取先**；实际落地窗口 2027 Q1（与 Wave 9-GH、Wave 11-B/C 并行不同模块）
- **例外 4 方向按触发条件不按日历**: Lyapunov 稳定性度量（自我进化 Phase 1-2 启用时）/ Triple-Barrier（AutoResearch 标签需求）/ 因子正交化（因子库 50+）/ 行为异象因子化（情绪因子 v5.0）— 触发即评估，可能早于 12 月
- **指针**: `cairn/recommended-reading-20260819.md` §九（10 方向+触发条件）§十（与系统计划关系）；`docs/高价值项目集成排期_Wave11_20260829.md`（11-C 纯文献轨道 2027-02-15~02-28）
...
- **分工**：ROADMAP 章节 = 结论事实源（简洁）；docs/ 文档 = 详细导航 + 冲突核对 C1~C5 + 备选方案审计记录
- **指针**: `cairn/ROADMAP.md` §策略优化排期决策；`docs/策略优化排期计划_20260903.md`

## 2026-09-03 · 策略排期 D-1~D-5 全部拍板（按推荐方案）+ ROADMAP/知识文档结论回写（纯文档）

- **用户拍板（"按最优方案执行"）**：D-1/D-2 = MVSK P5-3 与 qlib W7.3.8 切换随 12-31 v8.7 发布实施（10-13 shadow Δ夏普>0 且无异常换仓为前提，≤0 证伪归档）；D-3 = ERL ER-2.x 双签提前至 09-13~09-18、ER-3.x 冻结窗内只观察、12-31 发布后完成灰度 Stage 2→3；D-4 = ETF Phase 4/5 重定义（验收改 S12 防御口径：收益正向/30 日滚动回撤≤5%/与主组合相关性<0.3；取消独立资金灰度）；D-5 = ETF 子组合 = v9_200w_preset"ETF 核心仓+期权保护仓"两层的策略原型与 shadow 载体，不设独立第二实盘账户
- **回写**：ROADMAP（MVSK P5-3/W7.3.7、W7.3.8、ER-2.1/2.2、ER-3.1~3.3 任务行结论 + ETF Phase 4/5 章节按 S12 口径原位重写——旧 P4.1~P4.3/P5.1 划线保留 + v9 preset 灰度路线 S12 并入注记）；`cairn/etf-option-hedge-model.md`（顶部定位修正 + §十一 排期表重定义）；`docs/策略优化排期计划_20260903.md`（§五 拍板状态 + 全文状态更新）；`docs/排期计划总览_20260826.md`（§0/§3 同步，新增 ER 双签 P1 行）
- **近期执行锚点**：09-13~09-18 ER-2.1/2.2 Flag 双签（D-3）；Sprint3-1 期间 S12 防御层配置加载验证（D-4）；10-09~10-16 统一评估周产出合并评估报告（C5）
- **指针**: `docs/策略优化排期计划_20260903.md` §五；`cairn/ROADMAP.md` §ETF期权对冲排期 Phase 4/5

## 2026-09-03 · 策略优化排期计划产出：策略六线导航 + Q4 冻结窗冲突核对（D-1~D-5 待拍板）+ ROADMAP 口径修正（纯文档）

- **第三轮排期工作**（08-29 路线优化 → 09-02 运营收敛 → 本轮策略视角）：新建 `docs/策略优化排期计划_20260903.md` —— 把散落 6 条线的策略工作收敛为一张排期网（收益 MVSK/qlib、防御 S12、进化 ERL、研究 GNN、资金 200 万灰度、基建 v8.7.1 G3/G4+多策略组合优化顶层项），含 Stage A~D 分阶段排期与关键日历速查
- **核对出系统性冲突**：4 项生产启用动作原排期全部落在 09-19~12-10 冻结窗内，与 09-02 决策 1 冲突——① MVSK P5-3（10-13 起）② qlib W7.3.8 信号源切换（10-13 起）③ ERL ER-2.x 双签（09-27 起）+ ER-3.x 灰度（11-01 起）④ ETF Phase 4/5 灰度（10-06 起，另叠加 S13 关闭后内容过时 + 双"200 万"口径未定义）→ 逐项附推荐方案，待拍板 D-1~D-5（死线：D-3 → 09-12 / D-4·D-5 → 09-30 / D-1·D-2 → 12-10）
