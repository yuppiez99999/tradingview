...
try:
    ...
    from qlib.contrib.model.gbdt import (
        LGBModel,
    )  # G2 FIX: LGBModel 在 gbdt.py, 非 lightgbm.py
...
try:
    ...
    from qlib.contrib.model.pytorch_transformer import Transformer  # noqa: F401
...
def v75_to_qlib_features(
    df: pd.DataFrame, symbol: str, feature_cols: list | None = None
) -> pd.DataFrame:
    """
...
def train_qlib_model(
    dataset: DatasetH, model_type: str = DEFAULT_MODEL, **model_kwargs
) -> Any | None:
    """
    训练 Qlib 模型
...
def predict_qlib_signal(
    model: Any, dataset: DatasetH, segment: str = "test"
) -> pd.Series | None:
    """
    使用 Qlib 模型生成信号
    ...
    try:
        ...
        logger.info(
            f"Qlib 信号生成完成: mean={signal.mean():.4f}, std={signal.std():.4f}"
        )
        return signal
...
def _load_data_provider():
    """加载 data_provider 模块，返回 MarketDataProvider 类"""
    ...
    if spec and spec.loader:
        ...
        return data_provider.MarketDataProvider  # noqa: N806
    raise ImportError(f"无法加载 data_provider: {data_provider_path}")


def _select_period_by_days(days: int) -> str:
    """根据天数选择合适的数据周期"""
    if days <= 20:
        return "1m"
    if days <= 60:
...
def _validate_and_clean_ohlcv(
    df: pd.DataFrame, symbol: str, days: int
) -> pd.DataFrame | None:
    """验证 OHLCV 列完整性并清理数据，不合法返回 None"""
    # 只保留最近 N 天
    if len(df) > days:
        df = df.tail(days)
...
def fetch_ifind_historical(symbol: str, days: int = 120) -> pd.DataFrame | None:
    ...
    """
    try:
        MarketDataProvider = _load_data_provider()  # noqa: N806
        period = _select_period_by_days(days)
        ...
        df = _validate_and_clean_ohlcv(df, symbol, days)
        if df is None:
            return None
...
def generate_qlib_signal(
    ...
    **kwargs,
    ...
    # 准备数据集
    dataset = prepare_qlib_dataset(
        df, symbol, train_start, train_end, valid_start, valid_end, test_start, test_end
    )
    if dataset is None:
        return None
...
def _add_technical_features(data: pd.DataFrame) -> pd.DataFrame:
    ...
    # v8.6.13 HIGH FIX (2026-08-01 量化审计): OHLCV 列存在性校验, 防止 KeyError
    # 原代码直接 df["close"] / df["high"] 等裸访问, 上游数据源 (Wind/iFinD/AKShare)
    # 返回的 DataFrame 若缺失某列会抛 KeyError 导致整个信号生成链路崩溃.
    # 参照 utils/qlib_data_bridge.py 的 df.get() 保护模式.
    _required = ["close", "high", "low", "volume"]
    _missing = [c for c in _required if c not in df.columns]
    if _missing:
        raise KeyError(
            f"_add_technical_features 缺少必要 OHLCV 列: {_missing} "
            f"(现有列: {list(df.columns)}). 请检查上游数据源是否返回完整 OHLCV."
        )
    ...
    # ---------- OBV ----------
    obv = (
        df["volume"] * df["close"].pct_change().where(df["close"].diff() > 0, -1)
    ).cumsum()
    df["obv"] = obv
    ...
    # v8.6.13 CRITICAL FIX (2026-08-01 量化审计):
    # 删除 bfill(), 仅用 ffill() — 同步 v8.3_institutional/src/alpha/qlib_signal_adapter.py 的 BUG-R4 修复
    # bfill() 会用未来数据填充历史 NaN, 引入前视偏差 (look-ahead bias):
    #   - 回测中: 用未来观测值回填 rolling 窗口前的 NaN, 导致信号"偷看未来", 回测虚高
    #   - 实盘中: 无未来数据所以不泄漏, 但与回测行为不一致, 模型上线后表现骤降
    # 项目自带的 ci_lookahead_guard.py 已将 bfill() 标记为 CRITICAL 级别.
    # rolling 窗口前产生的 NaN 应丢弃 (dropna) 或前向填充 (ffill), 不可后向填充.
    df = df.ffill()
...
def _local_lightgbm_signal(
    ...
    try:
        ...
        X = data[feature_cols].values  # noqa: N806
        y = data["target"].values

        # 训练/测试分割
        split = int(len(X) * 0.8)
        X_train, X_test = X[:split], X[split:]  # noqa: N806
        ...
        logger.info(
            f"本地 LightGBM 训练完成 [{symbol}]: train_r2={train_score:.4f}, test_r2={test_score:.4f}"
        )
        ...
        if cached is not None:
            ...
            try:
                ...
                if len(data) > 0:
                    ...
                    if signal.std() > 0:
                        signal = (
                            2 * (signal - signal.min()) / (signal.max() - signal.min())
                            - 1
                        )
                    signal.name = "local_lgb_signal"
