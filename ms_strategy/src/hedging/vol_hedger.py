...
class VolHedger:
    ...
    def __init__(
        ...
        Args:
            vix_trigger: 触发阈值
            vix_emergency: 紧急阈值
            vix_mid_threshold: 中级分段阈值 (原硬编码40.0, HG-7配置化)
        ...
        self.vix_mid_threshold = float(vix_mid_threshold)
    ...
    def compute_hedge(self, vix: float, portfolio_value: float) -> dict[str, object]:
        ...
        if vix <= self.vix_mid_threshold:
            ...
            strike_short = None
            # 流动性折价: VIX > mid_threshold 时实际覆盖率衰减
            coverage = max(0.015, 1.0 - (vix - self.vix_mid_threshold) / 50.0)
        ...
        result = {
            ...
            "budget": float(budget),
            "budget_pct": (
                float(budget / portfolio_value) if portfolio_value > 0 else 0.0
            ),
