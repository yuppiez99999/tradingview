...
class HedgeCoordinator:
    ...
    def max_total_hedge(self) -> float:
        ...
        if regime == MarketRegime.WARNING:
    ...
    def coordinate(self,
        ...
        if total_hedge_pct > current_max:
            ...
            self._scale_orders(orders, scale)
    ...
    @staticmethod
    def _scale_orders(orders: list[dict], scale: float) -> None:
        """按比例缩放订单的名义价值/成本字段 (HG-4 修复, 防止过度对冲后字段不一致).

        随 scale 缩放的字段: notional/contracts/gold_value/gold_weight/
        repo_value/repo_weight/budget/estimated_cost。所有"金额/数量/成本"类
        字段必须同步缩放, 否则单个订单与聚合 total_* 对不上, 下游资金预算失真。
        """
        for o in orders:
            if "notional" in o:
            ...
            if "estimated_cost" in o:
                o["estimated_cost"] = o["estimated_cost"] * scale
