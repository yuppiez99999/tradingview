...
class HedgeStrategy(ABC):
    ...
    def on_tick(self, ctx: HedgeContext, tick: TickData) -> None:  # noqa: B027 - 可选回调钩子, 非抽象方法
        """Tick 回调 — 可选钩子, 子类按需重写"""
        ...

    def on_bar(self, ctx: HedgeContext, bar: BarData) -> None:  # noqa: B027 - 可选回调钩子, 非抽象方法
        """Bar 回调 — 可选钩子, 子类按需重写"""
        ...
...
class HedgeContext:
    ...
    def get_portfolio_metrics(self, prices: dict[str, float] | None = None) -> PortfolioMetrics:
        """获取组合指标"""
        if prices:
    ...
    def calc_target_hedge_volume(self, hedge_code: str,
                                 target_ratio: float | None = None) -> float:
        """计算目标对冲手数
    ...
    def open_hedge(self, hedge_code: str, hands: float, price: float | None = None) -> bool:
        """开空对冲"""
        if hands <= 0:
            return False
    ...
    def close_hedge(self, hedge_code: str, hands: float, price: float | None = None) -> bool:
        """平空对冲"""
