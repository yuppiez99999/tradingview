...
def _extract_signals_from_report(report):
    """从报告中提取信号列表、分布和股票信号（兼容多种报告格式）"""
...
def _evaluate_signal_quality(ic_ir):
    """根据 IC IR 评估信号质量等级，返回 (质量描述, 颜色表情)"""
    if ic_ir > 0.3:
        return "优秀", "🟢"
    if ic_ir > 0.2:
        return "良好", "🟡"
...
def _build_signal_monitor_report(report, report_path, quality, metrics, dist, signals, stock_signals):
    """构建信号监控报告字典"""
    return {
...
def analyze_signal_effectiveness():
    ...
    signals, dist, stock_signals = _extract_signals_from_report(report)

    total = sum(dist.values())

    print("--- 信号分布统计 ---")
    safe_total = total if total > 0 else 1.0
    print(f"看多: {dist.get('long', 0)} ({dist.get('long', 0)/safe_total*100:.1f}%)")
    print(f"看空: {dist.get('short', 0)} ({dist.get('short', 0)/safe_total*100:.1f}%)")
    print(f"中性: {dist.get('neutral', 0)} ({dist.get('neutral', 0)/safe_total*100:.1f}%)")
    ...
    print("\n--- 信号质量评估 ---")
    ic_ir = metrics.get("ic_ir", 0)
    quality, color = _evaluate_signal_quality(ic_ir)
    ...
    monitor_report = _build_signal_monitor_report(
        report, report_path, quality, metrics, dist, signals, stock_signals
    )
