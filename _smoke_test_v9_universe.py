...
from cache.symbol_universe import EXISTING_SYMBOLS, get_universe
...
def main() -> int:
    ...
    if not all_special_ok:
        print(f"  ⚠ 警告: 部分特殊标的缺失 (但流水线可继续)")
    else:
        print(f"  ✓ 全部 4 个特殊标的数据可用 (范围受限但可回测)")
    ...
    print(f"  旧 23 标的: 全部向后兼容")
    print(f"  数据范围: 5 年日 K (~1211 天), 平均 {avg_len:.0f} 天")
    print(f"  4 个特殊标的: 数据可用 (范围受限)")
