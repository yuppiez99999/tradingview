...
# Map positions to Wind MCP codes
wind_codes = {}
...
for key, item in positions.items():
    ...
    if cost_price > 0 and price > 0:
        deviation = abs(price - cost_price) / cost_price
        if deviation > 0.20:
            price_warning = f'价格偏离成本价 {deviation*100:.1f}%'
...
report_path = os.path.join(report_dir, f'收盘报告_{datetime.now().strftime("%Y%m%d")}_wind.md')
...
lines.append('## 持仓明细')
lines.append('')
lines.append('| 代码 | 名称 | 风格 | 持仓 | 现价 | 成本价 | 成本 | 市值 | 盈亏 | 盈亏率 | 来源 | 更新时间 | 备注 |')
lines.append('| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |')
for r in sorted(rows, key=lambda x: x['position_value'], reverse=True):
    lines.append(f'| {r["code"]} | {r["name"]} | {r["style"]} | {r["shares"]} | {r["price"]:.2f} | {r["cost_price"]:.2f} | {r["cost"]:,.0f} | {r["position_value"]:,.0f} | {r["pnl"]:+,.0f} | {r["pnl_pct"]:+.2f}% | {r["source"]} | {r["last_update"]} | {r["price_warning"]} |')
lines.append('')
lines.append('## 风格分布')
lines.append('')
