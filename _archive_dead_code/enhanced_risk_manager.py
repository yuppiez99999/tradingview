...
class RealTimeRiskMonitor:
    ...
    def __init__(self, total_capital: float = 1000000, risk_manager_callback=None):
    ...
    def _get_real_time_data(self) -> Dict:
        ...
        return {
            ...
            'is_simulated': True  # 模拟数据标记，非实盘数据
        }
        logger.warning("[模拟数据] _get_real_time_data 使用随机生成数据，不可用于实盘决策")
    ...
    def _stop_trading(self):
        ...
        # 设置本地停止标志（避免重复执行）
        if getattr(self, 'trading_stopped', False):
            logger.info("交易已处于停止状态，跳过重复执行")
            return
        
        self.trading_stopped = True
        self.trading_stop_time = datetime.now()
        
        # 优先回调给 EnhancedRiskManager 做统一处理
        if callable(getattr(self, 'risk_manager_callback', None)):
            try:
                self.risk_manager_callback.on_monitor_stop_trading()
            except Exception as e:
                logger.warning(f"stop_trading 回调失败: {e}")
        
        # 通知自动化执行系统停止
        try:
            from automated_execution_system import AutomatedExecutionSystem
            executor = AutomatedExecutionSystem(total_capital=self.total_capital)
            if hasattr(executor, 'stop_system'):
                executor.stop_system()
                logger.info("已通知 AutomatedExecutionSystem 停止系统")
        except ImportError:
            logger.info("automated_execution_system 不可用，仅记录本地停止状态")
        except Exception as e:
            logger.warning(f"通知执行系统停止交易失败: {e}")
        ...
        logger.warning(f"交易已停止，时间: {self.trading_stop_time}")
    ...
    def _emergency_hedging(self):
        ...
        if callable(getattr(self, 'risk_manager_callback', None)):
            try:
                cb_result = self.risk_manager_callback.on_monitor_emergency_hedging()
                if isinstance(cb_result, dict):
                    hedge_summary.update(cb_result)
            except Exception as e:
                logger.warning(f"emergency_hedging 回调失败: {e}")
        
        # 兜底：若回调未完成，则在本层继续尝试
        if hedge_summary.get('status') not in ('completed', 'skipped'):
            try:
                from multi_layer_hedge_manager import MultiLayerHedgeManager
                hedge_mgr = MultiLayerHedgeManager(total_capital=self.total_capital)
                ...
                if hasattr(hedge_mgr, 'execute_multi_layer_hedge'):
                    result = hedge_mgr.execute_multi_layer_hedge(market_data)
                    hedge_summary['status'] = 'completed'
                    hedge_summary['details'].append({"module": "MultiLayerHedgeManager", "result": result})
                    logger.error(f"多层次紧急对冲执行完成")
                else:
                    hedge_summary['status'] = 'skipped'
                    hedge_summary['details'].append("MultiLayerHedgeManager 不支持 execute_multi_layer_hedge")
            except ImportError:
                hedge_summary['status'] = 'skipped'
                hedge_summary['details'].append("multi_layer_hedge_manager 不可用")
                logger.error("multi_layer_hedge_manager 不可用，无法执行紧急对冲")
            except Exception as e:
                hedge_summary['status'] = 'failed'
                hedge_summary['details'].append(str(e))
                logger.error(f"紧急对冲执行失败: {e}")
        
        # 记录到风险历史
        try:
            self.risk_history.append({
                'timestamp': datetime.now().isoformat(),
                'event': 'emergency_hedging',
...
class RiskBudgetOptimizer:
    ...
    def _calculate_risk_correlation(self, market_data: Dict) -> Dict:
        """计算风险相关性"""
        logger.warning("[模拟数据] _calculate_risk_correlation 使用随机生成数据，不可用于实盘决策")
        return {
...
class StressTestEngine:
    ...
    def __init__(self):
        self.test_scenarios = {
            ...
            'internet_bubble_2000': {
                'name': '2000互联网泡沫',
                'description': '纳斯达克估值泡沫破裂，科技股深度回调',
                'parameters': {
                    'market_shock': -0.45,
                    'volatility_multiplier': 2.0,
                    'liquidity_haircut': 0.2,
                    'correlation_breakdown': 0.7,
                    'tech_stock_shock': -0.60,
                    'valuation_collapse': 0.50
                }
            }
        }
    ...
    def _run_scenario_test(self, scenario: Dict, portfolio_data: Dict, 
        ...
        try:
            ...
            # 科技股额外冲击（可选）
            tech_stock_shock = parameters.get('tech_stock_shock')
            tech_weight = portfolio_data.get('tech_weight', 0.16)
            tech_loss = initial_value * tech_weight * tech_stock_shock if tech_stock_shock is not None else 0.0
            
            # 估值回归冲击（可选）
            valuation_collapse = parameters.get('valuation_collapse')
            valuation_loss = initial_value * valuation_collapse * 0.1 if valuation_collapse is not None else 0.0
            ...
            # 计算总损失
            total_loss = market_loss + volatility_loss + liquidity_loss + correlation_loss + tech_loss + valuation_loss
    ...
    def _generate_assessment(self, test_results: Dict, 
        ...
        total_scenarios = len(self.test_scenarios) + 1  # +1 for Monte Carlo
...
class EnhancedRiskManager:
    ...
    def __init__(self, total_capital: float = 1000000,
        ...
        # 初始化组件
        self.risk_monitor = RealTimeRiskMonitor(total_capital, risk_manager_callback=self)
    ...
    def on_monitor_stop_trading(self):
        """接收 RealTimeRiskMonitor 的停止交易回调，统一处理停仓/停机动作"""
        self.risk_management_enabled = False
        try:
            self.risk_management_history.append({
                'timestamp': datetime.now().isoformat(),
                'event': 'monitor_stop_trading',
                'source': 'RealTimeRiskMonitor'
            })
        except Exception:
            pass
        logger.warning("[CALLBACK] 监控器触发停止交易，EnhancedRiskManager 已关闭进一步风险管理动作")

    def on_monitor_emergency_hedging(self):
        """接收 RealTimeRiskMonitor 的紧急对冲回调，统一协调多层次对冲"""
        summary = {'status': 'initiated', 'source': 'RealTimeRiskMonitor'}
        try:
            # 优先使用已加载的流动性控制或对冲相关模块
            if getattr(self, 'liquidity_controller', None) is not None:
                summary['liquidity_controller'] = 'available'

            # 记录统一事件
            self.risk_management_history.append({
                'timestamp': datetime.now().isoformat(),
                'event': 'monitor_emergency_hedging',
                'summary': summary
            })
        except Exception:
            pass
        logger.error("[CALLBACK] 监控器触发紧急对冲，EnhancedRiskManager 已接管统一处理")
        return summary
    ...
    def assess_factor_signals(self, klines: Dict = None, fundamentals: Dict = None) -> Dict:
        """运行五维因子模型，并附带 GTJA191 technical_alpha 明细。"""
        ...
        try:
            ...
            signal = self.factor_model.generate_signal(results)
            # 附加每只股票的 technical_alpha 明细，便于风险预警直接引用
            per_stock_technical_alpha = []
            for code, result in results.items():
                per_stock_technical_alpha.append({
                    'code': code,
                    'composite': result.composite,
                    'signal': result.signal,
                    'technical_alpha': result.factors.get('technical_alpha', 0.0),
                })
            signal['per_stock_technical_alpha'] = per_stock_technical_alpha
            return signal
        except Exception as e:
    ...
    def run_risk_management_cycle(self, market_data: Dict, 
        ...
        try:
            ...
            current_risk_level = risk_summary.get('current_risk_level', 'low')
            if not current_risk_level:
                current_risk_level = 'low'
            risk_summary['current_risk_level'] = current_risk_level
            logger.info(f"当前风险状态: {current_risk_level}")
    ...
    def _make_risk_decision(self, risk_summary: Dict, 
        ...
        current_level = risk_summary.get('current_risk_level') or 'low'
        decision = {
        ...
        if factor_signals:
            ...
            # GTJA191 technical_alpha 明细接入风险预警
            technical_alpha_alerts = []
            for item in factor_signals.get('per_stock_technical_alpha', []) or []:
                try:
                    ta = float(item.get('technical_alpha', 0.0))
                except (TypeError, ValueError):
                    ta = 0.0
                if ta <= -0.6:
                    technical_alpha_alerts.append({
                        'code': item.get('code'),
                        'technical_alpha': ta,
                        'signal': item.get('signal'),
                        'level': 'strong_sell_alpha',
                    })
                elif ta <= -0.3:
                    technical_alpha_alerts.append({
                        'code': item.get('code'),
            ...
            if technical_alpha_alerts:
                if decision['priority'] not in ('critical', 'high'):
                    decision['priority'] = 'medium'
                decision['measures'].append('technical_alpha_alert_review')
                logger.warning(f"GTJA191 technical_alpha 预警: {technical_alpha_alerts}")
            decision['technical_alpha_alerts'] = technical_alpha_alerts
