...
class TailRiskHedge:
    ...
    def _execute_option_protection(self, option_ratio: float, protection_capital: float, 
        ...
        # 提取市场触发条件
        vix_level = market_data.get('vix', 0.0)
        daily_drop = market_data.get('daily_drop', 0.0)
        weekly_drop = market_data.get('weekly_drop', 0.0)
        
        # 三层深度OTM Put保护逻辑
        protection_layers = []
        
        # Layer 1: 轻度保护（85% OTM）- 日常震荡
        light_put_ratio = 0.40
        light_moneyness = 0.85
        protection_layers.append({
            'name': '轻度保护',
            'ratio': light_put_ratio,
            'moneyness': light_moneyness,
            'trigger': '日常保护',
            'capital_ratio': light_put_ratio
        })
        
        # Layer 2: 中度保护（80% OTM）- VIX>30 或 单日跌>3%
        if vix_level > 30 or daily_drop < -0.03:
            ...
            protection_layers.append({
                'name': '中度保护',
                'ratio': mid_put_ratio,
                'moneyness': mid_moneyness,
                'trigger': f'VIX>{vix_level:.1f}或日跌{daily_drop:.1%}',
                'capital_ratio': mid_put_ratio
            })
        
        # Layer 3: 深度保护（75% OTM）- 双周跌>10% 或 危机态
        if weekly_drop < -0.10 or self.market_regime == 'crisis':
            deep_put_ratio = 0.25
            deep_moneyness = 0.75
            protection_layers.append({
                'name': '深度保护',
                'ratio': deep_put_ratio,
                'moneyness': deep_moneyness,
                'trigger': f'双周跌{weekly_drop:.1%}或危机态',
                'capital_ratio': deep_put_ratio
            })
        ...
        for layer in protection_layers:
            layer_capital = option_capital * layer['capital_ratio']
            option_premium = self._estimate_option_premium('put', layer['moneyness'], market_data)
            
            if option_premium <= 0:
                continue
            
            protection_units = layer_capital / (underlying_price * 100)
            quantity = int(protection_units)
            
            if quantity <= 0:
                continue
            
            strike_price = int(underlying_price * layer['moneyness'])
            ...
            trade = {
                ...
                'protection_layer': layer['name'],
                'strike_price': strike_price,
                'moneyness': layer['moneyness'],
                'premium': option_premium,
                'capital_required': quantity * option_premium * 100,
                'protection_ratio': option_ratio * layer['capital_ratio'],
                'market_regime': self.market_regime,
                'trigger': layer['trigger']
            }
            trades.append(trade)
        
        logger.info(f"期权多层保护执行完成: {len(trades)} 个期权合约 (Layer 1+2+3)")
