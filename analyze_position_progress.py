...
with open(r'e:\各种PY程序\28-终极量化交易系统7.1\500万建仓计划_20260706.json', 'r', encoding='utf-8') as f:
...
print("风格分布分析")
print("=" * 80)
print(f"{'风格':<10} {'当前市值':<15} {'目标金额':<15} {'实际占比':<10} {'目标占比':<10} {'偏差':<10}")
print("-" * 80)
for style, stats in sorted(style_stats.items(), key=lambda x: x[1]['target_amount'], reverse=True):
    current_pct = stats['current_value'] / current_market_value * 100 if current_market_value > 0 else 0
    target_pct = stats['target_amount'] / target_total * 100 if target_total > 0 else 0
    deviation = current_pct - target_pct
    print(f"{style:<10} ¥{stats['current_value']:>12,.0f} ¥{stats['target_amount']:>12,.0f} {current_pct:>8.1f}% {target_pct:>8.1f}% {deviation:>+8.1f}%")

print("\n" + "=" * 80)
print("各标的建仓进度")
print("=" * 80)
print(f"{'代码':<12} {'名称':<16} {'持仓':<10} {'当前市值':<12} {'目标金额':<12} {'完成率':<8} {'风格':<8}")
print("-" * 80)
sorted_positions = sorted(positions['positions'].items(), key=lambda x: x[1].get('target_weight', 0), reverse=True)
for code, pos in sorted_positions:
    shares = pos.get('shares', 0)
    est_price = pos.get('est_price', pos.get('avg_cost', 0)) if pos.get('est_price', 0) > 0 else pos.get('avg_cost', 0)
    current_value = shares * est_price
    target_amount = pos.get('amount', 0)
    completion = current_value / target_amount * 100 if target_amount > 0 else 0
    print(f"{code:<12} {pos['name'][:16]:<16} {shares:<10} ¥{current_value:>10,.0f} ¥{target_amount:>10,.0f} {completion:>6.1f}% {pos.get('style', '其他'):<8}")
...
for style, stats in style_stats.items():
    ...
    if abs(current_pct - target_pct) > 10:
        if current_pct > target_pct:
            print(f"⚠️ {style}超配: 实际{current_pct:.1f}% vs 目标{target_pct:.1f}%, 超配{current_pct-target_pct:.1f}个百分点")
        else:
