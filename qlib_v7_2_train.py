# -*- coding: utf-8 -*-
"""
QLib v7.2训练脚本 — 抗过拟合优化
目标：降低训练/测试集IC差距，提升Rank IC和IC IR
"""
import os
import sys
import json
...
# v7.2: 扩大训练样本到CSI300，但控制总量避免内存溢出
CSI300_AUX = [
    ...
    ("300408", "SZ", "三环集团"), ("600745", "SH", "闻泰科技"),
    ("002938", "SZ", "鹏鼎控股"), ("600584", "SH", "长电科技"),
    ("688012", "SH", "中微公司"), ("688036", "SH", "传音控股"),
    ("000538", "SZ", "云南白药"), ("002007", "SZ", "华兰生物"),
    ("300003", "SZ", "乐普医疗"), ("300015", "SZ", "爱尔眼科"),
    ("603288", "SH", "海天味业"), ("002466", "SZ", "天齐锂业"),
    ("600362", "SH", "江西铜业"), ("000630", "SZ", "铜陵有色"),
...
def main():
    ...
    # 限制最大数量避免内存溢出
    all_train_symbols = all_train_symbols[:300]
    all_train_symbols = [s for s in all_train_symbols if s.lower() in available_symbols]
    print(f"训练池规模: {len(all_train_symbols)} 只股票", flush=True)
    ...
    if os.path.exists(features_path):
        ...
        print(f"已加载Top {TOP_N}特征配置", flush=True)
    else:
        ...
        if os.path.exists(features_50_path):
            ...
            print(f"从Top50特征中取前{TOP_N}", flush=True)
        else:
            print(f"警告: 未找到特征配置，使用全部特征", flush=True)
            top_features = feature_names
    
    top_feature_indices = [i for i, name in enumerate(feature_names) if name in top_features]
    print(f"\n特征筛选: {len(feature_names)} → {len(top_feature_indices)}", flush=True)
    ...
    # v7.2: 降低复杂度，加强正则化，减少过拟合
    params = {
        "objective": "reg:squarederror",
        "max_depth": 4,  # 降低深度
        "learning_rate": 0.008,  # 降低学习率
        "subsample": 0.6,
        "colsample_bytree": 0.6,
        "gamma": 0.6,  # 提高gamma
        "reg_alpha": 0.8,  # 提高L1正则
        "reg_lambda": 1.5,  # 提高L2正则
        "min_child_weight": 80,  # 提高最小样本权重
        "verbosity": 0,
        "random_state": 42,
        "importance_type": "gain",
    }
    
    print("训练XGBoost模型(v7.2)...", flush=True)
    print(f"正则化参数: gamma={params['gamma']}, reg_alpha={params['reg_alpha']}, reg_lambda={params['reg_lambda']}", flush=True)
    print(f"树参数: max_depth={params['max_depth']}, min_child_weight={params['min_child_weight']}", flush=True)
    print(f"训练样本: {len(y_train)}, 验证样本: {len(y_valid)}, 测试样本: {len(y_test)}", flush=True)
    ...
    print(f"\n{'='*70}", flush=True)
    print(f"XGBoost v7.2 (Top{TOP_N}特征+抗过拟合+扩大样本) 训练完成", flush=True)
    print(f"{'='*70}", flush=True)
    print(f"训练集 IC: {ic_train:.4f}", flush=True)
    print(f"验证集 IC: {ic_valid:.4f}", flush=True)
    print(f"测试集 IC: {ic_test:.4f}", flush=True)
    print(f"训练集 Rank IC: {rank_ic_train:.4f}", flush=True)
    print(f"验证集 Rank IC: {rank_ic_valid:.4f}", flush=True)
    print(f"测试集 Rank IC: {rank_ic_test:.4f}", flush=True)
    ...
    print(f"\n日均 IC: {avg_ic:.4f}", flush=True)
    print(f"IC 标准差: {ic_std:.4f}", flush=True)
    print(f"IC IR: {ic_ir:.4f}", flush=True)
    print(f"IC > 0 占比: {ic_positive_ratio:.4%}", flush=True)
    ...
    report = {
        "version": "v7.2",
        "model": "XGBoost_Top40_AntiOverfit",
    ...
    report_path = os.path.join(reports_dir, f"qlib_v7_2_train_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json")
