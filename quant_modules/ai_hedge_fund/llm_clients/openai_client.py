...
class LocalCompatibleChatOpenAI(NormalizedChatOpenAI):
    ...
    def with_structured_output(self, schema, *, method=None, **kwargs):
        resolved = (
            method or get_capabilities(self.model_name).preferred_structured_method
        )
...
class DeepSeekChatOpenAI(NormalizedChatOpenAI):
    ...
    def _get_request_payload(self, input_, *, stop=None, **kwargs):
        ...
        for message_dict, message in zip(
            outgoing, _input_to_messages(input_), strict=False
        ):
...
class ProviderSpec:
    ...
    base_url_env: str | None = (
        None  # env var that overrides base_url (e.g. OLLAMA_BASE_URL)
    )
    key_optional: bool = False  # don't require/prompt; send a placeholder if unset
    placeholder_key: str = (
        "EMPTY"  # sent when no key is available (keyless local servers)
    )
    require_base_url: bool = (
        False  # error if no base_url is resolved (generic endpoint)
    )
...
OPENAI_COMPATIBLE_PROVIDERS: dict[str, ProviderSpec] = {
    ...
    "minimax": ProviderSpec(
        base_url="https://api.minimax.io/v1", chat_class=MinimaxChatOpenAI
    ),
