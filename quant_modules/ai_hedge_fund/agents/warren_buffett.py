...
def analyze_moat(metrics: list) -> dict[str, Any]:
    ...
    if len(historical_roes) >= 5:
        ...
        if roe_consistency >= 0.8:  # 80%+ of periods with ROE > 15%
            ...
            reasoning.append(
                f"Excellent ROE consistency: {high_roe_periods}/{len(historical_roes)} periods >15% (avg: {avg_roe:.1%}) - indicates durable competitive advantage"  # noqa: E501
            )
...
def calculate_intrinsic_value(financial_line_items: list) -> dict[str, Any]:
    ...
    present_value = 0
    details.append(
        f"Using three-stage DCF: Stage 1 ({stage1_growth:.1%}, {stage1_years}y), Stage 2 ({stage2_growth:.1%}, {stage2_years}y), Terminal ({terminal_growth:.1%})"  # noqa: E501
    )
