...
def apply_llm_decisions(report_date: str, plan_date: str) -> Path:
    ...
    has_build_seq = any(
        kw in rec_text
        for kw in ["建仓顺序", "优先建仓", "调整建仓", "仓位顺序", "build_sequence"]
    )
    if has_build_seq:
        plan.setdefault("llm_overrides", {})
...
def _parse_position_adjustments(ai_recs: list) -> list:
    ...
    for rec in ai_recs:
        ...
        # 必须包含减持/加仓/调整仓位关键词
        if not any(
            kw in rec_text for kw in ["减持", "加仓", "调整仓位", "仓位调整", "减仓"]
        ):
            continue
...
def _parse_position_conversions(ai_recs: list) -> list:
    ...
    for rec in ai_recs:
        ...
        # 必须包含转换关键词
        if not any(
            kw in rec_text for kw in ["转换", "转为", "转换为", "调仓至", "切换至"]
        ):
            continue

        # 提取 "将A...转换为B" 模式
        conv_match = re.search(
            r"将\s*([^,，。；;]+?)\s*(?:的\d+份?)?\s*转(?:换)?为?\s*([^,，。；;]+)",
            rec_text,
        )
        if not conv_match:
            continue
...
def _parse_sector_adjustments(ai_recs: list) -> list:
    ...
    for rec in ai_recs:
        ...
        for sector, keywords in sector_keywords.items():
            ...
            has_action = any(
                kw in rec_text
                for kw in ["优先", "增加", "降低", "减少", "平衡", "提升", "调整"]
            )
