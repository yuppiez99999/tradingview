...
mock_context = {
    ...
    "news_items": [
        {"title": "某公司业绩增长", "content": "利好", "symbol": "600276.SH"}
    ],
    "macro_data": {
...
print(
    f"OK: mock orchestrate 完成, action={result2.action}, strength={result2.strength:.3f}"
)
...
print(
    f"OK: shadow_compare, diff={diff.diff:.3f}, direction_match={diff.direction_match}"
)
...
mock_veto = {
    ...
    "news_items": [
        {"title": "某公司被立案调查", "content": "财务造假", "symbol": "600276.SH"}
    ],
...
print(
    f"OK: veto path, action={result3.action}, veto={result3.veto}, reason={result3.veto_reason}"
)
