...
def main():
    ...
    for code, info in NEW_TARGETS.items():
        ...
        print(
            f"  {code} {info['name']}: {info['weight']:.4%} | ¥{target_amount} | {total_shares}股"
        )
    ...
    for code, info in NEW_TARGETS.items():
        ...
        for tmpl in PHASE_TEMPLATES:
            ...
            phase_shares = (
                int(total_shares * tmpl["capital_ratio"] // info["lots"]) * info["lots"]
            )
            phases.append(
                {
                    "phase": tmpl["phase"],
    ...
    for code, info in plan["target_portfolio"].items():
        print(
            f"    {code} {info['name']:>10}  权重 {info['weight']:.4%}  ¥{info['target_amount']:>10}"
        )
