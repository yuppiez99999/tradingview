...
def update_prices():
    ...
    for _key, item in positions.items():
        ...
        if not quote or quote.get('price') is None:
            fail_count += 1
            print(f'{code:12s} {item.get("name", ""):12s} 获取价格失败: Wind MCP 返回空数据')
            continue

        real_time_price = float(quote['price'])
        if real_time_price <= 0:
            fail_count += 1
            print(f'{code:12s} {item.get("name", ""):12s} 获取价格失败: Wind MCP 价格非正')
            continue
    ...
    report_path = os.path.join(report_dir, f'price_update_{datetime.now().strftime("%Y%m%d_%H%M%S")}_wind.json')
