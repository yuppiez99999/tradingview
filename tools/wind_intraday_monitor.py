...
def run(target_date: Optional[str] = None) -> Dict:
    ...
    for item in plan_codes:
        ...
        is_fund = clean_code.startswith(("51", "58", "15", "16")) or (item.get("type", "") in ("ETF", "基金", "债券"))
        items.append(item)
    ...
    for item, clean_code in zip(items, clean_codes):
        ...
        if quote:
            print(f"  ✅ {clean_code} => price={quote.get('price')} change={quote.get('change')}")
        else:
    ...
    with open(report_path, "w", encoding="utf-8") as f:
        ...
        f.write("---\n\n")
        f.write("## 一、标的行情总览\n\n")
        f.write("| 代码 | 名称 | 风格 | 最新价 | 估算价 | 价格偏离 | 涨跌幅 | 成交量 | 估算净流入(亿) | 状态 | 研判 |\n")
        f.write("|------|------|------|--------|--------|----------|--------|--------|----------------|------|------|\n")
        for r in rows:
            ...
            flow_str = f"{r['net_flow_yi']:+.2f}" if r["net_flow_yi"] is not None else "-"
            f.write(f"| {r['code']} | {r['name']} | {r['style'] or '-'} | {price_str} | {est_str} | {deviation_str} | {chg_str} | {vol_str} | {flow_str} | {r['status']} | {r['judgment']} |\n")
        f.write("\n")
        f.write("---\n\n")
        f.write("## 二、风格涨跌分布\n\n")
        ...
        if abnormal_rows:
            for r in abnormal_rows:
                f.write(f"- **{r['name']}**({r['code']})：{r['judgment']}，涨跌幅 {r['change_pct']:+.2f}%\n")
        else:
            f.write("当前无明显异动。\n")
        f.write("\n")
        f.write("---\n\n")
