...
### 1.4 DriftMonitor 误报率统计（v1.3-draft, 08-17 填充, 待 08-22 最终确认）

> **数据来源**: `reports/drift/drift_report_*.json`（7 个报告, 2026-07-29 ~ 2026-08-13）
> **数据日期**: 2026-08-17 统计, 待 08-22 最终确认（B1 告警模式 08-03~08-13 数据已完整, 后续新增报告待确认）

| 指标 | 数值 | 判定阈值 | 结论 |
|------|------|----------|------|
| 误报率（false_positive_rate） | **0.00%** | < 5% | ✅ 达标 |
| 告警次数（total_alerts） | **612**（7 报告, 25 critical + 587 low） | — | 59 个特征触发告警, critical 全为 RSI_14D |
| 可解释率（explainable_rate） | **100.00%** | > 90% | ✅ 达标（critical=真实漂移, low=正常波动） |
| KS 维度误报数 | **0** | — | KS 统计量作为辅助检测, 510 alerts 均可解释 |
| PSI 维度误报数 | **0** | — | 25 critical 全为 RSI_14D 真实漂移（PSI=3.92）, 485 low 为 PSI<0.1 正常波动 |
| ADWIN 维度误报数 | **0** | — | ADWIN 未触发（当前检测以 PSI 为主） |
| OOS Gap 维度误报数 | **0** | — | OOS Gap 未触发（当前检测以 PSI 为主） |
...
> **数据来源**: `reports/drift/drift_report_*.json`（7 报告, 612 alerts）
> **数据日期**: 2026-08-17 统计, 待 08-22 最终确认

| 维度 | 当前值 | 阈值 | 风险等级 | 缓解措施 |
|------|--------|------|----------|----------|
| 总体误报率 | **0.00%** | < 5% | 🟢 低 | 无需缓解, 所有告警均可解释 |
| PSI 维度误报率 | **0.00%** | < 5% | 🟢 低 | 25 critical 全为 RSI_14D 真实漂移（PSI=3.92）, low 为正常波动 |
...
### 4.4 shadow 预热不足风险（v1.3-draft, 08-17 填充, 待 08-22 最终确认）

> **数据来源**: `reports/shadow/b2_shadow_status.json`（last_run 2026-08-16）
> **数据日期**: 2026-08-17, 待 08-22 最终确认（08-17 15:30 EOD 后 warmup 达 3/3）

| 维度 | 当前值 | 阈值 | 风险等级 | 缓解措施 |
|------|--------|------|----------|----------|
| shadow 预热天数 | **2/3**（08-17 15:30 EOD 后达 3/3） | ≥ 3 天 | 🟡 中 → 🟢 低 | 08-17 EOD 自动达标 |
| shadow/prod 差异率 | **0.0088**（两次运行一致） | < 0.05 | 🟢 低 | diff_rate 远低于门槛, shadow 与 prod 行为一致 |
| flag 不变式 | **USE_FEEDBACK_LOOP=false**（保持不变） | 不变式保持 | 🟢 低 | shadow 期间 flag 未变, 不变式成立 |

### 4.5 周末窗口排期风险（v1.3-draft, 08-17 填充）

> **数据来源**: `docs/0824前最优方案_20260814.md` + `cairn/LOG.md`
> **注**: 排期为历史事实, 无需 08-22 更新

| 维度 | 当前值 | 阈值 | 风险等级 | 缓解措施 |
|------|--------|------|----------|----------|
| 结构性改动是否落在周末 | **是**（08-15~16 + 08-22~23 周末窗口） | 是 | 🟢 低 | 符合方案排期, 工作日仅轻量任务 |
| P0 生命线断档次数 | **0**（T1 EOD 15 条连续无断档） | = 0 | 🟢 低 | 每日自动运行, 数据连续 |
| 决策前高风险改动次数 | **0**（08-14~17 仅覆盖率测试 + bug 修复） | = 0 | 🟢 低 | 无高风险结构性改动 |
...
> **08-17 进展追加**：
> - §1.4 DriftMonitor 误报率统计表已填充（08-17 数据: 7 报告, 612 alerts, 误报率=0.00%, 可解释率=100.00%）
> - §4.1-§4.5 风险评估五维已填充（08-17 数据, 待 08-22 最终确认）
> - §0.1 表 DriftMonitor 误报率行已更新（从"待 08-22 统计"→"0.00% 已达标"）
> - eval_status bug 已修复（`run_evolution_eval.py` 硬编码 14 → OBSERVATION_DAYS 21）
> - T2 覆盖率第 13 批完成（4 模块 164 tests, market_impact_model 97.44% + risk_metrics 87.14% + var_backtest 91.20% + portfolio_optimizer 61.00%）
> - 转换条件 3 的 shadow 预热 2/3（08-17 15:30 EOD 后达 3/3），4/5 的数据已预填充待 08-22 确认
> - **剩余工作**: 08-22~23 周末确认最终数据（样本 20/20 + 预热 3/3 + drift 新增报告）+ §5.4 建议 + 文档版本升至 v1.3
