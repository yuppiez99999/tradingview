# ARCHITECTURE — 自我进化框架 (Self-Evolution Framework)

> 灵感来源: AIDE² (Weco AI, 2026-07-14) — Level 1 递归自我改进 (RSI)
> 创建时间: 2026-07-29
> 状态: 设计阶段 (观察期内, 仅原型, 不修改 V9 基线)
...
AIDE² 通过 **双层优化循环** (bi-level optimization) 实现 RSI:
...
### 2.1 Public/Private 分数分离 (AIDE² 核心机制)
...
**实现**: `StrategyEvaluator` 同时返回两个分数, 内层 agent 只接收 public 部分.

---

## 3. 现有基础设施复用清单

### 3.1 验证工具 (v8.3_institutional/src/validation/)
...
### 3.3 反 Reward Hacking 机制 (已有 24 项)
...
### 4.1 StrategyEvaluator (策略多维评估器)

**文件**: `utils/alpha/strategy_evaluator.py`
**定位**: 策略级/组合级评估 (区别于 `utils/alpha_evaluator.py` 的因子级 IC 评估)
...
### 4.2 EvolutionOrchestrator (进化编排器)

**文件**: `utils/alpha/evolution_orchestrator.py`
**定位**: 外层循环编排, 整合 Evaluator + DriftMonitor + ABTestFramework + ModelRegistry
...
3. 根据 private_score 和 reward_hacking_risk 决策:
   ...
   - 其他 → 继续
4. 记录决策日志到 `reports/evolution/decisions.jsonl`

### 4.3 数据收集钩子 (只读, 不影响基线)

**修改文件**: `v8.3_institutional/daily_workflow.py` Phase 10 末尾
**新增内容**: 调用 `evolution_orchestrator.collect_metrics()` (只读历史数据, 不写入生产路径)
**约束**: 观察期内仅收集数据, 不触发任何进化动作

---

## 5. 分阶段实施路线图

### 5.1 第 1 阶段: 评估器原型 (观察期内, 现在 ~ 2026-08-10)

**目标**: 建立"客观衡量好坏"的能力, 不修改 V9 基线

| 任务 | 文件 | 优先级 | 状态 |
|------|------|--------|------|
| 创建架构文档 | `docs/模块整合_8.4/ARCHITECTURE_自我进化框架.md` | 高 | ✅ 本文档 |
| 创建评估器原型 | `utils/alpha/strategy_evaluator.py` | 高 | 待开发 |
| 验证评估器与现有数据兼容 | 测试脚本 | 中 | 待开发 |
| 数据收集钩子 (只读) | `daily_workflow.py` Phase 10 | 低 | 观察期后 |

**约束**:
- 评估器只读 `reports/shadow/daily_returns.jsonl`, 不写入生产路径
- 不修改 V9 策略代码
- Feature Flag `USE_STRATEGY_EVALUATOR` 默认 False

### 5.2 第 2 阶段: 自动化重训 (观察期后, 2026-08-10 ~ 2026-08-30)

**目标**: 实现"漂移检测 → 重训 → 注册 → A/B 测试"闭环

| 任务 | 文件 | 依赖 |
|------|------|------|
| 完善 DriftMonitor 触发逻辑 | `utils/alpha/drift_monitor.py` | Stage 2 通过 |
| 启用 MLOpsPipeline | `utils/alpha/mlops_pipeline.py` | Feature Flag |
| 集成 AutoRetrainScheduler | `utils/alpha/auto_retrain_scheduler.py` | DriftMonitor |
| 评估器接入 A/B 测试 | `strategy_evaluator.py` + `ab_testing.py` | MLOpsPipeline |

### 5.3 第 3 阶段: LLM 驱动的策略创新 (2026-09-01 ~ 2026-09-30)

**目标**: 让系统不仅改进参数, 还能提出新策略思路

| 任务 | 文件 | 依赖 |
|------|------|------|
| 策略思想生成器 | `utils/alpha/strategy_ideation.py` | LLM Client |
| 假设验证框架 | `utils/alpha/hypothesis_tester.py` | ABTestFramework |
| 知识沉淀库 | `reports/evolution/knowledge_base.jsonl` | 决策日志 |

### 5.4 第 4 阶段: 完整双层闭环 (2026-10-01+)

**目标**: 无人值守的进化管道 (人类仅审批实盘上线)
...
### 6.1 为什么评估器放在 `utils/alpha/` 而非 `utils/eval/`?
...
- 观察期结束后, 评估器可直接接入第 2 阶段的自动化重训

### 6.4 为什么不直接用 MLOpsPipeline 作为进化编排器?
...
```
                    ...
                    └──────────┬───────────┘
                               │
                    ┌──────────▼───────────┐
                    │  StrategyEvaluator    │  ← 第 1 阶段 (只读)
                    │  (评估器原型)         │
                    └──────────┬───────────┘
                               │
                    ┌──────────▼───────────┐
                    │  EvolutionOrchestrator│  ← 第 2 阶段 (观察期后)
                    │  (进化编排器)         │
                    └──────────┬───────────┘
                               │
              ┌────────────────┼────────────────┐
    ...
    └────────────────┘ └──────────────┘ └──────────────┘
              │                │                │
    ┌─────────▼──────┐ ┌──────▼───────┐ ┌──────▼───────┐
    │ ModelRegistry  │ │ AutoRetrain  │ │ Strategy     │  ← 第 3 阶段
