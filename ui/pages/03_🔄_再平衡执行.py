...
with st.sidebar:
    ...
    st.divider()
    st.caption(
        f"低风险理财: {(total_capital - equity_capital)/1e4:.0f}万 ({100-equity_capital/total_capital*100:.0f}%)"
    )
...
if not positions:
    render_alert_card(
        "持仓数据缺失",
        "📋 暂无持仓数据，请在 config/positions.json 配置持仓",
        level="info",
    )
...
with tab1:
    ...
    if result.signals:
        ...
        st.dataframe(
            df.style.map(color_action, subset=["操作"]),
        ...
        sc4.metric(
            "📊 平均置信度",
            f"{sum(s.confidence for s in result.signals)/max(len(result.signals),1):.0%}",
        )
    else:
        render_alert_card(
            "组合已平衡", "✅ 组合权重已接近目标，无需调整", level="success"
        )
    ...
    if names:
        fig = go.Figure()
        fig.add_trace(
            go.Bar(name="当前权重", x=names, y=current_w, marker_color="#91d5ff")
        )
        fig.add_trace(
            go.Bar(name="目标权重", x=names, y=target_w, marker_color="#52c41a")
        )
        fig.add_trace(go.Bar(name="风险平价", x=names, y=rp_w, marker_color="#faad14"))
        fig.update_layout(
            barmode="group", height=400, margin=dict(t=20), yaxis_title="权重(%)"
        )
        ...
        budget_rows.append(
            {
                ...
                "风控等级": (
                    "🔴高"
                    if info["avg_risk"] >= 0.25
                    else ("🟡中" if info["avg_risk"] >= 0.12 else "🟢低")
                ),
            }
        )
...
with tab3:
    ...
    for day_name in ["Day1", "Day2", "Day3", "Day4", "Day5"]:
        ...
        with st.expander(
            f"{day_name} — 买入{len(buys)}只 / 卖出{len(sells)}只",
            expanded=(day_name == "Day1"),
        ):
            c1, c2 = st.columns(2)
            with c1:
                st.markdown("**📥 买入**")
    ...
    st.divider()
    st.caption(
        "建仓纪律: ①首日建仓50%确认趋势后补满 ②单日≤20%总资金 ③沪深300跌>3%暂停 ④每笔设置止损委托"
    )
...
with tab5:
    ...
    if os.path.exists(history_file):
        ...
        render_alert_card(
            "暂无历史记录", "暂无历史记录 | 运行分析后可保存", level="info"
        )
