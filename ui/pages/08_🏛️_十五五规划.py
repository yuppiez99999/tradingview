...
if not mod.FIFTEEN_FIVE_AVAILABLE:
    render_alert_card(
        "模块不可用",
        "❌ 十五五规划分析模块不可用，请检查 `utils/five_year_plan.py`",
        level="error",
    )
    st.stop()
...
if st.button("🚀 运行十五五规划分析", type="primary"):
    ...
    if overview:
        ov_data = [
            {
                "战略方向": o.get("direction", ""),
                "权重": f"{o.get('weight', 0):.0%}",
                "优先级评分": o.get("relevance_score", 0),
            }
            for o in overview
        ]
    ...
    if holdings:
        ...
        st.dataframe(
            hold_df.style.map(color_grade, subset=["等级"]),
    ...
    if adjustments:
        adj_data = [
            {
                "名称": adj.get("name", ""),
                "建议": adj.get("suggestion", ""),
                "调整幅度": f"{adj.get('weight_adjust_pct', 0):+.1f}%",
            }
        ...
        st.dataframe(
            adj_df.style.map(highlight_adjust, subset=["调整幅度"]),
            use_container_width=True,
            hide_index=True,
        )
    ...
    st.download_button(
        "📥 下载十五五规划报告",
        report,
        file_name=f"十五五规划适配_{now_bj().strftime('%Y%m%d')}.md",
        mime="text/markdown",
    )

else:
    render_alert_card(
        "待运行分析", "👆 点击上方按钮开始十五五规划适配分析", level="info"
    )
    st.markdown("""
    ### 分析内容
