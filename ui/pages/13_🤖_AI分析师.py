...
try:
    ...
    render_alert_card("安装提示", "请确保已安装: pip install langgraph langchain langchain-openai python-dotenv pandas numpy", level="info")
    st.stop()
    _AVAILABLE = False
...
if run_btn:
    ...
    render_alert_card("分析中", f"正在分析 {len(tickers)} 只标的，使用 {len(selected_analysts) if selected_analysts else '全部'} 位分析师...", level="info")
    ...
    try:
        ...
        if not result.get('success'):
            render_alert_card("分析失败", f"分析失败: {result.get('error', '未知错误')}", level="error")
            st.stop()
