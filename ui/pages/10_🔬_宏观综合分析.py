...
if not any(avail.values()):
    render_alert_card(
        "模块不可用", "所有 v5.2 分析模块均不可用，请检查 utils/ 目录", level="error"
    )
    st.stop()
...
def _run_macro_kondratiev():
    ...
    return (
        analyzer.get_current_phase(),
        analyzer.get_sector_allocation(),
        analyzer.get_commodity_signals(),
    )
...
if st.button("🚀 一键运行宏观综合分析", type="primary"):
        ...
        if mod.KONDRATIEV_AVAILABLE:
            with st.spinner("运行康波周期分析..."):
                try:
                    ...
                    render_alert_card(
                        "推荐风格",
                        f"**推荐风格**: {phase.get('recommended_style', '')}",
                        level="info",
                    )
                    ...
                    if sectors:
                        ...
                        st.dataframe(
                            sec_df[["sector", "combined_score", "recommendation"]],
                    ...
                    render_alert_card(
                        "康波周期分析失败", f"康波周期分析失败: {e}", level="error"
                    )
                ...
                try:
                    ...
                    if adjustments:
                        adj_data = [
                            ...
                            if a.get("weight_adjust_pct", 0) != 0
                        ]
                        if adj_data:
                            st.dataframe(
                    ...
                    render_alert_card(
                        "十五五规划分析失败", f"十五五规划分析失败: {e}", level="error"
                    )
    ...
    render_alert_card(
        "待运行分析", "👆 点击上方按钮一键运行三大宏观分析模块", level="info"
    )
    st.markdown(f"""
    ### 运行内容
