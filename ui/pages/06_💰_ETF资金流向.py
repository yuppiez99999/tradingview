...
with st.sidebar:
    ...
    st.metric(
        "🟡 中等置信度", f">= {SIGNAL_THRESHOLD['medium']} 亿", delta="加/减仓信号"
    )
...
with tab1:
    ...
    if not analyze:
        render_alert_card(
            "分析提示",
            "👆 点击上方按钮开始分析（将尝试连接实时数据源，失败时使用模拟数据回退）",
            level="info",
        )
        st.stop()
    ...
    with top_left:
        ...
        if overall == "净流入":
            render_status_card(
                "整体流向",
                f"📈 整体 **{overall}** | 净流入: **{total_net_flow:+.1f}** 亿 — 国家队资金积极入场",
                level="success",
            )
        elif overall == "净流出":
            render_alert_card(
                "整体流向",
                f"📉 整体 **{overall}** | 净流出: **{total_net_flow:+.1f}** 亿 — 国家队资金有所流出",
                level="warning",
            )
        else:
            render_alert_card(
                "整体流向",
                f"➡️ 整体 **{overall}** | 净流动: **{total_net_flow:+.1f}** 亿",
                level="info",
            )
    ...
    if high_signals:
        ...
        for i, s in enumerate(high_signals):
            with sig_cols[i]:
                ...
                icon = "📈" if is_buy else "📉"
                st.markdown(
                    f"""<div style="padding:16px;border-radius:10px;border-left:5px solid {border};
    ...
    flow_list = (
        ...
        else list(monitor.flow_data.values())
    )
    ...
    if category_flows:
        ...
        with cat_col2:
            for cat, net in sorted(category_flows.items(), key=lambda x: -x[1]):
                ...
                st.markdown(
                    f"{icon} **{cat}**: <span style='color:{color}'>{net:+.1f} 亿</span>",
                    unsafe_allow_html=True,
                )
        ...
        flow_table.append(
            {
                ...
                "成交额(亿)": (
                    f"{f.get('amount_yi', 0):.1f}" if f.get("amount_yi") else "-"
                ),
                "净流入(亿)": f"{net_flow:+.2f}",
                "趋势": f.get("trend", "-"),
            }
    ...
    if style_rotation:
        ...
        for i, (style, action) in enumerate(style_rotation.items()):
            with rot_cols[i]:
                ...
                color = {"增持": "#cf1322", "减持": "#389e0d", "持有": "#666"}.get(
                    action, "#666"
                )
                bg = {"增持": "#fff1f0", "减持": "#f6ffed", "持有": "#fafafa"}.get(
                    action, "#fafafa"
                )
                st.markdown(
...
with tab3:
    ...
    for alert in alerts:
        bg = (
            "#fff2f0"
            if "高" in alert["级别"]
            else "#fffbe6" if "中" in alert["级别"] else "#f6ffed"
        )
        border = (
            "#ff4d4f"
            if "高" in alert["级别"]
            else "#faad14" if "中" in alert["级别"] else "#52c41a"
        )
        st.markdown(
