...
with st.sidebar:
    ...
    if st.button("🔄 重新同步止损规则", use_container_width=True):
        try:
            ...
            engine = mod.ExcelDrivenRebalancingEngineV4(
...
with tab1:
    if not rules:
        render_alert_card(
            "规则缺失", "未找到止损止盈规则。请在左侧面板同步规则。", level="warning"
        )
        st.stop()
        ...
        status_data.append(
            {
                ...
                "距止损": (
                    f"{((current - sl_price)/sl_price*100):+.1f}%"
                    if sl_price and current
                    else "-"
                ),
                "止盈价": f"￥{tp_price:.2f}" if tp_price else "-",
                "距止盈": (
                    f"{((tp_price - current)/current*100):+.1f}%"
                    if tp_price and current
                    else "-"
                ),
                "状态": status,
    ...
    attention = [
        s for s in status_data if "关注" in s["状态"] and "触及" not in s["状态"]
    ]
    ...
    if heat_data:
        ...
        fig.add_hline(
            y=5, line_dash="dot", line_color="orange", annotation_text="警戒线"
        )
    ...
    if status_data:
        pos_weights = [
            (d["名称"], d["风险权重"]) for d in status_data if d["风险权重"] > 0
        ]
        if pos_weights:
            fig2 = px.pie(
                names=[w[0] for w in pos_weights],
...
with tab2:
    ...
    if weights:
        ...
        rho = 0.5
        port_var = sum(
            norm_weights[i] ** 2 * volatilities[i] ** 2 for i in range(len(weights))
        )
        ...
        vc2.metric(
            "VaR (1日, 95%)",
            f"{var_1d_pct*100:.1f}%",
            delta=f"￥{var_1d_pct*total_value:,.0f}",
        )
        vc3.metric(
            "VaR (5日, 95%)",
            f"{var_5d_pct*100:.1f}%",
            delta=f"￥{var_5d_pct*total_value:,.0f}",
        )
        vc4.metric(
            "VaR (月度, 95%)",
            f"{var_20d_pct*100:.1f}%",
            delta=f"￥{var_20d_pct*total_value:,.0f}",
        )
        vc5.metric(
            "CVaR (1日)",
            f"{cvar_1d_pct*100:.1f}%",
            delta=f"预期尾部损失 ￥{cvar_1d_pct*total_value:,.0f}",
        )
        ...
        vol_contrib = pd.DataFrame(
            {
                ...
                "风险贡献(%)": [
                    norm_weights[i] * volatilities[i] * 100 for i in range(len(weights))
                ],
            }
...
with tab3:
    ...
    names = [
        rule.get("name", c) for c in codes for rule in rules if rule.get("code") == c
    ]
    ...
    if len(codes) >= 2:
        ...
        n = (
            len(names_list_unique)
            if len(names_list_unique) == len(codes)
            else len(codes)
        )
        ...
        for i in range(n):
            for j in range(i + 1, n):
                c = est_corr(
                    list(industry_map.values())[i] if i < len(industry_map) else "科技",
        ...
        display_names = (
            names_list_unique[:n]
            if n <= len(names_list_unique)
            else list(industry_map.keys())[:n]
        )
        ...
        if weights:
            ...
            hhi_color = (
                "#ff4d4f" if hhi > 2500 else "#faad14" if hhi > 1500 else "#52c41a"
            )

            conc1, conc2, conc3 = st.columns(3)
            conc1.metric("HHI 指数", f"{hhi:.0f}", delta=hhi_level)
            conc2.metric(
                "有效持仓数", f"{1/(sum((w/sum(weights))**2 for w in weights)):.1f}"
            )
            ...
            if hhi > 2500:
                render_alert_card(
                    "集中度风险",
                    "组合集中度偏高，建议增加不同行业标的分散风险",
                    level="warning",
                )
            elif hhi > 1500:
                render_status_card(
                    "集中度状态", "组合集中度适中，可适当增加差异化资产", level="info"
                )
            else:
...
with tab4:
    ...
    if status_data:
        ...
        dd1.metric(
            "最大回撤",
            f"{max_dd:.2f}%",
            delta="模拟数据" if abs(max_dd) < 20 else "⚠️ 超出阈值",
        )
