...
with st.sidebar:
    ...
    show_reasoning = st.checkbox(
        "显示详细推理", value=True, help="展示每阶段的详细推理过程"
    )
    skip_ai = st.checkbox("跳过AI分析", value=False, help="仅使用规则引擎，不调用LLM")
    ...
    try:
        ...
        with st.spinner("🔍 正在执行五阶段联动分析 v5.9..."):
            ...
            with st.status("Phase 1/5: 评估组合风险...", expanded=True) as status1:
                ...
                status1.update(
                    label="Phase 1/5: 风险评估完成 ✅ (多指数Beta)", state="complete"
                )

            # Phase 2 (v5.9: 组合自触发)
            with st.status(
                "Phase 2/5: 对冲决策 (组合自触发)...", expanded=True
            ) as status2:
            ...
            with st.status(
                "Phase 3/5: 再平衡检查 (波动率驱动)...", expanded=True
            ) as status3:
                rebalance = integrator.check_rebalance(
                    risk, portfolio_volatility=est_vol
                )
                status3.update(label="Phase 3/5: 再平衡检查完成 ✅", state="complete")

            # Phase 4
            with st.status("Phase 4/5: 联合优化...", expanded=True) as status4:
                adj_hedge, adj_rebalance, warnings = integrator.joint_optimize(
                    risk, hedge, rebalance
                )
                status4.update(
                    label=f"Phase 4/5: 联合优化完成 ✅ ({len(warnings)} 项警告)",
                    state="complete",
                )
            ...
            with st.status("Phase 5/5: 生成执行计划...", expanded=True) as status5:
                plan = integrator.generate_execution_plan(
                    risk, adj_hedge, adj_rebalance, warnings
                )
...
if result is None and st.session_state.analysis_error is None:
    render_alert_card(
        "待运行分析",
        "👆 点击左侧「运行联动分析 v5.9」按钮开始分析 — 组合自触发 + 多指数Beta加权",
        level="info",
    )
    st.stop()
...
with col1:
    st.metric(
        "组合市值",
        f"¥{risk.total_value:,.0f}",
        delta=(
            f"敞口 {risk.stock_exposure/risk.total_value*100:.0f}%"
            if risk.total_value > 0
            else None
        ),
    )
with col2:
    st.metric("执行优先级", plan.execution_priority, delta=plan.execution_window)
with col3:
    st.metric(
        "预估年化收益",
...
with col4:
    delta_str = (
        f"-{plan.estimated_max_drawdown*100:.0f}bp"
        if plan.estimated_max_drawdown < 0.18
        else f"{plan.estimated_max_drawdown*100:.0f}%"
    )
    st.metric(
        "预估最大回撤",
...
with tab1:
    ...
    regime_display = {
        "calm": "🟢 平静期 (vol<18%,DD<8%)",
        "mild": "🟡 温和波动 (vol 18-25%)",
        "high": "🟠 中高波动 (vol 25-35%)",
        "tail": "🔴 尾部事件 (vol>35%或DD>18%)",
    }

    c1, c2, c3 = st.columns(3)
    with c1:
        st.metric(
            "组合状态", regime_display.get(hedge.regime.value, hedge.regime.value)
        )
    ...
    with c2:
        ...
        st.metric(
            "最大Beta (对冲后)",
            f"{beta_after:.2f}",
            delta=f"{beta_after-max_beta:+.2f}",
        )
        # 显示多指数Beta
        st.caption(
            f"CSI300β={risk.beta_csi300:.2f} | CSI500β={risk.beta_csi500:.2f} | CSI1000β={risk.beta_csi1000:.2f}"
        )
    with c3:
        st.metric("总名义价值", f"¥{hedge.total_notional:,.0f}")
        st.metric(
            "总保证金",
            f"¥{hedge.total_margin:,.0f}",
            delta=(
                f"{hedge.total_margin/risk.total_value*100:.1f}% 资产"
                if risk.total_value > 0
                else None
            ),
        )
        # v5.9: 显示触发条件
        vol_str = (
            f"vol={result.get('est_vol', 0)*100:.0f}%" if result.get("est_vol") else ""
        )
        dd_str = (
            f"dd={result.get('est_dd', 0)*100:.0f}%" if result.get("est_dd") else ""
        )
    ...
    if hedge.needed and hedge.futures_contracts:
        ...
        st.dataframe(
            pd.DataFrame(contracts_data), use_container_width=True, hide_index=True
        )

    if hedge.fallback_used:
        render_alert_card(
            "回退价格",
            f"⚠️ 以下品种使用回退价格: {', '.join(hedge.fallback_used)}",
            level="warning",
        )
...
with tab3:
    ...
    # 敞口对比
    after_hedge = (
        risk.stock_exposure * (1 - adj_hedge.hedge_ratio)
        if adj_hedge.needed
    ...
    with c3:
        discrepancy = (
            abs(after_hedge - after_rebalance) / risk.stock_exposure * 100
            if risk.stock_exposure > 0
            else 0
        )
        color = (
            "normal" if discrepancy < 5 else ("off" if discrepancy < 15 else "inverse")
        )
        st.metric(
            "敞口偏差",
            f"{discrepancy:.1f}%",
            delta="可接受" if discrepancy < 5 else "需调整",
        )
    ...
    if result["warnings"]:
        ...
        render_status_card(
            "一致性检查", "对冲后敞口与再平衡后分布一致，无需调整", level="success"
        )
    ...
    if adj_hedge.needed:
        ...
        margin_ratio = (
            required_margin / risk.total_value * 100 if risk.total_value > 0 else 0
        )
        st.metric(
            "保证金/总资产",
            f"{margin_ratio:.1f}%",
...
with tab4:
    ...
    if plan.warning_flags:
        render_alert_card(
            "注意事项",
            "\n".join([f"- {w}" for w in plan.warning_flags]),
            level="warning",
        )
    ...
    if auto_execute:
        ...
        if adj_rebalance.needed and adj_rebalance.positions_to_adjust:
            st.markdown(
                f"**再平衡操作**: {len(adj_rebalance.positions_to_adjust)} 只标的"
            )
        ...
        if st.button("✅ 确认执行", use_container_width=True, type="primary"):
            try:
                ...
                render_alert_card(
                    "执行记录已保存", f"✅ 执行记录已保存: {exec_path}", level="success"
                )
                render_alert_card(
                    "执行提示",
                    "💡 实际交易需通过券商API完成，当前仅记录执行意图。",
                    level="info",
                )
            except Exception as e:
