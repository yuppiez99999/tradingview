...
# Ensure project root in sys.path so local module imports can be top-level
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(
            ROOT / "logs" / f"backtest_rerun_{time.strftime('%Y%m%d_%H%M%S')}.log",
            encoding="utf-8",
        ),
...
def main() -> int:
    ...
    if not acc.get("passed"):
        ...
        if annual < MIN_ANNUAL_RETURN:
            ...
            logger.warning(
                "    月度收益: 正 %d 个月 (均值 +%.2f%%), 负 %d 个月 (均值 %.2f%%)",
                len(pos),
                sum(pos) / len(pos) * 100 if pos else 0,
                len(neg),
                sum(neg) / len(neg) * 100 if neg else 0,
            )
            logger.warning(
                "    最大单月盈利: +%.2f%%, 最大单月亏损: %.2f%%",
                max(rets) * 100 if rets else 0,
                min(rets) * 100 if rets else 0,
            )
            # 分析 regime 分布
            regimes = {}
    ...
    logger.info(
        "【达标确认】修复后代码回测验证通过: 年化 %.2f%% >= 8%%, 回撤 %.2f%% <= 15%%",
        annual * 100,
        max_dd * 100,
    )
    return 0
