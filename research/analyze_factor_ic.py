...
def main():
    ...
    logger.info("=" * 70)
    univ_df = get_universe(args.universe)
    if "code" in univ_df.columns:
        codes = univ_df["code"].tolist()
    else:
        codes = univ_df.index.tolist()
    codes = codes[: args.n_stocks]
    if not codes:
        logger.error("未能获取股票池")
        return
    logger.info(f"股票池: {args.universe}  取样 {len(codes)} 只")
    ...
    loader = KlinesLoader(count=args.kline_count)
    adapter = get_vibe_adapter()
    config = ScoringConfig(kline_count=args.kline_count)
    ...
    for d in rebal_dates:
        ...
        for theme in theme_factors:
            ...
            if col in scores.columns:
                ...
                m2 = xt.notna() & y.notna()
                if m2.sum() >= 20:
                    tic = scipy_stats.spearmanr(xt[m2], y[m2]).correlation
    ...
    if abs(ic_mean) >= 0.05 and t_stat >= 2.0:
        verdict = "有效 - 因子对系统有稳定预测力"
    elif abs(ic_mean) >= 0.03 and t_stat >= 1.5:
        verdict = "弱有效 - 因子有边际预测力，但偏弱"
    else:
        verdict = "基本无效 - 因子得分与未来收益无显著相关"
