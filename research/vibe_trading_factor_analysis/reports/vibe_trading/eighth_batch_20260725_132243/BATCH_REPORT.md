# 第八批次流水线跑批报告 - eighth_batch_20260725_132243

> CIO 视角批次质量评估（P2.1b 反向信号因子增强）
> 生成时间：2026-07-25 13:22:58

## 1. 批次基本信息

| 项目 | 值 |
|------|----|
| batch_id | `eighth_batch_20260725_132243` |
| 开始时间 | 2026-07-25T13:22:43.452047 |
| 结束时间 | 2026-07-25T13:22:58.230869 |
| 数据标的数 | 105 |
| 候选因子总数 | 20 |
| n_trials (多重检验基数) | 105 |
| portfolio_value | 1e8 |
...
| Gate | 通过数 | 通过率 | 说明 |
|------|--------|--------|------|
| G1 正交性 | 19 | 95.0% | 与现有 50+ 因子相关性 \|corr\| < 0.5 |
| G2 IC 稳定性 | 4 | 20.0% | IC_IR_120d >= 0.3, decay < 0.6 |
| G3 DSR 防过拟合 | 1 | 5.0% | DSR > 0, n_trials >= 5 |
| G4 经济逻辑 | 1 | 5.0% | 评分 >= 7 |
| Enhancement | 0 | 0.0% | Capacity + Regime |
| Shadow 90d | 0 | 0.0% | live_DSR > 0.5, max_dd < 12% |
| **Committee Approved** | **0** | **0.0%** | avg >= 7, no veto |
| Rejected | 20 | 100.0% | 任一 Gate 未通过 |
| Failed | 0 | 0.0% | 异常崩溃 |
...
### 7.2 G2 通过率分析
- **G2 通过率 20.0%** (未达 > 30% 目标)
- 微观结构因子通过 G2 数量: 2
- 与第六批次 v3 (20.0% G2) 对比，验证 P2.1 微观结构维度是否带来 Alpha 提升

### 7.3 准入决议
- **G2 通过率 20.0%** ≤ 30% 阈值，暂缓 S5
- **Approved 因子数 0** 个
...
完整流水线状态持久化于：
```
reports/vibe_trading/eighth_batch_20260725_132243/pipeline_state.json
```
