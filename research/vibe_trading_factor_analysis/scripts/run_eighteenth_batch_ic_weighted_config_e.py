...
def main() -> int:
    ...
    print("=" * 70)
    print("S3 第十八批次：IC 加权组合 + Config_E 超激进参数测试（P2.2 v6.6）")
    print("=" * 70)
    print("v6.6 改进内容:")
    print(f"  - Factor A: {FACTOR_A}（信号反转因子）")
    print(f"  - Factor B: {FACTOR_B}（信号正常因子）")
    print(f"  - 组合方法：IC 加权（lookback={ROLLING_LOOKBACK}）")
    print("  - Shadow 参数：4 组对比（Config_A → Config_E 梯度激进）")
    print()
    print("v6.6 验证目标:")
    print("  - IC 加权组合在 Config_E 下能否通过 Shadow（live_dsr > 0.5, max_dd < 0.12）")
    print("  - 不同参数下的 max_dd / live_dsr / total_return 权衡")
    print("  - 找到 IC 加权组合的最优 Shadow 参数")
    ...
    if passed_configs:
        ...
        print(f"  最低 max_dd 配置: {min_dd_config}")
    ...
    print("\n  观察要点:")
    print("  1. target_vol 越低 → max_dd 越低（缩放更激进）")
    print("  2. dd_threshold 越低 → 触发去杠杆越早（max_dd 控制更好）")
    print("  3. dd_factor 越低 → 去杠杆幅度越大（敞口降得更多）")
    print("  4. 但过度激进 → Alpha 信号被压缩（live_dsr 可能下降）")
    print("  5. 需要在 max_dd 控制和 live_dsr 保持之间找平衡")

    # ============ v6.6 结论 ============
    print("\n[v6.6 验证结论]")
    if passed_configs:
        print(f"  🎉 IC 加权组合在 {passed_configs} 配置下通过 Shadow 验证")
        print(f"  → IC 加权组合 + 超激进参数 = 完整 Shadow 通过")
        print(f"  → 验证了 IC 加权方法学的有效性（适应信号反转 + 风险管理控制回撤）")
    else:
        ...
        print(f"  ⚠️ 即使 Config_E 超激进参数也无法让 IC 加权组合通过 Shadow")
        print(f"     最低 max_dd: {best_max_dd:.4f} {'< 0.12 ✅' if best_max_dd < 0.12 else '>= 0.12 ❌'}")
        print(f"     最高 live_dsr: {best_live_dsr:+.4f} {'> 0.5 ✅' if best_live_dsr > 0.5 else '<= 0.5 ❌'}")
        print(f"  → 根因可能是 VT_MICRO_VOL_SKEW_INV 信号反转过于严重")
        print(f"  → 后续可尝试：剔除 VT_MICRO_VOL_SKEW_INV，只用 MARGIN_EXP 单因子")
        print(f"  → 或寻找其他与 MARGIN_EXP 信号方向一致的因子组合")
