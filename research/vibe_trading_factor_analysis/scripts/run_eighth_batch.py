...
def main() -> int:
    ...
    logger.info("=" * 70)
    logger.info("S3 第八批次流水线跑批（P2.1b 反向信号因子增强）")
    logger.info("=" * 70)
    logger.info("P2.1b 改进内容:")
    logger.info("  新增 VT_MICRO_VOL_SKEW_INV (第七批次 IC_IR=-0.42 反向使用)")
    logger.info("  预期通过 G2 (IC_IR>=0.3) 和 G3 DSR (DSR>0)")
    ...
    logger.info("\n" + "=" * 70)
    logger.info("S3 第八批次流水线跑批完成（P2.1b 反向信号因子增强）")
    logger.info("=" * 70)
    return 0
