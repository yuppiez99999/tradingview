# -*- coding: utf-8 -*-
"""S3 第八批次流水线跑批脚本（P2.1b 反向信号因子增强）

P2.1b 改进内容（基于第七批次诊断）：
    - 新增 VT_MICRO_VOL_SKEW_INV（-1 * skew(volume)）
      - 第七批次实测 VT_MICRO_VOL_SKEW IC_IR=-0.42（强反向信号）
      - 反向使用预期 IC_IR=+0.42，远超 0.3 阈值
      - 同时 Gate3 DSR=-6.865 反向后预期为 +6.865（通过）
    - 保留第七批次所有因子

P2.1b 设计依据：
    - 第七批次诊断结论：VT_MICRO_VOL_SKEW 与 VT_REV_VOL_DRAIN 都是强反向信号
    - 反向使用 IC_IR 负值因子（|IC_IR| >= 0.3）是有效的 Alpha 提取策略
    - VT_REV_VOL_DRAIN_INV 已在第七批次验证通过 G2（IC_IR=+0.33）
    - 本批次进一步验证 VT_MICRO_VOL_SKEW_INV 是否能通过 G2+G3 双重检验

验收标准：
    - VT_MICRO_VOL_SKEW_INV 通过 G2（IC_IR >= 0.3）
    - VT_MICRO_VOL_SKEW_INV 通过 G3 DSR（DSR > 0）
    - 整体 G2 通过率 > 30%
"""
...
def main() -> int:
    ...
    print("=" * 70)
    print("S3 第八批次流水线跑批（P2.1b 反向信号因子增强）")
    print("=" * 70)
    print("P2.1b 改进内容:")
    print("  新增 VT_MICRO_VOL_SKEW_INV (第七批次 IC_IR=-0.42 反向使用)")
    print("  预期通过 G2 (IC_IR>=0.3) 和 G3 DSR (DSR>0)")
    ...
    batch_id = f"eighth_batch_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    print(f"  batch_id: {batch_id}")

    # ============ Step 3: 跑流水线 ============
    print("\n[3/5] 执行 8 级流水线（P2.1b: 105 标的 + VT_MICRO_VOL_SKEW_INV 增强因子）")
    ...
    print(f"  G2 通过率: {g2_rate:.1f}% (P2.1 目标 > 30%)")
    print(f"  Deferred 比例: {result.deferred_fundamentals/total*100:.1f}%")
    ...
    print(f"  P2.1 微观结构因子通过 G2: {micro_factors_g2_passed} 个")

    if g2_rate > 30:
        print(f"  [OK] P2.1 验收达成: G2 通过率 {g2_rate:.1f}% > 30%")
    else:
        print(f"  [INFO] P2.1 验收未达: G2 通过率 {g2_rate:.1f}% <= 30%")
        print(f"         （但微观结构因子是新增维度，可能需要 P2.2+ 进一步改进）")

    # ============ Step 5: 写入报告 ============
    print("\n[5/5] 写入批次报告")
    md_path = _write_eighth_batch_report(
    ...
    print("\n" + "=" * 70)
    print("S3 第八批次流水线跑批完成（P2.1b 反向信号因子增强）")
    print("=" * 70)
    return 0
...
def _write_eighth_batch_report(
    ...
    content = f"""# 第八批次流水线跑批报告 - {result.batch_id}

> CIO 视角批次质量评估（P2.1b 反向信号因子增强）
...
| 改进项 | 目标 | 实际 | 验收 |
|--------|------|------|------|
| 新增 VT_MICRO_VOL_SKEW_INV | IC_IR >= 0.3 | (详见 G2 详情) | 待验证 |
| 沿用 P2.1 微观结构因子 | 7 个 | VT_MICRO_* x6 + VT_REV_VOL_DRAIN_INV | ✅ |
...
---
*本报告由 PipelineOrchestrator v1.0 + P2.1b 反向信号因子增强自动生成。*
"""
