...
def _optimize_and_eval(
    ...
    return {
        "gamma_s": gs, "gamma_k": gk, "delta": delta,
        "年化收益": m["年化收益"], "年化夏普": m["年化夏普"],
        "组合偏度": m["组合偏度"], "超额峰度": m["超额峰度"],
        "日CVaR95": m["日CVaR95"], "换手率": turn,
        "成本后净收益": m["年化收益"] - cost,
...
def phase1_in_sample(R, mu, cov, w_bench):  # noqa: N806
    ...
    print(f"  MV 基准: 夏普={mv['年化夏普']:.4f} 净收益={mv['成本后净收益']:.4f} "
    ...
    print(f"  {'γ_s':<8}{'γ_k':<8}{'δ':<6}{'夏普':<10}{'净收益':<10}{'偏度':<10}{'峰度':<10}")
    for r in top10:
        print(f"  {r['gamma_s']:<8.2f}{r['gamma_k']:<8.2f}{r['delta']:<6.1f}"
...
def phase2_rolling_stability(R, w_bench, top10):  # noqa: N806
    ...
    for params in top10:
        ...
        stability.append({
            "gamma_s": gs, "gamma_k": gk, "delta": d,
            "窗口夏普均值": float(sharpes.mean()),
            "窗口夏普std": float(sharpes.std()),
            "窗口夏普CV": float(sharpes.std() / abs(sharpes.mean())) if sharpes.mean() != 0 else 99,
            "窗口夏普序列": [float(x) for x in sharpes],
            "稳定": bool(sharpes.std() / abs(sharpes.mean()) < 0.5) if sharpes.mean() != 0 else False,
        })
    ...
    for s in sorted(stability, key=lambda x: x["窗口夏普CV"]):
        print(f"  {s['gamma_s']:<8.2f}{s['gamma_k']:<8.2f}{s['delta']:<6.1f}"
...
def phase3_out_of_sample(R, w_bench, stability):  # noqa: N806
    ...
    res_mv = opt_mv.optimize(
        symbols=symbols, expected_returns=mu_train, cov_matrix=cov_train,
        benchmark_weights=w_bench, max_tracking_error=0.06,
        max_weight=0.15, min_weight=0.0,
    )
    mv_test = compute_risk_metrics(R_test, res_mv.optimal_weights)
    print(f"  MV 样本外: 夏普={mv_test['年化夏普']:.4f} 收益={mv_test['年化收益']:.4f} "
    ...
    for c in candidates:
        ...
        res = opt.optimize(
            ...
            benchmark_weights=w_bench, max_tracking_error=0.06,
            max_weight=0.15, min_weight=0.0,
            return_matrix=R_train, skew_aversion=gs, kurtosis_aversion=gk,
        )
    ...
    print("\n  样本外对比 (Top 5 稳定参数):")
    print(f"  {'γ_s':<8}{'γ_k':<8}{'δ':<6}{'夏普':<10}{'收益':<10}{'偏度':<10}{'峰度':<10}{'胜MV':<10}")
    for r in oos_results:
        beat = ("夏普✓" if r["胜MV夏普"] else "夏普✗") + ("峰度✓" if r["胜MV峰度"] else "峰度✗")
        print(f"  {r['gamma_s']:<8.2f}{r['gamma_k']:<8.2f}{r['delta']:<6.1f}"
    ...
    if valid:
        ...
        print("\n  *** 最终最优 ***")
        print(f"  γ_s={best['gamma_s']:.2f} γ_k={best['gamma_k']:.2f} δ={best['delta']:.1f}")
        print(f"  样本外: 夏普={best['样本外夏普']:.4f} 收益={best['样本外收益']:.4f} "
              f"偏度={best['样本外偏度']:.4f} 峰度={best['样本外峰度']:.4f}")
        print(f"  vs MV: 夏普={mv_test['年化夏普']:.4f} 收益={mv_test['年化收益']:.4f} "
              f"偏度={mv_test['组合偏度']:.4f} 峰度={mv_test['超额峰度']:.4f}")
...
def main() -> int:
    ...
    out = {
        ...
        "MV样本内": mv_in,
        "MV样本外": {
            "年化夏普": mv_oos["年化夏普"], "年化收益": mv_oos["年化收益"],
            "组合偏度": mv_oos["组合偏度"], "超额峰度": mv_oos["超额峰度"],
        },
        "最终最优": best,
    }
