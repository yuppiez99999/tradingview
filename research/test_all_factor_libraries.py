...
try:
    ...
    print(
        f"  ✅ 动量: {mom}, 价值: {val}, 质量: {qua}, 低波: {vol}, 规模: {size}, 流动性: {liq}"
    )
...
try:
    ...
    results["Vibe-Trading"] = {
        "status": "参考资料已就绪",
        "qlib158": qlib158_count,
...
try:
    ...
    try:
        ...
        qlib_available = True
        print(
            f"  ✅ QLib 已安装: {qlib.__version__ if hasattr(qlib, '__version__') else '已安装'}"
        )
    except ImportError:
    ...
    results["QLib Alpha158"] = {
        "status": "已集成" if (has_adapter and has_train) else "部分集成",
    ...
    df = pd.DataFrame(
        {
            ...
            "high": np.cumsum(np.random.randn(n)) * 0.01
            + 100
            + np.abs(np.random.randn(n)),
            "low": np.cumsum(np.random.randn(n)) * 0.01
            + 100
            - np.abs(np.random.randn(n)),
