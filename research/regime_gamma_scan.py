...
def main() -> int:
    ...
    print(
        f"γ 网格: {len(GAMMA_S_GRID)}×{len(GAMMA_K_GRID)}={len(GAMMA_S_GRID)*len(GAMMA_K_GRID)} 组合\n"
    )
    ...
    for gs in GAMMA_S_GRID:
        for gk in GAMMA_K_GRID:
            ...
            print(
                f"{tag:<20} 夏普={m['夏普']:+.4f}  净值={m['末净值']:.4f}  "
    ...
    if beat_sharpe:
        ...
        print(
            f"  最佳: γ_s={best['gamma_s']} γ_k={best['gamma_k']} "
