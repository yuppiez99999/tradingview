# -*- coding: utf-8 -*-
"""
QLib v6训练脚本 — 使用Top50特征筛选后的XGBoost模型
"""
import os
import sys
import json
...
def main():
    ...
    if os.path.exists(features_path):
        ...
        print(f"警告: 未找到{features_path}，使用全部特征")
    ...
    print(f"\n{'='*70}")
    print(f"XGBoost v6 (Top{TOP_N}特征) 训练完成")
    print(f"{'='*70}")
    ...
    report_path = os.path.join(reports_dir, f"qlib_v6_train_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json")
