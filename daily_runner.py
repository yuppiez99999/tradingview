...
def step_update_data() -> str:
    ...
    try:
        ...
        if r.returncode == 0:
            logger.info(
                f"[数据更新] data_download.py 执行成功\n{r.stdout[-500:] if len(r.stdout) > 500 else r.stdout}"
            )
            return "iFinD数据更新完成"
        logger.warning(
            f"[数据更新] data_download.py 返回非零: {r.returncode}, stderr={r.stderr[:200]}"
        )
...
def step_fast_backtest() -> str:
    ...
    if r.returncode == 0:
        ...
        summary_lines = [
            line
            for line in lines
            if any(kw in line for kw in ["年化", "回撤", "胜率", "夏普", "收益"])
        ]
...
def run_daily_pipeline(
    ...
    logger.info("=" * 70)
    logger.info(
        f"  任务完成 | 总耗时: {total_duration/60:.1f}分钟 | "
...
示例:
  ...
  python daily_runner.py --no-trendcast    禁用 TrendCast AI 预测（默认开启）
    ...
    parser.add_argument("--skip-download", action="store_true", help="跳过数据更新步骤")
    parser.add_argument("--skip-backtest", action="store_true", help="跳过快速回测步骤")
    parser.add_argument("--trading", action="store_true", help="报告生成后启动模拟交易")
    parser.add_argument("--report-only", action="store_true", help="仅生成每日报告")
    parser.add_argument(
        "--no-ai", action="store_true", help="禁用AI分析（YiZhao增强模块）"
    )
    parser.add_argument(
        "--trendcast",
        action="store_true",
