...
将"五 Agent 加权投票结果"与"辩论裁决结果"统一融合, 采用:
  - Brier Score 动态权重: 基于 AICoordinator ai_decision_accuracy 表滚动窗口
    计算每个来源的历史校准度, 校准越好权重越高
  - 语义去重: 轻量词重叠近似余弦相似度, 过滤冗余观点避免重复计数
  - 多样性奖励: 对"预测多数偏见但坚持正确"的来源加权, 鼓励独立判断
...
_DB_PATH = "data/ai_decision_accuracy.db"  # 与 AICoordinator 约定对齐


def _word_overlap(a: str, b: str) -> float:
    """词重叠近似相似度 (Jaccard), 用于语义去重"""
    sa = set(a.split())
    sb = set(b.split())
...
def aggregate(views: list[ModelView],
    ...
    # 置信度: 观点一致度 * 平均置信度
    actions = [v.action for v in views]
    agree = max(Counter(actions).values()) / max(1, len(actions))
    ...
    if debate is not None:
        ...
        if debate.verdict_type == "AUTO":
            confidence = max(confidence, debate.confidence)
