...
NEWS_KEYWORDS_POSITIVE: dict[str, float] = {
    # ── 强利好 (1.0) ──
    "回购": 1.0, "涨停": 1.0, "超预期": 1.0, "创历史新高": 1.0,
    "业绩大增": 1.0, "营收增长": 0.9, "净利增长": 0.9, "毛利率提升": 0.9,
    # ── 中等利好 (0.6) ──
    "预增": 0.8, "中标": 0.8, "增持": 0.7, "突破": 0.7, "新高": 0.7,
    "盈利": 0.6, "扩产": 0.6, "订单": 0.6, "合作": 0.6, "放量": 0.6,
    "增长": 0.5, "景气": 0.5, "上调": 0.6, "龙头": 0.6, "技术领先": 0.6,
    "股权激励": 0.7, "并购": 0.7, "重组": 0.7, "获批": 0.6,
    # ── 弱利好 (0.3) ──
    "提升": 0.3, "改善": 0.3, "研发": 0.3, "向好": 0.3, "成长": 0.3,
    "启动": 0.3, "上线": 0.3, "推出": 0.3, "入选": 0.3, "份额提升": 0.4,
    "投产": 0.3, "出海": 0.4, "强势": 0.3, "推荐": 0.2, "加仓": 0.3,
    "战略协议": 0.5, "护城河": 0.4, "补贴": 0.4, "首破": 0.5,
}

NEWS_KEYWORDS_NEGATIVE: dict[str, float] = {
    # ── 强利空 (1.0) ──
    "退市": 1.0, "跌停": 1.0, "暴雷": 1.0, "爆雷": 1.0,
    "资金链断裂": 1.0, "债务危机": 1.0, "业绩大降": 0.9, "不及预期": 0.9,
    # ── 中等利空 (0.6) ──
    "亏损": 0.8, "处罚": 0.8, "减持": 0.7, "诉讼": 0.7, "违规": 0.7,
    "暴跌": 0.7, "下滑": 0.6, "下降": 0.5, "萎缩": 0.6, "重挫": 0.7,
    "质押": 0.6, "问询": 0.6, "调查": 0.7, "立案": 0.8, "风险": 0.5,
    "警告": 0.6, "破发": 0.5, "破净": 0.5, "过剩": 0.5, "利空": 0.6,
    "ST": 0.9, "风险警示": 0.8, "停牌核查": 0.7,
    # ── 弱利空 (0.3) ──
    "承压": 0.3, "走弱": 0.3, "回落": 0.3, "下跌": 0.3, "断供": 0.4,
    "降价": 0.3, "下调": 0.4, "经营困难": 0.5, "高管辞职": 0.5,
    "预减": 0.8,  # 业绩预告亏损属于强信号
}
...
def _save_news_sentiment_cache(
    ...
    try:
        cache_data = {
            code: {
                "sentiment_score": [round(float(v), 4) for v in df["sentiment_score"].tolist()],
            ...
            df = pd.DataFrame(
                {
                    ...
                    "news_count": data.get("news_count", [0] * len(data["sentiment_score"])),
                },
                index=dates,
            )
            df.index.name = "date"
...
def _score_article_weighted(article: dict[str, str]) -> dict[str, Any]:
    ...
    if match_count == 0:
        return {
            "score": 0.0, "weighted_pos": 0.0, "weighted_neg": 0.0,
            "matched_count": 0, "signal_strength": "none",
        }
