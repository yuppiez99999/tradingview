# -*- coding: utf-8 -*-
"""
QLib v4 训练 — 模型集成 + 自定义特征增强

改进措施:
1. 模型集成: 5个不同random_seed的LightGBM模型预测取平均，降低方差
2. 自定义特征: 在Alpha158基础上添加动量、波动率、成交量比率等因子
3. 保留v3.5的内存优化（gc.collect + 数据范围控制）
"""
import os
import sys
import json
...
def main():
    print(f"\n{'='*70}")
    print(f"QLib v4 训练 — 模型集成 + 自定义特征增强")
    print(f"{'='*70}\n")
    ...
    print(f"[3/4] 训练 {len(ENSEMBLE_SEEDS)} 个 LightGBM 模型 (模型集成)...")
    predictions = []
    for i, seed in enumerate(ENSEMBLE_SEEDS):
        print(f"      子模型 {i+1}/{len(ENSEMBLE_SEEDS)} (seed={seed})...")
        model = LGBModel(
            loss="mse",
            num_leaves=31,
            learning_rate=0.01,
    ...
    common_idx = ensemble_pred.index.intersection(label_series.index)
    pred_aligned = ensemble_pred.loc[common_idx]
    ...
    print(f"数据范围:          {DATA_START} ~ {cal_end}")
    print(f"标签:              {FORWARD_DAYS}日前瞻收益")
    print(f"子模型数:          {len(ENSEMBLE_SEEDS)}")
    print(f"特征数:            158 (Alpha158)")
    ...
    latest_preds = ensemble_pred.groupby(level=1).last()
    ...
    for symbol in portfolio_symbols:
        ...
        for cv in [symbol, symbol.lower()]:
            try:
                stock_pred = ensemble_pred.xs(cv, level=1)
                if len(stock_pred) > 0:
    ...
    report_path = os.path.join(report_dir, f"qlib_v4_train_{ts}.json")
    report = {
        "timestamp": datetime.datetime.now().isoformat(),
        "qlib_version": qlib.__version__,
        "model": f"LightGBM Ensemble ({len(ENSEMBLE_SEEDS)} models, Alpha158, lr=0.01)",
        "improvements": [
            f"训练池: {len(all_train_symbols)}只股票",
            f"数据起始时间从2015缩短到{DATA_START}",
            f"标签保持 {FORWARD_DAYS}日前瞻收益",
            f"模型集成: {len(ENSEMBLE_SEEDS)}个不同seed的LightGBM取平均",
            "降低 learning_rate 至 0.01",
            "增加 num_boost_round 至 2000",
            "增加 early_stopping_rounds 至 200",
            "添加 gc.collect() 内存清理",
        ],
