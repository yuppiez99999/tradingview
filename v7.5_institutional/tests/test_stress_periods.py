...
class TestStressScenarioConfig(unittest.TestCase):
    ...
    def test_covid_scenario(self):
        s = STRESS_SCENARIOS['COVID_CRASH']
        self.assertGreater(abs(s.asset_shocks['SPX']), 0.1)
        self.assertIn('vix_peak', s.key_metrics)
        self.assertGreater(s.key_metrics['vix_peak'], 50)

    def test_luna_scenario(self):
        s = STRESS_SCENARIOS['LUNA_CRASH']
        self.assertAlmostEqual(s.key_metrics['luna_loss'], -0.999, delta=0.01)

    def test_yen_scenario(self):
        s = STRESS_SCENARIOS['YEN_CARRY']
        self.assertGreater(s.key_metrics['vix_peak'], 50)
        self.assertLess(s.asset_shocks['NIKKEI'], -0.10)
