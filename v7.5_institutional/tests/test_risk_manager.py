...
class TestRiskBudgeter(unittest.TestCase):
    ...
    def test_initial_state(self):
        ...
        self.assertEqual(self.budgeter.position_size, 1.0)
    ...
    def test_defense_trigger(self):
        # 11% 回撤 → DEFENSE
        result = self.budgeter.update_drawdown(4_450_000, pd.Timestamp('2026-07-05'))
        self.assertEqual(result, "HALVE_POSITION")
        self.assertEqual(self.budgeter.mode, "DEFENSE")
        self.assertEqual(self.budgeter.position_size, 0.5)

    def test_defense_hold(self):
        # 先进入 DEFENSE，再来一次信号
        self.budgeter.update_drawdown(4_450_000, pd.Timestamp('2026-07-05'))
        result = self.budgeter.update_drawdown(4_400_000, pd.Timestamp('2026-07-06'))
        self.assertEqual(result, "DEFENSE_HOLD")

    def test_circuit_breaker(self):
        result = self.budgeter.update_drawdown(4_250_000, pd.Timestamp('2026-07-05'))
        self.assertEqual(result, "LIQUIDATE_ALL")
        self.assertEqual(self.budgeter.mode, "CIRCUIT_BREAKER")
        self.assertAlmostEqual(self.budgeter.position_size, 0.0)

    def test_kelly_weight(self):
        w = self.budgeter.kelly_weight(
            mu_hist=0.12, sigma=0.25, beta=1.0,
            rf=0.02, mu_market=0.08, n=252
        )
        # 应在 (0, 0.20] 范围内
    ...
    def test_kelly_single_trade_cap(self):
        ...
        w = self.budgeter.kelly_weight(
            mu_hist=0.50, sigma=0.80, beta=1.5,
            rf=0.02, mu_market=0.08, n=252
        )
    ...
    def test_risk_parity_weights(self):
        ...
        weights = self.budgeter.risk_parity_weights(cov)
