...
DEEP_SHOCK_SCENARIOS = [
    ShockScenario(
        ...
        vol_multipliers={
            "equity_global": 5.0, "equity_cn": 5.5, "vix": 5.0,
        },
    ...
    ShockScenario(
        ...
        vol_multipliers={
            "equity_global": 3.0, "vix": 5.0, "commodity_oil": 6.0,
        },
    ...
    ShockScenario(
        ...
        vol_multipliers={
            "equity_global": 2.5, "rates_10y": 3.0,
        },
    ...
    ShockScenario(
        ...
        vol_multipliers={
            "equity_cn": 6.0, "vix": 8.0, "commodity_oil": 4.0,
        },
...
class DeepStressTester:
    ...
    def resolve_shock(self, scenario: ShockScenario,
                      portfolio_exposures: Dict[str, float]) -> Dict[str, float]:
    ...
    def run_scenario(self, scenario: ShockScenario,
                     portfolio_exposures: Dict[str, float]) -> Dict:
        """运行单个深度压力场景
        ...
        # 相关性崩溃 → 分散化失效的额外损失
        diversification_benefit_before = sum(
            abs(v) for v in impacts.values()
        ) - abs(total_impact)
        diversification_benefit_after = diversification_benefit_before * (
            1 - scenario.correlation_converge
        )
    ...
    def run_all(self, portfolio_exposures: Dict[str, float]) -> Dict:
        ...
        # 概率加权损失
        prob_weighted_loss = sum(
            r['nav_impact_pct'] * r['probability']
            for r in results
        )
        ...
        overall = "FAIL" if ratings["FAIL"] > 0 else (
            "SHIELD_UP" if ratings["SHIELD_UP"] > 2 else (
                "REVIEW" if ratings["REVIEW"] > 2 else "PASS"
            )
        )
    ...
    # ---------- 机制条件测试 ----------
    def regime_stress(self, portfolio_exposures: Dict[str, float],
