...
class RiskControlManager:
    ...
    def pre_trade_check(self,
                        ...
                        sector_map: Dict[str, str] = None) -> PreTradeResult:
        """
        交易前综合检查。
        ...
        with self._lock:
            ...
            if sector_map and code in sector_map and action == OrderAction.BUY:
                ...
                sector_exposure = sum(
                    v for c, v in current_positions.items()
                    if sector_map.get(c) == sector
                )
    ...
    def portfolio_risk_report(self,
                              positions: Dict[str, float],
                              prices: Dict[str, float],
                              sector_map: Dict[str, str] = None) -> Dict[str, Any]:
        """生成组合风险报告"""
        with self._lock:
...
def check_correlation_risk(
    ...
    for i, code_i in enumerate(codes):
        for j, code_j in enumerate(codes):
            if i >= j:
                continue
            corr = corr_matrix.loc[code_i, code_j] if code_i in corr_matrix.columns and code_j in corr_matrix.columns else 0
            if corr > high_corr_threshold:
                alerts.append(
                    f"[相关性告警] {code_i} ↔ {code_j}: 相关系数 {corr:.2f} > {high_corr_threshold} "
                    f"(集中度风险)"
                )
    ...
    if len(returns_df) > lookback_days * 2:
        ...
        for i, code_i in enumerate(codes):
            for j, code_j in enumerate(codes):
                if i >= j:
                    continue
                current_corr = corr_matrix.loc[code_i, code_j] if code_i in corr_matrix.columns and code_j in corr_matrix.columns else 0
                hist_c = hist_corr.loc[code_i, code_j] if code_i in hist_corr.columns and code_j in hist_corr.columns else 0
