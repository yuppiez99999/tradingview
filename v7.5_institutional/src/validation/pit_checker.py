...
class PITChecker:
    ...
    def check_indicator_offset(
        ...
        known_fwd_params = [
            'shift', 'future_offset', 'look_ahead', 'forward',
            'future_bar', 'next',
        ]
                        ...
                        try:
                            ...
                            if val < 0:
                                self.violations.append(PITViolation(
                                    ...
                                    severity="CRITICAL",
                                    description=(
                                        f"指标'{name}'参数'{param_key}={param_val}'为负数偏移, "
                                        f"可能引用未来数据"
                                    ),
                ...
                try:
                    ...
                    if ud < ad:
                        self.violations.append(PITViolation(
                            ...
                            severity="CRITICAL",
                            description=(
                                f"{symbol}: 策略使用日{usage}早于公告日{announce}, "
                                f"存在财务数据前视偏差"
                            ),
    ...
    def generate_report(self) -> PITReport:
        ...
        if passed and not self.violations:
            ...
            summary = (
                f"PIT检查通过 (无CRITICAL违规), "
                f"{len(high)}项HIGH/{len(self.violations)}项总计, 请注意修复"
            )
        else:
            summary = (
                f"PIT检查未通过 — {len(critical)}项CRITICAL违规, "
                f"{len(high)}项HIGH, 必须在部署前修复"
            )
    ...
    def print_report(self):
        ...
        print("=" * 60)
        print(f"  Point-in-Time (PIT) 未来函数检查报告")
        print("=" * 60)
