...
class MLFeatureEngineer:
    ...
    def get_feature_vector(self, df: pd.DataFrame, feature_list: List[str]) -> np.ndarray:
        ...
        if len(available_features) < len(feature_list):
            ...
            logger.warning("[WARN] 缺失特征: %s", missing)

        # 取最后一行有效数据
        last_row = df[available_features].dropna().iloc[-1:]
        return cast(np.ndarray, last_row.values)
...
class MLModelPredictor:
    ...
    def _load_optimized_best(self, meta_files: List[str]) -> bool:
        ...
        def sort_key(item: Tuple[str, str, float, float, float]) -> Tuple[float, float]:
        ...
        logger.info("[ML] 从 %d 份元数据中择优 → %s (Acc=%.2f%%, F1=%.4f, AUC=%.4f)",
                    len(meta_files), best_model_name, best_acc * 100, best_f1, best_auc)

        return self._load_optimized(best_meta_path)

    def _load_optimized(self, meta_path: str) -> bool:
        """加载优化版模型（含特征选择）"""
        logger.info("[ML] 加载优化版模型: %s", os.path.basename(meta_path))
        ...
        try:
            ...
            # 加载特征选择器（容错：失败不阻断主模型加载，观测路径 fail-open）
            ...
            if model_files:
                try:
                    ...
                    if 'results' in meta and best_model_name in meta['results']:
                        r = meta['results'][best_model_name]
                        logger.info("[ML]   准确率: %.2f%%  F1分数: %.4f",
                                    r['accuracy'] * 100, r['f1'])
                    self._loaded = True
                    return True
        ...
        for model_name in self.MODEL_PREFERENCE:
            ...
            if model_files:
                try:
                    ...
                    logger.info("[ML] 降级加载成功: %s", model_name)
                    self._loaded = True
    ...
    def _load_enhanced(self, meta_path: str) -> bool:
        """加载增强版模型"""
        logger.info("[ML] 加载增强版模型: %s", os.path.basename(meta_path))
    ...
    def _load_basic(self, meta_path: str) -> bool:
        """加载基础版模型"""
        logger.info("[ML] 加载基础版模型: %s", os.path.basename(meta_path))
    ...
    def predict(self, kline_df: pd.DataFrame,
        ...
        if model_name not in self.models:
            logger.warning("[ML] 模型未加载: %s", model_name)
            return None
        ...
        if len(available_features) < len(self.selected_features) * 0.5:
            logger.warning("[ML] 特征不足: 需要 %d, 可用 %d",
                           len(self.selected_features), len(available_features))
            return None

        # 去除NaN行
        feat_clean = feat_df[available_features].dropna()
        if len(feat_clean) == 0:
            logger.warning("[ML] 无有效特征数据")
            return None

        # 取最后一行做预测
        feat_mat = feat_clean.iloc[-1:].values

        # 预测
        try:
            pred = model.predict(feat_mat)[0]
            prob = model.predict_proba(feat_mat)[0]
...
class StackingPredictor:
    ...
    def auto_discover_and_load(self) -> bool:
        ...
        if not optuna_meta:
            logger.warning("[Stacking] 未找到优化版元数据")
            return False

        # 加载最新的元数据
        meta_path = optuna_meta[-1]
        logger.info("[Stacking] 加载元数据: %s", os.path.basename(meta_path))
        with open(meta_path, encoding='utf-8') as f:
        ...
        if loaded_count < 2:
            logger.warning("[Stacking] 模型不足 (%d个)，Stacking 至少需要 2 个模型", loaded_count)
            return False
        ...
        self._loaded = True
        logger.info("[Stacking] 已加载 %d 个模型，权重方法: %s", loaded_count, self.weight_method)
        return True
    ...
    def predict(self, kline_df: pd.DataFrame) -> Optional[Dict[str, Any]]:
        ...
        if not self._loaded:
            if not self.auto_discover_and_load():
                # 回退到单一最佳模型
                logger.info("[Stacking] 回退到单一模型预测")
        ...
        for name, model in self.models.items():
            try:
                proba = model.predict_proba(feat_mat)[0]
...
def run_ml_signal_scan(codes: Optional[List[str]] = None,
...
class EnhancedPredictor:
    ...
    def __init__(self, model_dir: str = 'models', weight_method: str = 'f1_weighted'):
        ...
        # 特征工程器 — 优先增强版, 缺失时回退基础版 (Any: 两版均暴露 build_features/build_enhanced_features)
        self.feature_engineer: Any = MLFeatureEngineer()
        try:
            from .ml_enhanced_trainer import EnhancedFeatureEngineer as _EnhancedFeatureEngineer
            self.feature_engineer = _EnhancedFeatureEngineer(
                data_dir=model_dir.replace('models', 'data/cache'))
        except ImportError:
            pass  # fail-open: 增强版不可用时保持基础版
    ...
    def auto_discover_and_load(self, prefer_enhanced: bool = True) -> bool:
        ...
        logger.warning("[EnhancedPredictor] 未找到训练元数据")
        return False

    def _load_from_meta(self, meta_path: str) -> bool:
        """从元数据加载所有模型"""
        logger.info("[EnhancedPredictor] 加载: %s", os.path.basename(meta_path))
        with open(meta_path, encoding='utf-8') as f:
            self.metadata = json.load(f)
        ...
        self._loaded = True
        logger.info("  [CFG] 预测窗口: T+%d | 过滤震荡: %s", self.prediction_horizon,
                    self.filter_oscillation)
        logger.info("  [SUM] 加载 %d 个模型, 权重: %s", loaded_count, self.weight_method)
        return True

    @staticmethod
    def _signal_metrics(final_prob: float, agreement: float,
                        filter_oscillation: bool) -> Tuple[float, float, str]:
        """将 Stacking 融合概率映射为信号强度 / 置信度 / 方向分类。

        filter_oscillation=True 时使用四分类（strong_buy/buy/sell/strong_sell），
        0.45~0.55 判为 hold（震荡区域不参与交易）；否则三分类。
        """
        ...
        if filter_oscillation:
            ...
            if final_prob > 0.55:
                ...
                strength = 'hold'
        return signal_strength, confidence, strength

    def predict(self, kline_df: pd.DataFrame,
                market_data: Optional[Dict[str, pd.DataFrame]] = None,
                northbound_data: Optional[pd.DataFrame] = None) -> Optional[Dict[str, Any]]:
        """增强预测 — 支持多窗口 + 新特征
        ...
        if not available:
            logger.warning("[EnhancedPredictor] 无可用特征")
            return None
        ...
        # 信号强度 / 置信度 / 方向分类
        signal_strength, confidence, strength = self._signal_metrics(
            final_prob, agreement, self.filter_oscillation)
...
if __name__ == '__main__':
    ...
    logger.info("%s", json.dumps(result, indent=2, ensure_ascii=False, default=str))
