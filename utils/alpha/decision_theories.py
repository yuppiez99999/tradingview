...
标准输出: 每个引擎返回 TheoryDecision dataclass, 包含:
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
迁移说明 (T2.2, 2026-07-27):
  - 原路径: v8.3_institutional/src/factors/decision_theories.py
  - 新路径: utils/alpha/decision_theories.py (本文件)
  - 旧路径已改为 re-export 兼容层 (HC-1 保护 V9 基线)
...
class DalioEconomicMachine:
    ...
    def assess_debt_cycle(
        ...
        _real_rate = debt_data.get("real_rate", 0.01)  # noqa: F841
    ...
    def generate_decision(
        ...
        if debt_score > 0.7 and regime in ("OVERHEAT", "RECESSION"):
            ...
            signal = "BUY"
            conviction = "HIGH"
            summary = f"达利奥框架: 经济处于再通胀复苏期，债务评分{debt_score:.2f}偏低，是积极配置窗口"
        else:
            signal = "HOLD"
            conviction = "MEDIUM"
            summary = f"达利奥框架: {regime_result.get('regime_name', '')}，债务评分{debt_score:.2f}，维持标准配置"
...
class FirstPrinciplesAnalyzer:
    ...
    def generate_decision(
        ...
        if sell_count > buy_count:
            ...
            signal = "HOLD"
            score = 0.5
            summary = "第一性原理分析: 估值与基本面基本匹配，无显著偏离"
...
class BuffettMungerFramework:
    ...
    def evaluate_moat(
        ...
        for code, data in stock_data.items():
            ...
            _baseline = self.SECTOR_MOAT_BASELINE.get(sector, 25)  # noqa: F841
    ...
    def generate_decision(
        ...
        if wide_moat + narrow_moat > total * 0.5 and deep_value_count > 0:
            ...
            signal = "BUY"
            conviction = "MEDIUM"
            summary = f"巴菲特芒格框架: 有{wide_moat}只优质标的，但安全边际不足，耐心等待更好价格"
...
class TheoryFusionEngine:
    ...
    def fuse_decisions(
        ...
        if "BUY" in unique_signals and "SELL" in unique_signals:
            ...
            conflicts.append(
                f"⚠️ 理论冲突: {', '.join(buy_theories)} 建议买入 vs {', '.join(sell_theories)} 建议卖出 → 建议保持观望"
            )
