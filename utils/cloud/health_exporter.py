...
if PROMETHEUS_AVAILABLE:
    m_broker_connected = Gauge(
        "quant_broker_connected", "Broker 连接状态 (1=连接, 0=断开)"
    )
    ...
    m_rpc_latency = Histogram(
        "quant_rpc_latency_seconds",
        "云→Win 网关 RPC 延迟",
        buckets=(0.05, 0.1, 0.25, 0.5, 1, 2, 5),
    )
    m_last_job_success = Gauge(
        "quant_last_job_success_timestamp", "最近一次任务成功时间 (unix ts)"
    )
