...
def evaluate_batch(input_data, output_path=None):
    ...
    for idx, row in input_data.iterrows():
        ...
        finance_data = {
            ...
            "data_type": row.get(
                "data_type", row.get("finance_data_type", "credit_score")
            ),
            "data_quality_score": row.get(
                "data_quality_score", row.get("finance_quality", 95.0)
            ),
            "completeness": row.get(
                "completeness", row.get("finance_completeness", 95.0)
            ),
            "accuracy": row.get("accuracy", row.get("finance_accuracy", 95.0)),
            "timeliness": row.get("timeliness", row.get("finance_timeliness", 95.0)),
            "compliance_score": row.get(
                "compliance_score", row.get("finance_compliance", 100.0)
            ),
        }
        ...
        results.append(
            {
                "index": idx,
                "supplier_id": row.get(
                    "supplier_id", row.get("entity_id", f"SUPPLIER_{idx:03d}")
                ),
...
def interactive_evaluation():
    ...
    while True:
        ...
        finance_data_type = (
            input("  数据类型 (默认 credit_score): ").strip() or "credit_score"
        )
        ...
        if include_energy:
            energy_category = (
                input("  能源类别 (默认 electricity): ").strip() or "electricity"
            )

            energy_data_type = (
                input("  数据类型 (默认 production): ").strip() or "production"
            )
        ...
        # 保存结果
        all_results.append(
            {"supplier_name": supplier_name, "supplier_id": supplier_id, **result}
        )
...
def main():
    ...
    parser.add_argument(
        "--interactive", "-i", action="store_true", help="交互式评估模式"
    )
    parser.add_argument("--batch", "-b", action="store_true", help="批量评估模式")
    parser.add_argument(
        "--input", "-f", type=str, help="批量评估输入文件路径 (CSV/Excel)"
    )
    parser.add_argument("--output", "-o", type=str, help="批量评估输出文件路径")
    parser.add_argument("--info", action="store_true", help="显示模型信息")
    parser.add_argument("--demo", "-d", action="store_true", help="运行演示示例")
