...
class FeatureFlags:
    ...
    def reload(self) -> None:
        ...
        with self._lock:
            self._load_config()
            logger.info(
                "FeatureFlags reloaded: %d flags, %d overrides",
                len(self._flags_cache),
                len(self._overrides),
            )
    ...
    def enable(self, name: str, signer: str, co_signer: str, reason: str = "") -> None:
        ...
        with self._lock:
            ...
            logger.info(
                "Flag ENABLED: %s by %s + %s (reason: %s)",
                name,
                signer,
                co_signer,
                reason,
    ...
    def _set_override(
        ...
        override_dir = Path(
            os.environ.get(_OVERRIDE_DIR_ENV, "").strip() or _DEFAULT_OVERRIDE_DIR
        )
        ...
        if override_file.exists():
            logger.info(
                "Overwriting existing flag override: %s at %s", name, override_file
            )
