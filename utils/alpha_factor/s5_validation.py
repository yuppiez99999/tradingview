...
def _build_universe(
    days: int,
) -> tuple[dict[str, dict], list[str], Any, dict[str, str]]:
    ...
    logger.info(
        f"[3/4] 图: {graph_info['node_count']} 节点 / {graph_info['edge_count']} 边"
    )
...
def _top_bottom_ls(
    ...
    rows = sorted(
        zip(factor_values, returns, strict=True), key=lambda x: x[0], reverse=True
    )
    n = len(rows)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="S5 组合层面检验 — CHAIN_MOM_60D 边际夏普改善"
    )
    ...
    logger.info(
        f"窗口数: {result.get('n_windows', 0)} | horizon={result.get('horizon', 0)}"
    )
    logger.info("-" * 80)
    logger.info(
        f"{'窗口':>6} {'股票数':>8} {'基准多空':>12} {'增强多空':>12} {'差值':>12}"
    )
    logger.info("-" * 80)
    for d in result.get("window_details", []):
        logger.info(
            f"{d['window']:>6} {d['n_stocks']:>8} {d['benchmark_ls']:>12.6f} "
    ...
    logger.info(
        f"S5 判定: {tag} (边际夏普改善 {'>' if verdict == 'PASS' else '<='} 阈值)"
    )
    logger.info("=" * 80)
