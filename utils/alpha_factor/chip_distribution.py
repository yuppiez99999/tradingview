...
class ChipDistributionEngine:
    ...
    def _bin_edges_fixed(
        lo: float, hi: float, n_bins: int, pad_ratio: float = 0.5
    ) -> np.ndarray:
    ...
    @staticmethod
    def _extend_edges(
        old_edges: np.ndarray, new_lo: float, new_hi: float
    ) -> np.ndarray:
        ...
        while cur < new_hi:
            ...
            if right_pads > 5000:
                break
        left_part = (
            np.linspace(old_edges[0] - left_pads * step, old_edges[0] - step, left_pads)
            if left_pads > 0
            else np.array([])
        )
        right_part = (
            np.linspace(
                old_edges[-1] + step, old_edges[-1] + right_pads * step, right_pads
            )
            if right_pads > 0
            else np.array([])
        )
    ...
    def _align_distribution(
        ...
        idx = np.clip(
            np.searchsorted(new_centers, old_centers), 0, len(new_centers) - 1
        )
    ...
    def _triangle_weights(
        centers: np.ndarray, peak: float, spread: float
    ) -> np.ndarray:
    ...
    def update(
        ...
        if prev is None:
            ...
            edges = self._bin_edges_fixed(
                anchor_lo, anchor_hi, self.n_bins, pad_ratio=0.5
            )
        ...
        is_limit_block = (
            abs(float(hs[-1]) - float(ls[-1])) < float(cs[-1]) * 0.001
        ) and (float(vol_arr[-1]) > 2.0 * float(np.median(vol_arr[-10:] + 1e-12)))
        if is_limit_block:
