...
class EnhancedFeatureEngineer:
    ...
    def _build_market_breadth(self, data: pd.DataFrame,
        ...
        up_ratios = []
        for _, mdf in market_data.items():
    ...
    @staticmethod
    def _calc_atr(data: pd.DataFrame, period: int = 14) -> pd.Series:
    ...
    @staticmethod
    def _calc_cci(data: pd.DataFrame, period: int = 14) -> pd.Series:
    ...
    @staticmethod
    def _calc_wr(data: pd.DataFrame, period: int = 14) -> pd.Series:
        high = data.get('high', data['close'])
        low = data.get('low', data['close'])
    ...
    @staticmethod
    def _calc_obv(data: pd.DataFrame) -> pd.Series:
    ...
    @staticmethod
    def _calc_mfi(data: pd.DataFrame, period: int = 14) -> pd.Series:
        tp = (data.get('high', data['close']) + data.get('low', data['close']) + data['close']) / 3
        vol = data.get('volume', data.get('VOLUME', 1))
        rmf = tp * vol
...
class EnhancedMLTrainer:
    ...
    def __init__(self,
        ...
        Args:
            ...
            use_purged_cv: 是否使用 Purged K-Fold (P0-3 修复, 消除前视偏差)
        """
        ...
        # feature_selector 运行时在 select_features 中赋值为 SelectKBest; Any 使其在各 sklearn 阶段均可访问
    ...
    def load_all_data(self, data_dir: str = 'data/cache',
                      northbound_path: Optional[str] = None) -> Tuple[pd.DataFrame, Dict[str, pd.DataFrame]]:
        """加载全部K线数据 — 兼容两种布局:
        1) v5.10 原生平铺: data_dir/kline_*.parquet
        2) v8.6 子目录:    data_dir/<kline子目录>/*.parquet (如 data/cache/klines/{code}.parquet)
        """
        logger.info('[DATA] 加载历史数据...')
        logger.info('[PATH] 数据目录: %s', os.path.abspath(data_dir))

        def _code_from_fname(fname: str) -> str:
            code = fname
            if code.startswith('kline_'):
                code = code[len('kline_'):]
            if code.endswith('_daily.parquet'):
                code = code[:-len('_daily.parquet')]
            elif code.endswith('.parquet'):
                code = code[:-len('.parquet')]
            return code

        if not os.path.isdir(data_dir):
            logger.error('[ERROR] 数据目录不存在: %s', data_dir)
            return None, {}

        # 布局 1: 平铺 kline_*.parquet (优先, v5.10 原生)
        filepaths = [
            os.path.join(data_dir, fname)
            for fname in sorted(os.listdir(data_dir))
            if fname.startswith('kline_') and fname.endswith('.parquet')
        ]
        # 布局 2: 仅扫描名称含 kline 的子目录 (v8.6 data/cache/klines),
        #         排除 factor_series 等非K线子目录, 避免同 code 冲突污染
        if not filepaths:
            for sub in sorted(os.listdir(data_dir)):
                subpath = os.path.join(data_dir, sub)
                if os.path.isdir(subpath) and 'kline' in sub.lower():
                    filepaths += [
                        os.path.join(subpath, fname)
        ...
        if not all_data:
            logger.error('[ERROR] 未找到任何parquet数据文件!')
            return None, {}

        combined = pd.concat(all_data, ignore_index=True)
        logger.info('[DATA] 共 %d 个文件, %d 条记录', file_count, len(combined))
    ...
    def prepare_dataset(self, df: pd.DataFrame,
        ...
        Returns:
            ...
            feature_names: 特征名列表
        """
        logger.info('[FEATURE] 构建增强特征 (horizon=T+%d, filter_oscillation=%s)...',
                    self.prediction_horizon, self.filter_oscillation)
        ...
        if label_col not in combined.columns:
            logger.error('[ERROR] 标签列 %s 不存在', label_col)
            return None, None, None, []
        ...
        if self.filter_oscillation:
            ...
            logger.info('[LABEL] 过滤震荡日后样本: %d (过滤前: %d)', len(combined), total_before)
        else:
        ...
        X = combined[feature_names].values  # noqa: N806 - sklearn 惯例特征矩阵
        y = combined[label_col].values.astype(int)
        w = combined['sample_weight'].values.astype(np.float32) if 'sample_weight' in combined.columns else np.ones(len(X), dtype=np.float32)

        del combined

        unique, counts = np.unique(y, return_counts=True)
        logger.info('[LABEL] 分布 (horizon=T+%d):', self.prediction_horizon)
        for u, c in zip(unique, counts, strict=True):
            name = {0: '跌', 1: '涨'}.get(u, str(u))
            logger.info('  %s(label=%s): %6d (%.1f%%)', name, u, c, c / len(y) * 100)

        logger.info('[FEATURE] 特征数: %d, 样本数: %d', len(feature_names), len(X))
    ...
    def select_features(self, X: np.ndarray, y: np.ndarray,  # noqa: N803 - sklearn 惯例特征矩阵 X/y
                         k: int = 30, method: str = 'mutual_info') -> np.ndarray:
        """SelectKBest 特征选择"""
        logger.info('[FEATURE SEL] SelectKBest (%s), top=%d', method, k)
        ...
        X_sel = cast(np.ndarray, self.feature_selector.fit_transform(X, y))  # noqa: N806 - sklearn 惯例
        ...
        logger.info('[FEATURE SEL] Top 15 特征:')
        ranked = sorted(zip(self.feature_cols, scores, strict=True),
                        key=lambda x: x[1], reverse=True)
        for i, (name, score) in enumerate(ranked[:15], 1):
            star = '*' if name in self.selected_features else ' '
            logger.info('  %s %2d. %-30s %.4f', star, i, name, score)

        return X_sel

    # ── 模型训练 ──

    def train_all_models(self, X: np.ndarray, y: np.ndarray,  # noqa: N803 - sklearn 惯例
                          sample_weight: Optional[np.ndarray] = None,
                          use_optuna: bool = False,
                          n_trials: int = 50) -> Dict[str, Any]:
        """训练所有模型并评估"""
        logger.info('[TRAIN] 训练模型 (时序分割, 80/20)...')
        split_idx = int(len(X) * 0.8)
        X_train, X_test = X[:split_idx], X[split_idx:]  # noqa: N806 - sklearn 惯例
        y_train, y_test = y[:split_idx], y[split_idx:]
        w_train = sample_weight[:split_idx] if sample_weight is not None else None

        logger.info('  [SPLIT] 训练: %d | 测试: %d', len(X_train), len(X_test))
        logger.info('  [WEIGHT] 样本加权: %s',
                    '启用' if sample_weight is not None else '未启用')
        ...
        for name, model in model_configs.items():
            ...
            try:
                ...
                logger.info('    Acc=%.4f  F1=%.4f  AUC=%.4f  Prec=%.4f  Rec=%.4f',
                            acc, f1, auc, prec, rec)
                ...
                try:
                    stat_result = EnhancedMLTrainer.run_statistical_significance_test(
                        y_test, y_pred, y_prob if y_prob is not None else None,
        ...
        if best_model_name:
            logger.info('[RESULT] 最佳模型: %s (F1=%.4f)', best_model_name, best_f1)

        # P0-2 修复: 统计显著性检验
        logger.info('\n%s', '=' * 60)
        logger.info('  [P0-2] 统计显著性检验')
        for name in results:
    ...
    @staticmethod
    def run_statistical_significance_test(y_test: np.ndarray, y_pred: np.ndarray,
        ...
        # ── 3. Bootstrap 置信区间 ──
        bootstrap_accs: List[float] = []
        bootstrap_f1s: List[float] = []
        ...
        bootstrap_acc_arr = np.asarray(bootstrap_accs, dtype=np.float64)
        bootstrap_f1_arr = np.asarray(bootstrap_f1s, dtype=np.float64)
        acc_ci_low = float(np.percentile(bootstrap_acc_arr, 2.5))
        acc_ci_high = float(np.percentile(bootstrap_acc_arr, 97.5))
        f1_ci_low = float(np.percentile(bootstrap_f1_arr, 2.5))
        f1_ci_high = float(np.percentile(bootstrap_f1_arr, 97.5))
        ...
        for i in range(n_permutation):
            ...
            perm_acc = accuracy_score(y_shuffled[split_idx:],
                                       rf.predict(y_pred[split_idx:].reshape(-1, 1)))
            perm_accs.append(perm_acc)
        perm_acc_arr = np.asarray(perm_accs, dtype=np.float64)
        permutation_p = float((perm_acc_arr >= acc).mean())  # null分布中≥真实准确率的比例
        ...
        if rank_ic is not None:
            ic_ir = rank_ic / max(bootstrap_acc_arr.std(), 0.0001)  # 标准差估计

        # ── 7. 显著性判定 ──
        is_significant = (
            (binom_p is not None and binom_p < 0.05) or
            (permutation_p < 0.05) or
            (t_stat > 2.0) or
            (acc_ci_low > 0.5)
        )
        ...
        result = {
            ...
            'bootstrap': {
                ...
                'acc_std': float(bootstrap_acc_arr.std()),
            },
            # 统计检验
            'binomial_test': {
            ...
            'permutation_test': {
                ...
                'null_mean_accuracy': float(perm_acc_arr.mean()),
                'null_std_accuracy': float(perm_acc_arr.std()),
            },
        ...
        # 打印报告
        logger.info(f'    Rank IC: {result["rank_ic"]:.4f}' if result['rank_ic'] else '    Rank IC: N/A (无概率输出)')
        logger.info(f'    IC_IR:   {result["ic_ir"]:.4f}' if result['ic_ir'] else '    IC_IR:   N/A')
        logger.info('    Acc 95%CI: [%.4f, %.4f]', acc_ci_low, acc_ci_high)
        logger.info('    二项检验 p=%s',
                    f'{binom_p:.4f}' if binom_p is not None else 'N/A')
        logger.info('    置换检验 p=%.4f (n=%s)', permutation_p, n_permutation)
        logger.info('    t统计量: %.2f (Renaissance标准: > 2.0)', t_stat)
        logger.info('    判定: %s', result['verdict'])

        return result

    # ── Optuna 优化 ──

    def optimize_with_optuna(self, X: np.ndarray, y: np.ndarray,  # noqa: N803 - sklearn 惯例
                              sample_weight: Optional[np.ndarray] = None,
                              n_trials: int = 50) -> Dict[str, Any]:
        """使用 Optuna 进行贝叶斯超参数优化"""
        if not _HAS_OPTUNA:
            logger.warning('[WARN] Optuna 未安装，跳过优化')
            return self.train_all_models(X, y, sample_weight)

        logger.info('[OPTUNA] 贝叶斯优化 (n_trials=%d)...', n_trials)
        ...
        # 优化目标函数 — 风险约束版本
        def objective(trial: optuna.Trial, model_type: str = 'lightgbm') -> float:
            if model_type == 'lightgbm' and _HAS_LGB:
                ...
                model = lgb.LGBMClassifier(**params)  # type: ignore[arg-type]  # lightgbm py.typed 签名不接受动态 dict kwargs
            ...
            for ti, vi in tscv.split(X_train):
                ...
                y_vprob = cast(np.ndarray, model.predict_proba(X_train[vi]))[:, 1]
        ...
        for mt in model_types:
            logger.info('  [OPT] Optuna 优化 %s...', mt)
            ...
            # B023 修复: 默认参数在定义时求值快照 mt, 避免晚绑定捕获循环变量
            def _objective_for_mt(trial: optuna.Trial, mt: str = mt) -> float:
                return objective(trial, mt)
            ...
            y_pred = cast(np.ndarray, final_model.predict(X_test))
            y_prob = cast(Optional[np.ndarray], final_model.predict_proba(X_test)) \
            ...
            logger.info('    F1=%.4f  AUC=%.4f  Best_CV_F1=%.4f', f1, auc, study.best_value)
    ...
    def save_all(self, feature_names: List[str]) -> str:
        ...
        logger.info('[SAVE] 元数据: %s', meta_path)
        return meta_path
...
def run_enhanced_training(
    ...
    Args:
        ...
        use_purged_cv: 使用 Purged K-Fold 验证 (P0-3)
    """
    ...
    # 准备数据集
    X, y, w, feature_names = trainer.prepare_dataset(combined, market_data, northbound)  # noqa: N806
    if X is None or y is None:
        return {'error': '特征构建失败'}

    # P0-3 修复：先进行时间序列分割，再在训练集上进行特征选择（避免数据泄漏）
    split_idx = int(len(X) * 0.8)
    X_train = X[:split_idx]  # noqa: N806 - sklearn 惯例
    y_train = y[:split_idx]
    w_train = w[:split_idx] if w is not None else None
    logger.info('  [SPLIT] 训练样本: %d', len(X_train))

    # 特征选择（仅在训练集上进行）
    X_train_sel = trainer.select_features(X_train, y_train, k=n_features)  # noqa: N806
    logger.info('  [FEATURE SEL] 完成: %d 特征', X_train_sel.shape[1])
...
if __name__ == '__main__':
    ...
    if 'error' in result:
        ...
        logger.info('  F1: %.4f', result['best_f1'])
        logger.info('  AUC: %.4f', result['best_auc'])
        logger.info('  准确率: %.4f', result['best_accuracy'])
        logger.info('  特征数: %d', result['n_features'])
        logger.info('  样本数: %d', result['n_samples'])
        logger.info('  元数据: %s', result['meta_path'])
