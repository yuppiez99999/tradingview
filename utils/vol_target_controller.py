...
class VolTargetController:
    ...
    def calc_realized_vol(self, daily_returns: List[float] = None) -> float:
        """计算已实现波动率 (EWMA)
        ...
        # EWMA 权重
        weights = np.array([(1 - self.EWMA_LAMBDA) * (self.EWMA_LAMBDA ** i) 
    ...
    def adjust_daily_budget(self, original_budget: float,
                            realized_vol: float = None,
                            force_scale: float = None) -> Dict[str, float]:
        """调整当日建仓预算
...
if __name__ == "__main__":
    ...
    print(f"\n{'='*50}")
    print(f"原始预算: ¥{result['original_budget']:,.0f}")
    print(f"Vol Scale: {result['vol_scale']:.3f} ({'触发缩仓' if result['threshold_active'] else '正常'})")
    print(f"调整后预算: ¥{result['adjusted_budget']:,.0f}")
