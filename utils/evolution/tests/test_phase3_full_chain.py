...
class MockV1Orchestrator:
    ...
    def evaluate_current(
        self, metrics: Any = None, signal_history: Any = None
    ) -> MockScoreReport | None:
        self.signal_history = signal_history
        return self.report

    def log_decision(
        self, report: Any = None, action: str = "", metrics: Any = None
    ) -> None:
        pass
...
class TestScenario1DriftABTestPromote:
    ...
    def test_ab_test_promote_flow_via_orchestrator(
        ...
        ), f"预期 promote, 实际推荐 {result.recommendation}"
    ...
    def test_ab_test_result_written_to_memory(
        ...
        memory_result = orchestrator._write_ab_test_to_memory(
            "memory_test", mock_result
        )
...
class TestScenario4L3HumanApproval:
    ...
    def test_l3_proposal_passes_guard_with_shadow(
        ...
        assert (
            decision.passed
        ), f"合规 L3 提案应通过 Guard, 但被防御 {decision.violated_defense} 拒绝"
...
class TestScenario8EndToEnd:
    ...
    def test_full_evolution_lifecycle(
        ...
        assert (
            weight_records[0].learned == "动量因子 IC 提升后适度增配, 7 日观察收益增强"
        )
