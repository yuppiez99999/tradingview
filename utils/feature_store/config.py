...
class FeatureStoreConfig:
    ...
    def from_dict(cls, data: dict[str, Any] | None) -> FeatureStoreConfig:
        ...
        if not isinstance(data, dict):
            logger.warning(
                "[FeatureStore] config is not dict (%s), using all defaults",
                type(data).__name__,
            )
            return cls()
        ...
        if online_backend not in _VALID_ONLINE_BACKENDS:
            logger.warning(
                "[FeatureStore] invalid online_backend '%s', using default '%s'",
                online_backend,
                defaults.online_backend,
            )
        ...
        if offline_backend not in _VALID_OFFLINE_BACKENDS:
            logger.warning(
                "[FeatureStore] invalid offline_backend '%s', using default '%s'",
                offline_backend,
                defaults.offline_backend,
            )
        ...
        if not isinstance(online_ttl_days, int) or online_ttl_days <= 0:
            logger.warning(
                "[FeatureStore] invalid online_ttl_days %r, using default %d",
                online_ttl_days,
                defaults.online_ttl_days,
            )
        ...
        if not isinstance(batch_size, int) or batch_size <= 0:
            logger.warning(
                "[FeatureStore] invalid batch_size %r, using default %d",
                batch_size,
                defaults.batch_size,
            )
            batch_size = defaults.batch_size

        query_timeout_seconds = data.get(
        ...
        if (
            ...
            logger.warning(
                "[FeatureStore] invalid query_timeout_seconds %r, using default %s",
        ...
        return cls(
            ...
            online_redis_url=str(
                data.get("online_redis_url", defaults.online_redis_url)
            ),
            offline_duckdb_path=str(
                data.get("offline_duckdb_path", defaults.offline_duckdb_path)
            ),
            offline_parquet_dir=str(
                data.get("offline_parquet_dir", defaults.offline_parquet_dir)
            ),
            batch_size=batch_size,
            reject_nan=bool(data.get("reject_nan", defaults.reject_nan)),
            query_timeout_seconds=float(query_timeout_seconds),
            feature_flag_name=str(
                data.get("feature_flag_name", defaults.feature_flag_name)
            ),
        )
    ...
    def from_system_config(cls) -> FeatureStoreConfig:
        ...
        try:
            ...
            if not isinstance(sys_cfg, dict):
                logger.warning(
                    "[FeatureStore] system_config is not dict, using all defaults"
                )
                return cls()
            fs_cfg = sys_cfg.get("feature_store")
            if fs_cfg is None:
                logger.warning(
                    "[FeatureStore] feature_store section not found in system_config, using all defaults"
                )
                return cls()
            return cls.from_dict(fs_cfg)
        except Exception as e:
            logger.warning(
                "[FeatureStore] failed to load system_config: %s, using all defaults", e
            )
            return cls()
