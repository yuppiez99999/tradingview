...
class ProtectivePutEngine:
    ...
    # P1-2 (2026-09-11) + 口径拍板 (2026-09-11): 认沽保护的被保护对象是
    # **证券/ETF 腿** (PROTECTION_TARGETS 全为 ETF), 故年度期权预算基数
    # 应用 capital_base.stock_etf_capital (200 万), 而非含期货腿的
    # total_capital。原 5M → 预算 12.5 万 (高估); 现 2M → 5 万。
    # 类属性在 import 时求值一次; 显式传参 total_capital 仍优先。
    TOTAL_CAPITAL = get_stock_etf_capital()  # 证券/ETF 腿 (被保护组合规模)
