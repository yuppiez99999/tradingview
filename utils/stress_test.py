...
def generate_stress_report(
    ...
    lines.append("")
    lines.append(
        "| 情景 | 权益冲击 | 波动率倍增 | 相关性增加 | 预估回撤 | 是否触发硬止损 |"
    )
    lines.append("|------|----------|-----------|-----------|---------|--------------|")
    for name, params in scenarios.items():
        ...
        lines.append(
            f"| {name} | {eq_shock:.0%} | {vol_spike:.1f}x | +{corr_inc:.0%} | {est_drawdown:.1%} | {trigger} |"
        )
    lines.append("")
    ...
    try:
        ...
        lines.append(
            f"- **触发硬止损概率**: {sum(1 for r in returns if r < -HARD_STOP_MAX_DRAWDOWN) / monte_carlo_runs:.1%}"
        )
    ...
    lines.append("")
    lines.append(
        f"- **硬止损线**: {HARD_STOP_MAX_DRAWDOWN:.0%} (回撤超过此值触发 Kill Switch)"
    )
    lines.append(
        f"- **尾部对冲阈值**: {TAIL_HEDGE_THRESHOLD:.0%} (回撤超过此值启动期权保护)"
    )
    lines.append("")
