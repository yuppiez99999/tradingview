...
class BacktestResult:
    ...
    # v2.4 因子生命周期
    factor_registry: Dict[str, Dict] = field(default_factory=dict)  # 因子注册表
...
class BacktestDataLoader:
        ...
        for i, code in enumerate(codes):
            ...
            try:
                ...
                if rows:
                    ...
                    if self._try_akshare_etf(code, name, i + 1, n, start, end):
                        continue
    ...
    def _try_akshare_etf(self, code, name, idx, total, start, end):
        if not (code.startswith('51') or code.startswith('15')):
            return False
        try:
            ...
            if df is not None and len(df) > 0:
                ...
                if len(df) > 0:
                    ...
                    cpath = os.path.join(self.cache_dir, f'kline_{code}_daily.parquet')
                    df.to_parquet(cpath)
                    print(f"  [{idx}/{total}] OK {name} ({code}): {len(df)} 条 [akshare]")
                    return True
        except Exception as e:
            print(f"  [{idx}/{total}] -- {name} ({code}): akshare失败 {e}")
        return False
...
class HedgeRebalanceBacktest:
    ...
    def _init_factor_registry(self):
        self.factor_registry = {
            "volatility_30d": {
                "name": "30日波动率", "active": True, "ic": 0.0, "sharpe": 0.0,
                "decay_weeks": 0, "last_checked": "", "source": "portfolio"
            },
            "drawdown_60d": {
                "name": "60日回撤", "active": True, "ic": 0.0, "sharpe": 0.0,
                "decay_weeks": 0, "last_checked": "", "source": "portfolio"
            },
            "csi300_regime": {
                "name": "CSI300市场状态", "active": True, "ic": 0.0, "sharpe": 0.0,
                "decay_weeks": 0, "last_checked": "", "source": "index"
            },
            "multi_beta_if": {
                "name": "IF指数Beta", "active": True, "ic": 0.0, "sharpe": 0.0,
                "decay_weeks": 0, "last_checked": "", "source": "beta"
            },
            "multi_beta_ic": {
                "name": "IC指数Beta", "active": True, "ic": 0.0, "sharpe": 0.0,
                "decay_weeks": 0, "last_checked": "", "source": "beta"
            },
            "multi_beta_im": {
                "name": "IM指数Beta", "active": True, "ic": 0.0, "sharpe": 0.0,
                "decay_weeks": 0, "last_checked": "", "source": "beta"
            },
            "tail_trigger": {
                "name": "尾部对冲触发", "active": True, "ic": 0.0, "sharpe": 0.0,
                "decay_weeks": 0, "last_checked": "", "source": "strategy"
            },
        }

    def _update_factor_ic(self, factor_name, ic_value):
        if factor_name in self.factor_registry:
            self.factor_registry[factor_name]["ic"] = ic_value
            self.factor_registry[factor_name]["last_checked"] = str(self.dates[-1].date())

    def _scan_factor_decay(self):
        for name, info in self.factor_registry.items():
            if info["ic"] < 0.02:
                info["decay_weeks"] += 1
                if info["decay_weeks"] >= 8:
                    info["active"] = False
            else:
                info["decay_weeks"] = 0
    ...
    def _run_s2(self):
        ...
        for i in range(1, self.n_days):
            ...
            if len(turnover_daily) > TURNOVER_WINDOW:
                window_turnover = sum(turnover_daily[-(TURNOVER_WINDOW + 1):-1]) / max(1, eq[-2])
            ...
            turnover_daily[-1] = self._compute_turnover(prev_pos, pos, px) / max(1, total)

        return BacktestResult(
            name="S2:仅再平衡", equity_curve=eq, dates=self.dates,
            daily_returns=self._rets(eq), trade_count=trades,
            hedge_costs=hc, transaction_costs=tc, n_days=self.n_days,
            turnover_budget=TURNOVER_BUDGET, turnover_window=TURNOVER_WINDOW,
    ...
    def _run_s4_v2(self):
        ...
        for i in range(1, self.n_days):
            ...
            prev_ratio = current_ratio  # v2.3: 保存前一天的current_ratio用于滑点计算
            ...
            # v2.3: 滑点仅在比率变化时产生
            ratio_delta = abs(target_ratio - prev_ratio)
    ...
    def _metrics(self, r: BacktestResult):
        ...
        r.total_hedge_cost = float(getattr(r, 'roll_cost', 0.0) + getattr(r, 'margin_cost', 0.0) + getattr(r, 'slippage_cost', 0.0))
...
def format_comparison_report(multi: MultiStrategyResult, bt: HedgeRebalanceBacktest = None) -> str:
    lines = []
    lines.append("=" * 90)
    ...
    lines.append("  v2.3 优化要点:")
    lines.append("    - 滑点成本改为仅在比率变化时产生，避免无交易时的持续滑点侵蚀")
    lines.append("  v2.4 优化要点:")
    lines.append("    - 新增因子生命周期管理，自动监控信号衰减并标记失效因子")
    lines.append("")
    ...
    # v2.0 新增: 对冲有效性指标
    lines.append("")
    lines.append("  [1b] 对冲有效性指标 (v2.0新增 / v2.2成本拆分)")
    lines.append("  " + "-" * 90)
    lines.append(f"  {'策略':<18s} {'对冲盈亏':>10s} {'对冲天数':>8s} "
                f"{'展期':>8s} {'保证金':>8s} {'滑点':>8s} {'总成本':>10s} {'净效果':>10s}")
    lines.append("  " + "-" * 90)
    for s in ss:
        hpnl = getattr(s, 'hedge_pnl_total', 0)
        hdays = getattr(s, 'hedge_days', 0)
        roll = getattr(s, 'roll_cost', 0)
        margin = getattr(s, 'margin_cost', 0)
        slip = getattr(s, 'slippage_cost', 0)
        total = s.total_hedge_cost
        net_effect = hpnl - total
        lines.append(f"  {s.name:<18s} {hpnl:>+10,.0f} {hdays:>8d} "
                    f"{roll:>8,.0f} {margin:>8,.0f} {slip:>8,.0f} "
                    f"{total:>10,.0f} {net_effect:>+10,.0f}")
    lines.append("  " + "-" * 90)
    ...
    if bt is not None:
        lines.append("")
        lines.append("  [7] 因子生命周期状态 (v2.4新增)")
        lines.append("  " + "-" * 60)
        lines.append(f"  {'因子':<18s} {'状态':>6s} {'IC':>8s} {'衰减周':>8s} {'来源':>8s}")
        lines.append("  " + "-" * 60)
        for name, info in bt.factor_registry.items():
            status = "激活" if info["active"] else "失效"
            lines.append(f"  {info['name']:<18s} {status:>6s} "
                        f"{info['ic']:.3f} {info['decay_weeks']:>8d} {info['source']:>8s}")
        lines.append("  " + "-" * 60)
...
def run_backtest(start=START_DATE, end=END_DATE, output_dir=None, force_dl=False):
    ...
    report = format_comparison_report(multi, bt)
