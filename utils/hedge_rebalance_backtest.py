...
class HedgeRebalanceBacktest:
    ...
    def run_all(self) -> MultiStrategyResult:
        ...
        logger.info("  [2/5] 策略2: 仅动态再平衡 ...")
        s2 = self._run_s2()
        self._metrics(s2)

        logger.info("  [3/5] 策略3: 仅固定对冲(CSI300) ...")
        s3 = self._run_s3()
        self._metrics(s3)

        logger.info("  [4/5] 策略4: 多指数Beta加权联动 ...")
        s4 = self._run_s4_v2()
        self._metrics(s4)

        logger.info("  [5/5] 策略5: S2+组合自触发尾部对冲 ...")
        s5 = self._run_s5()
        self._metrics(s5)
...
def run_backtest(start: str = START_DATE, end: str = END_DATE, output_dir: Optional[str] = None, force_dl: bool = False) -> Tuple[MultiStrategyResult, str]:
    ...
    logger.info("=" * 60)
    logger.info("  对冲+再平衡联动 — 五策略对比回测 v2.0")
    logger.info("=" * 60)
    ...
    logger.info(f"  组合多指数Beta: IF={betas['IF']:.2f}, IC={betas['IC']:.2f}, IM={betas['IM']:.2f}")
