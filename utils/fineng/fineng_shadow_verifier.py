...
DIR_CONSISTENCY_ACCEPTANCE = (
    0.40  # 方向一致性 > 40% (放宽: 统计估计自然波动, 仅捕获系统性反转)
)
...
class FinengShadowVerifier:
    ...
    def _run_garch_window(self, returns: list[float]) -> ModuleWindowResult:
        try:
            ...
            if not (
                GARCH_PERSISTENCE_MIN <= garch.persistence <= GARCH_PERSISTENCE_MAX
            ):
            ...
            return ModuleWindowResult(
                -1, "", "", len(returns), {}, [str(e)], converged=False
            )

    def _run_kalman_window(
        self, pf: list[float], idx: list[float]
    ) -> ModuleWindowResult:
        try:
            ...
            # 运行时错误/除零/矩阵奇异
            return ModuleWindowResult(
                -1, "", "", len(pf), {}, [str(e)], converged=False
            )
    ...
    def _run_pathsim_window(self, returns: list[float]) -> ModuleWindowResult:
        try:
            ...
            metrics = {
                "dd_p50": result.dd_p50,
                "dd_p90": result.dd_p90,
                "dd_p95": result.dd_p95,
                "dd_p99": result.dd_p99,
    ...
    def _run_stage_a(
        self, pf: list[float], idx: list[float] | None, n: int
    ) -> dict[str, ModuleFullResult]:
        ...
        if self._garch_ok:
            ...
            full_results["garch"] = ModuleFullResult(
                module="garch",
                available=True,
                full_converged=fw.converged,
                full_metrics=fw.metrics,
                full_warnings=fw.warnings,
            )
            logger.info(
                "  GARCH 全量: converged=%s persistence=%.4f ratio=%.2f",
                fw.converged,
        ...
        if self._kalman_ok and idx and len(idx) == n:
            ...
            logger.info(
                "  Kalman 全量: converged=%s beta=%.3f var_reduction=%.1f%%",
                fw.converged,
        ...
        if self._evt_ok:
            ...
            logger.info(
                "  EVT 全量: converged=%s xi=%.3f n_excess=%d ES99=%.4f",
                fw.converged,
                fw.metrics.get("xi", float("nan")),
        ...
        if self._pathsim_ok:
            ...
            logger.info(
                "  PathSim 全量: DD_P50=%.2f%% DD_P99=%.2f%% gradient_ok=%s",
    ...
    def _run_stage_b(
        ...
        for mod_name in ["garch", "kalman", "evt", "pathsim"]:
            ...
            all_cvs = [
                _cv(values) for values in key_metrics.values() if len(values) >= 3
            ]
            ...
            wf_ok = (
                mr.cross_window_cv < CV_ACCEPTANCE
            ...
            if not mr.passed:
                ...
                if mr.cross_window_cv >= CV_ACCEPTANCE:
                    reasons.append(
                        f"CV={mr.cross_window_cv:.1%} >= {CV_ACCEPTANCE:.0%}"
                    )
                if mr.direction_consistency <= DIR_CONSISTENCY_ACCEPTANCE:
                    reasons.append(
                        f"方向性={mr.direction_consistency:.1%} <= {DIR_CONSISTENCY_ACCEPTANCE:.0%}"
                    )
                mr.rejection_reason = "; ".join(reasons)

            logger.info(
                "  %s: converged=%s/%d CV=%.1f%% dir=%.1f%% -> %s",
                mod_name,
                mr.n_converged_windows,
                mr.n_windows,
                mr.cross_window_cv * 100,
                mr.direction_consistency * 100,
    ...
    def _compute_final_verdict(
        ...
        failed = [
            ...
            if r.available and not r.blocked and not r.passed
        ]
        ...
        report.accepted = len(passed) >= max(
            2, len(passed) + len(failed) - 1
        )  # 最多 1 个失败
    ...
    def run_verification(
        ...
        n = len(pf)
        dates = list(date_labels) if date_labels else [f"d{i}" for i in range(n)]

        logger.info("=" * 60)
        logger.info("T4.7 Fineng 影子验证器 v2 启动")
        logger.info(
            "  数据: %d 日, 最少窗口: %d, 窗口大小: %d, 步进: %d",
            n,
            self.min_windows,
            self.window_size,
            self.window_step,
        ...
        logger.info(
            "  模块: GARCH=%s Kalman=%s EVT=%s PathSim=%s",
            self._garch_ok,
            self._kalman_ok,
            self._evt_ok,
            self._pathsim_ok,
        ...
        if n < MIN_DATA_LENGTH:
            ...
            report.acceptance_detail = (
                f"数据不足: {n} < {MIN_DATA_LENGTH} 日 (最少统计推断)"
            )
        ...
        if len(windows) < self.min_windows:
            ...
            report.next_step = (
                "调整窗口参数 (减小 window_size 或 window_step) 或补充历史数据"
            )
            self._save_report(report)
            return report

        logger.info(
            "  滑动窗口: %d 个 (%d 日/窗口, 步进 %d 日)",
            len(windows),
            self.window_size,
            self.window_step,
        )
...
def _extract_key_metrics(
    mod: str, windows: list[ModuleWindowResult]
) -> dict[str, list[float]]:
    """从各窗口提取关键指标列表."""
            ...
            wf.append(
                {
                    ...
                    "data_start": w.data_start,
                    "data_end": w.data_end,
                    "n_days": w.n_days,
                    "metrics": {
                        k: (float(f"{v:.6f}") if isinstance(v, (int, float)) else v)
