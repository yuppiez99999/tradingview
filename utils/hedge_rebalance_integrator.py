...
class HedgeRebalanceIntegrator:
    ...
    def __init__(
        ...
        self.portfolio_config = (
            _load_yaml(os.path.join(self.config_dir, "portfolio.yaml")) or {}
        )
        # v5.10+: normalize positions dict → assets list for backward compat
        if (
            "assets" not in self.portfolio_config
            and "positions" in self.portfolio_config
        ):
        ...
        self.positions = (
            _load_json(os.path.join(self.config_dir, "positions.json")) or {}
        )
        ...
        if portfolio_value is not None:
            ...
            self.portfolio_value = float(
                self.portfolio_config.get("global", {})
                .get("capital", {})
        ...
        # v8.8: hysteresis 状态 — 对冲触发后5天内渐减衰减
        self._hedge_cooldown = 0
        self._last_active_ratio = 0.0
        self._hysteresis_days = 5
                    ...
                    try:
                        ...
                        if quote is not None:
                            ...
                            if not px and isinstance(quote, dict):
                                px = float(
                                    quote.get("price", 0) or quote.get("latest", 0)
                                )
                            if px > 0:
        ...
        if not assets and isinstance(pos_cfg, dict) and pos_cfg:
            for code, item in pos_cfg.items():
                ...
                if item.get("name", "").endswith("ETF") or item.get("sector") == "黄金":
                    continue  # ETF另处理
                assets.append(
                    {"code": code, "target_weight": item.get("target_weight", 0.07)}
                )
    ...
    def _determine_market_regime(
        ...
        vol = (
            ...
            else _estimate_portfolio_vol()
        )
        dd = (
            portfolio_drawdown_60d
            if portfolio_drawdown_60d is not None
            else _estimate_portfolio_dd_60d()
        )
    ...
    def _compute_tail_hedge_ratio(
        ...
        v8.7: 触发阈值随 VIX regime 自适应 (低波提高门槛, 危机大幅降低).
        仅在组合自触发条件满足时激活，日常为0。
        """
        ...
        if _HEDGE_OK and vix is not None:
            ...
            max_hedge = TAIL_MAX_HEDGE

        ratio = 0.0

        if vol > vol_trigger:
            excess_vol = vol - vol_trigger
            ratio = min_hedge + excess_vol * 2.0
            ratio = min(ratio, max_hedge)

        if dd > dd_trigger:
            excess_dd = dd - dd_trigger
    ...
    def decide_hedge(
        ...
        v8.7: 自动获取 VIX (若未传入), 用 RegimeFolio 动态阈值.
        v8.8: VIX趋势过滤(高VIX但稳定时回退normal) + hysteresis(触发后5天渐减).
        ...
        if vix is None and _HEDGE_OK:
            ...
            if vix is not None:
                logger.info(
                    "[v8.7] VIX=%.1f → regime=%s",
                    vix,
                    _get_regime_triggers(vix).get("vol_trigger", 0),
                )

        # v8.8: VIX趋势过滤 — 高VIX但稳定时回退normal阈值
        effective_vix = vix
        if _HEDGE_OK and vix is not None and vix > 20.0:
            try:
                vix_trend = compute_vix_trend()
                if vix_trend < 0.10:
                    effective_vix = 18.0
                    logger.info(
                        "[v8.8] VIX=%.1f 但趋势稳定(%.3f), 回退normal", vix, vix_trend
                    )
            except (ValueError, TypeError, RuntimeError):
                pass

        regime = self._determine_market_regime(
            risk, portfolio_volatility, portfolio_drawdown_60d
        )
        hconfig = PORTFOLIO_HEDGE_THRESHOLDS[regime]

        # 基础对冲比率 — v8.8: 用 effective_vix (VIX趋势过滤后)
        if self.hedge_mode == HedgeMode.TAIL_ONLY:
            # 尾部保护模式: 仅在触发时激活
            base_ratio = self._compute_tail_hedge_ratio(
                portfolio_volatility, portfolio_drawdown_60d, vix=effective_vix
            )
        ...
        # v8.8: hysteresis — 对冲触发后5天内渐减衰减
        if base_ratio > 0:
            self._hedge_cooldown = self._hysteresis_days
            self._last_active_ratio = base_ratio
        elif self._hedge_cooldown > 0:
            base_ratio = self._last_active_ratio * (
                self._hedge_cooldown / self._hysteresis_days
            )
            self._hedge_cooldown -= 1
        ...
        if (
            _HEDGE_OK
            and portfolio_volatility is not None
            and portfolio_volatility > self.VOLATILITY_TARGET * 1.3
        ):
        ...
        if hedge_ratio <= 0:
            return HedgeDecision(
                ...
                expected_beta_after=max(
                    risk.beta_csi300, risk.beta_csi500, risk.beta_csi1000
                ),
        ...
        vol_str = (
            f"波动率={portfolio_volatility * 100:.1f}%" if portfolio_volatility else ""
        )
        dd_str = (
            f"回撤={portfolio_drawdown_60d * 100:.1f}%"
            if portfolio_drawdown_60d
            else ""
        )
        ...
        return HedgeDecision(
            ...
            reasoning=(
                f"{actual_desc}。{vol_str}{dd_str}。{reason}" if reason else actual_desc
            ),
        )
    ...
    def _get_dynamic_rebalance_threshold(
        self, portfolio_vol: float
    ) -> tuple[float, str, int]:
    ...
    def _get_sector_adjusted_weights(self) -> dict[str, float]:
        ...
        if self.portfolio_config.get("categories"):
            return {
                cat: self.portfolio_config.get("categories", {})
                .get(cat, {})
                .get("weight", 0.25)
    ...
    def check_rebalance(
        ...
        total_stock_value = (
            ...
            if risk.stock_exposure > 0
                ...
                self.positions.get(a["code"], {}).get("shares", 0)
                * prices.get(a["code"], 50)
        ...
        threshold, check_freq, max_adjust = self._get_dynamic_rebalance_threshold(
            portfolio_vol
        )
        ...
        for asset in self.portfolio_config.get("assets", []):
            ...
            if category in sector_weights:
                ...
                cat_default_w = (
                    self.portfolio_config.get("categories", {})
                    .get(category, {})
                    .get("weight", 0.25)
                )
                if cat_default_w > 0:
            ...
            current_weight = (
                current_mv / total_stock_value if total_stock_value > 0 else 0
            )
    ...
    def joint_optimize(
        ...
        discrepancy = (
            abs(after_hedge_exposure - after_rebalance_exposure) / risk.stock_exposure
            if risk.stock_exposure > 0
            else 0
        )
        ...
        if discrepancy > 0.15:
            adj_ratio = (
                1 - (after_rebalance_exposure / risk.stock_exposure)
                if risk.stock_exposure > 0
                else hedge.hedge_ratio
            )
    ...
    def generate_execution_plan(
        ...
        after_hedge_exposure = (
            risk.stock_exposure * (1 - hedge.hedge_ratio)
            if hedge.needed
            else risk.stock_exposure
        )
        est_return, est_drawdown, est_sharpe, est_vol = self._estimate_performance(
            risk, hedge, rebalance
        )

        summary_parts = []
        if hedge.needed:
            instruments_str = (
                "+".join(hedge.futures_instruments)
                if hedge.futures_instruments
                else "无"
            )
    ...
    def run_full_workflow(
        ...
        logger.info(
            f"  组合Beta(IF/IC/IM): {risk.beta_csi300:.2f}/{risk.beta_csi500:.2f}/{risk.beta_csi1000:.2f}"
        )

        logger.info("[Phase 2/5] 对冲决策 (组合自触发)...")
        hedge = self.decide_hedge(risk, portfolio_volatility, portfolio_drawdown_60d)
        logger.info(
            f"  模式: {hedge.mode.value}, 比率: {hedge.hedge_ratio * 100:.0f}%, 状态: {hedge.regime.value}"
        )

        logger.info("[Phase 3/5] 再平衡检查...")
        rebalance = self.check_rebalance(risk, portfolio_volatility)
        logger.info(
            f"  决策: {'需要' if rebalance.needed else '无需'}, 类型: {rebalance.rebalance_type}"
        )
            ...
            try:
                ...
                for code, pos in (
                    ...
                    if isinstance(pos, dict) and pos.get("shares", 0) > 0:
                        pos_for_stress[code] = {
                            "shares": pos["shares"],
                            "cost": pos.get("avg_cost", 50),
                        }
                if pos_for_stress:
                    stress_tests = self.hedge_engine.run_historical_stress_tests(
                        pos_for_stress, prices
                    )
                    if stress_tests:
                        breach_count = sum(
                            1 for r in stress_tests.values() if r["breaches_limit"]
                        )
                        logger.info(
                            f"[P0-8] 压力测试完成: {breach_count}/6 情景突破15%回撤上限"
                        )
            except (
                ValueError,
                KeyError,
        ...
        plan = self.generate_execution_plan(
            risk, adj_hedge, adj_rebalance, warnings, stress_tests
        )
        logger.info(
            f"  优先级: {plan.execution_priority}, 窗口: {plan.execution_window}"
        )
    ...
    def format_report(self, plan: JointPlan) -> str:
        ...
        if plan.hedge:
            ...
            if h.futures_contracts:
                for code, n in h.futures_contracts.items():
                    ...
                    lines.append(
                        f"    {code}: 做空 {n} 手 | 名义{notional:,.0f} | 保证金{margin:,.0f}"
                    )
                lines.append(
        ...
        if plan.rebalance:
            ...
            lines.append(
                f"  净现金流: {r.net_cash_flow:,.0f} ({'净流入' if r.net_cash_flow > 0 else '净流出'})"
            )
            lines.append("")
            lines.append(
        ...
        if plan.stress_tests:
            ...
            for scenario, result in plan.stress_tests.items():
                ...
                lines.append(
                    f"  {scenario}: 预估回撤{dd:.1f}%, 损失{loss:,.0f} — {breach}"
                )
            if breach_count > 0:
                lines.append(
                    f"  ⚠️  {breach_count}/6 个历史情景突破15%上限，强烈建议启用尾部保护"
                )
            else:
