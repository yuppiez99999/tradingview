...
LLM降级链: Ollama本地(Qwen2.5:7b) → GLM-5 → DeepSeek → 规则引擎
...
class ETFFlowDecisionEngine:
        ...
        if self._local_llm_client is None:
            try:
                ...
                if llm_path.exists():
                    ...
                    logger.info("LLM客户端已加载 (Ollama/GLM/DeepSeek降级链)")
    ...
    def _call_llm_analysis(self, prompt: str, system: str = "") -> Optional[str]:
        ...
        降级链: Ollama (qwen2.5:7b) → GLM-5 → DeepSeek → 规则引擎
        """
...
请输出结构化分析结果，不超过500字。"""
    ...
    def pre_market_decision(self) -> Dict[str, Any]:
        ...
        logger.info(f"【盘前决策】完成: {decision_result['summary']['strong_signals']} 强信号, "
    ...
    def intraday_decision(self, refresh_interval: int = 300) -> Dict[str, Any]:
        ...
        logger.info(f"【盘中决策】完成: {decision_result['summary']['sudden_changes']} 个突变信号, "
                   f"耗时 {elapsed:.1f}秒")
        
        return decision_result
    
    def _build_intraday_llm_prompt(self, flow_data: Dict, realtime_data: Dict, 
                                    sudden_changes: List[Dict], timestamp: str) -> str:
...
请快速响应，输出不超过300字。"""
    ...
    def post_market_review(self) -> Dict[str, Any]:
        ...
        logger.info(f"【盘后复盘】完成: 信号准确率 {signal_effectiveness.get('accuracy_rate', 0):.1%}, "
                   f"耗时 {elapsed:.1f}秒")
    ...
    def _generate_recommendations(self, signals: Dict, realtime_data: Dict, 
                                   sudden_changes: Optional[List[Dict]] = None) -> List[Dict]:
        """生成交易建议"""
        recommendations = []
        sudden_changes = sudden_changes or []
        sudden_codes = {c.get("code") for c in sudden_changes}
    ...
    def _generate_tomorrow_preview(self, signals: Dict, effectiveness: Dict) -> Dict:
        ...
        return {
            ...
            "risk_warning": [
                {"code": c, "action": "减仓"} for c, _ in top_outflow[:2]
            ],
        }
...
if __name__ == "__main__":
    ...
    parser.add_argument("phase", nargs="?", default="pre_market",
