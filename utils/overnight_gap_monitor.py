...
class OvernightGapMonitor:
    ...
    def apply_to_plan(self, plan: Dict, risk: Dict) -> Dict:
        ...
        if level >= 3:
            ...
            logger.warning(
                "[OvernightGapMonitor] L2 熔断: 已过滤 BUY 订单, 仅允许平仓"
            )
...
if __name__ == "__main__":
    ...
    print("=" * 70)
    print("Overnight Gap Monitor 自检")
    print("=" * 70)

    ogm = OvernightGapMonitor()
    risk = ogm.evaluate_overnight_risk()
    print(f"\nS&P500 跌幅: {risk['sp500_change_pct']:.2%}")
    print(f"ADR 偏离度: {risk['adr_deviation_pct']:.2%}")
    print(f"级别: L{risk['level']} ({risk['level_name']})")
    print(f"数据源: {risk['data_source']}")
    print(f"触发原因: {risk.get('trigger', 'none')}")
    print(f"动作: {risk['actions']}")
    ...
    print(f"  risk_guard: {result['risk_guard'].get('overnight_gap', {})}")
