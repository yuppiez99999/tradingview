...
from deflated_sharpe import deflated_sharpe_ratio, DeflatedSharpeResult
from walk_forward import walk_forward_stability_test, purged_walk_forward_split, check_cv_leakage
...
def run_deflated_sharpe(daily_returns: pd.Series, n_trials: int = 100) -> Dict:
    ...
    print(f"\n{'='*60}")
    print(f"  Deflated Sharpe Ratio 检验")
    print(f"{'='*60}")
    print(f"  样本数: {len(rets_list)} 个交易日")
    print(f"  观测年化夏普: {result.sharpe_ratio:.4f}")
    print(f"  偏度: {result.skewness:.4f}")
    print(f"  超额峰度: {result.kurtosis:.4f}")
    print(f"  E[max_SR] (噪音上限): {result.e_max_sr:.4f}")
    print(f"  DSR (去膨胀夏普): {result.deflated_sharpe_ratio:.4f}")
    print(f"  p-value: {result.p_value:.6f}")
    print(f"  阈值: {result.required_dsr}")
    print(f"  通过: {'✅ 是' if result.is_pass else '❌ 否'}")
...
def run_walk_forward_stability(daily_returns: pd.Series) -> Dict:
    ...
    print(f"\n{'='*60}")
    print(f"  Walk-Forward 多窗口稳定性检验 ({len(rets_list)}天 / {result.get('sharpe_cv', 0):.2f} CV)")
    print(f"{'='*60}")
    ...
    print(f"  夏普变异系数: {result.get('sharpe_cv', 0):.4f}")
    print(f"  收益变异系数: {result.get('return_cv', 0):.4f}")
    print(f"  稳定: {'✅ 是' if result.get('stable') else '❌ 否'}")
...
def run_purged_cv_alpha_ic() -> Dict:
    ...
    if all_oos_ics:
        ...
        print(f"\n{'='*60}")
        print(f"  Purged K-Fold CV (5折, embargo=1%) — OOS IC")
        print(f"{'='*60}")
        print(f"  测试标的数: {len(per_symbol_results)}")
        print(f"  总折叠数: {len(all_oos_ics)}")
        print(f"  整体 OOS IC: {overall_ic:.4f} ± {overall_std:.4f}")
        print(f"  IC IR: {overall_ic / (overall_std + 1e-9):.4f}")
        print(f"  正 IC 占比: {positive_rate*100:.1f}%")
        print(f"  显著 (IC > 0.02): {'✅' if overall_ic > 0.02 else '❌'}")
        print(f"  ---")
        print(f"  逐标的 OOS IC (前 10):")
...
def run_lookahead_check() -> Dict:
    ...
    print(f"  前视偏差泄露检查")
    print(f"{'='*60}")
    print(f"  检查特征数: {report.get('features_checked', 0)}")
    print(f"  通过: {report.get('pass_count', 0)}")
    print(f"  可疑: {report.get('leak_count', 0)}")
...
def main():
    print("=" * 60)
    print("  LGB Alpha 集成 — 统计显著性验证")
    print("  对象: 38.81% 年化收益回测结果")
    print("=" * 60)
    print()
