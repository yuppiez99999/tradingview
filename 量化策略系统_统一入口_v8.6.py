...
if '--gemma' in sys.argv or any(arg.startswith('--gemma-') for arg in sys.argv):
    ...
    if args.gemma or args.gemma_news or args.gemma_stock or args.gemma_prompt:
        import subprocess
        gemma_args = [sys.executable, os.path.join(os.path.dirname(os.path.abspath(__file__)), 'run_llm_analyze.py')]
        if args.gemma_model:
...
from utils.stress_test import HARD_STOP_MAX_DRAWDOWN, TAIL_HEDGE_THRESHOLD, generate_stress_report
...
# ============================================================
try:
    from utils.ml_predictor import EnhancedPredictor, MLModelPredictor, run_ml_signal_scan
...
# v2.0 增强训练引擎
try:
    from utils.ml_enhanced_trainer import EnhancedFeatureEngineer, EnhancedMLTrainer, run_enhanced_training
...
run_daily_workflow           = _deprecated_mode_stub('每日工作流',       '--daily',           'py -3.8 v8.3_institutional/daily_workflow.py --phase all')
...
run_social_security_analysis = _deprecated_mode_stub('社保基金ETF追踪', '--social-security')
...
run_hedge_rebalance_joint    = _deprecated_mode_stub('对冲+再平衡联动', '--hedge-rebalance')
        ...
        if external_signals and (buy_signals or sell_signals):
            ...
            if consistency_rows:
                ...
                for r in consistency_rows:
                    ml_label = "🟢" if r['ml_action'] == 'BUY' else "🔴" if r['ml_action'] == 'SELL' else "🟡"
                    ext_label = "🟢" if r['ext_action'] == 'BUY' else "🔴" if r['ext_action'] == 'SELL' else "🟡"
                    glm5_label = "🟢" if r['glm5'] == 'BUY' else "🔴" if r['glm5'] == 'SELL' else \
                                  "🟡" if r['glm5'] == 'HOLD' else "-"
                    lines.append(
    ...
    if fallback_prices:
        ...
        for code in [a['code'] for a in assets]:
            if code not in result and code in fallback_map:
                ...
                logger.warning(f"使用兜底价格 {code}: {fallback_map[code]} (最后更新: {last_updated})")
    ...
    if ml_result:
        ...
        if 'signals' in result:
            ...
            for s in sig.get('buy', [])[:5]:
                name = get_stock_name(s['code'])
                logger.info(f"    买入 {s['code']} {name:8s} 概率:{s['probability']:.1%}")
    else:
...
def run_report_generation(args):
    ...
    if config_hub and not config_hub.check_and_reload():
        ...
        logger.info(f"  📋 配置摘要: {summary['asset_count']}个标的, "
    ...
    if generate_daily_report:
        try:
            ...
            archive_report(report_content, '综合日报', ext='.txt')  # 归档路径由 archive_report 内部 logger 输出
...
def run_quick_check(args):
    ...
    if ETFFundFlowMonitor is not None:
        ...
        logger.warning("  ⚠️ ETFFundFlowMonitor 未加载 (engine/managers 缺失), 跳过ETF监控检查")
    etf_config = config_manager.get('etf_monitor', 'signal_high_threshold')
    if etf_config:
        try:
            logger.info(f"  ✅ 信号阈值: 高{etf_config/1e8:.0f}亿/中{config_manager.get('etf_monitor', 'signal_medium_threshold')/1e8:.0f}亿/低{config_manager.get('etf_monitor', 'signal_low_threshold')/1e8:.0f}亿")
    ...
    try:
        ...
        logger.info(f"  当前活跃连接器: {connector_status.get('active_connector', 'None')}")
        logger.error(f"  是否降级模式: {'✅ 是' if connector_status.get('fallback_mode') else '❌ 否'}")
        logger.info(f"  注册连接器数: {connector_status.get('total_connectors', 0)}")
        logger.info(f"  可用连接器数: {connector_status.get('available_connectors', 0)}")
    except (AttributeError, TypeError):
        active = connector_manager.get_active_connector()
        n = len(connector_manager.list_connectors())
        logger.warning(f"  ⚠️ 连接器状态 API 不完整 (get_status 缺失), 简化展示: 活跃={'有' if active else '无'}, 已注册 {n} 个")
    ...
    try:
        ...
        router = ModelRouter()
        logger.info(f"  ✅ 多模型路由器: 已注册 {len(router.config.get('providers', {}))} 个模型提供商")
    ...
    try:
        ...
        logger.warning(f"  {'✅' if status.get('wind_mcp_available') else '⚠️'} Wind MCP: {wind_ok}")
...
def run_enhanced_training_mode(args):
    ...
    logger.info("=" * 70)
    logger.info(f"  预测窗口: T+{horizon} | 过滤震荡: {filter_osc} | Optuna: {use_optuna}")
...
def run_enhanced_prediction_mode(args):
    ...
    logger.info(f"  🟢 买入: {len(signals['buy'])} | 🔴 卖出: {len(signals['sell'])} | 🟡 震荡/持有: {len(signals['hold'])}")
    ...
    if args.list:
        ...
        if not hypotheses:
            ...
            for i, hyp in enumerate(hypotheses, 1):
                ...
                logger.info(f"     ID: {list(strategy_registry.hypotheses.keys())[i-1]}")
        ...
        if os.path.exists(cache_file):
            try:
                ...
                if 'close' in df.columns and len(df) > 60:
                    ...
                    returns_data.append(rets[-252:] if len(rets) >= 252 else rets)  # 最近一年
...
def run_stop_loss_config_mode(args):
    ...
    logger.info(f"\n标的配置 ({len(assets)} 只):")
    logger.info("-" * 70)
    logger.info(f"{'代码':<12} {'名称':<10} {'板块':<8} {'基准价':>8} {'ATR止损':>8} {'固定止损':>8} {'风险'}")
    logger.info("-" * 70)
    ...
    for a in assets:
        ...
        logger.info(f"{code:<12} {name:<10} {sector:<8} {base:>8.2f} {atr_sl:>8.2f} {fixed_sl:>8.2f} {risk}")
...
def main():
    ...
    MODES = [
        ...
        ('--ai-decision',     'ai_decision',     'AI盘中决策 v5.10 - 场景路由+并行对冲+Wind MCP动态数据', run_ai_decision),
        ...
        ('--hedge',           'hedge',           '对冲分析 v5.10 — Taleb+Burry+Druck 多指数期货/期权风险对冲',run_hedge_mode),
        ('--hedge-rebalance', 'hedge_rebalance', '对冲+再平衡联动分析 v5.10 — 组合自触发+多指数Beta加权',           run_hedge_rebalance_joint),
        ('--hedge-detail',    'hedge_detail',    '期货对冲明细 v5.10 — 完整对冲规格+Beta表+触发机制+回测', run_hedge_detail_mode),
        ('--stress-test',     'stress_test',     '极端压力测试 v5.10 — 6历史情景+蒙特卡洛+硬止损',    run_stress_test_mode),
        ('--stop-loss',       'stop_loss',       '止损配置 v5.10 — 查看/更新止损止盈规则(含ATR动态止损)', run_stop_loss_config_mode),
        ('--ml-significance',  'ml_significance', 'ML显著性验证 v5.10 — Bootstrap+置换检验+Rank IC',      run_ml_significance_mode),
        ('--kronos',           'kronos',          'Kronos 金融K线预测 — 时序基础模型信号',                 run_kronos_predict_mode),
        ('--gemma',            'gemma',           'Gemma 4 分析增强 — 新闻情绪/异动解读/报告生成',          run_gemma_analyze_mode),
...
架构特点 (借鉴Vibe-Trading):
  ...
  • 时序预测: Transformer模型骨架集成
    ...
    parser.add_argument('--include-valuation', action='store_true', help='报告生成时追加 DCF + Comps 估值摘要')
    ...
    # v5.10: 对冲明细选项
    parser.add_argument('--json', action='store_true', help='期货对冲明细: 输出 JSON 格式')
    ...
    # Kronos 预测选项
    parser.add_argument('--kronos-code', type=str, default=None, help='Kronos: 单股代码 (如 000001)')
    parser.add_argument('--kronos-name', type=str, default=None, help='Kronos: 股票名称 (可选)')
    parser.add_argument('--kronos-pred-len', type=int, default=24, help='Kronos: 预测窗口 (默认 24)')
    parser.add_argument('--kronos-device', type=str, default='cpu', help='Kronos: 运行设备 (cpu 或 cuda:0)')
    parser.add_argument('--kronos-batch', type=str, default=None, help='Kronos: 批量预测 JSON 数组')
    parser.add_argument('--kronos-output', type=str, default=None, help='Kronos: 输出文件名 (保存到 reports/)')
    # Gemma 分析选项
    parser.add_argument('--gemma-model', type=str, default=None, help='Gemma: Ollama 模型名 (默认 qwen2.5:7b)')
    parser.add_argument('--gemma-news', type=str, default=None, help='Gemma: 新闻内容 (用于情绪分析)')
    parser.add_argument('--gemma-stock', type=str, default=None, help='Gemma: 股票代码 (用于基本面分析)')
    parser.add_argument('--gemma-prompt', type=str, default=None, help='Gemma: 自定义提示')
    parser.add_argument('--gemma-output', type=str, default=None, help='Gemma: 输出文件名 (保存到 reports/)')
    # ── v5.7 Phase 2: 高级训练选项 ──
    ...
    parser.add_argument('--horizon', type=int, default=1, help='训练: 预测窗口 T+N (1/5/10, 默认 1)')
    parser.add_argument('--trials', type=int, default=50, help='训练: Optuna 试验次数 (默认 50)')
