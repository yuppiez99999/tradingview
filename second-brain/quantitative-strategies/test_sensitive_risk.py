...
def test_sensitive_risk_detection():
    ...
    for risk_type in RiskType:
        risk_system.update_control_thresholds(
            risk_type, {"low": 0.1, "medium": 0.3, "high": 0.5, "critical": 0.7}
        )
...
def compare_risk_sensitivity():
    ...
    # 设置不同敏感度
    low_sensitivity.update_control_thresholds(
        RiskType.MARKET, {"low": 0.4, "medium": 0.6, "high": 0.8, "critical": 0.9}
    )
    medium_sensitivity.update_control_thresholds(
        RiskType.MARKET, {"low": 0.2, "medium": 0.4, "high": 0.6, "critical": 0.8}
    )
    high_sensitivity.update_control_thresholds(
        RiskType.MARKET, {"low": 0.1, "medium": 0.2, "high": 0.3, "critical": 0.5}
    )
    ...
    for name, system in systems:
        ...
        for alert in alerts[:2]:  # 只显示前两个告警
            print(
                f"  告警: [{alert.risk_type.value}] {alert.risk_level.value} ({alert.risk_score:.3f})"
            )
...
def test_operational_emotional_risks():
    ...
    # 测试操作风险
    print("\n操作风险测试:")
    operational_score = risk_system.risk_controls[
        RiskType.OPERATIONAL
    ].calculate_risk_score(high_risk_data)
    print(f"操作风险分数: {operational_score:.3f}")
    print(
        f"操作风险等级: {risk_system.risk_controls[RiskType.OPERATIONAL].get_risk_level(operational_score)}"
    )

    # 测试情绪风险
    print("\n情绪风险测试:")
    emotional_score = risk_system.risk_controls[
        RiskType.EMOTIONAL
    ].calculate_risk_score(high_risk_data)
    print(f"情绪风险分数: {emotional_score:.3f}")
    print(
        f"情绪风险等级: {risk_system.risk_controls[RiskType.EMOTIONAL].get_risk_level(emotional_score)}"
    )

    # 测试整体影响
    print("\n整体影响测试:")
    overall_score, _individual_scores = risk_system.calculate_overall_risk(
        high_risk_data
    )
    print(f"整体风险分数: {overall_score:.3f}")
