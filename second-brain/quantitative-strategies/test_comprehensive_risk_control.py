...
def create_crisis_test_data():
    ...
    return {
        ...
        "portfolio": {
            ...
            "correlation_matrix": np.array(
                [[1.0, 0.95, 0.90], [0.95, 1.0, 0.88], [0.90, 0.88, 1.0]]
            ),
...
def test_basic_functions(risk_system, test_data):
    ...
    for alert in alerts:
        print(
            f"  [{alert.risk_type.value}] {alert.risk_level.value} - {alert.description}"
        )
    ...
    print("\n风险摘要:")
    print(
        f"  整体风险: {summary['overall_risk_score']:.3f} ({summary['overall_risk_level']})"
    )
...
def test_risk_configuration(risk_system):
    ...
    for risk_type in RiskType:
        risk_system.update_control_thresholds(
            risk_type, {"low": 0.05, "medium": 0.15, "high": 0.25, "critical": 0.35}
        )
    ...
    for risk_type in RiskType:
        risk_system.update_control_thresholds(
            risk_type, {"low": 0.3, "medium": 0.5, "high": 0.7, "critical": 0.9}
        )
    ...
    for risk_type in RiskType:
        risk_system.update_control_thresholds(
            risk_type, {"low": 0.1, "medium": 0.25, "high": 0.4, "critical": 0.6}
        )
...
def test_realtime_monitoring(risk_system):
    ...
    for i in range(5):
        ...
        print(f"\n时间点 {i+1}:")
        print(
            f"  整体风险: {overall_score:.3f} ({risk_system._get_overall_risk_level(overall_score)})"
        )
...
def test_risk_recommendations(risk_system, normal_data, crisis_data):
    ...
    scenarios = [
        ...
        {
            "name": "组合集中",
            "data": {"portfolio": {"positions": {"STOCK_A": 0.6, "STOCK_B": 0.4}}},
        },
        {
            "name": "系统故障",
            "data": {"operational": {"system_health": 0.3, "execution_quality": 0.2}},
        },
    ...
    for result in simulation_results:
        print(
            f"  {result['scenario_name']}: {result['risk_level']} (风险分数: {result['overall_risk_score']:.3f})"
        )
...
def generate_risk_report(risk_system, test_data):
    ...
    report = {
        ...
        "system_status": {
            "active_controls": sum(
                1 for control in risk_system.risk_controls.values() if control.is_active
            ),
