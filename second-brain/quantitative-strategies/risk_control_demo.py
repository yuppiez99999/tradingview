...
def demo_risk_scenarios():
    ...
    print("-" * 80)
    print(
        f"{'场景名称':<12} {'整体风险':<10} {'风险等级':<8} {'市场':<8} {'个股':<8} {'组合':<8} {'操作':<8} {'情绪':<8}"
    )
    print("-" * 80)
...
def demo_real_time_monitoring():
    ...
    base_data = {
        ...
        "portfolio": {
            "positions": {"STOCK_A": 0.1, "STOCK_B": 0.09, "STOCK_C": 0.08},
    ...
    for i in range(10):
        ...
        if len(alerts) > 0:
            print(
                f"    ⚠️  告警: {', '.join([f'{alert.risk_type.value}' for alert in alerts])}"
            )
