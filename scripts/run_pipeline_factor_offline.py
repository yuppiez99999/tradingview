...
def main() -> int:
    ...
    try:
        ...
        print(f"  因子组合: {combo.get('factor_a', '?')} + {combo.get('factor_b', '?')}")
        print(f"  组合方法: {combo.get('method', 'N/A')}")
        print(f"  lookback: {combo.get('lookback', 'N/A')}")
        print(f"  combined_ic_ir: {combo.get('combined_ic_ir', 0):+.4f}")
        print(f"  live_dsr: {combo.get('live_dsr', 0):+.4f}")
        print(f"  max_drawdown: {combo.get('max_drawdown', 0):.4f}")
        print(f"  权重: w_a={combo.get('w_a', 0):+.4f}, w_b={combo.get('w_b', 0):+.4f}")
        print(f"\n  信号统计:")
        print(f"    标的数: {stats.get('n_symbols', 0)}")
        print(f"    正信号: {stats.get('n_positive', 0)}")
        print(f"    负信号: {stats.get('n_negative', 0)}")
        print(f"    最大值: {stats.get('max_signal', 0):+.4f}")
        print(f"    最小值: {stats.get('min_signal', 0):+.4f}")
        print(f"    均值:   {stats.get('avg_signal', 0):+.4f}")
