...
for d in sorted(details, key=lambda x: x['pnl_pct']):
    print(f'{d["name"]:14s} {d["code"]:12s} 持{d["shares"]:>6d} 成本¥{d["avg_cost"]:.3f} 现价¥{d["est_price"]:.3f} 盈亏¥{d["pnl"]:>+10,.0f} ({d["pnl_pct"]:>+6.1f}%)')
...
print(f'持有期: {start_date} → {clearance_date} = {holding_days}天 = {holding_years:.2f}年')
print()

# 场景分析: 不同2030年平仓总市值下的年化收益率
# 年化收益率 = (最终市值 / 总成本)^(1/年数) - 1
print(f'{"场景":20s} {"2030市值":>12s} {"总盈亏":>12s} {"总收益率":>10s} {"年化收益率":>10s}')
print('-' * 80)
...
for name, final_value in scenarios:
    ...
    if total_cost > 0 and final_value > 0:
        ...
        ann_str = 'N/A'
    print(f'{name:20s} ¥{final_value:>10,.0f} ¥{pnl:>+10,.0f} {total_return:>+9.1f}% {ann_str}')
...
print(f'{"目标年化":>10s} {"需要2030市值":>14s} {"需要总盈亏":>14s} {"较当前盈亏":>14s}')
print('-' * 60)
for target_ann in [0.05, 0.08, 0.10, 0.12, 0.15, 0.20, 0.25, 0.30]:
    ...
    print(f'{target_ann*100:>9.0f}% ¥{needed_value:>12,.0f} ¥{needed_pnl:>+12,.0f} ¥{diff:>+12,.0f}')