...
EXCLUDE_DIRS = {".venv", ".git", "__pycache__", ".mypy_cache", ".ruff_cache",
                ".pytest_cache", "qlib", "external", "unsloth_compiled_cache",
                "ifind-finance-data-1.3.0", "data_cache", "models", "mlruns",
                ".qlib_experiments", "node_modules", "build", "dist",
                "量化系统升级为实盘系统—1783205278319", ".codeartsdoer",
                ".codebuddy", ".workbuddy", ".loopx_handoffs", ".cairn",
                ".agents", ".arts", ".claude", ".ocr_home", ".tmp_pip",
                "sim_snapshots", "downloads", "cache", "logs", ".benchmarks"}

# 密钥模式 (高置信度)
PATTERNS = [
    (r'(?:api_key|apikey|api-key)\s*[=:]\s*["\'][a-zA-Z0-9]{20,}["\']', "API Key"),
    (r'(?:secret|token|password|passwd|pwd)\s*[=:]\s*["\'][a-zA-Z0-9]{16,}["\']', "Secret/Token"),
...
# 误报过滤 (这些是配置键名而非实际密钥)
FALSE_POSITIVES = [
    "os.environ", "os.getenv", "os.environ.get", "config.get",
    "settings.get", "getenv", "YOUR_API_KEY", "YOUR_TOKEN",
            ...
            if matches:
                ...
                if not is_false_positive:
                    findings.append({
                        ...
                        "line": line_num,
                        "type": key_type,
                        "match": matches[0][:60] + "..." if len(matches[0]) > 60 else matches[0],
                    })
    return findings
...
def main():
    ...
    for dirpath, dirnames, filenames in os.walk(ROOT):
        ...
        for filename in filenames:
            if not filename.endswith((".py", ".yaml", ".yml", ".json", ".toml", ".ini", ".cfg", ".env")):
                continue
