#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""薄包装: 让 CI 的 scripts/_tdd_guard.py 路径可解析。

实际实现在 _archive/one_time_scripts/_tdd_guard.py。
此文件同时支持两种调用方式:
    1. 直接运行: python scripts/_tdd_guard.py  -> 执行归档脚本的 __main__
    2. 被 import: import _tdd_guard  -> 不执行副作用, 暴露归档脚本的符号
...
_ARCHIVE_SCRIPT = _PROJECT_ROOT / "_archive" / "one_time_scripts" / "_tdd_guard.py"
...
def _load_archive_module():
    spec = importlib.util.spec_from_file_location("_tdd_guard_impl", str(_ARCHIVE_SCRIPT))
