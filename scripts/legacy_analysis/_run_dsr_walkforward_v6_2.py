...
# 自动查找最新的V6.2回测结果
v6_files = glob.glob("output/validation_reports/lgb_backtest_v6_2*.json")
if not v6_files:
    print("ERROR: 未找到V6.2回测结果文件")
    sys.exit(1)
v6_files.sort(key=lambda x: os.path.getmtime(x))
v6_file = Path(v6_files[-1])
print(f"加载V6.2结果: {v6_file}")
...
print("=" * 70)
print("V6.2 DSR + Walk-Forward 验证 (止盈精调: 组合>12%→×0.80)")
print("=" * 70)
...
print(f"偏度:     {skew:.4f}")
print(f"峰度:     {kurt:.4f}  (V6: 4.99, V6.1: 3.83, V5.1: 1.89)")
print()
...
try:
    ...
    print(f"\nDSR 最大通过 n_trials: {max_pass} (目标 >=5, V6: 3, V6.1: 5)")
...
print(f"\nSharpe CV: {sharpe_cv:.4f} (目标 <0.5, V6: 0.46, V6.1: 0.55, {'达标' if stable else '未达标'})")
...
print(f"去除最高2月后年化: {ann_ret_without*100:.2f}% (目标 >=8%, V6: 8.92%, V6.1: 6.48%)")
...
out_file = output_path / f"dsr_walkforward_v6_2_{ts}.json"
...
print()
print("=" * 70)
print("V6.2 vs V6.1 vs V6 vs V5.1 对比:")
print(f"{'指标':<18} {'V5.1':<12} {'V6':<12} {'V6.1':<12} {'V6.2':<12} {'V6.2-V6':<10}")
print("-" * 80)
print(f"{'止盈规则':<18} {'12/0.85':<12} {'12/0.85':<12} {'10/0.80':<12} {'12/0.80':<12} {'':<10}")
print(f"{'年化收益':<18} {'15.57%':<12} {'16.84%':<12} {'13.32%':<12} {f'{ann_ret*100:.2f}%':<12} {f'{(ann_ret-0.1684)*100:+.2f}%':<10}")
print(f"{'最大回撤':<18} {'9.60%':<12} {'7.57%':<12} {'7.90%':<12} {f'{max_dd*100:.2f}%':<12} {f'{(max_dd-0.0757)*100:+.2f}%':<10}")
print(f"{'Sharpe':<18} {'1.176':<12} {'1.215':<12} {'1.103':<12} {f'{sharpe:.3f}':<12} {f'{sharpe-1.215:+.3f}':<10}")
print(f"{'峰度':<18} {'1.89':<12} {'4.99':<12} {'3.83':<12} {f'{kurt:.2f}':<12} {f'{kurt-4.99:+.2f}':<10}")
print(f"{'偏度':<18} {'1.00':<12} {'1.74':<12} {'1.42':<12} {f'{skew:.2f}':<12} {f'{skew-1.74:+.2f}':<10}")
print(f"{'WF Sharpe CV':<18} {'0.67':<12} {'0.46':<12} {'0.55':<12} {f'{sharpe_cv:.2f}':<12} {f'{sharpe_cv-0.46:+.2f}':<10}")
print(f"{'DSR n_trials':<18} {'8':<12} {'3':<12} {'5':<12} {f'{max_pass}':<12} {f'{max_pass-3:+d}':<10}")
print(f"{'去极端月年化':<18} {'8.65%':<12} {'8.92%':<12} {'6.48%':<12} {f'{ann_ret_without*100:.2f}%':<12} {f'{(ann_ret_without-0.0892)*100:+.2f}%':<10}")
print("=" * 80)
...
print()
if all_pass:
    print("🎉 V6.2 全部验收通过! 止盈精调成功平衡峰度与Sharpe CV稳定性!")
else:
    print("⚠️  V6.2 部分验收未通过, 需继续调优")
print("=" * 70)
print(f"\n结果已保存: {out_file}")
