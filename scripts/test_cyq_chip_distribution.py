...
def main() -> int:
    ...
    for sym in patterns:
        ...
        s = snap
        print(
            f"  {sym:<13}: profit={s.profit_ratio:.3f}  conc={s.concentration:.3f}  "
    ...
    print(
        f"[断言2] 成本偏离方向: UP_BIG={up_big_dev:+.4f}>0  DOWN_BIG={down_big_dev:+.4f}<0 ✓"
    )
    ...
    print(
        f"[断言3] SIDEWAYS 集中度 = {side_conc:.3f} (预期 0<conc≤1, 横盘筹码自然集中)"
    )
    ...
    print(
        f"[断言4] FLASH_CRASH: cost_dev={flash.cost_deviation:+.4f}<0 (套牢), peak/px={flash_peak_pos:.3f}"
    )
    assert (
        flash.cost_deviation < 0
    ), f"FLASH_CRASH 成本偏离应为负, 实际{flash.cost_deviation:+.4f}"
    print("  ✓ 急跌后套牢特征符合")
    ...
    assert (
        ent_high >= ent_low * 0.95
    ), f"高成交量熵应更高 (低量{ent_low:.3f}, 高量{ent_high:.3f})"
    ...
    for fname, fval in chip4.items():
        ...
        print(
            f"  {fname:<22}: 覆盖 {n}/6 标的  min={min(vals):+.4f}  max={max(vals):+.4f}  category={fval.category}"
        )
    ...
    print(
        f"  [成本偏离横截面] UP_BIG={dev_map['UP_BIG']:+.4f}  DOWN_BIG={dev_map['DOWN_BIG']:+.4f}"
    )
    ...
    # ---------- 5. 流通股本退化 (vs 提供流通股本) ----------
    print("\n[退化 vs 提供流通股本] 同一标的对比")
    ffs = dict.fromkeys(pd_, 10000000000.0)  # 提供极大流通股本 → 换手率极低 → 筹码几乎不变
    ...
    for fname in chip4:
        ...
        assert len(chip_ffs[fname].values) == len(
            chip4[fname].values
        ), f"{fname} 标的数不一致"
    ...
    print(
        f"  debug_info.chip_covered_symbols = {result.debug_info.get('chip_covered_symbols')}"
    )
    ...
    print(
        f"  CYQ_PROFIT_RATIO: mode={fv.ic_mode}  n_samples={fv.ic_n_samples}  "
    ...
    print(
        f"  换手衰减一致性: 高成交量熵 {ent_high:.2f} ≥ 低成交量熵 {ent_low:.2f} (×≥0.95) ✓"
    )
