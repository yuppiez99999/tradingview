...
print("=" * 110)
print(
    f"  持仓盈亏明细  |  价格快照日期: {latest_price_date or '未知'}  |  总资金: {total_capital:,.0f}  |  持仓标的: {len(rows)} 个"  # noqa: E501
)
print("=" * 110)
print(
    f"{'代码':<12}{'名称':<14}{'类型':<6}{'风格':<8}{'持仓':>8}{'成本价':>10}{'现价':>10}{'市值':>12}{'盈亏':>12}{'盈亏率':>9}"  # noqa: E501
)
print("-" * 110)
for r in rows:
    pnl_sign = "🔴" if r["pnl"] < 0 else ("🟢" if r["pnl"] > 0 else "⚪")
    print(
        f"{r['code']:<12}{r['name']:<14}{r['type']:<6}{r['style']:<8}"
...
print("-" * 110)
print(
    f"{'合计':<40}{'':>8}{'':>10}{'':>10}{total_mv:>12,.0f}{total_pnl:>11,.0f}  {total_pnl_pct:>7.2f}%"
)
print("=" * 110)
...
for style, g in sorted(style_groups.items(), key=lambda x: -x[1]["mv"]):
    ...
    print(
        f"{style:<10}{g['count']:>6}{g['mv']:>14,.0f}{pct:>7.1f}%{g['pnl']:>13,.0f} {sign}{g_pnl_pct:>7.2f}%"
    )
print("-" * 65)
print(
    f"{'合计':<10}{len(rows):>6}{total_mv:>14,.0f}{100.0:>7.1f}%{total_pnl:>13,.0f}  {total_pnl_pct:>7.2f}%"
)
...
if winners:
    ...
    print(
        f"   最大盈利: {best['name']} ({best['code']}) +{best['pnl_pct']:.2f}%  盈亏 {best['pnl']:+,.0f}"
    )
if losers:
    worst = min(losers, key=lambda r: r["pnl_pct"])
    print(
        f"   最大亏损: {worst['name']} ({worst['code']}) {worst['pnl_pct']:.2f}%  盈亏 {worst['pnl']:+,.0f}"
    )
