...
_FS_TAIL = re.compile(r"\s*#.*(?:fail-safe|模块 fail-safe|待后续精确化).*$", re.IGNORECASE)

# try 体信号 -> 异常族
_IMPORT_HINTS = ("import ", "from ")
_NET_HINTS = (
    "request", "http", "urlopen", "url", "fetch", "get(", "post(",
    "source_health", "read_text", "read_csv", "json.load", "json.dumps",
    "loads(", "pd.read", "open(", "mkdir", "connect", "client", "get_history",
    "get_realtime", "to_datetime", "get_macro", "get_risk", "get_news",
)
_PROBE_HINTS = ("is_ready", "test_connection", "return None", "return False", "return {}")

# 顶层清理/日志写: 必须保留宽捕获的豁免标记 (改写后)
# 仅明确清理语义 (atexit.register / sys.exit 包裹) 才豁免, 避免误吞数据层异常
_SAFE_KEEP_RE = re.compile(r"(atexit\.register|sys\.exit|os\._exit|atexit\b)", re.IGNORECASE)
...
def _infer_exception_family(try_src: str) -> tuple[str, bool]:
    ...
    has_import = first_line.startswith("import ") or first_line.startswith("from ") or (
        "importlib.import_module" in src or "import " in src
    )
    ...
    if net and not probe:
        return "(ValueError, KeyError, TypeError, AttributeError, OSError, RuntimeError)", False
