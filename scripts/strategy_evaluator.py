...
class ScoreReport:
    ...
    divers_metrics: DiversificationMetrics = field(
        default_factory=DiversificationMetrics
    )
...
def evaluate_diversification(positions: dict) -> DiversificationMetrics:
    ...
    variance = (
        sum((w - mean_w) ** 2 for w in weights) / len(weights) if weights else 0.0
    )
...
class StrategyEvaluator:
    ...
    def __init__(self, feature_flag_name: str = "USE_STRATEGY_EVALUATOR"):
        ...
        logger.info(
            f"StrategyEvaluator init: enabled={self._enabled}, flag={feature_flag_name}"
        )
    ...
    def evaluate(
        ...
        if not self._enabled:
            return self._build_degraded_report(
                reason=f"feature_flag_disabled ({self._feature_flag_name})"
            )
        ...
        # ========== III: 综合评分 ==========
        public_score, private_score = self._compute_overall_scores(
            return_metrics, divers_metrics
        )
        ...
        recommendation, reason = self._make_recommendation(
            public_score, private_score, return_metrics, divers_metrics
        )
        ...
        report = ScoreReport(
            ...
            overall_score=round(
                public_score * EVOLUTION_CONFIG["public_weight"]
                + private_score * EVOLUTION_CONFIG["private_weight"],
                4,
            ),
                ...
                if total_value > 0:
                    ...
                    return (
                        ReturnMetrics(
                            ...
                            max_drawdown=0.0,
                            sharpe_ratio=0.0,
                            total_return=pnl_sum / total_value,
    ...
    def _compute_overall_scores(
        ...
        # - 年化收益打分
        excess_return = max(
            0, return_metrics.annual_return - EVOLUTION_CONFIG["min_annual_return"]
        )
        ...
        sharpe_score = max(
            ...
            min(
                1.0,
                (return_metrics.sharpe_ratio - EVOLUTION_CONFIG["benchmark_sharpe"])
                / 1.0,
            ),
        )
        ...
        # - 稳定性: 最大回撤越小越好
        max_dd_penalty = min(
            1.0, return_metrics.max_drawdown / EVOLUTION_CONFIG["max_allowed_drawdown"]
        )
        ...
        diversity_score = (
            0.6 * divers_metrics.style_diversity_score + 0.4 * concentration_penalty
        )
    ...
    def _make_recommendation(
        ...
        if public_score >= 0.6 and private_score >= 0.65:
            ...
            if return_metrics.max_drawdown < EVOLUTION_CONFIG["max_allowed_drawdown"]:
                return (
                    "promote",
                    f"public={public_score:.2f}, private={private_score:.2f}, max_dd={return_metrics.max_drawdown:.2f}",
                )
            return (
                "continue",
                f"score OK but max_drawdown={return_metrics.max_drawdown:.2f} exceeds limit",
            )
        ...
        return (
            "continue",
            f"public={public_score:.2f}, private={private_score:.2f}, 待更多数据收敛",
        )
...
def evaluate_strategy_simple(
    ...
    evaluator = StrategyEvaluator(
        feature_flag_name=EVOLUTION_CONFIG["feature_flag_name"]
    )
...
if __name__ == "__main__":
    ...
    logger.info("\n回报指标:")
    logger.info(
        f"  年化收益率: {report.return_metrics.annual_return:.4f} ({report.return_metrics.sample_count} 个样本)"
    )
