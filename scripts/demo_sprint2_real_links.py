...
_LOG_FILE = os.path.join(
    ...
    "logs", "demo_sprint2_real_links.log",
)
...
def demo_a_memory_reflection_with_mock_prices(tmp_dir: str) -> None:
    ...
    records = [
        {
            ...
            "evaluated": False, "eval_date": "",
            "forward_return_1d": None, "forward_return_5d": None, "forward_return_10d": None,
            "correct_1d": None, "correct_5d": None, "correct_10d": None,
            "reflection": "",
        },
        {
    ...
    for r in records:
        print(f"    - {r['ticker']}: signal={r['final_signal']}, conf={r['final_confidence']}, date={r['date']}")
    ...
    for ticker, dates in price_data.items():
        prices = " → ".join(f"{d.split('-')[2]}: {info['close']}" for d, info in sorted(dates.items()))
...
def demo_b_debate_layer_with_rate_limiter(tmp_dir: str) -> None:
    ...
    def _mock_call_llm(prompt, pydantic_model, **kwargs):
        ...
        return DebateStance(
            ...
            key_arguments=["多空证据均衡"], rebuttals=[], evidence_summary="",
        )
    ...
    analyst_signals = {
        "warren_buffett": {
            "AAPL": {"signal": "bullish", "confidence": 85, "reasoning": "护城河无可匹敌, 长期持有"},
        },
        "ben_graham": {
            "AAPL": {"signal": "bullish", "confidence": 72, "reasoning": "内在价值测算溢价 15%"},
        },
        "michael_burry": {
            "AAPL": {"signal": "bearish", "confidence": 68, "reasoning": "PE 处于历史 85 分位, 拐点临近"},
        },
        "cathie_wood": {
            "AAPL": {"signal": "bullish", "confidence": 80, "reasoning": "AI 战略布局领先"},
        },
        "stanley_druckenmiller": {
            "AAPL": {"signal": "neutral", "confidence": 50, "reasoning": "等待宏观信号明确"},
        },
    }
    ...
    with patch.object(DebateLayer, "_llm_available", return_value=True):
        with patch("quant_modules.ai_hedge_fund.utils.llm.call_llm",
                   side_effect=_mock_call_llm) as mock_call:
            layer2 = DebateLayer(
                use_llm=True, log_dir=os.path.join(tmp_dir, "debates_b"),
...
def demo_c_end_to_end_loop(tmp_dir: str) -> None:
    ...
    logger.info("步骤 1 开始: 运行辩论 | tickers=['AAPL','TSLA','GOOG'], use_llm=True, use_rate_limiter=True, model=gpt-4o-mini")
    ...
    def _mock_call_llm(prompt, pydantic_model, **kwargs):
        ...
        if "bull" in agent_name:
            return DebateStance(
                stance="bullish", confidence=78,
                key_arguments=["基本面强劲", "估值合理", "动量正向"],
                rebuttals=[], evidence_summary="看多证据充分",
            )
        elif "bear" in agent_name:
            return DebateStance(
                stance="bearish", confidence=55,
                key_arguments=["估值偏高", "技术面走弱"],
                rebuttals=[], evidence_summary="看空证据中等",
            )
        return DebateStance(
            stance="neutral", confidence=50,
    ...
    layer = DebateLayer(
        use_llm=True, log_dir=os.path.join(tmp_dir, "debates_c"),
        use_rate_limiter=True, model_name="gpt-4o-mini",
    )
    logger.debug("DebateLayer 实例化完成 | rounds=%s, use_llm=%s, use_rate_limiter=%s, _rate_limited_caller=%s",
                 layer.rounds, layer.use_llm, layer.use_rate_limiter,
                 type(layer._rate_limited_caller).__name__ if layer._rate_limited_caller else "None")
    ...
    logger.info("分析师信号就绪 | agents=%s, tickers=%s",
                list(analyst_signals.keys()), list(next(iter(analyst_signals.values())).keys()))

    try:
        with patch.object(DebateLayer, "_llm_available", return_value=True):
            with patch("quant_modules.ai_hedge_fund.utils.llm.call_llm",
                       side_effect=_mock_call_llm):
                session = layer.run_full_debate(["AAPL", "TSLA", "GOOG"], analyst_signals)
        step_elapsed = time.monotonic() - step_start
        logger.info("步骤 1 完成: 辩论结束 | tickers=%d, errors=%d, 耗时=%.2fs",
                    len(session.debate_results), len(session.errors), step_elapsed)
    ...
    for ticker, result in session.debate_results.items():
        logger.info("  辩论结果 [%s] | winner=%s, signal=%s, conf=%d, net_conf=%d",
                    ticker, result.winner, result.final_signal,
                    result.final_confidence, result.net_confidence)
        logger.debug("  辩论详情 [%s] | bull_r1_conf=%d, bear_r1_conf=%d, bull_final_conf=%d, bear_final_conf=%d",
                     ticker, result.bull_round1.confidence, result.bear_round1.confidence,
                     result.bull_final.confidence, result.bear_final.confidence)
        logger.debug("  裁决理由 [%s] | %s", ticker, result.reasoning)
        print(f"    {ticker}: winner={result.winner}, signal={result.final_signal}, conf={result.final_confidence}")
    ...
    try:
        ...
        step_elapsed = time.monotonic() - step_start
        logger.info("步骤 2 完成: 决策记录写入 | count=%d, 耗时=%.2fs, file=%s",
                    count, step_elapsed, mem.memory_file)
    except Exception as exc:
    ...
    logger.info("步骤 3 开始: T+10 评估 | lookback_days=30, eval_date=2026-08-11, provider_type=dict")

    print("  mock 价格 (决策日 08-01 → 评估日 08-11):")
    for ticker, dates in price_data.items():
        ret_10d = (dates["2026-08-11"]["close"] - dates["2026-08-01"]["close"]) / dates["2026-08-01"]["close"]
        logger.info("  价格数据 [%s] | 08-01=%.2f → 08-11=%.2f (10d收益=%+.2f%%)",
                    ticker, dates["2026-08-01"]["close"], dates["2026-08-11"]["close"], ret_10d * 100)
        print(f"    {ticker}: {dates['2026-08-01']['close']} → {dates['2026-08-11']['close']} (10d收益={ret_10d:+.2%})")
    ...
    try:
        ...
        step_elapsed = time.monotonic() - step_start
        logger.info("步骤 3 完成: 评估结束 | updated=%d, 耗时=%.2fs", updated, step_elapsed)
    except Exception as exc:
    ...
    try:
        ...
        for rec in eval_records:
            ...
            logger.info("  评估结果 [%s] | signal=%s, conf=%d, 5d=%s, 10d=%s, correct_5d=%s, correct_10d=%s",
                        ticker, signal, conf,
                        f"{ret_5d:+.2%}" if ret_5d is not None else "None",
                        f"{ret_10d:+.2%}" if ret_10d is not None else "None",
                        correct_5d, correct_10d)
    ...
    # 步骤 5: 提取反思上下文 (注入下次分析)
    # ============================================================
    _print_sub("步骤 5: 提取反思上下文 (get_reflection_context) — 可注入下次分析 prompt")
    step_start = time.monotonic()
    ...
    try:
        ctx = mem.get_reflection_context(days=30)
        step_elapsed = time.monotonic() - step_start
        logger.info("步骤 5 完成 | total_evaluated=%d, overall_win_rate=%.4f, 耗时=%.2fs",
                    ctx["total_evaluated"], ctx["overall_win_rate"], step_elapsed)
    except Exception as exc:
    ...
    for ticker, stats in ctx["by_ticker"].items():
        logger.info("  反思明细 [%s] | total=%d, evaluated=%d, correct_5d=%d, win_rate=%.4f",
                    ...
                    stats["correct_5d"], stats["win_rate"])
        for r in stats["recent_reflections"]:
            logger.debug("  反思记录 [%s] | %s", ticker, r)
        print(f"    {ticker}: win_rate={stats['win_rate']:.0%} ({stats['correct_5d']}/{stats['evaluated']} 正确)")
        for r in stats["recent_reflections"]:
            print(f"      ↳ {r}")
    ...
    # ============================================================
    _print_sub("步骤 6: RateLimitedLLMCaller 全局统计 (3 ticker × 2 轮 × 2 方 = 12 次调用)")
    logger.info("步骤 6 开始: 汇总 RateLimitedLLMCaller 统计")

    stats = caller.stats
    logger.info("统计维度数=%d", len(stats))
    for key, s in stats.items():
        logger.info("  RateLimiter统计 [%s] | total=%d, success=%d, cache_hits=%d, rate_limited=%d, failed=%d, avg_latency=%.2fms, success_rate=%.4f",
                    key, s["total_calls"], s["successful"], s["cache_hits"],
                    s["rate_limited"], s["failed"],
                    s.get("avg_latency_ms", 0.0), s.get("success_rate", 0.0))
        print(f"\n  [{key}]")
        for k, v in s.items():
    ...
    logger.info("端到端闭环完成 | tickers=%d, 决策记录=%d, 评估=%d, 整体胜率=%.4f, 总耗时=%.2fs",
                len(session.debate_results), count, updated,
                ctx["overall_win_rate"], loop_elapsed)
