# -*- coding: utf-8 -*-
"""
QLib v8训练脚本 — 适中正则化 + Top40特征 + 扩展训练样本(CSI100)
"""
import os
import sys
import json
...
CSI100_AUX = [
    ...
    ("600085", "SH", "同仁堂"), ("601009", "SH", "南京银行"),
    ("600919", "SH", "江苏银行"), ("000001", "SZ", "平安银行"),
...
def main():
    ...
    print(f"\n{'='*70}")
    print(f"XGBoost v8 (Top{TOP_N}特征+适中正则化) 训练完成")
    print(f"{'='*70}")
    ...
    report_path = os.path.join(reports_dir, f"qlib_v8_train_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json")
