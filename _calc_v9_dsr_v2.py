...
print(f'\n{"=" * 70}')
print(f'综合评估 (修正后标准, 2026-07-25 审计 P0-2/P0-3 修复)')
print(f'{"=" * 70}')

checks = [
    ('Sharpe CV < 1.0 (12月滚动)', sharpe_cv < 1.0, f'{sharpe_cv:.4f} (滚动窗口数={len(rolling_sharpes)})'),
    (f'DSR max_pass >= 5 (标准公式)', max_pass >= 5, f'max_pass={max_pass}'),
...
print(f'  注: 实际研究过程测试了 V6/V6.1/V6.2/V7/V7.1/V7.2/V8/V9 共 8+ 版本')
print(f'  Bailey 标准: DSR > 0.95 (单尾 p<0.05) 才能拒绝"无 alpha"零假设')

print(f'\n{"=" * 70}')
if all_pass:
    print(f'✓ V9 在修正后标准下全部达标!')
else:
    print(f'✗ V9 在修正后标准下未全部达标')
