...
def run_hedge_detail_mode(args):
    ...
    from utils.hedge_engine import (
        ...
        DEFAULT_FUTURES_PRICES, FALLBACK_PRICES_UPDATED,
    ...
    try:
        ...
        if os.path.exists(price_history):
            with open(price_history, 'r', encoding='utf-8') as f:
                for line in f:
                    try:
    ...
    # 计算组合估值
    stock_value = sum(
        v.get('shares', 0) * prices_est.get(k, 0)
    ...
    print(f"  股票敞口:        ¥{stock_value:,.0f} ({stock_value/total_value*100:.1f}%)" if total_value > 0 else "")
    print(f"  现金:            ¥{cash:,.0f}")
    ...
    print(f"\n  'One' 股指期货品种规格")
    print(f"  {'─' * 70}")
    print(f"  {'品种':<6s} {'名称':<18s} {'乘数':>6s} {'保证金':>6s} {'Tick':>6s}  {'实时价格':>10s}  {'数据来源'}")
    print(f"  {'─' * 70}")

    price_sources = {
        "wind": set(), "akshare": set(), "sina": set(),
        "efinance": set(), "default": set(),
    }
    default_prices = DEFAULT_FUTURES_PRICES
    ...
    for name in INDEX_ALLOCATION_ORDER + ["IH", "IF"]:
        if name in INDEX_FUTURES_SPECS:
            ...
            if live > 0:
                ...
                source = "默认回退"
            print(f"  {name:<6s} {spec['name']:<18s} {spec['multiplier']:>6d} {spec['margin_pct']:>5.0%} "
    ...
    print(f"  日VaR(95%)触发:           > {trigger.get('min_var_pct', 0.015)*100:.1f}% 总资产")
    print(f"  恐慌指数阈值:             > {trigger.get('panic_index_threshold', 0.6)}")
    print(f"  ── v5.9 组合自触发 ──")
    print(f"  波动率触发(年化):         > {PORTFOLIO_TAIL_HEDGE_TRIGGERS['vol_trigger']*100:.0f}%")
    print(f"  60日回撤触发:              > {PORTFOLIO_TAIL_HEDGE_TRIGGERS['dd_trigger']*100:.0f}%")
    print(f"  触发后对冲范围:           {PORTFOLIO_TAIL_HEDGE_TRIGGERS['min_hedge_ratio']*100:.0f}%-{PORTFOLIO_TAIL_HEDGE_TRIGGERS['max_hedge_ratio']*100:.0f}%")
    ...
    for level, cfg in strength.items():
        print(f"  {level:<10s} 对冲 {cfg['ratio']*100:3.0f}% 敞口  — {cfg.get('description', '')}")

    # ── 4. 保证金约束 ──
    margin_cfg = hedge_config.get('margin', {})
    print(f"\n  'Four' 保证金与成本")
    print(f"  {'─' * 70}")
    print(f"  最大保证金占比:           < {margin_cfg.get('max_margin_pct', 0.20)*100:.0f}%")
    print(f"  最低现金留存:             ¥{margin_cfg.get('min_cash_reserve', 100000):,}")
    print(f"  年化展期成本(基差+费):    ~2.5%")
    print(f"  保证金机会成本(无风险):   ~2.0%")
    print(f"  成本效益阈值:             预期收益 > 成本 x 1.5")
    ...
    print(f"  {'─' * 70}")
    print(f"  {'情景':<20s} {'CSI300':>6s} {'CSI500':>6s} {'CSI1000':>6s} {'SSE50':>6s} {'黄金':>6s}")
    print(f"  {'─' * 70}")
    for scenario, shocks in HedgeEngine.HISTORICAL_STRESS_SCENARIOS.items():
        print(f"  {scenario:<20s} {shocks['csi300']:>+5.0%}  {shocks['csi500']:>+5.0%}  "
              f"{shocks['csi1000']:>+5.0%}  {shocks['sse50']:>+5.0%}  {shocks['gold']:>+5.0%}")

    # ── 7. Beta 表 ──
    print(f"\n  'Seven' 主要标的Beta值")
    print(f"  {'─' * 70}")
    print(f"  {'代码':<10s} {'名称':<10s} {'CSI300':>8s} {'CSI500':>8s} {'CSI1000':>8s} {'SSE50':>8s}")
    print(f"  {'─' * 70}")
    beta_name_map = {
        "300308": "中际旭创", "688041": "海光信息", "002371": "北方华创",
        "688981": "中芯国际", "300750": "宁德时代", "000425": "徐工机械",
        "601088": "中国神华", "600219": "南山铝业", "600019": "宝钢股份",
        "518880": "黄金ETF", "000792": "盐湖股份",
        "600900": "长江电力", "600276": "恒瑞医药", "603259": "药明康德",
        "002422": "科伦药业",
    }
    for code, betas in sorted(HedgeEngine.DEFAULT_BETAS.items()):
        name = beta_name_map.get(code, "")
        print(f"  {code:<10s} {name:<10s} {betas[0]:>8.2f} {betas[1]:>8.2f} {betas[2]:>8.2f} {betas[3]:>8.2f}")

    # ── 8. CLI 命令参考 ──
    print(f"\n  'Eight' 相关CLI命令")
    print(f"  {'─' * 70}")
    print(f"  --hedge             对冲分析 (含AI分析师)")
    print(f"  --hedge --no-ai     纯规则引擎对冲")
    print(f"  --hedge-rebalance   对冲+再平衡联动")
    print(f"  --hedge-rebalance --mode=tail_only   尾部保护 (默认)")
    print(f"  --hedge-rebalance --mode=dynamic     动态对冲")
    print(f"  --hedge-detail      本明细文档")
    print(f"  --stress-test       极端压力测试")
    ...
    print(f"  ⚠️ 以上信息仅供参考，不构成投资建议。")
    print(f"  ⚠️ 期货/期权交易有杠杆风险，请谨慎执行。\n")
