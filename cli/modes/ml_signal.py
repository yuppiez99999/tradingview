...
def run_ml_signal_mode(args):
    ...
    if 'signals' in result:
        ...
        if buy_n:
            print("  🟢 买入TOP3:")
            for s in sorted(sig['buy'], key=lambda x: x['probability'], reverse=True)[:3]:
                name = get_stock_name(s['code'])
                print(f"    {s['code']} {name:<10} 概率={s['probability']:.2%} 置信={s['confidence']:.2%}")
        if sell_n:
            print("  🔴 卖出TOP3:")
            for s in sorted(sig['sell'], key=lambda x: x['probability'])[:3]:
                name = get_stock_name(s['code'])
                print(f"    {s['code']} {name:<10} 概率={1-s['probability']:.2%} 置信={s['confidence']:.2%}")
