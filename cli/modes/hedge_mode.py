...
def run_hedge_mode(args):
    ...
    try:
        from utils.hedge_engine import (
            ...
            INDEX_FUTURES_SPECS,  # noqa: F401
            HedgeEngine,
            HedgeSignalStrength,
            calculate_portfolio_beta,  # noqa: F401
        )
    ...
    os.path.join(BASE_DIR, "config", "portfolio.yaml")
    ...
    print(
        f"  股票敞口:      ¥{risk.stock_exposure:,.0f} ({risk.stock_exposure/risk.total_value*100:.1f}%)"
        if risk.total_value > 0
        else f"  股票敞口:      ¥{risk.stock_exposure:,.0f}"
    )
    ...
    if futures_prices:
        print(
            f"  📡 期货价格来源: 已获取 {len([k for k in futures_prices if futures_prices[k] > 0])}/4 品种"
        )
    ...
    if strength != HedgeSignalStrength.NO_HEDGE:
        ...
        print(
            f"  对冲比率:      {hedge_ratio*100:.0f}% ({risk.stock_exposure*hedge_ratio:,.0f})"
            if risk.stock_exposure > 0
            else f"  对冲比率:      {hedge_ratio*100:.0f}%"
        )

        futures_result = engine.generate_futures_hedge(
        ...
        if futures_result.get("contracts"):
            ...
            print(
                f"  保证金占比:    {futures_result['total_margin']/total_value*100:.1f}%"
                if total_value > 0
                else "  保证金占比:    N/A"
            )
        else:
        ...
        # 期权方案 (作为替代/补充)
        options_result = engine.generate_options_hedge(
            risk, hedge_ratio, strategy="protective_put"
        )
        print("\n  📊 期权对冲方案 (保护性看跌)")
        print(f"  {'─' * 50}")
        ...
        print(
            f"  预估权利金:    ¥{options_result.get('total_premium', 0):,.0f}"
            if options_result.get("total_premium")
            else f"  预估权利金:    ¥{options_result.get('total_premium', 0):,.0f}"
        )
        ...
        if not getattr(args, "no_ai", False):
            ...
            try:
                ...
                hedge_signal = result["data"]["analyst_signals"].get(
                    "hedge_analyst_agent", {}
                )
                ...
                if hedge_signal:
                    ...
                    print(
                        f"    对冲比率: {hedge_signal.get('hedge_ratio', 0)*100:.0f}%"
                    )
                    print(
                        f"    紧急程度: {hedge_signal.get('urgency_score', 0)*100:.0f}%"
                    )
                    ...
                    if reasoning:
                        try:
                            ...
                            if "hedge_recommendation" in r:
                                ...
                                print(
                                    f"    推荐工具: {rec.get('preferred_instrument', 'N/A')}"
                                )
                                print(
                                    f"    执行时机: {rec.get('execution_timing', 'N/A')}"
                                )
                        except (json.JSONDecodeError, KeyError):
