...
from core.context import (
    BASE_DIR, logger, connector_manager, stop_loss,
    SOCIAL_SECURITY_ETF_AVAILABLE, ML_PREDICTOR_AVAILABLE,
...
def run_unified_monitor(args):
    ...
    def stock_monitor_func():
        module_logger = get_module_logger("股票实时监控")
    ...
    def futures_scan_func():
        """调用期货期权扫描模块"""
        module_logger = get_module_logger("期货期权扫描")
        try:
    ...
    # 止损止盈风险快照（单次，非阻塞）
    def risk_check_func():
        module_logger = get_module_logger("风险评估")
    ...
    def etf_flow_monitor_func():
        """ETF资金流监控 - 每10分钟"""
        module_logger = get_module_logger("ETF资金流监控")
        try:
    ...
    def ml_signal_monitor_func():
        """ML信号扫描 - 每15分钟"""
        module_logger = get_module_logger("ML信号扫描")
        try:
    ...
    def kommo_monitor_func():
        """康波周期监控 - 每1小时"""
        module_logger = get_module_logger("康波周期监控")
        try:
    ...
    def connector_health_func():
        """数据源健康探测 - 每2分钟"""
        module_logger = get_module_logger("数据源健康探测")
        try:
    ...
    def hedge_rebalance_func():
        """对冲+再平衡联动分析 - 每30分钟"""
        module_logger = get_module_logger("对冲再平衡联动")
        try:
    ...
    modules_config = [
        ...
        # v5.10 新增模块
        ("对冲再平衡联动", hedge_rebalance_func, 1800),       # 30分钟
    ]
